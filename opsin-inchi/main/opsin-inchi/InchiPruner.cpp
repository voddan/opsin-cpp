...
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
    ...
    if (std::isalpha(
            inchiLayers[0][inchiLayers[0].length() - 1])) { //remove the S indicating this to be a standard InChI
    ...
    for (int i = 2; i < inchiLayers.size(); i++) {
        ...
        } else if (c != L'b' && c != L't' && c != L'm' &&
                   c != L's') { //ignore stereochemistry but continue as there may be an isotopic layer
            break;
        }
    }
...
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
    ...
    for (int i = 2; i < inchiLayers.size(); i++) {
        boost::optional<wchar_t> c = inchiLayers[i][0];
        if (c == L'c' || c == L'h' || c == L'q' || c == L'p' || c == L'b' || c == L't' || c == L'm' || c == L's' ||
            c == L'i') {
            retainedLayers.push_back(inchiLayers[i]);
        } else {
            break;
