...
//ORIGINAL LINE: @Test public void verifySMILES() throws Exception
						...
						void VerifyFragmentsTest::verifySmilesInTokenList(XMLStreamReader *reader) throw(XMLStreamException)
						{
							...
							if (tagname == XmlDeclarations::GROUP_EL || tagname == XmlDeclarations::FUNCTIONALGROUP_EL || tagname == XmlDeclarations::HETEROATOM_EL || tagname == XmlDeclarations::SUFFIXPREFIX_EL)
							{
								...
								while (reader->hasNext())
								{
									...
									case XMLStreamConstants::START_ELEMENT:
										if (reader->getLocalName().equals(L"token"))
										{
											...
											assertNotNull(std::wstring(L"The following token's SMILES or labels were in error: ") + smiles, mol);
											try
											{
												mol->checkValencies();
											}
											...
											{
												fail(std::wstring(L"The following token's SMILES produced a structure with invalid valency: ") + smiles);
											}
										}
										break;
