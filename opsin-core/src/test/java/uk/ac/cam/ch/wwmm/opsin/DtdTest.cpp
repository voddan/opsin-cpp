...
//ORIGINAL LINE: @Test public void testTokenFilesValueValidity() throws Exception
						void DtdTest::testTokenFilesValueValidity() throw(std::exception)
						{
							...
							{
								if (indexReader->next() == XMLStreamConstants::START_ELEMENT && indexReader->getLocalName().equals(L"tokenFile"))
								{
							...
							while (reader->hasNext())
							{
								...
								case XMLStreamConstants::START_ELEMENT:
									if (reader->getLocalName().equals(L"token"))
									{
										...
										assertTrue(tokenString + std::wstring(L" occurred more than once in a tokenList"),!terms->contains(tokenString));
										terms->add(tokenString);
										std::vector<wchar_t> characters = tokenString.toCharArray();
										for (auto c : characters)
										{
											assertTrue(std::wstring(L"Non ascii character found in token: ") + tokenString + OpsinTools::NEWLINE + std::wstring(L"An ASCII replacement should be used!"),static_cast<int>(c) < 128);
											assertTrue(std::wstring(L"Capital letter found in token: ") + tokenString + OpsinTools::NEWLINE + std::wstring(L"Only lower case letters should be used!"), !(c >= L'A' && c <= L'Z'));
										}
									}
									break;
