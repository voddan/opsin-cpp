...
//ORIGINAL LINE: @Test public void acceptableInterWordBreaks() throws ParsingException
						void TokenizerTest::acceptableInterWordBreaks() throw(ParsingException)
						{
							...
							assertEquals(true, tokenizer->tokenizeRightToLeft(reverseParseRules, L"methane ethane", false)->isSuccessfullyTokenized());
							assertEquals(true, tokenizer->tokenizeRightToLeft(reverseParseRules, L"methane-ethane", false)->isSuccessfullyTokenized());
							assertEquals(true, tokenizer->tokenizeRightToLeft(reverseParseRules, L"methane - ethane", false)->isSuccessfullyTokenized());
							assertEquals(false, tokenizer->tokenizeRightToLeft(reverseParseRules, L"methane -ethane", false)->isSuccessfullyTokenized());
							assertEquals(false, tokenizer->tokenizeRightToLeft(reverseParseRules, L"methane - ", false)->isSuccessfullyTokenized());
						}
...
//ORIGINAL LINE: @Test public void CCCP_RL() throws ParsingException
						...
						{
							TokenizationResult *result = tokenizer->tokenizeRightToLeft(reverseParseRules, L"Carbonyl cyanide m-chlorophenyl oxime", true);
							assertEquals(true, result->isSuccessfullyTokenized());
							assertEquals(true, result->isFullyInterpretable());
							assertEquals(L"", result->getUninterpretableName());
							assertEquals(L"", result->getUnparsableName());
