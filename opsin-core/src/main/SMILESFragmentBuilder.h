...
public:
    ...
    /// @return </param>
    /// <exception cref="StructureBuildingException"> </exception>
    virtual Fragment *build(const std::wstring &smiles, const std::wstring &type,
    ...
    /// <exception cref="StructureBuildingException"> </exception>
    virtual Fragment *build(const std::wstring &smiles, Element *tokenEl,
                            const std::wstring &labelMapping) throw(StructureBuildingException);

private:
    Fragment *build(const std::wstring &smiles, Fragment *fragment,
                    const std::wstring &labelMapping) throw(StructureBuildingException);

    void
    processLabelling(const std::wstring &labelMapping, std::vector<Atom *> &atomList) throw(StructureBuildingException);

    void
    verifyAndTakeIntoAccountLonePairsInAtomParities(std::vector<Atom *> &atomList) throw(StructureBuildingException);
