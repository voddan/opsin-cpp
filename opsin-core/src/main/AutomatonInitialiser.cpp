...
AutomatonInitialiser::AutomatonInitialiser(const std::wstring &resourcePath) : resourceGetter(
        new ResourceGetter(resourcePath)) {
}

RunAutomaton *
AutomatonInitialiser::loadAutomaton(const std::wstring &automatonName, const std::wstring &regex, bool tableize,
                                    bool reverseAutomaton) {
    if (reverseAutomaton) {
...
RunAutomaton *AutomatonInitialiser::loadCachedAutomaton(const std::wstring &automatonName) throw(IOException) {
    InputStream *automatonInput = resourceGetter->getInputstreamFromFileName(
            automatonName + std::wstring(L"SerialisedAutomaton.aut"));
    try {
...
void AutomatonInitialiser::cacheAutomaton(const std::wstring &automatonName, RunAutomaton *automaton,
...
//ORIGINAL LINE: throw new RuntimeException("Java VM is broken; UTF-8 should be supported", e);
        ...
        automatonOutputStream = resourceGetter->getOutputStream(
                automatonName + std::wstring(L"SerialisedAutomaton.aut"));
