...
void
StructureBuildingMethods::resolveWordOrBracket(BuildState *state, Element *word) throw(StructureBuildingException) {
    if (word->getName() == XmlDeclarations::WORDRULE_EL) { //already been resolved
        return;
    }
    ...
    std::vector<Element *> subsBracketsAndRoots = OpsinTools::getDescendantElementsWithTagNames(word,
    ...
    for (auto subsBracketsAndRoot : subsBracketsAndRoots) {
        if (subsBracketsAndRoot->getAttribute(XmlDeclarations::MULTIPLIER_ATR) != nullptr) {
            throw StructureBuildingException(
                    std::wstring(L"Structure building problem: multiplier on :") + subsBracketsAndRoot->getName() +
                    std::wstring(L" was never used"));
        }
    }
    ...
    for (int i = 0; i < groups.size(); i++) {
        ...
        if (group->getAttribute(XmlDeclarations::RESOLVED_ATR) == nullptr && i != groups.size() - 1) {
            throw StructureBuildingException(
                    std::wstring(L"Structure building problem: Bond was not made from :") + group->getValue() +
                    std::wstring(L" but one should of been"));
        }
    }
}

void StructureBuildingMethods::recursivelyResolveLocantedFeatures(BuildState *state,
                                                                  Element *word) throw(StructureBuildingException) {
    if (word->getName() != XmlDeclarations::WORD_EL && word->getName() != XmlDeclarations::BRACKET_EL) {
        throw StructureBuildingException(L"A word or bracket is the expected input");
    }
    std::vector<Element *> subsBracketsAndRoots = OpsinTools::getChildElementsWithTagNames(word,
                                                                                           std::vector<std::wstring>{
                                                                                                   XmlDeclarations::BRACKET_EL,
                                                                                                   XmlDeclarations::SUBSTITUENT_EL,
                                                                                                   XmlDeclarations::ROOT_EL});
...
void StructureBuildingMethods::recursivelyResolveUnLocantedFeatures(BuildState *state,
                                                                    Element *word) throw(StructureBuildingException) {
    if (word->getName() != XmlDeclarations::WORD_EL && word->getName() != XmlDeclarations::BRACKET_EL) {
        throw StructureBuildingException(L"A word or bracket is the expected input");
    }
...
void StructureBuildingMethods::resolveRootOrSubstituentLocanted(BuildState *state,
                                                                Element *subOrRoot) throw(StructureBuildingException) {
    ...
    if (foundSomethingToSubstitute) {
        performAdditiveOperations(state,
                                  subOrRoot); //e.g. ethylenediimino, oxyethylene (operations where two outAtoms are used to produce the bond and no locant is required as groups)
        performLocantedSubstitutiveOperations(state, subOrRoot); //e.g. 2-methyltoluene
    }
}

void StructureBuildingMethods::resolveRootOrSubstituentUnLocanted(BuildState *state,
                                                                  Element *subOrRoot) throw(StructureBuildingException) {
...
void StructureBuildingMethods::performLocantedSubstitutiveOperations(BuildState *state,
    ...
    if (frag->getOutAtomCount() >= 1 && subBracketOrRoot->getAttribute(XmlDeclarations::LOCANT_ATR) != nullptr) {
        ...
        if (subBracketOrRoot->getAttribute(XmlDeclarations::MULTIPLIER_ATR) != nullptr) { //e.g. 1,2-diethyl
            ...
            if (parentFrag == nullptr) {
                throw StructureBuildingException(
                        std::wstring(L"Cannot find in scope fragment with atom with locant ") + locantString +
                        std::wstring(L"."));
            }
            ...
            if (groupToAttachTo->getAttribute(XmlDeclarations::ACCEPTSADDITIVEBONDS_ATR) != nullptr &&
                parentFrag->getOutAtomCount() > 0 &&
                groupToAttachTo->getAttribute(XmlDeclarations::ISAMULTIRADICAL_ATR) != nullptr &&
                ...
                if (XmlDeclarations::PHOSPHO_SUBTYPE_VAL == group->getAttributeValue(XmlDeclarations::SUBTYPE_ATR) &&
                    frag->getOutAtom(0)->getValency() == 1) {
                    if (atomToSubstituteAt->getElement() != ChemEl::O) {
                        for (auto neighbour : atomToSubstituteAt->getAtomNeighbours()) {
                            if (neighbour->getElement() == ChemEl::O && neighbour->getBondCount() == 1 &&
                                neighbour->getFirstBond()->getOrder() == 1 && neighbour->getOutValency() == 0 &&
...
void StructureBuildingMethods::performUnLocantedSubstitutiveOperations(BuildState *state,
                                                                       Element *subBracketOrRoot) throw(StructureBuildingException) {
    Element *group;
    if (subBracketOrRoot->getName() == XmlDeclarations::BRACKET_EL) {
        group = findRightMostGroupInBracket(subBracketOrRoot);
...
//ORIGINAL LINE: throw new RuntimeException("Substituent has an unused outAtom and has a locant but locanted substitution should already have been performed!");
        ...
        if (subBracketOrRoot->getAttribute(XmlDeclarations::MULTIPLIER_ATR) != nullptr) { //e.g. diethyl
            multiplyOutAndSubstitute(state, subBracketOrRoot);
        } else {
            if (XmlDeclarations::PERHALOGENO_SUBTYPE_VAL == group->getAttributeValue(XmlDeclarations::SUBTYPE_ATR)) {
                ...
                if (atomsToJoinTo.empty()) {
                    throw StructureBuildingException(std::wstring(
                            L"Unlocanted substitution failed: unable to find suitable atom to bond atom with id:") +
                                                     frag->getOutAtom(0)->getAtom()->getID() + std::wstring(L" to!"));
                }
                if (AmbiguityChecker::isSubstitutionAmbiguous(atomsToJoinTo, 1)) {
                    state->addIsAmbiguous(std::wstring(L"Connection of ") + group->getValue() + std::wstring(L" to ") +
                                          atomsToJoinTo[0]->getFrag().getTokenEl().getValue());
                }
...
void StructureBuildingMethods::performPerHalogenoSubstitution(BuildState *state, Fragment *perhalogenFrag,
                                                              Element *subBracketOrRoot) throw(StructureBuildingException) {
...
void StructureBuildingMethods::multiplyOutAndSubstitute(BuildState *state,
                                                        Element *subOrBracket) throw(StructureBuildingException) {
...
//ORIGINAL LINE: throw new RuntimeException("Multiplier not found where multiplier expected");
    ...
    for (int i = multiplier - 1; i >= 0; i--) {
        ...
        if (i != 0) {
            ...
            addPrimesToLocantedStereochemistryElements(currentElement, StringTools::multiplyString(L"'",
                                                                                                   i)); //Stereochemistry elements with locants will need to have their locants primed (stereochemistry is only processed after structure building)
        } else {
    ...
    for (auto multipliedElement : multipliedElements) { //attach all the multiplied subs/brackets
        parentWordOrBracket->insertChild(multipliedElement, indexOfSubOrBracket);
    }
    for (auto el : elementsNotToBeMultiplied) { //re-add anything before multiplier to original subOrBracket
...
//ORIGINAL LINE: throw new RuntimeException("Substituent has an unused outAtom and has a locant but locanted substitution should already been been performed!");
        ...
        if (atomsToJoinTo.empty()) {
            atomsToJoinTo = findAtomsForSubstitution(subBracketOrRoot, numOfSubstituents,
                                                     frag->getOutAtom(0)->getValency());
        }
        if (atomsToJoinTo.empty()) {
            throw StructureBuildingException(std::wstring(
                    L"Unlocanted substitution failed: unable to find suitable atom to bond atom with id:") +
        ...
        if (AmbiguityChecker::isSubstitutionAmbiguous(atomsToJoinTo, numOfSubstituents)) {
            state->addIsAmbiguous(std::wstring(L"Connection of ") + group->getValue() + std::wstring(L" to ") +
                                  atomsToJoinTo[0]->getFrag().getTokenEl().getValue());
            std::vector<Atom *> atomsPreferredByEnvironment = AmbiguityChecker::useAtomEnvironmentsToGivePlausibleSubstitution(
                    atomsToJoinTo, numOfSubstituents);
...
void StructureBuildingMethods::resolveLocantedFeatures(BuildState *state,
                                                       Element *subOrRoot) throw(StructureBuildingException) {
    std::vector<Element *> groups = subOrRoot->getChildElements(XmlDeclarations::GROUP_EL);
    if (groups.size() != 1) {
        throw StructureBuildingException(
                L"Each sub or root should only have one group element. This indicates a bug in OPSIN");
    }
    ...
    for (int i = subtractivePrefixElements.size() - 1; i >= 0; i--) {
        ...
        if (type == XmlDeclarations::DEOXY_TYPE_VAL) {
            ...
            ChemEl chemEl = StringHelper::fromString<ChemEl>(
                    subtractivePrefix->getAttributeValue(XmlDeclarations::VALUE_ATR));
            if (locant == L"") {
            ...
            if (locant != L"") {
                ...
                throw StructureBuildingException(
                        L"locants are assumed to be required for the use of dehydro to be unambiguous");
            }
        } else {
    ...
    if (atomsToDehydro.size() > 0) {
        ...
        if (group->getAttributeValue(XmlDeclarations::TYPE_ATR) == XmlDeclarations::CARBOHYDRATE_TYPE_VAL) {
            ...
            if (uniquifiedDehydroAtoms->size() ==
                atomsToDehydro.size()) { //need to rule out case where dehydro is being used to form triple bonds on carbohydrates
                isCarbohydrateDehydro = true;
            }
        }
        if (isCarbohydrateDehydro) {
            for (auto a : atomsToDehydro) {
                std::vector<Atom *> hydroxyAtoms = FragmentTools::findHydroxyLikeTerminalAtoms(a->getAtomNeighbours(),
                                                                                               ChemEl::O);
                if (hydroxyAtoms.size() > 0) {
                    hydroxyAtoms[0]->getFirstBond().setOrder(2);
                } else {
                    throw StructureBuildingException(std::wstring(L"atom with locant ") + a->getFirstLocant() +
                                                     std::wstring(
                                                             L" did not have a hydroxy group to convert to a ketose"));
                }
            }
        } else {
            ...
            for (auto atom : atomsToFormDoubleBonds) { //check that all the dehydro-ed atoms are next to another atom with spare valency
                ...
                if (!hasSpareValency) {
                    throw StructureBuildingException(
                            L"Unexpected use of dehydro; two adjacent atoms were not unsaturated such as to form a double bond");
                }
            }
        ...
        if (locant != L"") {
            ...
            if (a->hasSpareValency()) {
                ...
                if (!acdNameSpiroIndicatedHydrogenBug(group, locant)) {
                    throw StructureBuildingException(std::wstring(L"hydrogen addition at locant: ") + locant +
                                                     std::wstring(L" was requested, but this atom is not unsaturated"));
                }
            }
...
//JAVA TO C++ CONVERTER TODO TASK: The Java Collection 'remove(Object)' method is not converted:
            ...
            if (matcher->find()) {
                ...
                FragmentTools::unsaturate(thisFrag->getAtomByLocantOrThrow(locant), compoundLocant, bondOrder,
                                          thisFrag);
            } else {
    ...
    for (int i = heteroatoms.size() - 1; i >= 0; i--) {
        ...
        if (locant != L"") {
            Atom *heteroatom = state->fragManager->getHeteroatom(
                    heteroatomEl->getAttributeValue(XmlDeclarations::VALUE_ATR));
            Atom *atomToBeReplaced = thisFrag->getAtomByLocantOrThrow(locant);
            if (heteroatom->getElement() == atomToBeReplaced->getElement() &&
                heteroatom->getCharge() == atomToBeReplaced->getCharge()) {
                throw StructureBuildingException(std::wstring(L"The replacement term ") + heteroatomEl->getValue() +
                                                 std::wstring(L" was used on an atom that already is a ") +
                                                 heteroatom->getElement());
            }
            ...
            if (heteroatomEl->getAttribute(XmlDeclarations::LAMBDA_ATR) != nullptr) {
                thisFrag->getAtomByLocantOrThrow(locant)->setLambdaConventionValency(
                        std::stoi(heteroatomEl->getAttributeValue(XmlDeclarations::LAMBDA_ATR)));
            }
//JAVA TO C++ CONVERTER TODO TASK: The Java Collection 'remove(Object)' method is not converted:
...
bool StructureBuildingMethods::acdNameSpiroIndicatedHydrogenBug(Element *group,
                                                                const std::wstring &indicatedHydrogenLocant) {
    if (boost::starts_with(group->getValue(), L"spiro")) {
        for (auto suffix : group->getParent()->getChildElements(XmlDeclarations::SUFFIX_EL)) {
            ...
            if (suffixLocant != L"" && suffixLocant == indicatedHydrogenLocant) {
                LOG->debug(std::wstring(L"Indicated hydrogen at ") + indicatedHydrogenLocant +
                           std::wstring(L" ignored. Known bug in generated IUPAC name"));
                return true;
            }
        }
    }
...
void StructureBuildingMethods::applySubtractivePrefix(BuildState *state, Fragment *fragment, ChemEl chemEl,
                                                      const std::wstring &locant) throw(StructureBuildingException) {
    Atom *adjacentAtom = fragment->getAtomByLocantOrThrow(locant);
    std::vector<Atom *> applicableTerminalAtoms = FragmentTools::findHydroxyLikeTerminalAtoms(
            adjacentAtom->getAtomNeighbours(), chemEl);
    if (applicableTerminalAtoms.empty()) {
        throw StructureBuildingException(
                std::wstring(L"Unable to find terminal atom of type: ") + chemEl + std::wstring(L" at locant ") +
                locant + std::wstring(L" for subtractive nomenclature"));
    }
    Atom *atomToRemove = applicableTerminalAtoms[0];
    if (FragmentTools::isFunctionalAtom(
            atomToRemove)) { //This can occur with aminoglycosides where the anomeric OH is removed by deoxy
...
void StructureBuildingMethods::applyUnlocantedSubtractivePrefixes(BuildState *state, Fragment *fragment, ChemEl chemEl,
                                                                  int count) throw(StructureBuildingException) {
    std::vector<Atom *> applicableTerminalAtoms = FragmentTools::findHydroxyLikeTerminalAtoms(fragment->getAtomList(),
                                                                                              chemEl);
    if (applicableTerminalAtoms.empty() || applicableTerminalAtoms.size() < count) {
        throw StructureBuildingException(std::wstring(L"Unable to find terminal atom of type: ") + chemEl +
                                         std::wstring(L" for subtractive nomenclature"));
    }
...
void StructureBuildingMethods::applyAnhydroPrefix(BuildState *state, Fragment *frag,
                                                  Element *subtractivePrefix) throw(StructureBuildingException) {
...
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
    ...
    std::vector<Atom *> applicableTerminalAtoms = FragmentTools::findHydroxyLikeTerminalAtoms(
            backBoneAtom1->getAtomNeighbours(), chemEl);
    if (applicableTerminalAtoms.empty()) {
        throw StructureBuildingException(std::wstring(L"Unable to find terminal atom of type: ") + chemEl +
                                         std::wstring(L" for subtractive nomenclature"));
    }
...
void StructureBuildingMethods::addDehydroInducedTripleBonds(
        std::vector<Atom *> &atomsToFormTripleBondsBetween) throw(StructureBuildingException) {
    if (atomsToFormTripleBondsBetween.size() > 0) {
        ...
        for (int i = atomsToFormTripleBondsBetween.size() - 1;
             i >= 0; i = i - 2) { //two atoms will have a triple bond formed betwen them
            Atom *a = atomsToFormTripleBondsBetween[i];
            std::vector<Atom *> neighbours = a->getAtomNeighbours();
            for (auto neighbour : neighbours) {
                if (std::find(atomsToFormTripleBondsBetween.begin(), atomsToFormTripleBondsBetween.end(), neighbour) !=
                    atomsToFormTripleBondsBetween.end()) {
...
//JAVA TO C++ CONVERTER TODO TASK: The Java Collection 'remove(Object)' method is not converted:
            ...
            throw StructureBuildingException(
                    L"dehydro indicated atom should form a triple bond but no adjacent atoms also had hydrogen removed!");
            atomLoopContinue:
        }
        atomLoopBreak:
    }
...
void StructureBuildingMethods::resolveUnLocantedFeatures(BuildState *state,
                                                         Element *subOrRoot) throw(StructureBuildingException) {
    std::vector<Element *> groups = subOrRoot->getChildElements(XmlDeclarations::GROUP_EL);
    if (groups.size() != 1) {
        throw StructureBuildingException(
    ...
    for (auto currentEl : children) {
        ...
        if (elName == XmlDeclarations::UNSATURATOR_EL) {
            ...
            heteroatoms.push_back(currentEl);
            currentEl->detach();
        } else if (elName == XmlDeclarations::HYDRO_EL || elName == XmlDeclarations::INDICATEDHYDROGEN_EL ||
                   elName == XmlDeclarations::ADDEDHYDROGEN_EL) {
            hydrogenElements.push_back(currentEl);
            currentEl->detach();
        } else if (elName == XmlDeclarations::ISOTOPESPECIFICATION_EL) {
            isotopeSpecifications.push_back(currentEl);
    ...
    if (frag->getOutAtomCount() >
        0) { //assign any outAtoms that have not been set to a specific atom to a specific atom
...
void StructureBuildingMethods::applyUnlocantedHydro(BuildState *state, Fragment *frag,
    ...
    for (auto hydrogenElement : hydrogenElements) {
        if (hydrogenElement->getValue().equals(L"perhydro")) {
            if (hydrogenElsCount != 1) {
                throw StructureBuildingException(
                        L"Unexpected indication of hydrogen when perhydro makes such indication redundnant");
            }
    ...
    prioritisedAtomsAcceptingHydro.insert(prioritisedAtomsAcceptingHydro.end(), otherAtomsThatCanHaveHydro.begin(),
                                          otherAtomsThatCanHaveHydro.end()); //these end up at the end of the list

    if (hydrogenElsCount > prioritisedAtomsAcceptingHydro.size()) {
        throw StructureBuildingException(std::wstring(L"Cannot find atom to add hydrogen to (") + hydrogenElsCount +
                                         std::wstring(L" hydrogens requested but only ") +
                                         prioritisedAtomsAcceptingHydro.size() +
                                         std::wstring(L" positions that can be hydrogenated)"));
    }
    ...
    if (svCountAfterRemoval > 1) { //ambiguity likely. If it's 1 then an atom will be implicitly hydrogenated
        ...
        if (!(AmbiguityChecker::allAtomsEquivalent(atomsWithDefiniteSV) &&
            ...
            state->addIsAmbiguous(std::wstring(L"Ambiguous choice of positions to add hydrogen to on ") +
                                  frag->getTokenEl()->getValue());
        }
    }
...
void StructureBuildingMethods::unsaturateBonds(BuildState *state, Fragment *frag,
...
void StructureBuildingMethods::unsaturateBonds(BuildState *state, Fragment *frag, int bondOrder,
    ...
    if (bondsThatCouldBeUnsaturated.size() < numToUnsaturate) {
        ...
        alternativeBondsThatCouldBeUnsaturated = findAlternativeBondsToUnSaturate(frag, bondOrder,
                                                                                  bondsThatCouldBeUnsaturated);
    }
    if (bondsThatCouldBeUnsaturated.size() < numToUnsaturate) {
        throw StructureBuildingException(
                std::wstring(L"Failed to find bond to change to a bond of order: ") + std::to_wstring(bondOrder));
    }
    if (bondsThatCouldBeUnsaturated.size() > numToUnsaturate) {
        ...
        if (!isCycloAlkaneSpecialCase(frag, numToUnsaturate, bondsThatCouldBeUnsaturated) &&
            XmlDeclarations::HANTZSCHWIDMAN_SUBTYPE_VAL != frag->getSubType()) {
            if (alternativeBondsThatCouldBeUnsaturated.size() >= numToUnsaturate) {
                std::vector<Bond *> allBonds(bondsThatCouldBeUnsaturated);
                allBonds.insert(allBonds.end(), alternativeBondsThatCouldBeUnsaturated.begin(),
                                alternativeBondsThatCouldBeUnsaturated.end());
...
bool StructureBuildingMethods::isCycloAlkaneSpecialCase(Fragment *frag, int numToUnsaturate,
                                                        std::vector<Bond *> &bondsThatCouldBeUnsaturated) {
    if (numToUnsaturate == 1) {
        Bond *b = bondsThatCouldBeUnsaturated[0];
        Atom *a1 = b->getFromAtom();
        Atom *a2 = b->getToAtom();
        if ((XmlDeclarations::ALKANESTEM_SUBTYPE_VAL == frag->getSubType() ||
             XmlDeclarations::HETEROSTEM_SUBTYPE_VAL == frag->getSubType()) && a1->getAtomIsInACycle() &&
            a2->getAtomIsInACycle() && (a1->equals(frag->getFirstAtom()) || a2->equals(frag->getFirstAtom()))) {
...
bool StructureBuildingMethods::isCycloAlkaneHeteroatomSpecialCase(Fragment *frag, int numHeteroatoms,
                                                                  std::vector<Atom *> &atomsThatCouldBeReplaced) {
    if (numHeteroatoms == 1) {
        if ((XmlDeclarations::ALKANESTEM_SUBTYPE_VAL == frag->getSubType() ||
             XmlDeclarations::HETEROSTEM_SUBTYPE_VAL == frag->getSubType()) &&
            frag->getFirstAtom()->getAtomIsInACycle() && atomsThatCouldBeReplaced[0]->equals(frag->getFirstAtom())) {
...
StructureBuildingMethods::HeteroAtomSmilesAndLambda::HeteroAtomSmilesAndLambda(const std::wstring &smiles,
                                                                               const std::wstring &lambdaConvention)
        : smiles(smiles), lambdaConvention(lambdaConvention) {
}
...
void StructureBuildingMethods::applyUnlocantedHeteroatoms(BuildState *state, Fragment *frag,
    ...
    for (auto entry : heteroatomDescriptionToCount) {
        ...
        for (auto atom : atomlist) {
            ...
            if (atom->getElement() != ChemEl::C && heteroatomChemEl != ChemEl::C) {
                if (atom->getElement() == ChemEl::O &&
                    (heteroatomChemEl == ChemEl::S || heteroatomChemEl == ChemEl::Se ||
                     heteroatomChemEl == ChemEl::Te)) {
        ...
        if (atomsThatCouldBeReplaced.size() > replacementsRequired &&
            !isCycloAlkaneHeteroatomSpecialCase(frag, replacementsRequired, atomsThatCouldBeReplaced)) {
...
void StructureBuildingMethods::applyIsotopeSpecifications(BuildState *state, Fragment *frag,
                                                          std::vector<Element *> &isotopeSpecifications,
...
//JAVA TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'toString':
            ...
            if (chemEl == ChemEl::H) {
                std::vector<Atom *> parentAtomsToApplyTo = FragmentTools::findnAtomsForSubstitution(frag, multiplier,
                                                                                                    1);
...
bool StructureBuildingMethods::casIsotopeAmbiguitySpecialCase(Fragment *frag, std::vector<Atom *> &parentAtomsToApplyTo,
...
Atom *StructureBuildingMethods::findAtomForUnlocantedRadical(BuildState *state, Fragment *frag,
                                                             OutAtom *outAtom) throw(StructureBuildingException) {
    std::vector<Atom *> possibleAtoms = FragmentTools::findnAtomsForSubstitution(frag, outAtom->getAtom(), 1,
                                                                                 outAtom->getValency(), true);
    if (possibleAtoms.empty()) {
        throw StructureBuildingException(
                L"Failed to assign all unlocanted radicals to actual atoms without violating valency");
    }
    if (!((XmlDeclarations::ALKANESTEM_SUBTYPE_VAL == frag->getSubType() ||
           XmlDeclarations::HETEROSTEM_SUBTYPE_VAL == frag->getSubType()) &&
          possibleAtoms[0]->equals(frag->getFirstAtom()))) {
...
std::vector<Bond *> StructureBuildingMethods::findAlternativeBondsToUnSaturate(Fragment *frag, int bondOrder,
                                                                               Collection<Bond *> *bondsToIgnore) {
...
StructureBuildingMethods::findBondsToUnSaturate(Fragment *frag, int bondOrder, bool allowAdjacentUnsaturatedBonds,
    ...
    for (auto atom1 : frag->getAtomList()) {
        if (atom1->hasSpareValency() || XmlDeclarations::SUFFIX_TYPE_VAL == atom1->getType() ||
            atom1->getProperty(Atom::ISALDEHYDE) != nullptr) {
            continue;
        }
        ...
        for (auto bond : bonds) {
            if (bond->getOrder() == 1 &&
                !std::find(bondsToUnsaturate.begin(), bondsToUnsaturate.end(), bond) != bondsToUnsaturate.end() &&
                !bondsToIgnore->contains(bond)) {
                Atom *atom2 = bond->getOtherAtom(atom1);
                if (frag->getAtomByID(atom2->getID()) != nullptr) { //check other atom is actually in the fragment!
                    if (atom2->hasSpareValency() || XmlDeclarations::SUFFIX_TYPE_VAL == atom2->getType() ||
                        atom2->getProperty(Atom::ISALDEHYDE) != nullptr) {
                        continue;
                    }
                    ...
                    for (auto bond2 : atom2->getBonds()) {
                        ...
                        if (std::find(bondsToUnsaturate.begin(), bondsToUnsaturate.end(), bond2) !=
                            bondsToUnsaturate.end()) {
                            if (!allowAdjacentUnsaturatedBonds) {
                                goto bondLoopContinue;
                            }
...
void StructureBuildingMethods::performAdditiveOperations(BuildState *state,
                                                         Element *subBracketOrRoot) throw(StructureBuildingException) {
    if (subBracketOrRoot->getAttribute(XmlDeclarations::LOCANT_ATR) !=
        nullptr) { //additive nomenclature does not employ locants
        return;
    }
    Element *group;
    if (subBracketOrRoot->getName() == XmlDeclarations::BRACKET_EL) {
    ...
    if (outAtomCount >= 1) {
        if (subBracketOrRoot->getAttribute(XmlDeclarations::MULTIPLIER_ATR) == nullptr) {
            ...
            if (nextSiblingEl->getAttribute(XmlDeclarations::MULTIPLIER_ATR) != nullptr &&
                (outAtomCount >= std::stoi(nextSiblingEl->getAttributeValue(XmlDeclarations::MULTIPLIER_ATR)) ||
                 outAtomCount == 1 && frag->getOutAtom(0)->getValency() ==
                                      std::stoi(nextSiblingEl->getAttributeValue(XmlDeclarations::MULTIPLIER_ATR))) &&
                hasRootLikeOrMultiRadicalGroup(
                        nextSiblingEl)) { //probably multiplicative nomenclature, should be as many outAtoms as the multiplier
            ...
            } else if (group->getAttribute(XmlDeclarations::ISAMULTIRADICAL_ATR) !=
                       nullptr) { //additive nomenclature e.g. ethyleneoxy
                ...
                if (nextFrag != nullptr) {
                    ...
                    if (state->currentWordRule !=
                        WordRule::polymer) { //imino does not behave like a substituent in polymers only as a linker
                        if (nextMultiRadicalGroup->getAttribute(XmlDeclarations::IMINOLIKE_ATR) !=
                            nullptr) { //imino/methylene can just act as normal substituents, should an additive bond really be made???
                            ...
                            if (nextFrag != adjacentFrag) { //imino is not the absolute next frag
                                if (potentiallyCanSubstitute(nextMultiRadicalGroup->getParent()) ||
                                    potentiallyCanSubstitute(nextMultiRadicalGroup->getParent()->getParent())) {
                                    return;
                                }
                            }
                        }
                        if (group->getAttribute(XmlDeclarations::IMINOLIKE_ATR) != nullptr &&
                            levelsToWordEl(group) > levelsToWordEl(nextMultiRadicalGroup)) {
                ...
                if (siblingFragments.size() > 0) {
                    ...
                    if (nextGroup->getAttribute(XmlDeclarations::ACCEPTSADDITIVEBONDS_ATR) != nullptr &&
                        nextGroup->getAttribute(XmlDeclarations::ISAMULTIRADICAL_ATR) != nullptr &&
                        (nextFrag->getOutAtomCount() > 1 ||
                         nextGroup->getAttribute(XmlDeclarations::RESOLVED_ATR) != nullptr &&
                         nextFrag->getOutAtomCount() >= 1)) {
                    ...
                    if (group->getAttribute(XmlDeclarations::RESOLVED_ATR) == nullptr && siblingFragments.size() > 1) {
                        for (int i = 0; i < siblingFragments.size() - 1; i++) {
                            ...
                            if (lastGroup->getAttribute(XmlDeclarations::ACCEPTSADDITIVEBONDS_ATR) != nullptr &&
                                lastGroup->getAttribute(XmlDeclarations::ISAMULTIRADICAL_ATR) != nullptr &&
                                (lastFrag->getOutAtomCount() > 1 ||
                                 lastGroup->getAttribute(XmlDeclarations::RESOLVED_ATR) != nullptr &&
                                 lastFrag->getOutAtomCount() >= 1)) {
                            ...
                            if (FragmentTools::findSubstituableAtoms(lastFrag, frag->getOutAtom(
                                    outAtomCount - 1)->getValency()).size() > 0) {
                                break;
                            }
                        }
                    }
            ...
            if (siblingFragments.size() > 0) {
                ...
                if (nextGroup->getAttribute(XmlDeclarations::ACCEPTSADDITIVEBONDS_ATR) != nullptr &&
                    nextGroup->getAttribute(XmlDeclarations::ISAMULTIRADICAL_ATR) != nullptr &&
                    (nextFrag->getOutAtomCount() >= multiplier ||
                     nextGroup->getAttribute(XmlDeclarations::RESOLVED_ATR) != nullptr &&
                     nextFrag->getOutAtomCount() >= multiplier + 1)) {
                    Atom *toAtom = nextFrag->getOutAtom(0)->getAtom();
                    if (calculateSubstitutableHydrogenAtoms(toAtom) == 0) {
                ...
                if (group->getAttribute(XmlDeclarations::RESOLVED_ATR) == nullptr && siblingFragments.size() > 1) {
                    for (int i = 0; i < siblingFragments.size() - 1; i++) {
                        ...
                        if (lastGroup->getAttribute(XmlDeclarations::ACCEPTSADDITIVEBONDS_ATR) != nullptr &&
                            lastGroup->getAttribute(XmlDeclarations::ISAMULTIRADICAL_ATR) != nullptr &&
                            (lastFrag->getOutAtomCount() >= multiplier ||
                             lastGroup->getAttribute(XmlDeclarations::RESOLVED_ATR) != nullptr &&
                             lastFrag->getOutAtomCount() >= multiplier + 1)) {
                            Atom *toAtom = lastFrag->getOutAtom(0)->getAtom();
                            if (calculateSubstitutableHydrogenAtoms(toAtom) == 0) {
                                Attribute tempVar5(XmlDeclarations::RESOLVED_ATR, L"yes");
                                group->addAttribute(&tempVar5);
                                multiplyOutAndAdditivelyBond(state, subBracketOrRoot,
                                                             lastFrag); //e.g. dihydroxyphosphoryloxy
                            }
                            break;
                        }
...
bool StructureBuildingMethods::hasRootLikeOrMultiRadicalGroup(Element *subBracketOrRoot) {
    std::vector<Element *> groups = OpsinTools::getDescendantElementsWithTagName(subBracketOrRoot,
...
void StructureBuildingMethods::multiplyOutAndAdditivelyBond(BuildState *state, Element *subOrBracket,
    ...
    for (int i = multiplier - 1; i >= 0; i--) {
        ...
        if (frag->getOutAtomCount() != 1) {
            throw StructureBuildingException(std::wstring(
                    L"Additive bond formation failure: Fragment expected to have one OutAtom in this case but had: ") +
                                             std::to_wstring(frag->getOutAtomCount()));
        }
        joinFragmentsAdditively(state, frag, fragToAdditivelyBondTo);
    }
    for (auto clone : clonedElements) { //make sure cloned substituents don't substitute onto each other!
...
void StructureBuildingMethods::performMultiplicativeOperations(BuildState *state, Element *group,
...
void StructureBuildingMethods::performMultiplicativeOperations(BuildState *state, BuildResults *multiRadicalBR,
                                                               Element *multipliedParent) throw(StructureBuildingException) {
    ...
    if (multiplier != multiRadicalBR->getOutAtomCount()) {
        ...
        if (multiplier != multiRadicalBR->getOutAtomCount()) {
            throw StructureBuildingException(std::wstring(
                    L"Multiplication bond formation failure: number of outAtoms disagree with multiplier(multiplier: ") +
                                             multiplier + std::wstring(L", outAtom count: ") +
                                             multiRadicalBR->getOutAtomCount() + std::wstring(L")"));
        }
    }
    ...
    if (inLocantsString != L"") { //true for the root of a multiplicative name
...
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
            ...
            if (inLocants.size() != multiplier) {
                throw StructureBuildingException(
                        L"Mismatch between multiplier and number of inLocants in multiplicative nomenclature");
            }
        }
    }
    std::vector<Element *> clonedElements;
    ...
    for (int i = multiplier - 1; i >= 0; i--) {
        ...
        if (i != 0) {
            ...
            addPrimesToLocantedStereochemistryElements(multipliedElement, StringTools::multiplyString(L"'",
                                                                                                      i)); //Stereochemistry elements with locants will need to have their locants primed (stereochemistry is only processed after structure building)
        ...
        if (multipliedElement->getName() == XmlDeclarations::BRACKET_EL) {
            ...
            if (multipliedGroup == nullptr) { //root will not have a multivalent group
                std::vector<Element *> groups = OpsinTools::getDescendantElementsWithTagName(multipliedElement,
                                                                                             XmlDeclarations::GROUP_EL);
                if (inLocants.empty()) {
                    throw StructureBuildingException(
                            L"OPSIN Bug? in locants must be specified for a multiplied root in multiplicative nomenclature");
                }
...
//JAVA TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'toString':
                    throw StructureBuildingException(std::wstring(
                            L"Locants for inAtoms on the root were either misassigned to the root or were invalid: ") +
                                                     inLocants.toString() + std::wstring(L" could not be assigned!"));
                }
            }
        } else {
        ...
        if (multiRadicalGroup->getAttribute(XmlDeclarations::RESOLVED_ATR) == nullptr) {
            resolveUnLocantedFeatures(state,
                                      multiRadicalGroup->getParent()); //the addition of unlocanted unsaturators can effect the position of radicals e.g. diazenyl
            Attribute tempVar(XmlDeclarations::RESOLVED_ATR, L"yes");
        ...
        if (inLocants.size() > 0) {
            ...
            Attribute tempVar2(XmlDeclarations::RESOLVED_ATR, L"yes");
            rightMostGroup->addAttribute(
                    &tempVar2); //this group will not be used further within this word but can in principle be a substituent e.g. methylenedisulfonyl dichloride
            if (multipliedGroup->getAttribute(XmlDeclarations::ISAMULTIRADICAL_ATR) !=
                nullptr) { //e.g. methylenedisulfonyl dichloride
                if (multipliedParent->getAttributeValue(XmlDeclarations::INLOCANTS_ATR) !=
                    XmlDeclarations::INLOCANTS_DEFAULT) {
                    throw StructureBuildingException(
                            L"inLocants should not be specified for a multiradical parent in multiplicative nomenclature");
                }
            } else {
                ...
                for (int j = inLocants.size() - 1; j >= 0; j--) {
                    ...
                    if (locant ==
                        XmlDeclarations::INLOCANTS_DEFAULT) { //note that if one entry in inLocantArray is default then they all are "default"
                        std::vector<Atom *> possibleAtoms = getPossibleAtomsForUnlocantedConnectionToMultipliedRoot(
                                multipliedGroup, bondOrder, i);
                        if (possibleAtoms.empty()) {
                            throw StructureBuildingException(L"No suitable atom found for multiplicative operation");
                        }
                        if (AmbiguityChecker::isSubstitutionAmbiguous(possibleAtoms, 1)) {
                            state->addIsAmbiguous(
                                    std::wstring(L"Connection to multiplied group: ") + multipliedGroup->getValue());
                        }
...
//JAVA TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'toString':
                ...
                if (LOG->isTraceEnabled()) {
                    LOG->trace(std::wstring(L"Substitutively bonded (multiplicative to root) ") + from->getID() +
                               std::wstring(L" (") + from->getFrag()->getTokenEl()->getValue() + std::wstring(L") ") +
                               atomToJoinTo->getID() + std::wstring(L" (") +
                               atomToJoinTo->getFrag()->getTokenEl()->getValue() + std::wstring(L")"));
                }
        ...
        if (multipliedElement->getName() == XmlDeclarations::BRACKET_EL) {
            recursivelyResolveUnLocantedFeatures(state,
                                                 multipliedElement); //there may be outAtoms that are involved in unlocanted substitution, these can be safely used now e.g. ...bis((3-hydroxy-4-methoxyphenyl)methylene) where (3-hydroxy-4-methoxyphenyl)methylene is the currentElement
        }
    ...
    if (newBr->getFragmentCount() == 1) {
        throw StructureBuildingException(
                L"Multiplicative nomenclature cannot yield only one temporary terminal fragment");
    }
    if (newBr->getFragmentCount() >= 2) {
        std::vector<Element *> siblings = OpsinTools::getNextSiblingsOfTypes(multipliedParent,
                                                                             std::vector<std::wstring>{
                                                                                     XmlDeclarations::SUBSTITUENT_EL,
                                                                                     XmlDeclarations::BRACKET_EL,
                                                                                     XmlDeclarations::ROOT_EL});
        if (siblings.empty()) {
            ...
            if (parentOfMultipliedEl->getName() == XmlDeclarations::BRACKET_EL) { //brackets are allowed
                siblings = OpsinTools::getNextSiblingsOfTypes(parentOfMultipliedEl,
                                                              std::vector<std::wstring>{XmlDeclarations::SUBSTITUENT_EL,
                                                                                        XmlDeclarations::BRACKET_EL,
                                                                                        XmlDeclarations::ROOT_EL});
                if (siblings[0]->getAttribute(XmlDeclarations::MULTIPLIER_ATR) == nullptr) {
                    throw StructureBuildingException(
                            L"Multiplier not found where multiplier was expected for succesful multiplicative nomenclature");
                }
                performMultiplicativeOperations(state, newBr, siblings[0]);
            } else {
                throw StructureBuildingException(
                        L"Could not find suitable element to continue multiplicative nomenclature");
            }
        } else {
            if (siblings[0]->getAttribute(XmlDeclarations::MULTIPLIER_ATR) == nullptr) {
                throw StructureBuildingException(
                        L"Multiplier not found where multiplier was expected for successful multiplicative nomenclature");
            }
            performMultiplicativeOperations(state, newBr, siblings[0]);
        }
    }

    for (auto clone : clonedElements) { //only insert cloned substituents now so they don't substitute onto each other!
        OpsinTools::insertAfter(multipliedParent, clone);
    }
}

std::vector<Atom *>
StructureBuildingMethods::getPossibleAtomsForUnlocantedConnectionToMultipliedRoot(Element *multipliedGroup,
                                                                                  int bondOrder,
                                                                                  int primesAdded) throw(StructureBuildingException) {
    Fragment *multipliedFrag = multipliedGroup->getFrag();
    if ((std::wstring(L"yes")) == multipliedGroup->getAttributeValue(XmlDeclarations::USABLEASJOINER_ATR) &&
        multipliedFrag->getDefaultInAtom() == nullptr) {
        ...
        if (previous != nullptr && previous->getName() == XmlDeclarations::MULTIPLIER_EL) {
            ...
            if (locant != L"") {
                ...
                std::vector<Atom *> possibleAtoms = FragmentTools::findnAtomsForSubstitution(
                        multipliedFrag->getAtomList(), preferredAtom, 1, bondOrder, true);
                if (possibleAtoms.empty()) {
...
Fragment *StructureBuildingMethods::getNextInScopeMultiValentFragment(
        Element *substituentOrBracket) throw(StructureBuildingException) {
    if (substituentOrBracket->getName() != XmlDeclarations::SUBSTITUENT_EL &&
        substituentOrBracket->getName() != XmlDeclarations::BRACKET_EL) {
    ...
    std::vector<Element *> children = OpsinTools::getChildElementsWithTagNames(parent, std::vector<std::wstring>{
            XmlDeclarations::SUBSTITUENT_EL, XmlDeclarations::BRACKET_EL,
            XmlDeclarations::ROOT_EL}); //will be returned in index order
    int indexOfSubstituent = parent->find(substituentOrBracket);
    for (auto child : children) {
        if (parent->find(child) <= indexOfSubstituent) { //only want things after the input
            continue;
        }
        ...
        if (child->getName().equals(XmlDeclarations::BRACKET_EL)) {
            childDescendants = OpsinTools::getDescendantElementsWithTagNames(child, std::vector<std::wstring>{
                    XmlDeclarations::SUBSTITUENT_EL, XmlDeclarations::ROOT_EL}); //will be returned in depth-first order
        } else {
        ...
        for (auto descendantChild : childDescendants) {
            ...
            if (group->getAttribute(XmlDeclarations::ISAMULTIRADICAL_ATR) != nullptr &&
                (possibleFrag->getOutAtomCount() >= 2 || (possibleFrag->getOutAtomCount() >= 1 &&
                                                          group->getAttribute(XmlDeclarations::RESOLVED_ATR) !=
                                                          nullptr))) {
                return possibleFrag;
            }
        }
...
Element *StructureBuildingMethods::getFirstMultiValentGroup(Element *bracket) throw(StructureBuildingException) {
    ...
    std::vector<Element *> groups = OpsinTools::getDescendantElementsWithTagName(bracket,
                                                                                 XmlDeclarations::GROUP_EL); //will be returned in index order
    for (auto group : groups) {
        Fragment *possibleFrag = group->getFrag();
        if (group->getAttribute(XmlDeclarations::ISAMULTIRADICAL_ATR) != nullptr &&
            (possibleFrag->getOutAtomCount() >= 2 ||
             (possibleFrag->getOutAtomCount() >= 1 && group->getAttribute(XmlDeclarations::RESOLVED_ATR) != nullptr))) {
            return group;
        }
    }
    return nullptr;
...
void StructureBuildingMethods::joinFragmentsAdditively(BuildState *state, Fragment *fragToBeJoined,
    ...
    if (XmlDeclarations::EPOXYLIKE_SUBTYPE_VAL == elOfFragToBeJoined->getAttributeValue(XmlDeclarations::SUBTYPE_ATR)) {
        for (int i = 0, l = fragToBeJoined->getOutAtomCount(); i < l; i++) {
            ...
            if (outAtom->getLocant() != L"") {
                throw StructureBuildingException(
                        std::wstring(L"Inappropriate use of ") + elOfFragToBeJoined->getValue());
            }
        }
    }
    ...
    if (outAtomCountOnFragToBeJoined == 0) {
        throw StructureBuildingException(
                L"Additive bond formation failure: Fragment expected to have at least one OutAtom but had none");
    }
    ...
    if (parentFrag->getOutAtomCount() > 1) {
        ...
        if (unresolvedAmbiguity) { //not all outAtoms on parent equivalent
            ...
            if (unresolvedAmbiguity &&
                outAtomCountOnFragToBeJoined == 2) { //not all outAtoms on frag to be joined are equivalent either!
                ...
                if (previousGroup != nullptr) {
                    ...
                    if (previousFrag->getOutAtomCount() > 1) {
                        ...
                        if (!unresolvedAmbiguity &&
                            previousGroupFirstOutAtomOrder == parentFrag->getOutAtom(0)->getValency()) {
                            for (int i = 1, l = parentFrag->getOutAtomCount(); i < l; i++) {
                                OutAtom *outAtom = parentFrag->getOutAtom(i);
                                if (outAtom->getValency() != previousGroupFirstOutAtomOrder) {
                                    in_Renamed = outAtom;
    ...
    if (!in_Renamed->isSetExplicitly()) { //not set explicitly so may be an inappropriate atom
    ...
    if (out == nullptr) {
        if (outAtomCountOnFragToBeJoined >=
            bondOrder) { //handles cases like nitrilo needing to be -N= (remove later outAtoms first as per usual)
            int valency = 0;
    ...
    if (LOG->isTraceEnabled()) {
        LOG->trace(std::wstring(L"Additively bonded ") + from->getID() + std::wstring(L" (") +
                   from->getFrag()->getTokenEl()->getValue() + std::wstring(L") ") + to->getID() + std::wstring(L" (") +
                   to->getFrag()->getTokenEl()->getValue() + std::wstring(L")"));
    }
}

void StructureBuildingMethods::joinFragmentsSubstitutively(BuildState *state, Fragment *fragToBeJoined,
    ...
    if (outAtomCount > 1) {
        throw StructureBuildingException(
                std::wstring(L"Substitutive bond formation failure: Fragment expected to have one OutAtom but had: ") +
                std::to_wstring(outAtomCount));
    }
    if (outAtomCount == 0) {
        throw StructureBuildingException(
                L"Substitutive bond formation failure: Fragment expected to have one OutAtom but had none");
    }
    if (elOfFragToBeJoined->getAttribute(XmlDeclarations::IMINOLIKE_ATR) !=
        nullptr) { //special case for methylene/imino
    ...
    if (!out->isSetExplicitly()) { //not set explicitly so may be an inappropriate atom
        std::vector<Atom *> possibleAtoms = FragmentTools::findnAtomsForSubstitution(fragToBeJoined->getAtomList(),
                                                                                     from, 1, bondOrder, false);
        if (possibleAtoms.empty()) {
            throw StructureBuildingException(
                    L"Failed to assign all unlocanted radicals to actual atoms without violating valency");
        }
        if (!((XmlDeclarations::ALKANESTEM_SUBTYPE_VAL == fragToBeJoined->getSubType() ||
               XmlDeclarations::HETEROSTEM_SUBTYPE_VAL == fragToBeJoined->getSubType()) &&
              possibleAtoms[0]->equals(fragToBeJoined->getFirstAtom()))) {
            if (AmbiguityChecker::isSubstitutionAmbiguous(possibleAtoms, 1)) {
                state->addIsAmbiguous(
                        std::wstring(L"Positioning of radical on: ") + fragToBeJoined->getTokenEl()->getValue());
            }
        }
    ...
    state->fragManager->createBond(from, atomToJoinTo, bondOrder);
    if (LOG->isTraceEnabled()) {
        LOG->trace(std::wstring(L"Substitutively bonded ") + from->getID() + std::wstring(L" (") +
                   from->getFrag()->getTokenEl()->getValue() + std::wstring(L") ") + atomToJoinTo->getID() +
                   std::wstring(L" (") + atomToJoinTo->getFrag()->getTokenEl()->getValue() + std::wstring(L")"));
    }
}

std::vector<Atom *> StructureBuildingMethods::formEpoxide(BuildState *state, Fragment *bridgingFragment,
                                                          Atom *atomToJoinTo) throw(StructureBuildingException) {
    ...
    if (bridgingFragment->getOutAtom(0)->getLocant() != L"") {
        ...
        std::vector<Atom *> possibleSecondAtom = FragmentTools::findnAtomsForSubstitution(fragToJoinTo->getAtomList(),
                                                                                          preferredAtom, 1, 1, true);
...
std::vector<Atom *>
StructureBuildingMethods::findAtomsForSubstitution(Element *subOrBracket, int numberOfSubstitutions, int bondOrder) {
    ...
    for (int i = 0, l = possibleParents.size(); i < l; i++) {
        ...
        if (possibleParent->getParent()->getName() ==
            XmlDeclarations::ROOT_EL) { //consider all root groups as if they were one
            if (rootHandled) {
                continue;
            }
            std::vector<Atom *> atoms = frag->getAtomList();
            ...
            rootHandled = true;
            substitutableAtoms = FragmentTools::findnAtomsForSubstitution(atoms, frag->getDefaultInAtom(),
                                                                          numberOfSubstitutions, bondOrder, true);
        } else {
...
std::vector<Element *> StructureBuildingMethods::findAlternativeGroups(Element *startingElement) {
    ...
    while (stack->size() > 0) {
        ...
        std::vector<Element *> siblings = OpsinTools::getChildElementsWithTagNames(currentElement,
                                                                                   std::vector<std::wstring>{
                                                                                           XmlDeclarations::BRACKET_EL,
                                                                                           XmlDeclarations::SUBSTITUENT_EL,
                                                                                           XmlDeclarations::ROOT_EL});
        ...
        for (auto bracketOrSubOrRoot : siblings) {
            if (!doneFirstIteration &&
                currentElement->find(bracketOrSubOrRoot) <= currentElement->find(startingElement)) {
                continue;
            }
            ...
            if (bracketOrSubOrRoot->getName().equals(XmlDeclarations::BRACKET_EL)) {
                if (XmlDeclarations::IMPLICIT_TYPE_VAL ==
                    bracketOrSubOrRoot->getAttributeValue(XmlDeclarations::TYPE_ATR)) {
...
Fragment *StructureBuildingMethods::findFragmentWithLocant(Element *startingElement,
                                                           const std::wstring &locant) throw(StructureBuildingException) {
    Deque < Element * > *stack = new ArrayDeque<Element *>();
    stack->add(startingElement->getParent());
    bool doneFirstIteration = false; //check on index only done on first iteration to only get elements with an index greater than the starting element
    ...
    while (stack->size() > 0) {
        Element *currentElement = stack->removeLast();
        if (currentElement->getName() == XmlDeclarations::SUBSTITUENT_EL ||
            currentElement->getName() == XmlDeclarations::ROOT_EL) {
            Fragment *groupFrag = currentElement->getFirstChildElement(XmlDeclarations::GROUP_EL)->getFrag();
            if (monoNuclearHydride != nullptr && currentElement->getAttribute(XmlDeclarations::LOCANT_ATR) !=
                                                 nullptr) { //It looks like all groups are locanting onto the monoNuclearHydride e.g. 1-oxo-1-phenyl-sulfanylidene
            ...
            for (auto bracketOrSubOrRoot : siblings) {
                ...
                if (bracketOrSubOrRoot->getName().equals(XmlDeclarations::BRACKET_EL)) {
                    ...
                    if (substituentToTryFirst == nullptr &&
                        bracketOrSubOrRoot->getAttribute(XmlDeclarations::LOCANT_EL) == nullptr &&
                        OpsinTools::MATCH_NUMERIC_LOCANT->matcher(locant).matches()) {
...
Element *StructureBuildingMethods::findRightMostGroupInBracket(Element *bracket) {
    std::vector<Element *> subsBracketsAndRoots = OpsinTools::getChildElementsWithTagNames(bracket,
                                                                                           std::vector<std::wstring>{
                                                                                                   XmlDeclarations::BRACKET_EL,
                                                                                                   XmlDeclarations::SUBSTITUENT_EL,
                                                                                                   XmlDeclarations::ROOT_EL});
    ...
    while (lastSubsBracketOrRoot->getName() == XmlDeclarations::BRACKET_EL) {
        subsBracketsAndRoots = OpsinTools::getChildElementsWithTagNames(lastSubsBracketOrRoot,
                                                                        std::vector<std::wstring>{
                                                                                XmlDeclarations::BRACKET_EL,
                                                                                XmlDeclarations::SUBSTITUENT_EL,
                                                                                XmlDeclarations::ROOT_EL});
...
std::wstring StructureBuildingMethods::checkForBracketedPrimedLocantSpecialCase(Element *subBracketOrRoot,
                                                                                const std::wstring &locantString) {
...
void StructureBuildingMethods::checkAndApplySpecialCaseWhereOutAtomsCanBeCombinedOrThrow(Fragment *frag,
    ...
    if (groupValue == L"oxy" || groupValue == L"thio" || groupValue == L"seleno" ||
        groupValue == L"telluro") { //always bivalent
        return;
    }
    ...
    for (int i = outAtomCount - 1;
         i >= 0; i--) { //remove all outAtoms and add one with the total valency of all those that have been removed
        OutAtom *out = frag->getOutAtom(i);
        if (!out->getAtom()->equals(firstOutAtom)) {
            throw StructureBuildingException(std::wstring(
                    L"Substitutive bond formation failure: Fragment expected to have one OutAtom but had: ") +
                                             std::to_wstring(outAtomCount));
        }
...
void StructureBuildingMethods::addPrimesToLocantedStereochemistryElements(Element *subOrBracket,
                                                                          const std::wstring &primesString) {
    std::vector<Element *> stereoChemistryElements = OpsinTools::getDescendantElementsWithTagName(subOrBracket,
    ...
    for (auto stereoChemistryElement : stereoChemistryElements) {
        if (stereoChemistryElement->getAttribute(XmlDeclarations::LOCANT_ATR) != nullptr) {
            stereoChemistryElement->getAttribute(XmlDeclarations::LOCANT_ATR)->setValue(
                    stereoChemistryElement->getAttributeValue(XmlDeclarations::LOCANT_ATR) + primesString);
        }
    }
}
