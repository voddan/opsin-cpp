...
ResourceManager::ResourceManager(ResourceGetter *resourceGetter) throw(IOException) : resourceGetter(resourceGetter),
                                                                                      automatonInitialiser(
                                                                                              new AutomatonInitialiser(
                                                                                                      resourceGetter->getResourcePath() +
                                                                                                      std::wstring(
                                                                                                              L"serialisedAutomata/"))),
                                                                                      symbolTokenNamesDict(
                                                                                              std::vector<OpsinRadixTrie *>(
                                                                                                      grammarSymbolsSize)),
                                                                                      symbolRegexAutomataDict(
                                                                                              std::vector<RunAutomaton *>(
                                                                                                      grammarSymbolsSize)),
                                                                                      symbolRegexesDict(
                                                                                              std::vector<Pattern *>(
                                                                                                      grammarSymbolsSize)),
                                                                                      chemicalAutomaton(
                                                                                              processChemicalGrammar(
                                                                                                      false)) {
    ...
    try {
        while (filesToProcessReader->hasNext()) {
            ...
            if (event_Renamed == XMLStreamConstants::START_ELEMENT &&
                filesToProcessReader->getLocalName().equals(L"tokenFile")) {
...
void ResourceManager::addToken(const std::wstring &text, TokenEl *el, boost::optional<wchar_t> symbol, int index,
                               bool reversed) {
...
//ORIGINAL LINE: throw new RuntimeException(symbol +" is associated with the regex " + regex +" however it is not actually used in OPSIN's grammar!!!");
    ...
    if (!reversed) {
        if (determinise) { //should the regex be compiled into a DFA for faster execution?
            symbolRegexAutomataDict[index] = automatonInitialiser->loadAutomaton(
                    tokenTagName + std::wstring(L"_") + std::to_wstring(static_cast<int>(symbol)), regex, false, false);
        } else {
            symbolRegexesDict[index] = Pattern::compile(regex);
        }
    } else {
        if (determinise) { //should the regex be compiled into a DFA for faster execution?
            symbolRegexAutomataDictReversed[index] = automatonInitialiser->loadAutomaton(
                    tokenTagName + std::wstring(L"_") + std::to_wstring(static_cast<int>(symbol)), regex, false, true);
        } else {
...
TokenEl *ResourceManager::makeTokenElement(const std::wstring &tokenString,
                                           boost::optional<wchar_t> symbol) throw(ParsingException) {
    ...
    throw ParsingException(std::wstring(
            L"Parsing Error: This is a bug in the program. A token element could not be found for token: ") +
                           tokenString + std::wstring(L" using annotation symbol: ") + symbol);
}
