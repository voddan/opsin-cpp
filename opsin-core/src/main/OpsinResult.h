...
class OpsinResult {
private:
    static Logger *const LOG = Logger::getLogger(OpsinResult::
    typeid);
...
//JAVA TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'toString':
    OpsinResult(Fragment *frag, OPSIN_RESULT_STATUS status, std::vector<OpsinWarning *> &warnings,
                const std::wstring &chemicalName);

    OpsinResult(Fragment *frag, OPSIN_RESULT_STATUS status, const std::wstring &message,
                const std::wstring &chemicalName);
