...
//						import static uk.ac.cam.ch.wwmm.opsin.XmlDeclarations.*;
						...
						int SortAtomsForElementSymbols::compare(Atom *a, Atom *b)
						{
							...
							{ //prefer atoms with more hydrogen
								return -1;
							}
							if (expectedHydrogenA < expectedHydrogenB)
							{
						...
						int SortAtomsForMainGroupElementSymbols::compare(Atom *a, Atom *b)
						{
							...
							if (compare != 0)
							{ //only bother comparing properly if elements are the same
								return compare;
							}
							...
							{ //having substitutable hydrogen preferred
								return -1;
							}
							if (aExpectedHydrogen == 0 && bExpectedHydrogen > 0)
							{
							...
							if (locantsA.empty() && locantsB.size() > 0)
							{ //having no locants preferred
								return -1;
							}
						...
						{
							if (atoma->getType() == XmlDeclarations::SUFFIX_TYPE_VAL && atomb->getType() != XmlDeclarations::SUFFIX_TYPE_VAL)
							{ //suffix atoms go to the back
								return 1;
							}
						...
						void FragmentTools::assignElementLocants(Fragment *suffixableFragment, std::vector<Fragment*> &suffixFragments) throw(StructureBuildingException)
						{
...
//JAVA TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'toString':
							...
							if (fragType == XmlDeclarations::NONCARBOXYLICACID_TYPE_VAL || fragType == XmlDeclarations::CHALCOGENACIDSTEM_TYPE_VAL)
							{
							...
							else
							{
								if (suffixFragments.size() > 0)
								{
									...
									{ //look for special case violation of IUPAC rule, =(N)=(NN) is N//N' in practice rather than N/N'/N''
						...
						void FragmentTools::processMainGroupLabelling(Fragment *suffixableFragment, std::unordered_map<std::wstring, int> &elementCount, Set<Atom*> *atomsToIgnore)
						{
...
//JAVA TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'toString':
						...
						void FragmentTools::processSuffixLabelling(std::vector<Fragment*> &suffixFragments, std::unordered_map<std::wstring, int> &elementCount, Set<Atom*> *atomsToIgnore)
						{
...
//ORIGINAL LINE: java.util.Collections.sort(startingAtoms, new SortAtomsForElementSymbols());
						...
						void FragmentTools::processNonCarboxylicAcidLabelling(Fragment *suffixableFragment, std::unordered_map<std::wstring, int> &elementCount, Set<Atom*> *atomsToIgnore)
						{
							Set<Atom*> *atomsVisited = std::unordered_set<Atom*>();
...
//ORIGINAL LINE: java.util.Collections.sort(startingAtoms, new SortAtomsForElementSymbols());
						...
						void FragmentTools::assignLocantsAndExploreNeighbours(std::unordered_map<std::wstring, int> &elementCount, Set<Atom*> *atomsToIgnore, Set<Atom*> *atomsVisited, Deque<Atom*> *atomsToConsider)
						{
...
//ORIGINAL LINE: java.util.Collections.sort(atomsToExplore, new SortAtomsForElementSymbols());
						...
						{
//JAVA TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'toString':
							std::wstring element = atom->getElement()::toString();
						...
						Bond *FragmentTools::unsaturate(Atom *fromAtom, int bondOrder, Fragment *fragment) throw(StructureBuildingException)
						{
							...
							try
							{
								...
								else if (possibleToAtom == nullptr && fromAtom->getAtomIsInACycle())
								{ //allow something like cyclohexan-6-ene, something like butan-4-ene will still fail
							...
							if (toAtom == nullptr)
							{
								if (locant)
								{
									throw StructureBuildingException(std::wstring(L"Could not find bond to unsaturate starting from the atom with locant: ") + locant);
								}
								else
								{
						...
						void FragmentTools::unsaturate(Atom *fromAtom, const std::wstring &locantTo, int bondOrder, Fragment *fragment) throw(StructureBuildingException)
						{
...
//JAVA TO C++ CONVERTER TODO TASK: The Java Collection 'remove(Object)' method is not converted:
													...
													for (auto distance2Neighbour : distance2Neighbours)
													{
														if (distance2Neighbour->getElement() == ChemEl::N && distance2Neighbour->getAtomIsInACycle() && !std::find(originalIndicatedHydrogen.begin(), originalIndicatedHydrogen.end(), distance2Neighbour) != originalIndicatedHydrogen.end())
														{
										...
										if (!couldBeInvolvedInSimpleNitrogenTautomerism || detectSimpleNitrogenTautomer(indicatedAtom) != nullptr)
										{
							...
							while (svCount > 0)
							{
								...
								if (!foundBridgeHeadFlag)
								{
									throw StructureBuildingException(L"Failed to assign all double bonds! (Check that indicated hydrogens have been appropriately specified)");
								}
							}
						}
						...
						{
							for (auto a : atomCollection)
							{ //try and find an atom with SV that neighbours only one atom with SV
								if (a->hasSpareValency())
								{
									int atomsWithSV = 0;
									for (auto aa : frag->getIntraFragmentAtomNeighbours(a))
							...
							for (auto a : atomCollection)
							{ //try and find an atom with bridgehead atoms with SV on both sides c.f. phenoxastibinine == 10H-phenoxastibinine
								if (a->hasSpareValency())
								{
									std::vector<Atom*> neighbours = frag->getIntraFragmentAtomNeighbours(a);
						...
						Atom *FragmentTools::getAtomByAminoAcidStyleLocant(Atom *backboneAtom, const std::wstring &elementSymbol, const std::wstring &primes)
						{
...
//JAVA TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'toString':
								...
								if (atom->getElement() == ChemEl::N && atom->getIncomingValency() == 3 && atom->getCharge() == 0 && atomNeighbours.size() == 1 && atomNeighbours[0]->getElement() == ChemEl::N)
								{
...
//JAVA TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'toString':
						...
						{
							if (atom->getType() == XmlDeclarations::SUFFIX_TYPE_VAL || (atom->getElement()::isChalcogen() && XmlDeclarations::HETEROSTEM_SUBTYPE_VAL != atom->getFrag()->getSubType() && atom->getIncomingValency() == 1 && atom->getOutValency() == 0 && atom->getCharge() == 0))
							{
								return true;
							}
						...
						bool FragmentTools::isFunctionalAtomOrAldehyde(Atom *atom)
						{
							...
							{ //substituting an aldehyde would make it no longer an aldehyde
								return true;
							}
						...
						bool FragmentTools::isFunctionalAtom(Atom *atom)
						{
							...
							{ //potential chalcogen functional atom
						...
						void FragmentTools::removeTerminalAtom(BuildState *state, Atom *atomToRemove)
						{
							...
							{ //replace reference to atom with reference to implicit hydrogen
						...
						void FragmentTools::removeTerminalOxygen(BuildState *state, Atom *atom, int desiredBondOrder) throw(StructureBuildingException)
						{
							//TODO prioritise [N+][O-]
							std::vector<Atom*> neighbours = atom->getAtomNeighbours();
							for (auto neighbour : neighbours)
							{
								if (neighbour->getElement() == ChemEl::O && neighbour->getBondCount() == 1)
								{
									...
									if (b->getOrder() == desiredBondOrder && neighbour->getCharge() == 0)
									{
										...
										if (atom->getLambdaConventionValency())
										{ //corrects valency for phosphin/arsin/stibin
											atom->setLambdaConventionValency(atom->getLambdaConventionValency() - desiredBondOrder);
										}
										if (atom->getMinimumValency())
										{ //corrects valency for phosphin/arsin/stibin
							...
							{
								throw StructureBuildingException(L"Double bonded oxygen not found at suffix attachment position. Perhaps a suffix has been used inappropriately");
							}
							else if (desiredBondOrder == 1)
							{
								throw StructureBuildingException(L"Hydroxy oxygen not found at suffix attachment position. Perhaps a suffix has been used inappropriately");
							}
							else
							{
								throw StructureBuildingException(L"Suitable oxygen not found at suffix attachment position Perhaps a suffix has been used inappropriately");
							}
						...
						bool FragmentTools::notIn6MemberOrSmallerRing(Bond *bond)
						{
							...
							{ //obviously both must be in rings
...
//JAVA TO C++ CONVERTER TODO TASK: The Java Collection 'remove(Object)' method is not converted:
								...
								for (int i = 0; i < 5; i++)
								{ //up to 5 bonds from the neighbours of the fromAtom i.e. up to ring size 6
									...
									while (!atomsToInvestigate->isEmpty())
									{
										...
										for (auto neighbour : neighbours)
										{
											if (!std::find(visitedAtoms.begin(), visitedAtoms.end(), neighbour) != visitedAtoms.end() && neighbour->getAtomIsInACycle())
											{
...
//JAVA TO C++ CONVERTER TODO TASK: The Java Collection 'remove(Object)' method is not converted:
										...
										if (neighbours.size() >= 1 && neighbours[0]->getElement() == ChemEl::O && adjacentAtom->getBondToAtomOrThrow(neighbours[0])->getOrder() == 2)
										{
											continue;
										}
										if (neighbours.size() >= 2 && neighbours[1]->getElement() == ChemEl::O && adjacentAtom->getBondToAtomOrThrow(neighbours[1])->getOrder() == 2)
										{
											continue;
										}
						...
						std::vector<Atom*> FragmentTools::findnAtomsForSubstitution(std::vector<Atom*> &atomList, Atom *preferredAtom, int numberOfSubstitutionsRequired, int bondOrder, bool takeIntoAccountOutValency)
						{
						...
						std::vector<Atom*> FragmentTools::findnAtomsForSubstitution(Fragment *frag, Atom *preferredAtom, int numberOfSubstitutionsRequired, int bondOrder, bool takeIntoAccountOutValency)
						{
						...
						std::vector<Atom*> FragmentTools::findnAtomsForSubstitution(Fragment *frag, int numberOfSubstitutionsRequired, int bondOrder)
						{
