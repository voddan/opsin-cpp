...
//						import static uk.ac.cam.ch.wwmm.opsin.XmlDeclarations.*;
						...
						int FunctionalReplacement::SortInfixTransformations::compare(const std::wstring &infixTransformation1, const std::wstring &infixTransformation2)
						{
...
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
							...
							{ //infixTransformation1 preferred
								return -1;
							}
							if (allowedInputs1 > allowedInputs2)
							{ //infixTransformation2 preferred
								return 1;
							}
							else
							{
						...
						void FunctionalReplacement::processAcidReplacingFunctionalClassNomenclature(Element *finalSubOrRootInWord, Element *word) throw(ComponentGenerationException, StructureBuildingException)
						{
							Element *wordRule = OpsinTools::getParentWordRule(word);
							if (StringHelper::fromString<WordRule>(wordRule->getAttributeValue(XmlDeclarations::WORDRULE_ATR)) == WordRule::acidReplacingFunctionalGroup)
							{
...
//ORIGINAL LINE: throw new RuntimeException("OPSIN bug: problem with acidReplacingFunctionalGroup word rule");
						...
						bool FunctionalReplacement::processPrefixFunctionalReplacementNomenclature(std::vector<Element*> &groups, std::vector<Element*> &substituents) throw(StructureBuildingException, ComponentGenerationException)
						{
							...
							for (int i = originalNumberOfGroups - 1; i >= 0; i--)
							{
								...
								else if (XmlDeclarations::HALIDEORPSEUDOHALIDE_SUBTYPE_VAL == group->getAttributeValue(XmlDeclarations::SUBTYPE_ATR))
								{
									replacementType = PREFIX_REPLACEMENT_TYPE::halideOrPseudoHalide;
								}
								else if (XmlDeclarations::DEDICATEDFUNCTIONALREPLACEMENTPREFIX_SUBTYPE_VAL == group->getAttributeValue(XmlDeclarations::SUBTYPE_ATR))
								{
								...
								if (replacementType != nullptr)
								{
									...
									if (nextSubOrBracket != nullptr && (nextSubOrBracket->getName() == XmlDeclarations::ROOT_EL || nextSubOrBracket->getName() == XmlDeclarations::SUBSTITUENT_EL))
									{
										...
										if (groupPrecededByElementThatBlocksPrefixReplacementInterpetation(groupToBeModified))
										{
											...
											{
												throw ComponentGenerationException(std::wstring(L"dedicated Functional Replacement Prefix used in an inappropriate position :") + groupValue);
											}
										...
										if (possibleMultiplier != nullptr)
										{
											...
											{
												numberOfAtomsToReplace = std::stoi(possibleMultiplier->getAttributeValue(XmlDeclarations::VALUE_ATR));
											...
											if (possibleLocant != nullptr && possibleLocant->getName() == XmlDeclarations::LOCANT_EL && possibleLocant->getAttribute(XmlDeclarations::TYPE_ATR) == nullptr)
											{
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
												int numberOfLocants = possibleLocant->getValue().split(L",")->length;
												if (numberOfLocants == numberOfAtomsToReplace)
												{ //locants and number of replacements agree
													locantEl = possibleLocant;
												}
												else if (numberOfAtomsToReplace > 1)
												{ //doesn't look like prefix functional replacement
													if (replacementType == PREFIX_REPLACEMENT_TYPE::dedicatedFunctionalReplacementPrefix)
													{
														throw ComponentGenerationException(std::wstring(L"dedicated Functional Replacement Prefix used in an inappropriate position :") + groupValue);
													}
										...
										{
											oxygenReplaced = performChalcogenFunctionalReplacement(groupToBeModified, locantEl, numberOfAtomsToReplace, group->getAttributeValue(XmlDeclarations::VALUE_ATR));
										}
										...
										{
											if (groupToBeModified->getAttributeValue(XmlDeclarations::TYPE_ATR) != XmlDeclarations::NONCARBOXYLICACID_TYPE_VAL && !(groupToBeModified->getValue() == L"form" && groupValue == L"imido"))
											{
												throw ComponentGenerationException(std::wstring(L"dedicated Functional Replacement Prefix used in an inappropriate position :") + groupValue);
											}
											oxygenReplaced = performFunctionalReplacementOnAcid(groupToBeModified, locantEl, numberOfAtomsToReplace, group->getAttributeValue(XmlDeclarations::VALUE_ATR));
											if (oxygenReplaced == 0)
											{
												throw ComponentGenerationException(std::wstring(L"dedicated Functional Replacement Prefix used in an inappropriate position :") + groupValue);
											}
										...
										else if (replacementType == PREFIX_REPLACEMENT_TYPE::hydrazono || replacementType == PREFIX_REPLACEMENT_TYPE::halideOrPseudoHalide)
										{
											...
											if (groupToBeModified->getAttributeValue(XmlDeclarations::TYPE_ATR) != XmlDeclarations::NONCARBOXYLICACID_TYPE_VAL || acidHasSufficientHydrogenForSubstitutionInterpretation(acidFrag, group->getFrag()->getOutAtom(0)->getValency(), locantEl))
											{
...
//JAVA TO C++ CONVERTER TODO TASK: The Java Collection 'remove(Object)' method is not converted:
						...
						bool FunctionalReplacement::groupPrecededByElementThatBlocksPrefixReplacementInterpetation(Element *groupToBeModified)
						{
							...
							while (previous != nullptr && (previous->getName() == XmlDeclarations::SUBTRACTIVEPREFIX_EL || (previous->getName() == XmlDeclarations::STEREOCHEMISTRY_EL && previous->getAttributeValue(XmlDeclarations::TYPE_ATR) == XmlDeclarations::CARBOHYDRATECONFIGURATIONPREFIX_TYPE_VAL)))
							{
						...
						void FunctionalReplacement::processInfixFunctionalReplacementNomenclature(std::vector<Element*> &suffixes, std::vector<Fragment*> &suffixFragments) throw(StructureBuildingException, ComponentGenerationException)
						{
							for (int i = 0; i < suffixes.size(); i++)
							{
								...
								if (suffix->getAttribute(XmlDeclarations::INFIX_ATR) != nullptr)
								{
									...
									Element *possibleAcidGroup = OpsinTools::getPreviousSiblingIgnoringCertainElements(suffix, std::vector<std::wstring>{XmlDeclarations::MULTIPLIER_EL, XmlDeclarations::INFIX_EL, XmlDeclarations::SUFFIX_EL});
									if (possibleAcidGroup != nullptr && possibleAcidGroup->getName() == XmlDeclarations::GROUP_EL && (possibleAcidGroup->getAttributeValue(XmlDeclarations::TYPE_ATR) == XmlDeclarations::NONCARBOXYLICACID_TYPE_VAL || possibleAcidGroup->getAttributeValue(XmlDeclarations::TYPE_ATR) == XmlDeclarations::CHALCOGENACIDSTEM_TYPE_VAL))
									{
										fragToApplyInfixTo = possibleAcidGroup->getFrag();
									}
									if (fragToApplyInfixTo == nullptr)
									{
										throw ComponentGenerationException(std::wstring(L"infix has erroneously been assigned to a suffix which does not correspond to a suffix fragment. suffix: ") + suffix->getValue());
									}
...
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
									std::vector<std::wstring> infixTransformations = StringTools::arrayToList(suffix->getAttributeValue(XmlDeclarations::INFIX_ATR).split(L";"));
...
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
										...
										{
											throw StructureBuildingException(std::wstring(L"Atom to be replaced and replacement not specified correctly in infix: ") + infixTransformation);
										}
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
										...
										for (auto transformation : transformations)
										{
											...
											{
												throw StructureBuildingException(std::wstring(L"Malformed infix transformation. Expected to start with either - or =. Transformation was: ") + transformation);
											}
											if (transformation.length() < 2 || transformation.charAt(1) != L'O')
											{
												throw StructureBuildingException(L"Only replacement by oxygen is supported. Check infix defintions");
											}
										}
										...
										if ((acceptSingleBondedOxygen || nitrido) && !acceptDoubleBondedOxygen)
										{
											...
											{
												throw StructureBuildingException(std::wstring(L"Cannot find single bonded oxygen for infix with SMILES: ") + replacementSMILES + std::wstring(L" to modify!"));
											}
										...
										if (!acceptSingleBondedOxygen && (acceptDoubleBondedOxygen | nitrido))
										{
											...
											{
												throw StructureBuildingException(std::wstring(L"Cannot find double bonded oxygen for infix with SMILES: ") + replacementSMILES + std::wstring(L" to modify!"));
											}
										...
										if (acceptSingleBondedOxygen && acceptDoubleBondedOxygen)
										{
											...
											{
												throw StructureBuildingException(std::wstring(L"Cannot find oxygen for infix with SMILES: ") + replacementSMILES + std::wstring(L" to modify!"));
											}
										...
										{
											throw StructureBuildingException(std::wstring(L"Cannot find oxygen for infix with SMILES: ") + replacementSMILES + std::wstring(L" to modify!")); //this would be a bug
										}
										Fragment *replacementFrag = state->fragManager->buildSMILES(replacementSMILES, XmlDeclarations::SUFFIX_TYPE_VAL, XmlDeclarations::NONE_LABELS_VAL);
										if (replacementFrag->getOutAtomCount() > 0)
										{ //SMILES include an indication of the bond order the replacement fragment will have, this is not intended to be an outatom
										...
										if (infixAssignmentAmbiguous)
										{ //record what atoms could have been replaced. Often this ambiguity is resolved later e.g. S-methyl ethanthioate
						...
						void FunctionalReplacement::processAcidReplacingFunctionalClassNomenclatureFullWord(Element *acidContainingRoot, Element *acidReplacingWord) throw(ComponentGenerationException, StructureBuildingException)
						{
							...
							{
								throw ComponentGenerationException(L"OPSIN bug: acid replacing group not found where one was expected for acidReplacingFunctionalGroup wordRule");
							}
							...
							if (oxygenAtoms.empty())
							{
								std::vector<Element*> conjunctiveSuffixElements = OpsinTools::getNextSiblingsOfType(groupToBeModified, XmlDeclarations::CONJUNCTIVESUFFIXGROUP_EL);
							...
							{
								throw ComponentGenerationException(std::wstring(L"Insufficient oxygen to replace with ") + functionalGroupName + std::wstring(L"s in ") + acidContainingRoot->getFirstChildElement(XmlDeclarations::GROUP_EL)->getValue());
							}
							...
							if (isAmide)
							{
								...
								{
									throw ComponentGenerationException(std::wstring(L"OPSIN bug: ") + functionalGroupName + std::wstring(L" not found where expected"));
								}
						...
						void FunctionalReplacement::processAcidReplacingFunctionalClassNomenclatureFunctionalWord(Element *acidContainingRoot, Element *functionalWord) throw(ComponentGenerationException, StructureBuildingException)
						{
//JAVA TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'toString':
							if (functionalWord != nullptr && functionalWord->getAttributeValue(XmlDeclarations::TYPE_ATR) == WordType::functionalTerm->toString())
							{
								Element *functionalTerm = functionalWord->getFirstChildElement(XmlDeclarations::FUNCTIONALTERM_EL);
								if (functionalTerm == nullptr)
								{
									throw ComponentGenerationException(L"OPSIN bug: functionalTerm word not found where one was expected for acidReplacingFunctionalGroup wordRule");
								}
								...
								if (possibleLocantOrMultiplier != nullptr)
								{
									...
									{
										numberOfAcidicHydroxysToReplace = std::stoi(possibleLocantOrMultiplier->getAttributeValue(XmlDeclarations::VALUE_ATR));
...
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
								...
								{
									throw ComponentGenerationException(std::wstring(L"Insufficient oxygen to replace with nitrogen in ") + acidContainingRoot->getFirstChildElement(XmlDeclarations::GROUP_EL)->getValue());
								}
								bool isAmide = functionalGroupName == L"amide" || functionalGroupName == L"amid";
								if (isAmide)
								{
									...
									{
										Atom *functionalOxygenToReplace = locants.size() > 0 ? removeOxygenWithAppropriateLocant(oxygenAtoms, locants[i]) : oxygenAtoms[i];
								...
								else
								{
									...
									Fragment *acidReplacingFrag = state->fragManager->buildSMILES(groupValue, XmlDeclarations::SUFFIX_TYPE_VAL, labelsValue != L"" ? labelsValue : XmlDeclarations::NONE_LABELS_VAL);
									Fragment *acidFragment = groupToBeModified->getFrag();
									if (acidFragment->hasLocant(L"2"))
									{ //prefer numeric locants on group to those of replacing group
										for (auto atom : acidReplacingFrag->getAtomList())
										{
											atom->clearLocants();
										}
									}
									Atom *firstFunctionalOxygenToReplace = locants.size() > 0 ? removeOxygenWithAppropriateLocant(oxygenAtoms, locants[0]) : oxygenAtoms[0];
						...
						Atom *FunctionalReplacement::removeOxygenWithAppropriateLocant(std::vector<Atom*> &oxygenAtoms, const std::wstring &locant) throw(ComponentGenerationException)
						{
							for (std::vector<Atom*>::const_iterator iterator = oxygenAtoms.begin(); iterator != oxygenAtoms.end(); ++iterator)
							{
								Atom *atom = *iterator;
						...
						bool FunctionalReplacement::acidHasSufficientHydrogenForSubstitutionInterpretation(Fragment *acidFrag, int hydrogenRequiredForSubstitutionInterpretation, Element *locantEl)
						{
...
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
						...
						int FunctionalReplacement::performChalcogenFunctionalReplacement(Element *groupToBeModified, Element *locantEl, int numberOfAtomsToReplace, const std::wstring &replacementSmiles) throw(StructureBuildingException)
						{
							...
							if (locantEl != nullptr)
							{ //locants are used to indicate replacement on trivial groups
							...
							int atomsReplaced = 0;
							if (totalOxygen >= numberOfAtomsToReplace)
							{ //check that there atleast as many oxygens as requested replacements
								...
								if (prefixAssignmentAmbiguous)
								{ //record what atoms could have been replaced. Often this ambiguity is resolved later e.g. S-methyl thioacetate
									for (auto atom : ambiguousElementAtoms)
									{
										atom->setProperty(Atom::AMBIGUOUS_ELEMENT_ASSIGNMENT, ambiguousElementAtoms);
									}
						...
						int FunctionalReplacement::performPeroxyFunctionalReplacement(Element *groupToBeModified, Element *locantEl, int numberOfAtomsToReplace) throw(StructureBuildingException)
						{
							std::vector<Atom*> oxygenAtoms = findFunctionalOxygenAtomsInApplicableSuffixes(groupToBeModified);
							if (oxygenAtoms.empty())
							{
							...
							{ //check that there atleast as many oxygens as requested replacements
								...
								for (int j = 0; j < numberOfAtomsToReplace; j++)
								{
									...
									{ //etheric oxygen
										Fragment *newOxygen = state->fragManager->buildSMILES(L"O", XmlDeclarations::SUFFIX_TYPE_VAL, XmlDeclarations::NONE_LABELS_VAL);
									...
									{
										Fragment *replacementFrag = state->fragManager->buildSMILES(L"OO", XmlDeclarations::SUFFIX_TYPE_VAL, XmlDeclarations::NONE_LABELS_VAL);
						...
						int FunctionalReplacement::performFunctionalReplacementOnAcid(Element *groupToBeModified, Element *locantEl, int numberOfAtomsToReplace, const std::wstring &replacementSmiles) throw(StructureBuildingException)
						{
							int outValency;
							...
							{
								throw StructureBuildingException(L"OPSIN bug: Unexpected valency on fragment for prefix functional replacement");
							}
							...
							else
							{
								...
								{
									throw StructureBuildingException(L"Both a -OH and =O are required for nitrido prefix functional replacement");
								}
								oxygenAtoms.removeAll(singleBondedOxygen);
							}
							if (numberOfAtomsToReplace > 1 && oxygenAtoms.size() < numberOfAtomsToReplace)
							...
							{ //check that there atleast as many oxygens as requested replacements
								for (auto atomToReplace : oxygenAtoms)
								{
									...
									{
										Fragment *replacementFrag = state->fragManager->buildSMILES(replacementSmiles, atomToReplace->getFrag()->getTokenEl(), XmlDeclarations::NONE_LABELS_VAL);
										if (outValency == 3)
										{ //special case for nitrido
						...
						void FunctionalReplacement::disambiguateMultipliedInfixMeaning(std::vector<Element*> &suffixes, std::vector<Fragment*> &suffixFragments, Element *suffix, std::vector<std::wstring> &infixTransformations, int oxygenAvailable) throw(ComponentGenerationException, StructureBuildingException)
						{
							Element *possibleInfix = OpsinTools::getPreviousSibling(suffix);
							if (possibleInfix->getName() == XmlDeclarations::INFIX_EL)
							{ //the infix is only left when there was ambiguity
								...
								if (possibleMultiplier->getName() == XmlDeclarations::MULTIPLIER_EL)
								{
									...
									{ //multiplier means multiply the infix e.g. butandithiate
...
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
										...
										if (locants.size() > 0)
										{
											...
											{
												throw ComponentGenerationException(L"Multiplier/locant disagreement when multiplying infixed suffix");
											}
										...
										for (int j = 1; j < multiplierValue; j++)
										{ //multiplier means multiply the infixed suffix e.g. butandithione
						...
						void FunctionalReplacement::moveObsoleteOutAtoms(Atom *atomToBeReplaced, Fragment *replacementFrag)
						{
							...
							{ //this is not known to occur in well formed IUPAC names but would occur in thioxy (as a suffix)
								std::vector<Atom*> replacementAtomList = replacementFrag->getAtomList();
								Fragment *origFrag = atomToBeReplaced->getFrag();
						...
						void FunctionalReplacement::removeAssociatedFunctionalAtom(Atom *atomWithFunctionalAtom) throw(StructureBuildingException)
						{
						...
						std::vector<Atom*> FunctionalReplacement::pickOxygensWithAppropriateLocants(Element *locantEl, std::vector<Atom*> &oxygenAtoms)
						{
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
							std::vector<std::wstring> possibleLocants = locantEl->getValue().split(L",");
							...
							for (auto atom : oxygenAtoms)
							{
								...
								else
								{
									...
									if (atomWithNumericLocant != nullptr)
									{
										...
										for (auto locantVal : possibleLocants)
										{
											if (std::find(atomWithNumericLocantLocants.begin(), atomWithNumericLocantLocants.end(), locantVal) != atomWithNumericLocantLocants.end())
											{
												 oxygenWithAppropriateLocants.push_back(atom);
												 break;
											}
						...
						{
							std::vector<Element*> suffixElements = OpsinTools::getNextSiblingsOfType(groupToBeModified, XmlDeclarations::SUFFIX_EL);
							std::vector<Atom*> oxygenAtoms;
							for (auto suffix : suffixElements)
							{
								Fragment *suffixFrag = suffix->getFrag();
								if (suffixFrag != nullptr)
								{ //null for non carboxylic acids
						...
						std::vector<Atom*> FunctionalReplacement::findOxygenAtomsInApplicableSuffixes(Element *groupToBeModified)
						{
							...
							for (auto suffix : suffixElements)
							{
								...
								if (suffixFrag != nullptr)
								{ //null for non carboxylic acids
									if (suffixFrag->getFunctionalAtomCount() > 0 || groupToBeModified->getAttributeValue(XmlDeclarations::TYPE_ATR) == XmlDeclarations::ACIDSTEM_TYPE_VAL || suffix->getAttributeValue(XmlDeclarations::VALUE_ATR).equals(L"aldehyde"))
									{
						...
						void FunctionalReplacement::populateTerminalSingleAndDoubleBondedOxygen(std::vector<Atom*> &atomList, std::vector<Atom*> &singleBondedOxygen, std::vector<Atom*> &doubleBondedOxygen) throw(StructureBuildingException)
						{
							for (auto a : atomList)
							{
								if (a->getElement() == ChemEl::O)
								{ //find terminal oxygens
									if (a->getBondCount() == 1)
									{
										...
										{
											throw StructureBuildingException(std::wstring(L"Unexpected bond order to oxygen; excepted 1 or 2 found: ") + std::to_wstring(incomingValency));
										}
