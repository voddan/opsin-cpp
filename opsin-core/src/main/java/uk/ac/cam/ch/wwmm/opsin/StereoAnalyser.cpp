			...
			namespace ch
			{
				namespace wwmm
				{
					namespace opsin
					{
						...
						StereoAnalyser::StereoCentre::StereoCentre(StereoAnalyser *outerInstance, Atom *stereoAtom, boost::optional<bool> isTrueStereoCentre) : stereoAtom(stereoAtom), trueStereoCentre(isTrueStereoCentre), outerInstance(outerInstance)
						{
						}
						...
						std::vector<Atom*> StereoAnalyser::StereoCentre::getCipOrderedAtoms() throw(CipOrderingException)
						{
							...
							{ //lone pair is the 4th. This is represented by the atom itself and is always the lowest priority
								cipOrderedAtoms.push_back(0, stereoAtom);
							}
							return cipOrderedAtoms;
						}

						StereoAnalyser::StereoBond::StereoBond(StereoAnalyser *outerInstance, Bond *bond) : bond(bond), outerInstance(outerInstance)
						{
						}
						...
						StereoAnalyser::AtomNeighbouringColoursComparator::AtomNeighbouringColoursComparator(StereoAnalyser *outerInstance) : outerInstance(outerInstance)
						{
						}
						...
						StereoAnalyser::StereoAnalyser(Collection<Atom*> *atoms, Collection<Bond*> *bonds) : atoms(atoms), bonds(bonds), mappingToColour(std::unordered_map<Atom*, int>(atomsToSort->size())), atomNeighbourColours(std::unordered_map<Atom*, std::vector<int>>(atomsToSort->size()))
						{
...
//ORIGINAL LINE: java.util.Collections.sort(atomsToSort, atomicNumberThenAtomicMassComparator);
						...
						bool StereoAnalyser::populateColoursAndReportIfColoursWereChanged(std::vector<std::vector<Atom*>> &groupsByColour, std::vector<std::vector<Atom*>> &updatedGroupsByColour)
						{
						...
						std::vector<Atom*> StereoAnalyser::findParaStereoCentres(std::vector<Atom*> &potentialStereoAtoms, std::vector<Atom*> &trueStereoCentres)
						{
							...
							for (auto potentialStereoAtom : potentialStereoAtoms)
							{
								...
								if (neighbours.size() == 4)
								{
									...
									if (pairs == 1 || pairs == 2)
									{
										...
										{
											if (!branchesHaveTrueStereocentre(neighbours[entry.first], neighbours[entry.second], potentialStereoAtom, trueStereoCentres))
											{
						...
						bool StereoAnalyser::branchesHaveTrueStereocentre(Atom *branchAtom1, Atom *branchAtom2, Atom *potentialStereoAtom, std::vector<Atom*> &trueStereoCentres)
						{
							...
							while (!atomsToVisit.empty())
							{
								...
								while (!atomsToVisit.empty())
								{
									...
									if (std::find(atomsToVisit.begin(), atomsToVisit.end(), atom) != atomsToVisit.end())
									{ //the two branches have converged on this atom, don't investigate neighbours of it
										do
										{
						...
						bool StereoAnalyser::isAchiralDueToResonanceOrTautomerism(Atom *atom)
						{
							...
							if (chemEl == ChemEl::N || chemEl == ChemEl::P || chemEl == ChemEl::As || chemEl == ChemEl::S || chemEl == ChemEl::Se)
							{
								...
								for (auto neighbour : neighbours)
								{
									...
									if ((neighbourChemEl::isChalcogen() || neighbourChemEl == ChemEl::N) && isOnlyBondedToHydrogensOtherThanGivenAtom(neighbour, atom))
									{
...
//JAVA TO C++ CONVERTER TODO TASK: The Java Collection 'remove(Object)' method is not converted:
									...
									{
										if (neighbours1.size() == 2 && mappingToColour[neighbours1[0]].equals(mappingToColour[neighbours1[1]]))
										{
											continue;
										}
										Atom *a2 = bond->getToAtom();
...
//JAVA TO C++ CONVERTER TODO TASK: The Java Collection 'remove(Object)' method is not converted:
										...
										{
											if (neighbours2.size() == 2 && mappingToColour[neighbours2[0]].equals(mappingToColour[neighbours2[1]]))
											{
												continue;
											}
