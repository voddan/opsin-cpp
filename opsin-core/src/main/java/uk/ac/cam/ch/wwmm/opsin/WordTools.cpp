...
//						import static uk.ac.cam.ch.wwmm.opsin.OpsinTools.*;
						...
						std::vector<ParseWord*> WordTools::splitIntoParseWords(std::vector<ParseTokens*> &parseTokensList, const std::wstring &chemicalName)
						{
							...
							for (auto parseTokens : parseTokensList)
							{
								...
								std::vector<std::vector<wchar_t>> chunkedAnnotations = chunkAnnotations(annotations); //chunked into mainGroup/substituent/functionalTerm
								if (containsOmittedSpace(chunkedAnnotations))
								{
									...
									for (auto annotationList : chunkedAnnotations)
									{
										...
										if (finalAnnotationInList.equals(OpsinTools::END_OF_FUNCTIONALTERM) || finalAnnotationInList.equals(OpsinTools::END_OF_MAINGROUP) || annotPos == tokens.size())
										{
							...
							else
							{
								for (int i = 0; i < leastWordsInOmmittedSpaceParse; i++)
								{
									...
									{
										if (!std::find(parseTokensForWord.begin(), parseTokensForWord.end(), parseTokens.get(i)) != parseTokensForWord.end()))
									...
									ParseWord tempVar4(StringTools::stringListToString(parseTokensForWord[0]->getTokens(), L""), parseTokensForWord);
						...
						bool WordTools::containsOmittedSpace(std::vector<std::vector<wchar_t>> &chunkedAnnotations)
						{
							...
							{ //there are multiple subsitutents/maingroup/functionalterms
								for (auto annotationList : chunkedAnnotations)
								{
						...
						std::vector<std::vector<wchar_t>> WordTools::chunkAnnotations(std::vector<wchar_t> &annots)
						{
							...
							for (* : :optional<wchar_t> annot : annots)
							{
								...
								wchar_t ch = annot;
								if (ch == OpsinTools::END_OF_SUBSTITUENT || ch == OpsinTools::END_OF_MAINGROUP || ch == OpsinTools::END_OF_FUNCTIONALTERM)
								{
						...
						std::wstring WordTools::removeWhiteSpaceIfBracketsAreUnbalanced(const std::wstring &name) throw(ParsingException)
						{
							...
							for (int i = 0; i < stringLength; i++)
							{
								...
								else if (c == L' ' && bracketLevel > 0)
								{ //brackets unbalanced and a space has been encountered!
