			...
			namespace ch
			{
				namespace wwmm
				{
					namespace opsin
					{
						...
						TokenizationResult *Tokeniser::tokenize(const std::wstring &name, bool allowRemovalOfWhiteSpace) throw(ParsingException)
						{
							...
							while (!result->isSuccessfullyTokenized())
							{
								...
								std::wstring parsedName = result->getUnparsedName().substr(0, result->getUnparsedName().length() - result->getWorkingName().length());
						...
						TokenizationResult *Tokeniser::tokenizeRightToLeft(ReverseParseRules *reverseParseRules, const std::wstring &name, bool allowRemovalOfWhiteSpace) throw(ParsingException)
						{
						...
						{
							return parseTokens.size() > 0 && (result->isFullyInterpretable() || result->getWorkingName()[result->getWorkingName().length() - 1] == L' ' || result->getWorkingName()[result->getWorkingName().length() - 1] == L'-');
						}

						bool Tokeniser::isWordParsable(std::vector<ParseTokens*> &parseTokens, TokenizationResult *result)
						{
							return parseTokens.size() > 0 && (result->isFullyInterpretable() || result->getWorkingName()[0] == L' ' || result->getWorkingName()[0] == L'-');
						}

						void Tokeniser::parseWord(TokenizationResult *result, std::vector<ParseTokens*> &parseTokens, const std::wstring &parsedName, bool reverse)
						{
							...
							else
							{
								...
								{
									if (remainingName.length() > 3 && boost::starts_with(remainingName, L" - "))
									{ //this is a way of of indicating a mixture
						...
						void Tokeniser::addParseWords(std::vector<ParseTokens*> &parseTokens, const std::wstring &parsedName, Parse *parse, bool reverse)
						{
						...
						bool Tokeniser::fixWord(TokenizationResult *result, const std::wstring &parsedName, bool allowRemovalOfWhiteSpace) throw(ParsingException)
						{
							...
							else
							{
								if (allowRemovalOfWhiteSpace)
								{
									...
									if (!reverseSpaceRemoval(parsedWords, result))
									{
										...
										if (indexOfSpace != -1)
										{
											result->setUnparsedName(parsedName + result->getWorkingName().substr(0, indexOfSpace) + result->getWorkingName().substr(indexOfSpace + 1));
										}
										else
										{
											return false;
						...
						bool Tokeniser::fixWordInReverse(TokenizationResult *result, const std::wstring &parsedName, bool allowRemovalOfWhiteSpace)
						{
							if (allowRemovalOfWhiteSpace)
							{
								...
								if (indexOfSpace != -1)
								{
									result->setUnparsedName(result->getWorkingName().substr(0, indexOfSpace) + result->getWorkingName().substr(indexOfSpace + 1) + parsedName);
								}
								else
								{
									return false;
						...
						bool Tokeniser::reverseSpaceRemoval(std::vector<ParseWord*> &parsedWords, TokenizationResult *result) throw(ParsingException)
						{
							bool successful = false;

							if (!parsedWords.empty())
							{ //first see whether the space before the unparseable word is erroneous
								...
								std::wstring backParsedName = pw->getWord() + result->getUnparsedName().substr(0, result->getUnparsedName().length() - backUninterpretableName.length());
								if (backParsedName.length() > pw->getWord().length() && backParseTokens.size() > 0 && (backUninterpretableName == L"" || backUninterpretableName[0] == L' ' || backUninterpretableName[0] == L'-'))
								{ //a word was interpretable
									...
									{
										result->setUnparsedName(backUninterpretableName.substr(1)); //remove white space at start of uninterpretableName
									}
									else
									{
