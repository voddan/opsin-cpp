...
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
							...
							{
								if (matchCasCollectiveIndex->matcher(parentNameParts[parentNameParts.size() - 1]).matches())
								{ //CAS collective index description should be ignored
...
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
								...
								for (int c = 0, componentLen = components.size(); c < componentLen; c++)
								{
									...
									else
									{
										...
										if (parseTokens.size() > 0)
										{
											...
											{
												if (!firstWordType::equals(OpsinTools::determineWordType(firstParseWordTokens[j]->getAnnotations())))
												{
													throw ParsingException(component + std::wstring(L"can be interpreted in multiple ways. For the sake of precision OPSIN has decided not to process this as a CAS name"));
												}
											}
											...
											if (parseWords.size() == 1)
											{
												...
												case functionalTerm:
													if (component.equalsIgnoreCase(L"ester"))
													{
														...
														{
															throw ParsingException(L"ester encountered but no substituents were specified in potential CAS name!");
														}
														if (esterEncountered)
														{
															throw ParsingException(L"ester formation was mentioned more than once in CAS name!");
														}
												...
												case substituent:
													...
													break;
												case full:
													if (StringTools::endsWithCaseInsensitive(component, L"ate") || StringTools::endsWithCaseInsensitive(component, L"ite") || StringTools::endsWithCaseInsensitive(component, L"ium") || StringTools::endsWithCaseInsensitive(component, L"hydrofluoride") || StringTools::endsWithCaseInsensitive(component, L"hydrochloride") || StringTools::endsWithCaseInsensitive(component, L"hydrobromide") || StringTools::endsWithCaseInsensitive(component, L"hydroiodide"))
													{ //e.g. Piperazinium, 1,1-dimethyl-, 2,2,2-trifluoroacetate hydrochloride
														functionalTerms.push_back(component);
													}
													else if (StringTools::endsWithCaseInsensitive(component, L"ic") && c + 1 < componentLen && components[c + 1].equalsIgnoreCase(L"acid"))
													{
														functionalTerms.push_back(component);
													...
													{
														throw ParsingException(std::wstring(L"Unable to interpret: ") + component + std::wstring(L" (as part of a CAS index name)- A full word was encountered where a substituent or functionalTerm was expected"));
													}
													break;
												default:
											...
											else if (parseWords.size() == 2 && firstWordType::equals(WordType::substituent))
											{
												...
												WordType secondWordType = OpsinTools::determineWordType(secondParseWordTokens[0]->getAnnotations());
												for (int j = 1; j < secondParseWordTokens.size(); j++)
												{
													if (!secondWordType::equals(OpsinTools::determineWordType(secondParseWordTokens[j]->getAnnotations())))
													{
														throw ParsingException(component + std::wstring(L"can be interpreted in multiple ways. For the sake of precision OPSIN has decided not to process this as a CAS name"));
													}
												}
												if (secondWordType::equals(WordType::functionalTerm) && matchFunctionalTermAllowingSubstituentPrefix->matcher(parseWords[1]->getWord()).matches())
												{
													functionalTerms.push_back(component);
												}
												else
										...
										{
											if (!matchCasCollectiveIndex->matcher(component).matches())
											{ //CAS collective index description should be ignored
												throw ParsingException(std::wstring(L"Unable to interpret: ") + component + std::wstring(L" (as part of a CAS index name)"));
											}
										}
									}
								}
						...
						std::wstring CASTools::uninvertEster(const std::wstring &parent) throw(ParsingException)
						{
							...
							if (lastChar == L')')
							{
								...
								else if (StringTools::endsWithCaseInsensitive(parent, L"ine)"))
								{ //amino acid
