...
//JAVA TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'toString':
						OpsinResult::OpsinResult(Fragment *frag, OPSIN_RESULT_STATUS status, std::vector<OpsinWarning*> &warnings, const std::wstring &chemicalName) : structure(frag), status(status), message(sb->toString()), chemicalName(chemicalName), warnings(warnings)
						{
...
//JAVA TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'toString':
						...
						OpsinResult::OpsinResult(Fragment *frag, OPSIN_RESULT_STATUS status, const std::wstring &message, const std::wstring &chemicalName) : structure(frag), status(status), message(message), chemicalName(chemicalName), warnings(Collections::emptyList())
						{
						}
