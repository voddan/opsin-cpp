			...
			namespace ch
			{
				namespace wwmm
				{
					namespace opsin
					{
						...
						ParseTokens::ParseTokens(std::vector<std::wstring> &tokens, std::vector<wchar_t> &annotations) : tokens(Collections::unmodifiableList(std::vector<std::wstring>(tokens))), annotations(Collections::unmodifiableList(std::vector<wchar_t>(annotations)))
						{
