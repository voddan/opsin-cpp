...
const std::wstring OpsinTools::NEWLINE = System::getProperty(L"line.separator");
						...
						Element *OpsinTools::getNextNonChargeSuffix(Element *currentEl)
						{
							...
							while (next != nullptr)
							{
								if (next->getName() == XmlDeclarations::SUFFIX_EL && XmlDeclarations::CHARGE_TYPE_VAL != next->getAttributeValue(XmlDeclarations::TYPE_ATR))
								{
									return next;
								}
						...
						{
						  if (current->getName() == XmlDeclarations::GROUP_EL)
						  { //can start with a group or the sub/root the group is in
...
//ORIGINAL LINE: throw new RuntimeException("Cannot find enclosing wordRule element");
						...
						Element *OpsinTools::getNextSiblingIgnoringCertainElements(Element *startingEl, std::vector<std::wstring> &elNamesToIgnore)
						{
						...
						Element *OpsinTools::getPreviousSiblingIgnoringCertainElements(Element *startingEl, std::vector<std::wstring> &elNamesToIgnore)
						{
							Element *parent = startingEl->getParent();
							if (parent == nullptr)
							{
						...
						std::vector<Element*> OpsinTools::getDescendantElementsWithTagName(Element *startingElement, const std::wstring &elementName)
						{
						...
						std::vector<Element*> OpsinTools::getDescendantElementsWithTagNames(Element *startingElement, std::vector<std::wstring> &elementNames)
						{
							std::vector<Element*> matchingElements;
							Deque<Element*> *stack = new ArrayDeque<Element*>();
							for (int i = startingElement->getChildCount() - 1; i >= 0; i--)
						...
						std::vector<Element*> OpsinTools::getChildElementsWithTagNames(Element *startingElement, std::vector<std::wstring> &elementNames)
						{
							std::vector<Element*> matchingElements;
						...
						std::vector<Element*> OpsinTools::getDescendantElementsWithTagNameAndAttribute(Element *startingElement, const std::wstring &elementName, const std::wstring &attributeName, const std::wstring &attributeValue)
						{
							std::vector<Element*> matchingElements;
							Deque<Element*> *stack = new ArrayDeque<Element*>();
							for (int i = startingElement->getChildCount() - 1; i >= 0; i--)
						...
						std::vector<Element*> OpsinTools::getChildElementsWithTagNameAndAttribute(Element *startingElement, const std::wstring &elementName, const std::wstring &attributeName, const std::wstring &attributeValue)
						{
							std::vector<Element*> matchingElements;
							for (int i = 0, l = startingElement->getChildCount(); i < l; i++)
							{
