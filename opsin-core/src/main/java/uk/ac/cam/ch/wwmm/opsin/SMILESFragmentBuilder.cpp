...
SMILESFragmentBuilder::StaticConstructor SMILESFragmentBuilder::staticConstructor;
						...
						SMILESFragmentBuilder::ParserInstance::ParserInstance(SMILESFragmentBuilder *outerInstance, const std::wstring &smiles, Fragment *fragment) : smiles(smiles), endOfSmiles(smiles.length()), fragment(fragment), outerInstance(outerInstance)
						{
						}
						...
						void SMILESFragmentBuilder::ParserInstance::parseSmiles() throw(StructureBuildingException)
						{
							...
							for (; i < endOfSmiles; i++)
							{
								...
								case L'%':
									...
									break;
								default:
									throw StructureBuildingException(StringHelper::toString(ch) + std::wstring(L" is in an unexpected position. Check this is not a mistake and that this feature of SMILES is supported by OPSIN's SMILES parser"));
								}
							}
						...
						void SMILESFragmentBuilder::ParserInstance::processOrganicAtom(wchar_t ch) throw(StructureBuildingException)
						{
							...
							if (outerInstance->is_A_to_Z(ch))
							{ //normal atoms
								if (i + 1 < endOfSmiles && outerInstance->is_a_to_z(smiles[i + 1]) && organicAtoms->contains(smiles.substr(i, 2)))
								{
								...
								{
									throw StructureBuildingException(elementType + std::wstring(L" is not an organic Element. If it is actually an element it should be in square brackets"));
								}
							}
							else if (outerInstance->is_a_to_z(ch))
							{ //aromatic atoms
								if (!aromaticAtoms->contains(elementType))
								{
									throw StructureBuildingException(elementType + std::wstring(L" is not an aromatic Element. If it is actually an element it should not be in lower case"));
								}
						...
						void SMILESFragmentBuilder::ParserInstance::processBracketedAtom() throw(StructureBuildingException)
						{
							...
							{ //aromatic atoms
								if (outerInstance->is_a_to_z(smiles[i]))
								{
									...
									{
										throw StructureBuildingException(elementType + smiles[i] + std::wstring(L" is not an aromatic Element. If it is actually an element it should not be in lower case"));
									}
								}
								else
								{
							...
							for (; i < indexOfRightSquareBracket; i++)
							{
								ch = smiles[i];
								if (ch == L'@')
								{ // chirality-sets atom parity
									if (chiralitySet)
									{
										throw StructureBuildingException(L"Atom parity appeared to be specified twice for an atom in a square bracket!");
									}
								...
								{ // hydrogenCount
									...
									{
										throw StructureBuildingException(L"Hydrogen count appeared to be specified twice for an atom in a square bracket!");
									}
									if (smiles[i + 1] == L'?')
									{
								...
								{ // formalCharge
									if (charge != 0)
									{
										throw StructureBuildingException(L"Charge appeared to be specified twice for an atom in a square bracket!");
									}
									charge = (ch == L'+') ? 1 : -1;
									...
									{
										while (i + 1 < indexOfRightSquareBracket)
										{ //e.g. [C++]
											ch = smiles[i + 1];
											if (ch == L'+')
											{
												if (charge != 1)
												{
													throw StructureBuildingException(L"Atom has both positive and negative charges specified!"); //e.g. [C+-]
												}
											}
											else if (ch == L'-')
											{
						...
						void SMILESFragmentBuilder::ParserInstance::processRingOpening(const std::wstring &closure) throw(StructureBuildingException)
						{
							StackFrame *currentFrame = stack->getLast();
							...
							{ //replace ringclosureX with actual reference to id when it is known
						...
						void SMILESFragmentBuilder::ParserInstance::processRingClosure(const std::wstring &closure) throw(StructureBuildingException)
						{
							...
							if (currentFrame->slash != nullptr)
							{
								...
								b->setSmilesStereochemistry(currentFrame->slash);
								if (sf->slash != nullptr && sf->slash::equals(currentFrame->slash))
								{ //specified twice check for contradiction
							...
							if (closureAtomParity != nullptr)
							{ //replace dummy atom with actual atom e.g. N[C@@H]1C.F1 where the 1 initially holds a dummy atom before being replaced with the F atom
...
//ORIGINAL LINE: throw new RuntimeException("OPSIN Bug: Index of dummy atom representing ring closure atom not set");
						...
						int SMILESFragmentBuilder::ParserInstance::addAtomToAtomParity(AtomParity *atomParity, Atom *atom) throw(StructureBuildingException)
						{
							std::vector<Atom*> atomRefs4 = atomParity->getAtomRefs4();
							bool setAtom = false;
							int i = 0;
							...
							if (!setAtom)
							{
								throw StructureBuildingException(L"Tetrahedral stereocentre specified in SMILES appears to involve more than 4 atoms");
							}
							return i;
						}
						...
						Fragment *SMILESFragmentBuilder::build(const std::wstring &smiles, const std::wstring &type, const std::wstring &labelMapping) throw(StructureBuildingException)
						{
							Fragment tempVar(type);
							return build(smiles, &tempVar, labelMapping);
						}

						Fragment *SMILESFragmentBuilder::build(const std::wstring &smiles, Element *tokenEl, const std::wstring &labelMapping) throw(StructureBuildingException)
						{
							if (tokenEl == nullptr)
							{
						...
						Fragment *SMILESFragmentBuilder::build(const std::wstring &smiles, Fragment *fragment, const std::wstring &labelMapping) throw(StructureBuildingException)
						{
							if (smiles == L"")
							{
							...
							if (labelMapping == L"")
							{
								throw std::invalid_argument("labelMapping is null use \"none\" if you do not want any numbering or \"numeric\" if you would like default numbering");
							}
							if (smiles.length() == 0)
							{
								return fragment;
							...
							if (firstCharacter == L'-' || firstCharacter == L'=' || firstCharacter == L'#')
							{ //used by OPSIN to specify the valency with which this fragment connects
								firstIndex++;
							}
							wchar_t lastCharacter = smiles[lastIndex - 1];
							if (lastCharacter == L'-' || lastCharacter == L'=' || lastCharacter == L'#')
							{ //used by OPSIN to specify the valency with which this fragment connects and to indicate it connects via the last atom in the SMILES
								lastIndex--;
							}
							...
							{
								if (atom->getProperty(Atom::SMILES_HYDROGEN_COUNT) != nullptr && atom->getLambdaConventionValency() == nullptr)
								{
						...
						void SMILESFragmentBuilder::processLabelling(const std::wstring &labelMapping, std::vector<Atom*> &atomList) throw(StructureBuildingException)
						{
...
//JAVA TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'toString':
							...
							else if (labelMapping == XmlDeclarations::FUSEDRING_LABELS_VAL)
							{ //fragment is a fusedring with atoms in the correct order for fused ring numbering
...
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
								...
								{
									throw StructureBuildingException(std::wstring(L"Group numbering has been invalidly defined in resource file: labels: ") + labelMap.size() + std::wstring(L", atoms: ") + std::to_wstring(numOfAtoms));
								}
...
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
						...
						void SMILESFragmentBuilder::verifyAndTakeIntoAccountLonePairsInAtomParities(std::vector<Atom*> &atomList) throw(StructureBuildingException)
						{
							for (auto atom : atomList)
							{
								...
								if (atomParity != nullptr)
								{
									...
									if (nullAtoms != 0)
									{
										...
										{ //special case where lone pair is part of the tetrahedron
											if (atomList.find(atomRefs4[0]) < atomList.find(atom))
											{ //is there an atom in the SMILES in front of the stereocentre?
										...
										{
											throw StructureBuildingException(L"SMILES is malformed. Tetrahedral stereochemistry defined on a non tetrahedral centre");
										}
									}
								}
							}
						...
						void SMILESFragmentBuilder::addBondStereoElements(Fragment *currentFrag) throw(StructureBuildingException)
						{
							...
							for (auto centralBond : bonds)
							{ //identify cases of E/Z stereochemistry and add appropriate bondstereo tags
								if (centralBond->getOrder() == 2)
								{
									...
									for (auto preceedingBond : fromAtomBonds)
									{
										if (preceedingBond->getSmilesStereochemistry() != nullptr)
										{
											...
											{
												if (followingBond->getSmilesStereochemistry() != nullptr)
												{ //now found a double bond surrounded by two bonds with slashs
													bool upFirst;
													bool upSecond;
													...
													{
														throw StructureBuildingException(preceedingBond->getSmilesStereochemistry() + std::wstring(L" is not a slash!"));
													}
													...
													{
														throw StructureBuildingException(followingBond->getSmilesStereochemistry() + std::wstring(L" is not a slash!"));
													}
						...
						void SMILESFragmentBuilder::setupAtomValency(Atom *atom) throw(StructureBuildingException)
						{
							...
							if (atom->hasSpareValency())
							{
								...
								if (absoluteCharge != 0)
								{
									...
									{
										throw StructureBuildingException(chemEl + std::wstring(L" with charge ") + charge + std::wstring(L" is not expected to be aromatic!"));
									}
								}
							...
							{ //s or p block element
								if (defaultVal != incomingValency || charge != 0)
								{
									...
									{
										std::vector<boost::optional<int>> unchargedStableValencies = ValencyChecker::getPossibleValencies(chemEl, 0);
										...
										if (!hasPlausibleValency)
										{ //could be something like [Sn] which would be expected to be attached to later
											atom->setMinimumValency(incomingValency);
										}
									}
								}
