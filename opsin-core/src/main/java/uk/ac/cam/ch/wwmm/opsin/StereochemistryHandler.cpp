...
//						import static uk.ac.cam.ch.wwmm.opsin.XmlDeclarations.*;
						...
						StereochemistryHandler::StereochemistryHandler(BuildState *state, std::unordered_map<Atom*, StereoCentre*> &atomStereoCentreMap, std::unordered_map<Bond*, StereoBond*> &bondStereoBondMap) : state(state), atomStereoCentreMap(atomStereoCentreMap), bondStereoBondMap(bondStereoBondMap), notExplicitlyDefinedStereoCentreMap(std::unordered_map<Atom*, StereoCentre*>(atomStereoCentreMap)), notExplicitlyDefinedStereoBondMap(std::unordered_map<Bond*, StereoBond*>(bondStereoBondMap))
						{
						}

						void StereochemistryHandler::applyStereochemicalElements(std::vector<Element*> &stereoChemistryEls) throw(StructureBuildingException)
						{
							...
							for (auto stereoChemistryElement : stereoChemistryEls)
							{
								...
								else if (stereoChemistryElement->getAttributeValue(XmlDeclarations::TYPE_ATR).equals(XmlDeclarations::CARBOHYDRATECONFIGURATIONPREFIX_TYPE_VAL))
								{
						...
						void StereochemistryHandler::removeRedundantStereoCentres(std::vector<Atom*> &atomsWithPreDefinedAtomParity, std::vector<Bond*> &bondsWithPreDefinedBondStereo)
						{
						...
						void StereochemistryHandler::matchStereochemistryToAtomsAndBonds(Element *stereoChemistryEl) throw(StructureBuildingException, StereochemistryException)
						{
							...
							{
								throw StereochemistryException(stereoChemistryType + std::wstring(L" stereochemistry is not currently interpretable by OPSIN"));
							}
							...
							{
								state->addWarning(OpsinWarningType::STEREOCHEMISTRY_IGNORED, std::wstring(L"Optical rotation cannot be algorithmically used to assign stereochemistry. This term was ignored: ") + stereoChemistryEl->getValue());
							}
							else
							{
						...
						void StereochemistryHandler::processCarbohydrateStereochemistry(std::vector<Element*> &carbohydrateStereoChemistryEls) throw(StructureBuildingException)
						{
							...
							for (auto carbohydrateStereoChemistryEl : carbohydrateStereoChemistryEls)
							{
								...
								if (nextGroup == nullptr || (XmlDeclarations::SYSTEMATICCARBOHYDRATESTEMALDOSE_SUBTYPE_VAL != nextGroup->getAttributeValue(XmlDeclarations::SUBTYPE_ATR) && XmlDeclarations::SYSTEMATICCARBOHYDRATESTEMKETOSE_SUBTYPE_VAL != nextGroup->getAttributeValue(XmlDeclarations::SUBTYPE_ATR)))
								{
...
//ORIGINAL LINE: throw new RuntimeException("OPSIN bug: Could not find carbohydrate chain stem to apply stereochemistry to");
						...
						void StereochemistryHandler::assignStereoCentre(Element *stereoChemistryEl) throw(StructureBuildingException, StereochemistryException)
						{
							...
							std::vector<Fragment*> possibleFragments = StructureBuildingMethods::findAlternativeFragments(parentSubBracketOrRoot);
							std::vector<Element*> adjacentGroupEls = OpsinTools::getDescendantElementsWithTagName(parentSubBracketOrRoot, XmlDeclarations::GROUP_EL);
							...
							if (possibleWordParent->getName() == XmlDeclarations::WORD_EL && possibleWordParent->getChild(0)->equals(parentSubBracketOrRoot))
							{
								...
								for (auto word : words)
								{
									std::vector<Element*> possibleGroups = OpsinTools::getDescendantElementsWithTagName(word, XmlDeclarations::GROUP_EL);
							...
							throw StereochemistryException(std::wstring(L"Could not find atom that: ") + stereoChemistryEl->toXML() + std::wstring(L" appeared to be referring to"));
						}

						bool StereochemistryHandler::attemptAssignmentOfStereoCentreToFragment(Fragment *fragment, const std::wstring &rOrS, const std::wstring &locant) throw(StereochemistryException, StructureBuildingException)
						{
							if (locant == L"")
							{ //undefined locant
								...
								{
									if (notExplicitlyDefinedStereoCentreMap.find(potentialStereoAtom) != notExplicitlyDefinedStereoCentreMap.end())
									{
										applyStereoChemistryToStereoCentre(potentialStereoAtom, notExplicitlyDefinedStereoCentreMap[potentialStereoAtom], rOrS);
							...
							else
							{
								...
								if (potentialStereoAtom != nullptr && notExplicitlyDefinedStereoCentreMap.find(potentialStereoAtom) != notExplicitlyDefinedStereoCentreMap.end())
								{
									applyStereoChemistryToStereoCentre(potentialStereoAtom, notExplicitlyDefinedStereoCentreMap[potentialStereoAtom], rOrS);
									notExplicitlyDefinedStereoCentreMap.erase(potentialStereoAtom);
									return true;
						...
						void StereochemistryHandler::applyStereoChemistryToStereoCentre(Atom *atom, StereoCentre *stereoCentre, const std::wstring &rOrS) throw(StructureBuildingException, StereochemistryException)
						{
							...
							{ //from highest to lowest (true for S) hence atomParity 1 for S
						...
						void StereochemistryHandler::assignStereoBond(Element *stereoChemistryEl) throw(StructureBuildingException, StereochemistryException)
						{
...
//JAVA TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'toString':
							if (possibleWordParent->getName() == XmlDeclarations::WORD_EL && possibleWordParent->getAttributeValue(XmlDeclarations::TYPE_ATR) == WordType::substituent->toString())
							{
								//the element is in front of a substituent and may refer to the full group
								//i.e. the stereochemistry is in a different word to what it is applied to
//JAVA TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'toString':
								std::vector<Element*> words = OpsinTools::getChildElementsWithTagNameAndAttribute(possibleWordParent->getParent(), XmlDeclarations::WORD_EL, XmlDeclarations::TYPE_ATR, WordType::full->toString());
								for (auto word : words)
								{
									std::vector<Element*> possibleGroups = OpsinTools::getDescendantElementsWithTagName(word, XmlDeclarations::GROUP_EL);
									for (int i = possibleGroups.size() - 1; i >= 0; i--)
							...
							if (isCisTrans)
							{
								throw StereochemistryException(std::wstring(L"Could not find bond that: ") + stereoChemistryEl->toXML() + std::wstring(L" could refer unambiguously to"));
							}
							else
							{
								throw StereochemistryException(std::wstring(L"Could not find bond that: ") + stereoChemistryEl->toXML() + std::wstring(L" was referring to"));
							}
						}

						bool StereochemistryHandler::attemptAssignmentOfStereoBondToFragment(Fragment *fragment, const std::wstring &eOrZ, const std::wstring &locant, bool isCisTrans) throw(StereochemistryException)
						{
							if (locant == L"")
							{ //undefined locant
								...
								{
									if (notExplicitlyDefinedStereoBondMap.find(potentialBond) != notExplicitlyDefinedStereoBondMap.end() && (!isCisTrans || cisTransUnambiguousOnBond(potentialBond)))
									{
								...
								std::vector<Bond*> sortedInterFragmentBonds = sortInterFragmentBonds(state->fragManager->getInterFragmentBonds(fragment), fragment);
						...
						std::vector<Bond*> StereochemistryHandler::sortInterFragmentBonds(Set<Bond*> *interFragmentBonds, Fragment *preferredOriginatingFragment)
						{
						...
						void StereochemistryHandler::applyStereoChemistryToStereoBond(Bond *bond, StereoBond *stereoBond, const std::wstring &eOrZ) throw(StereochemistryException)
						{
						...
						bool StereochemistryHandler::attemptAssignmentOfCisTransRingStereoToFragment(Fragment *fragment, Element *stereoChemistryEl) throw(StructureBuildingException)
						{
							std::vector<Atom*> atomList = fragment->getAtomList();
								...
								if (potentialStereoAtom->getAtomIsInACycle())
								{
									...
									if (neighbours.size() == 4)
									{
										...
										for (auto neighbour : neighbours)
										{
											...
											if (!neighbour->getAtomIsInACycle() || !std::find(atomList.begin(), atomList.end(), neighbour) != atomList.end())
											{
												acylicOrNotInFrag++;
											}
										}
							...
							if (stereoAtoms.size() == 2)
							{
								...
								if (a1->getAtomParity() != nullptr && a2->getAtomParity() != nullptr)
								{ //one can have defined stereochemistry but not both
									return false;
								}
								...
								applyStereoChemistryToCisTransOnRing(a1, a2, paths, atomList, stereoChemistryEl->getAttributeValue(XmlDeclarations::VALUE_ATR));
						...
						void StereochemistryHandler::applyStereoChemistryToCisTransOnRing(Atom *a1, Atom *a2, std::vector<std::vector<Atom*>> &paths, std::vector<Atom*> &fragmentAtoms, const std::wstring &cisOrTrans) throw(StructureBuildingException)
						{
...
//JAVA TO C++ CONVERTER TODO TASK: The Java Collection 'remove(Object)' method is not converted:
						...
						Atom *StereochemistryHandler::getHydrogenOrAcyclicOrOutsideOfFragment(std::vector<Atom*> &atoms, std::vector<Atom*> &fragmentAtoms)
						{
							for (auto atom : atoms)
							{
							...
							for (auto atom : atoms)
							{
								if (!atom->getAtomIsInACycle() || !std::find(fragmentAtoms.begin(), fragmentAtoms.end(), atom) != fragmentAtoms.end())
								{
									return atom;
								}
							}
						...
						void StereochemistryHandler::assignAlphaBetaXiStereochem(Element *stereoChemistryEl) throw(StructureBuildingException)
						{
							...
							{
								Atom *potentialStereoAtom = fragment->getAtomByLocant(locant);
								if (potentialStereoAtom != nullptr && atomStereoCentreMap.find(potentialStereoAtom) != atomStereoCentreMap.end())
								{ //same stereocentre can be defined twice e.g. one subsituent alpha the other beta
									if (alphaOrBeta == L"xi")
									{
									...
									{
										std::wstring alphaBetaClockWiseAtomOrdering = fragment->getTokenEl()->getAttributeValue(XmlDeclarations::ALPHABETACLOCKWISEATOMORDERING_ATR);
										if (alphaBetaClockWiseAtomOrdering == L"")
										{
											throw StructureBuildingException(L"Identified fragment is not known to be able to support alpha/beta stereochemistry");
										}
							...
							throw StructureBuildingException(std::wstring(L"Could not find atom that: ") + stereoChemistryEl->toXML() + std::wstring(L" appeared to be referring to"));
						}

						void StereochemistryHandler::applyAlphaBetaStereochemistryToStereoCentre(Atom *stereoAtom, Fragment *fragment, const std::wstring &alphaBetaClockWiseAtomOrdering, const std::wstring &alphaOrBeta, Fragment *substituentGroup) throw(StructureBuildingException)
						{
...
//JAVA TO C++ CONVERTER TODO TASK: The Java Collection 'remove(Object)' method is not converted:
									...
									if ((std::find(fragment->getAtomList().begin(), fragment->getAtomList().end(), a1) != fragment->getAtomList().end() && std::find(ringOrder.begin(), ringOrder.end(), a1->getFirstLocant()) != ringOrder.end())))
									{
										atomRefs4[1] = a1;
										atomRefs4[2] = a2;
									}
									else if ((std::find(fragment->getAtomList().begin(), fragment->getAtomList().end(), a2) != fragment->getAtomList().end() && std::find(ringOrder.begin(), ringOrder.end(), a2->getFirstLocant()) != ringOrder.end())))
									{
										atomRefs4[1] = a2;
										atomRefs4[2] = a1;
									}
									...
									else if (substituentGroup != nullptr && fragment != substituentGroup && std::find(substituentGroup->getAtomList().begin(), substituentGroup->getAtomList().end(), a1) != substituentGroup->getAtomList().end())
									{
										atomRefs4[1] = a1;
										atomRefs4[2] = a2;
									}
									else if (substituentGroup != nullptr && fragment != substituentGroup && std::find(substituentGroup->getAtomList().begin(), substituentGroup->getAtomList().end(), a2) != substituentGroup->getAtomList().end())
									{
										atomRefs4[1] = a2;
										atomRefs4[2] = a1;
									}
									...
									{
										throw StructureBuildingException(std::wstring(L"alpha/beta stereochemistry could not be determined at position ") + stereoAtom->getFirstLocant());
									}
									...
									if (notExplicitlyDefinedStereoCentreMap.find(stereoAtom) == notExplicitlyDefinedStereoCentreMap.end())
									{ //stereocentre has already been defined, need to check for contradiction!
										AtomParity *newAtomParity = stereoAtom->getAtomParity();
										if (previousAtomParity == nullptr)
										{
											if (newAtomParity != nullptr)
											{
												throw StructureBuildingException(std::wstring(L"contradictory alpha/beta stereochemistry at position ") + stereoAtom->getFirstLocant());
											}
										}
						...
						void StereochemistryHandler::assignCarbohydratePrefixStereochem(Element *carbohydrateGroup, std::vector<Element*> &carbohydrateStereoChemistryEls) throw(StructureBuildingException)
						{
							...
							{
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
								std::vector<std::wstring> values = carbohydrateStereoChemistryEl->getAttributeValue(XmlDeclarations::VALUE_ATR).split(L"/");
								Collections::addAll(stereocentreConfiguration, values);
							}

							if (stereocentresInCarbohydrate.size() != stereocentreConfiguration.size())
							{
								throw StructureBuildingException(std::wstring(L"Disagreement between number of stereocentres on carbohydrate: ") + stereocentresInCarbohydrate.size() + std::wstring(L" and centres defined by configurational prefixes: ") + stereocentreConfiguration.size());
							}
...
//ORIGINAL LINE: throw new RuntimeException("OPSIN bug: unexpected carbohydrate stereochemistry configuration: " + configuration);
						...
						void StereochemistryHandler::assignDlStereochem(Element *stereoChemistryEl) throw(StructureBuildingException)
						{
							...
							Element *elementToApplyTo = OpsinTools::getNextSiblingIgnoringCertainElements(stereoChemistryEl, std::vector<std::wstring>{XmlDeclarations::STEREOCHEMISTRY_EL});
							if (elementToApplyTo != nullptr && elementToApplyTo->getName() == XmlDeclarations::GROUP_EL && attemptAssignmentOfDlStereoToFragment(elementToApplyTo->getFrag(), dOrL))
							{
							...
							throw StereochemistryException(std::wstring(L"Could not find stereocentre to apply ") + dOrL.toUpperCase(Locale::ROOT) + std::wstring(L" stereochemistry to"));
						}

						bool StereochemistryHandler::attemptAssignmentOfDlStereoToFragment(Fragment *fragment, const std::wstring &dOrL) throw(StereochemistryException, StructureBuildingException)
						{
							std::vector<Atom*> atomList = fragment->getAtomList();
							for (auto potentialStereoAtom : atomList)
							{
								if (notExplicitlyDefinedStereoCentreMap.find(potentialStereoAtom) != notExplicitlyDefinedStereoCentreMap.end() && potentialStereoAtom->getBondCount() == 4)
								{
									std::vector<Atom*> neighbours = potentialStereoAtom->getAtomNeighbours();
...
//ORIGINAL LINE: throw new RuntimeException("OPSIN bug: Unexpected value for D/L stereochemistry found: " + dOrL);
						...
						bool StereochemistryHandler::checkEquivalencyOfAtomsRefs4AndParity(std::vector<Atom*> &atomRefs1, int atomParity1, std::vector<Atom*> &atomRefs2, int atomParity2)
						{
