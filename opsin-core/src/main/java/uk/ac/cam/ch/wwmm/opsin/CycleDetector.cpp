				...
				namespace wwmm
				{
					namespace opsin
					{
						...
						void CycleDetector::assignWhetherAtomsAreInCycles(Fragment *frag)
						{
							...
							for (auto a : atomList)
							{ //as OPSIN does not disallow disconnected sections within a single "fragment" (e.g. in suffixes) for vigorousness this for loop is required
...
//JAVA TO C++ CONVERTER TODO TASK: The Java Collection 'remove(Object)' method is not converted:
						...
						CycleDetector::PathSearchState::PathSearchState(Atom *currentAtom, std::vector<Atom*> &orderAtomsVisited) : currentAtom(currentAtom), orderAtomsVisited(orderAtomsVisited)
						{
						}
						...
						std::vector<std::vector<Atom*>> CycleDetector::getPathBetweenAtomsUsingBonds(Atom *a1, Atom *a2, Set<Bond*> *peripheryBonds)
						{
							...
							while (stateStack->size() > 0)
							{
								...
								for (auto neighbourBond : neighbourBonds)
								{
									...
									if (std::find(orderAtomsVisited.begin(), orderAtomsVisited.end(), neighbour) != orderAtomsVisited.end())
									{ //atom already visited by this path
										continue;
									}
									if (neighbour == a2)
									{ //target atom found
									...
									{ //add atom to stack, its neighbours will be recursively investigated shortly
