...
//JAVA TO C++ CONVERTER WARNING: The following method was originally marked 'synchronized':
						...
						std::wstring NameToStructure::getVersion()
						{
							try
							{
								InputStream *is = NameToStructure::typeid->getResourceAsStream(L"opsinbuild.props");
								try
								{
...
//ORIGINAL LINE: java.util.Collections.sort(parses, new SortParses());
						...
						void NameToStructure::main(std::vector<std::wstring> &args) throw(std::exception)
						{
							...
							else if (outputType.equalsIgnoreCase(L"extendedsmi") || outputType.equalsIgnoreCase(L"extendedsmiles") || outputType.equalsIgnoreCase(L"cxsmi") || outputType.equalsIgnoreCase(L"cxsmiles"))
							{
							...
							else
							{
								...
								System::err::println(L"Expected output types are \"cml\", \"smi\", \"inchi\", \"stdinchi\" and \"stdinchikey\"");
								exit(1);
							}
						...
						void NameToStructure::displayUsage(Options *options)
						{
							...
							formatter->printHelp(std::wstring(L"java -jar opsin-") + (version != L"" ? version : L"[version]") + std::wstring(L"-jar-with-dependencies.jar [options] [inputfile] [outputfile]") + OpsinTools::NEWLINE + std::wstring(L"OPSIN converts systematic chemical names to CML, SMILES or InChI/StdInChI/StdInChIKey") + OpsinTools::NEWLINE + std::wstring(L"Names should be new line delimited and may be read from stdin (default) or a file and output to stdout (default) or a file"), options);
							exit(0);
						}
						...
						Options *NameToStructure::buildCommandLineOptions()
						{
							...
							options->addOption(L"a", L"allowAcidsWithoutAcid", false, L"Allows interpretation of acids without the word acid e.g. \"acetic\"");
							options->addOption(L"f", L"detailedFailureAnalysis", false, L"Enables reverse parsing to more accurately determine why parsing failed");
							options->addOption(L"r", L"allowRadicals", false, L"Enables interpretation of radicals");
							options->addOption(L"s", L"allowUninterpretableStereo", false, L"Allows stereochemistry uninterpretable by OPSIN to be ignored");
						...
						void NameToStructure::interactiveCmlOutput(InputStream *input, OutputStream *out, NameToStructureConfig *n2sconfig) throw(IOException, XMLStreamException)
						{
						...
						void NameToStructure::interactiveSmilesOutput(InputStream *input, OutputStream *out, NameToStructureConfig *n2sconfig, bool extendedSmiles) throw(IOException)
						{
							NameToStructure *nts = NameToStructure::getInstance();
							InputStreamReader tempVar(input, L"UTF-8");
							BufferedReader *inputReader = new BufferedReader(&tempVar);
						...
						void NameToStructure::interactiveInchiOutput(InputStream *input, OutputStream *out, NameToStructureConfig *n2sconfig, InchiType inchiType) throw(std::exception)
						{
							NameToStructure *nts = NameToStructure::getInstance();
							InputStreamReader tempVar(input, L"UTF-8");
							BufferedReader *inputReader = new BufferedReader(&tempVar);
