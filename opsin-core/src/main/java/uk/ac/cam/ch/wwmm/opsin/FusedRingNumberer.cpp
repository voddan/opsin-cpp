				...
				namespace wwmm
				{
					namespace opsin
					{
						...
						RingConnectivityTable *FusedRingNumberer::RingConnectivityTable::copy()
						{
							...
							copy->directionFromRingToNeighbouringRing.insert(copy->directionFromRingToNeighbouringRing.end(), directionFromRingToNeighbouringRing.begin(), directionFromRingToNeighbouringRing.end());
...
const std::unordered_map<ChemEl, int> FusedRingNumberer::heteroAtomValues = new java::util::EnumMap<ChemEl, int>(ChemEl::typeid);
...
FusedRingNumberer::StaticConstructor FusedRingNumberer::staticConstructor;
						...
						void FusedRingNumberer::numberFusedRing(Fragment *fusedRing) throw(StructureBuildingException)
						{
							...
							for (auto path : atomSequences)
							{ //TODO properly support interior atom labelling
								for (auto atom : atomList)
								{
...
//ORIGINAL LINE: java.util.Collections.sort(atomSequences, new SortAtomSequences());
						...
						void FusedRingNumberer::setupAdjacentFusedRingProperties(std::vector<Ring*> &rings)
						{
							for (int i = 0, l = rings.size(); i < l; i++)
							{
								...
								for (auto bond : curRing->getBondList())
								{ // go through all the bonds for the current ring
									for (int j = i + 1; j < l; j++)
									{
										Ring *otherRing = rings[j];
										if (std::find(otherRing->getBondList().begin(), otherRing->getBondList().end(), bond) != otherRing->getBondList().end())
										{ // check if this bond belongs to any other ring
...
//ORIGINAL LINE: throw new RuntimeException("Invalid ring size: " +ring.size());
						...
						std::vector<std::vector<Atom*>> FusedRingNumberer::determinePossiblePeripheryAtomOrders(std::vector<Ring*> &rings, int atomCountOfFusedRingSystem) throw(StructureBuildingException)
						{
...
//ORIGINAL LINE: throw new RuntimeException("OPSIN Bug: Non-fused bond from terminal ring not found");
							...
							std::unordered_map<RingConnectivityTable*, std::vector<int>> horizonalRowDirections = findLongestChainDirections(cts);
							std::vector<std::vector<std::vector<Ring*>>> ringMaps = createRingMapsAlignedAlongGivenhorizonalRowDirections(horizonalRowDirections);
							/* FR-5.2b-d */
						...
						std::vector<RingConnectivityTable*> FusedRingNumberer::buildRingConnectionTables(Ring *currentRing, Ring *previousRing, int previousDir, Bond *previousBond, Atom *atom, RingConnectivityTable *ct, std::vector<RingConnectivityTable*> &cts)
						{
...
//ORIGINAL LINE: throw new RuntimeException("OPSIN limitation, unsupported ring size in fused ring numbering");
							...
							for (int i = allowedShapes.size() - 1; i >= 0; i--)
							{
								...
								for (auto neighbourRing : currentRing->getNeighbours())
								{
									...
									if (!std::find(currentCT->usedRings.begin(), currentCT->usedRings.end(), neighbourRing) != currentCT->usedRings.end())
									{
										std::vector<RingConnectivityTable*> newCts;
										for (auto ctToExpand : ctsToExpand)
										{
											Atom *a = getAtomFromBond(currentRing, currentBond);
											std::vector<RingConnectivityTable*> generatedDownStreamCts = buildRingConnectionTables(neighbourRing, currentRing, dir, currentBond, a, ctToExpand, cts);
						...
						std::vector<FusionRingShape> FusedRingNumberer::removeDegenerateRingShapes(std::vector<FusionRingShape> &allowedRingShapes, std::vector<int> &distances, int ringSize)
						{
...
//JAVA TO C++ CONVERTER TODO TASK: The Java Collection 'remove(Object)' method is not converted:
							distances.remove(static_cast<boost::optional<int>*>(0)); //remove distance 0 if present, this invariably comes from the starting bond and is not of interest (and breaks getDirectionFromDist)
						...
						int FusedRingNumberer::calculateRingDirection(RingShape *ringShape, Bond *previousBond, Bond *currentBond, int previousDir)
						{
...
//ORIGINAL LINE: throw new RuntimeException("OPSIN Bug: Unrecognised fusion ring shape for 7 membered ring");
							...
							else if (ringSize % 2 == 0)
							{ //general case even number of atoms ring (a 6 membered ring or distortion of)
								if (dist == 1)
								{
									dir = -3;
								}
							...
							{ // general case odd number of atoms ring (distortion of an even numbered ring by insertion of one atom).
								if (dist == 1)
								{
									dir = -3;
								}
								else if (dist == ringSize / 2 || dist == ringSize / 2 + 1)
								{ //0 in both cases as effectively we are using a different depiction of the ring system. See FR-5.1.1 (this is done to give the longest horizontal row)
									dir = 0;
								}
								else if (dist == ringSize-1)
								{
...
//ORIGINAL LINE: throw new RuntimeException("OPSIN Bug: Unable to determine direction between odd number of atoms ring and next ring");
						...
						void FusedRingNumberer::removeCTsWithDistortedRingShapes(std::vector<RingConnectivityTable*> &cts)
						{
							...
							for (auto ct : cts)
							{
								...
								for (int i = 0; i < ringShapes.size(); i++)
								{
									...
									{
										if (ringShapes[j]->getRing().equals(r2) && ct->neighbouringRings.get(j)->equals(r1))
										{ //look for the reverse entry in the ring connection table
						...
						std::unordered_map<RingConnectivityTable*, std::vector<int>> FusedRingNumberer::findLongestChainDirections(std::vector<RingConnectivityTable*> &cts)
						{
...
//ORIGINAL LINE: throw new RuntimeException("OPSIN Bug: Sizes of arrays in fused ring numbering connection table are not equal");
								...
								for (int i = 0; i < ctEntriesSize; i++)
								{
									...
									{ //<= rather than < so buggy behaviour can be caught
...
//ORIGINAL LINE: throw new RuntimeException("OPSIN bug: fused ring numbering: Ring missing from connection table");
										...
										if (curChain >= maxChain)
										{
											...
											if (curChain > maxChain || (!std::find(directions.begin(), directions.end(), curDir) != directions.end() && !directions.contains(oDir)))
											{
...
//ORIGINAL LINE: throw new RuntimeException("OPSIN bug: fused ring layout contained a loop: more rings in a chain than there were rings!");
						...
						std::vector<std::vector<std::vector<Ring*>>> FusedRingNumberer::createRingMapsAlignedAlongGivenhorizonalRowDirections(std::unordered_map<RingConnectivityTable*, std::vector<int>> &horizonalRowDirectionsMap) throw(StructureBuildingException)
						{
...
//ORIGINAL LINE: throw new RuntimeException("OPSIN Bug: Sizes of arrays in fused ring numbering connection table are not equal");
								...
								for (* : :optional<int> horizonalRowDirection : entry.second)
								{
									...
									for (int i = 0; i < ctEntriesSize; i++)
									{
										...
										directionFromRingToNeighbouringRing[i] = determineAbsoluteDirectionUsingPreviousDirection(ringShape->getShape(), ringShape->getRing()->size(), ct->directionFromRingToNeighbouringRing[i], -horizonalRowDirection);
									}
									std::vector<std::vector<Ring*>> ringMap = generateRingMap(ct, directionFromRingToNeighbouringRing);
									if (ringMap.size() > 0)
									{ //null if overlapping bonds rings present
							...
							if (ringMaps.empty())
							{
								throw StructureBuildingException(L"Fused ring systems with overlapping rings such as in helices cannot currently be numbered");
							}
							return ringMaps;
						}

						std::vector<std::vector<Atom*>> FusedRingNumberer::findPossiblePaths(std::vector<std::vector<std::vector<Ring*>>> &ringMaps, int atomCountOfFusedRingSystem)
						{
							...
							for (int c = 0; c < chainQs.size(); c++)
							{
								...
								{
									std::vector<std::vector<Ring*>> qRingMap = transformQuadrantToUpperRightOfRingMap(ringMap, upperRightQuadrant);
						...
						std::vector<std::vector<Ring*>> FusedRingNumberer::generateRingMap(RingConnectivityTable *ct, std::vector<int> &directionFromRingToNeighbouringRing)
						{
							int ctEntriesSize = ct->ringShapes.size();
...
//ORIGINAL LINE: throw new RuntimeException("OPSIN Bug: Fused ring numbering bug, Coordinates have been calculated wrongly");
						...
						std::vector<Chain*> FusedRingNumberer::findChainsOfMaximumLengthInHorizontalDir(std::vector<std::vector<Ring*>> &ringMap)
						{
							int w = ringMap.size();
						...
						std::vector<boost::optional<double>> FusedRingNumberer::countQuadrants(std::vector<std::vector<Ring*>> &ringMap, int midChainXcoord, int yChain)
						{
...
//ORIGINAL LINE: throw new RuntimeException("OPSIN Bug: Fused ring numbering, no chains found?");
							...
							for (* : :optional<double>[] chainQ : chainQs)
							{
								for (int j = 0; j < 4; j++)
								{
						...
						std::vector<Atom*> FusedRingNumberer::orderAtoms(std::vector<std::vector<Ring*>> &ringMap, bool inverseAtoms, int atomCountOfFusedRingSystem)
						{
							int w = ringMap.size();
							int h = ringMap[0].length;
...
//ORIGINAL LINE: throw new RuntimeException("OPSIN Bug: Upper right ring not found when performing fused ring numbering");
							...
							while (isEntirelyFusionAtoms(upperRightRing))
							{ //c.f cyclopropa[de]anthracene
...
//ORIGINAL LINE: throw new RuntimeException("OPSIN Bug: Fused ring numbering may have been stuck in an infinite loop while enumerating peripheral numbering");
						...
						Ring *FusedRingNumberer::findClockwiseRingFromUpperRightRing(std::vector<std::vector<Ring*>> &ringMap, Ring *upperRightRing, std::vector<Ring*> &visitedRings)
						{
...
//ORIGINAL LINE: throw new RuntimeException("OPSIN Bug: Ring not found in ringMap when performing fused ring numbering");
						...
						Ring *FusedRingNumberer::findUpperLeftNeighbourOfUpperRightRing(std::vector<std::vector<Ring*>> &ringMap, Ring *upperRightRing)
						{
							Ring *nRing = nullptr;
...
//ORIGINAL LINE: throw new RuntimeException("OPSIN Bug: Ring not found in ringMap when performing fused ring numbering");
						...
						std::vector<std::vector<Ring*>> FusedRingNumberer::transformQuadrantToUpperRightOfRingMap(std::vector<std::vector<Ring*>> &ringMap, int upperRightQuadrant)
						{
							int w = ringMap.size();
							int h = ringMap[0].length;
...
//ORIGINAL LINE: Ring[][] rearrangedMap = new Ring[w][h];
							...
							for (int i = 0; i < w; i++)
							{
								for (int j = 0; j < h; j++)
								{
									...
									else if (upperRightQuadrant == 2)
									{ //flip in x and y axes
										rearrangedMap[w - i - 1][h - j - 1] = ringMap[i][j];
									}
									else if (upperRightQuadrant == 3)
									{ //flip in x axis
...
//ORIGINAL LINE: throw new RuntimeException("The cyclic bond list should already have been generated");
						...
						int FusedRingNumberer::determineAbsoluteDirectionUsingPreviousDirection(FusionRingShape fusionRingShape, int ringSize, int relativeDirection, int previousDir)
						{
							...
							if (std::abs(interimDirection) == 2 && ((ringSize % 2 == 0) || ringSize == 5 || ringSize == 7))
							{
								...
								if (std::abs(relativeDirection) == 1 && std::abs(previousDir) == 3 || std::abs(relativeDirection) == 3 && std::abs(previousDir) == 1)
								{
...
//ORIGINAL LINE: Ring[][] yxOrdered = new Ring[ringMap[0].length][ringMap.length];
							std::vector<std::vector<Ring*>> yxOrdered = RectangularVectors::ReturnRectangularRingVector(ringMap[0].length, ringMap.size());
