...
namespace uk { namespace ac { namespace cam { namespace ch { namespace wwmm { namespace opsin { class ParseRules; } } } } } }
namespace uk { namespace ac { namespace cam { namespace ch { namespace wwmm { namespace opsin { class ParsingException; } } } } } }
namespace uk { namespace ac { namespace cam { namespace ch { namespace wwmm { namespace opsin { class TokenizationResult; } } } } } }
namespace uk { namespace ac { namespace cam { namespace ch { namespace wwmm { namespace opsin { class ReverseParseRules; } } } } } }
...
namespace uk { namespace ac { namespace cam { namespace ch { namespace wwmm { namespace opsin { class ParseWord; } } } } } }
				...
				namespace wwmm
				{
					namespace opsin
					{
						...
						private:
							...
							Pattern *const matchCasCollectiveIndex = Pattern::compile(L"([\\[\\(\\{]([1-9][0-9]?[cC][iI][, ]?)+[\\]\\)\\}])+|[1-9][0-9]?[cC][iI]", Pattern::CASE_INSENSITIVE);
							Pattern *const matchCompoundWithPhrase = Pattern::compile(L"(compd\\. with|compound with|and) ", Pattern::CASE_INSENSITIVE);
						...
						public:
							...
							virtual TokenizationResult *tokenize(const std::wstring &name, bool allowRemovalOfWhiteSpace) throw(ParsingException);
							...
							/// @return </param>
							/// <exception cref="ParsingException">  </exception>
							virtual TokenizationResult *tokenizeRightToLeft(ReverseParseRules *reverseParseRules, const std::wstring &name, bool allowRemovalOfWhiteSpace) throw(ParsingException);
						...
						private:
							...
							void parseWord(TokenizationResult *result, std::vector<ParseTokens*> &parseTokens, const std::wstring &parsedName, bool reverse);

							void addParseWords(std::vector<ParseTokens*> &parseTokens, const std::wstring &parsedName, Parse *parse, bool reverse);

							bool fixWord(TokenizationResult *result, const std::wstring &parsedName, bool allowRemovalOfWhiteSpace) throw(ParsingException);
