...
//						import static uk.ac.cam.ch.wwmm.opsin.XmlDeclarations.*;
						...
						Parser::Parser() throw(IOException) : tokeniser(new Tokeniser(parseRules)), wordRules(new WordRules(resources)), resourceManager(new ResourceManager(resources)), parseRules(new ParseRules(this->resourceManager))
						{
							ResourceGetter *resources = new ResourceGetter(L"uk/ac/cam/ch/wwmm/opsin/resources/");
						}

						Parser::Parser(WordRules *wordRules, Tokeniser *tokeniser, ResourceManager *resourceManager) : tokeniser(tokeniser), resourceManager(resourceManager), parseRules(tokeniser->getParseRules())
						{
							this->wordRules = wordRules;
						}

						std::vector<Element*> Parser::parse(NameToStructureConfig *n2sConfig, const std::wstring &name) throw(ParsingException)
						{
							...
							{ //a mixture, spaces are sufficient for OPSIN to treat as a mixture. These spaces for obvious reasons must not be removed
								TokenizationResult *tokenizationResult = tokeniser->tokenize(matchSemiColonSpace->matcher(name).replaceAll(L" "), false);
								if (tokenizationResult->isSuccessfullyTokenized())
								{
									parse = tokenizationResult->getParse();
								}
							...
							if (parse == nullptr)
							{
								...
								else
								{
									...
									{
										throw ParsingException(name + std::wstring(L" is unparsable due to the following being uninterpretable: ") + tokenizationResult->getUninterpretableName() + std::wstring(L" The following was not parseable: ") + tokenizationResult->getUnparsableName());
									}
								}
							}
							else
							...
							for (auto pp : parses)
							{
								...
								for (auto pw : pp->getWords())
								{
									...
									{
										throw ParsingException(L"OPSIN bug: parseWord should have exactly 1 annotations after creating additional parses step");
									}
...
//JAVA TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'toString':
								...
								try
								{
									...
									if (moleculeEl->getAttributeValue(XmlDeclarations::ISSALT_ATR) != L"" && moleculeEl->getChildElements(XmlDeclarations::WORDRULE_EL).size() < 2)
									{
										throw ParsingException(name + std::wstring(L" is apparently a salt, but the name only contained one component. The name could be describing a class of compounds"));
									}
							...
							if (results.empty())
							{
								...
								throw ParsingException(name + std::wstring(L" could be parsed but OPSIN was unsure of the meaning of the words. This error will occur, by default, if a name is just a substituent"));
							}

							return results;
						}

						std::vector<boost::optional<int>> Parser::processStoichiometryIndication(const std::wstring &ratioString) throw(ParsingException)
						{
...
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
						...
						void Parser::generateExactParseFailureReason(TokenizationResult *tokenizationResult, const std::wstring &name) throw(ParsingException)
						{
...
//ORIGINAL LINE: throw new RuntimeException("Failed to load resources for parsing names from right to left!",e);
						...
						std::vector<Parse*> Parser::generateParseCombinations(Parse *parse) throw(ParsingException)
						{
							...
							for (auto pw : parseWords)
							{
								...
								if (numberOfCombinations > 128)
								{ //checked here to avoid integer overflow on inappropriate input
									throw ParsingException(L"Too many different combinations of word interpretation are possible (>128) i.e. name contains too many terms that OPSIN finds ambiguous to interpret");
								}
							}
						...
						void Parser::writeWordXML(Element *wordEl, std::vector<std::wstring> &tokens, std::vector<std::vector<wchar_t>> &annotations) throw(ParsingException)
						{
							int annotNumber = 0;
							int annotPos = 0;
							...
							for (auto token : tokens)
							{
								...
								if (tokenElement != nullptr)
								{ //null for tokens that have ignoreWhenWritingXML set
								...
								else if (lastTokenElement != nullptr && token.length() > 0)
								{
									...
									{
										lastTokenElement->getAttribute(XmlDeclarations::SUBSEQUENTUNSEMANTICTOKEN_ATR)->setValue(lastTokenElement->getAttributeValue(XmlDeclarations::SUBSEQUENTUNSEMANTICTOKEN_ATR) + token);
									}
									else
									{
						...
						void Parser::applyStoichiometryIndicationToWordRules(Element *moleculeEl, std::vector<Integer> &componentRatios) throw(ParsingException)
						{
							...
							{
								throw ParsingException(std::wstring(L"Component and stoichiometry indication indication mismatch. OPSIN believes there to be ") + wordRules.size() + std::wstring(L" components but ") + componentRatios.size() + std::wstring(L" ratios were given!"));
							}
