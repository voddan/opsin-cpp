			...
			namespace ch
			{
				namespace wwmm
				{
					namespace opsin
					{
						...
						CipSequenceRules::CipOrderingRunTimeException::CipOrderingRunTimeException(const std::wstring &message) : RuntimeException(message)
						{
						}
...
//ORIGINAL LINE: java.util.Collections.sort(neighbours, new SortByCipOrder(chiralAtom));
						...
						std::vector<Atom*> CipSequenceRules::getNeighbouringAtomsInCipOrderIgnoringGivenNeighbour(Atom *neighbourToIgnore) throw(CipOrderingException)
						{
							std::vector<Atom*> neighbours = chiralAtom->getAtomNeighbours();
//JAVA TO C++ CONVERTER TODO TASK: The Java Collection 'remove(Object)' method is not converted:
							...
							{
								throw std::invalid_argument(std::wstring(L"OPSIN bug: Atom") + neighbourToIgnore->getID() + std::wstring(L" was not a neighbour of the given stereogenic atom"));
							}
							try
							{
//JAVA TO C++ CONVERTER TODO TASK: The 'Compare' parameter of std::sort produces a boolean value, while the Java Comparator parameter produces a tri-state result:
//ORIGINAL LINE: java.util.Collections.sort(neighbours, new SortByCipOrder(chiralAtom));
						...
						CipSequenceRules::CipState::CipState(std::vector<AtomWithHistory*> &nextAtoms1, std::vector<AtomWithHistory*> &nextAtoms2) : nextAtoms1(nextAtoms1), nextAtoms2(nextAtoms2)
						{
						}

						CipSequenceRules::AtomWithHistory::AtomWithHistory(Atom *atom, std::vector<Atom*> &visitedAtoms, boost::optional<int> indexOfOriginalFromRoot) : atom(atom), visitedAtoms(visitedAtoms), indexOfOriginalFromRoot(indexOfOriginalFromRoot)
						{
						}

						CipSequenceRules::SortByCipOrder::SortByCipOrder(CipSequenceRules *outerInstance, Atom *chiralAtom) : chiralAtom(chiralAtom), outerInstance(outerInstance)
						{
						}
						...
						int CipSequenceRules::SortByCipOrder::compare(Atom *a, Atom *b)
						{
							...
							throw CipOrderingRunTimeException(L"Failed to assign CIP stereochemistry, this indicates a bug in OPSIN or a limitation in OPSIN's implementation of the sequence rules");
						}
						...
						int CipSequenceRules::SortByCipOrder::compareNeighboursByCipPriorityRules(std::vector<std::vector<AtomWithHistory*>> &neighbours1, std::vector<std::vector<AtomWithHistory*>> &neighbours2)
						{
						...
						std::vector<std::vector<AtomWithHistory*>> CipSequenceRules::SortByCipOrder::getNextLevelNeighbours(std::vector<AtomWithHistory*> &nextAtoms)
						{
...
//ORIGINAL LINE: java.util.Collections.sort(neighbourLists, atomListCipComparator);
						...
						std::vector<std::vector<AtomWithHistory*>> CipSequenceRules::SortByCipOrder::formListsWithSamePriority(std::vector<std::vector<AtomWithHistory*>> &neighbourLists)
						{
...
//ORIGINAL LINE: java.util.Collections.sort(neighbourList, cipComparator);
						...
						CipSequenceRules::SortByCipOrder::CipComparator::CipComparator(CipSequenceRules::SortByCipOrder *outerInstance) : outerInstance(outerInstance)
						{
						}
						...
						CipSequenceRules::SortByCipOrder::AtomListCipComparator::AtomListCipComparator(CipSequenceRules::SortByCipOrder *outerInstance) : outerInstance(outerInstance)
						{
						}

						int CipSequenceRules::SortByCipOrder::AtomListCipComparator::compare(std::vector<AtomWithHistory*> &a, std::vector<AtomWithHistory*> &b)
						{
							int aSize = a.size();
							int bSize = b.size();
						...
						CipSequenceRules::SortByCipOrder::ListOfAtomListsCipComparator::ListOfAtomListsCipComparator(CipSequenceRules::SortByCipOrder *outerInstance) : outerInstance(outerInstance)
						{
						}

						int CipSequenceRules::SortByCipOrder::ListOfAtomListsCipComparator::compare(std::vector<std::vector<AtomWithHistory*>> &a, std::vector<std::vector<AtomWithHistory*>> &b)
						{
							int aSize = a.size();
							int bSize = b.size();
							int differenceInSize = aSize - bSize;
						...
						std::vector<AtomWithHistory*> CipSequenceRules::SortByCipOrder::getNextAtomsWithAppropriateGhostAtoms(AtomWithHistory *atomWithHistory)
						{
							...
							for (auto b : atom->getBonds())
							{
								...
								if (!atomBondConnectsTo->equals(chiralAtom))
								{ //P-91.1.4.2.4 (higher order bonds to chiral centre do not involve duplication of atoms)
									for (int j = b->getOrder(); j > 1; j--)
									{ //add ghost atoms to represent higher order bonds
								...
								{
									if (std::find(visitedAtoms.begin(), visitedAtoms.end(), atomBondConnectsTo) != visitedAtoms.end())
									{ //cycle detected, add ghost atom instead
										Atom *ghost = new Atom(atomBondConnectsTo->getElement());
										if (rule > 0)
										{
											AtomWithHistory tempVar4(ghost, visitedAtomsIncludingCurrentAtom, visitedAtoms.find(atomBondConnectsTo));
