...
//						import static uk.ac.cam.ch.wwmm.opsin.XmlDeclarations.*;
						...
						FragmentManager::FragmentManager(SMILESFragmentBuilder *sBuilder, IDManager *idManager) : sBuilder(sBuilder), idManager(idManager)
						{
						...
						Fragment *FragmentManager::buildSMILES(const std::wstring &smiles, const std::wstring &type, const std::wstring &labelMapping) throw(StructureBuildingException)
						{
						...
						Fragment *FragmentManager::buildSMILES(const std::wstring &smiles, Element *tokenEl, const std::wstring &labelMapping) throw(StructureBuildingException)
						{
						...
						void FragmentManager::incorporateFragment(Fragment *childFrag, Fragment *parentFrag) throw(StructureBuildingException)
						{
							...
							{ //reassign inter-fragment bonds of child
						...
						void FragmentManager::incorporateFragment(Fragment *childFrag, Atom *fromAtom, Fragment *parentFrag, Atom *toAtom, int bondOrder) throw(StructureBuildingException)
						{
						...
						void FragmentManager::replaceAtomWithAtom(Atom *a, Atom *heteroAtom, bool assignLocant) throw(StructureBuildingException)
						{
...
//JAVA TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'toString':
								while (a->getFrag()->getAtomByLocant(chemEl::toString() + primes) != nullptr)
								{ //if element symbol already assigned, add a prime and try again
...
//JAVA TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'toString':
						...
						Fragment *FragmentManager::copyAndRelabelFragment(Fragment *originalFragment, int primesToAdd)
						{
							...
							for (auto atom : atomList)
							{
								...
								if (primesToAdd != 0)
								{
									for (int i = 0; i < newLocants.size(); i++)
									{
										...
										while (originalFragment->getAtomByLocant(locantSansPrimes + StringTools::multiplyString(L"'", highestNumberOfPrimesWithThisLocant + 1)) != nullptr)
										{
											highestNumberOfPrimesWithThisLocant++;
										}
										newLocants[i] = locantSansPrimes + StringTools::multiplyString(L"'", ((highestNumberOfPrimesWithThisLocant + 1)*primesToAdd) + currentPrimes);
									}
								}
							...
							for (int i = 0, l = originalFragment->getOutAtomCount(); i < l; i++)
							{
								...
								{
									newFragment->getOutAtom(newFragment->getOutAtomCount() - 1)->setLocant(outAtom->getLocant() + StringTools::multiplyString(L"'", primesToAdd));
								}
							}
							...
							{
								Bond *newBond = createBond(oldToNewAtomMap[bond->getFromAtom()], oldToNewAtomMap[bond->getToAtom()], bond->getOrder());
						...
						Element *FragmentManager::cloneElement(BuildState *state, Element *elementToBeCloned) throw(StructureBuildingException)
						{
							return cloneElement(state, elementToBeCloned, 0);
						}

						Element *FragmentManager::cloneElement(BuildState *state, Element *elementToBeCloned, int primesToAdd) throw(StructureBuildingException)
						{
							Element *clone = elementToBeCloned->copy();
							std::vector<Element*> originalGroups = OpsinTools::getDescendantElementsWithTagName(elementToBeCloned, XmlDeclarations::GROUP_EL);
							std::vector<Element*> clonedGroups = OpsinTools::getDescendantElementsWithTagName(clone, XmlDeclarations::GROUP_EL);
							...
							for (auto originalFragment : oldNewFragmentMapping)
							{ //add inter fragment bonds to cloned fragments
							...
							for (auto bond : interFragmentBondsToClone)
							{
								...
								if (oldNewFragmentMapping.find(originalFragment1) == oldNewFragmentMapping.end() || (oldNewFragmentMapping.find(originalFragment2) == oldNewFragmentMapping.end()))
								{
									throw StructureBuildingException(L"An element that was a clone contained a bond that went outside the scope of the cloning");
								}
						...
						void FragmentManager::makeHydrogensExplicit() throw(StructureBuildingException)
						{
							...
							{
								if (fragment->getSubType().equals(XmlDeclarations::ELEMENTARYATOM_SUBTYPE_VAL))
...
//JAVA TO C++ CONVERTER TODO TASK: The Java Collection 'remove(Object)' method is not converted:
									...
									if (neighbours.empty())
									{
										throw StructureBuildingException(L"OPSIN Bug: Unable to determine which atom has substituted a hydrogen at stereocentre");
									}
