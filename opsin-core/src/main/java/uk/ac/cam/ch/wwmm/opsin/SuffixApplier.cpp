...
//						import static uk.ac.cam.ch.wwmm.opsin.XmlDeclarations.*;
						...
						void SuffixApplier::resolveSuffixes(Element *group, std::vector<Element*> &suffixes) throw(StructureBuildingException, ComponentGenerationException)
						{
							...
							std::wstring suffixTypeToUse = isGroupTypeWithSpecificSuffixRules(groupType) ? groupType : XmlDeclarations::STANDARDGROUP_TYPE_VAL;

							std::vector<Fragment*> associatedSuffixFrags = state->xmlSuffixMap[group];
							if (associatedSuffixFrags.size() > 0)
							{ //null for non-final group in polycyclic spiro systems
							...
							for (auto suffix : suffixes)
							{
								...
								if (suffix->getFrag() != nullptr)
								{
									...
									if (boughtonIsotopeSpecification != nullptr && boughtonIsotopeSpecification->getName() == XmlDeclarations::ISOTOPESPECIFICATION_EL)
									{
										if (XmlDeclarations::BOUGHTONSYSTEM_TYPE_VAL == boughtonIsotopeSpecification->getAttributeValue(XmlDeclarations::TYPE_ATR))
										{
...
//ORIGINAL LINE: throw new RuntimeException("Unexpected isotope specification after suffix");
									...
									while (iupacIsotopeSpecification != nullptr && iupacIsotopeSpecification->getName() == XmlDeclarations::ISOTOPESPECIFICATION_EL && XmlDeclarations::IUPACSYSTEM_TYPE_VAL == iupacIsotopeSpecification->getAttributeValue(XmlDeclarations::TYPE_ATR))
									{
							...
							for (auto entry : suffixValToSuffixes)
							{
								...
								std::vector<SuffixRule*> rulesToApply = suffixRules->getSuffixRuleTags(suffixTypeToUse, suffixValue, subgroupType);
...
//ORIGINAL LINE: throw new RuntimeException("OPSIN Bug: Suffix was expected to have an associated fragment but it wasn't found");
												...
												{
													throw ComponentGenerationException(L"OPSIN Bug: Dummy atom in suffix should have at least one bond to it");
												}
												...
												else
												{
													...
													if (fragAtomToUse == nullptr)
													{
														if (possibleAtomsToAttachSuffixTo.empty())
														{
															...
															possibleAtomsToAttachSuffixTo = FragmentTools::findnAtomsForSubstitution(frag, atomList[0], substitutionsRequired, bondOrderRequired, true);
															if (possibleAtomsToAttachSuffixTo.empty())
															{
																throw StructureBuildingException(std::wstring(L"No suitable atom found to attach ") + suffixValue + std::wstring(L" suffix"));
															}
															for (auto atom : possibleAtomsToAttachSuffixTo)
															{
																if (FragmentTools::isCharacteristicAtom(atom))
																{
																	throw StructureBuildingException(L"No suitable atom found to attach suffix");
																}
															}
															if ((std::wstring(L"yes")) == suffixRule->getAttributeValue(XmlDeclarations::SUFFIXRULES_KETONELOCANT_ATR) && !atomList[0]->getAtomIsInACycle())
															{
																std::vector<Atom*> proKetoneAtoms = getProKetonePositions(possibleAtomsToAttachSuffixTo);
															...
															if (!(substitutionsRequired == 1 && (XmlDeclarations::ALKANESTEM_SUBTYPE_VAL == frag->getSubType() || XmlDeclarations::HETEROSTEM_SUBTYPE_VAL == frag->getSubType()) && possibleAtomsToAttachSuffixTo[0]->equals(frag->getFirstAtom())))
															{
																if (AmbiguityChecker::isSubstitutionAmbiguous(possibleAtomsToAttachSuffixTo, substitutionsRequired))
																{
																	state->addIsAmbiguous(std::wstring(L"Addition of ") + suffixValue + std::wstring(L" suffix to: ") + group->getValue());
																}
															}
														}
													...
													for (auto bondToSuffix : bonds)
													{
														...
														if (fragAtomToUse->getIncomingValency() > 2 && (suffixValue == L"aldehyde" || suffixValue == L"al" || suffixValue == L"aldoxime"))
														{ //formaldehyde/methanal are excluded as they are substitutable
															if ((std::wstring(L"X")) == suffixAtom->getFirstLocant())
															{ //carbaldehyde
											...
											{
												throw ComponentGenerationException(std::wstring(L"OPSIN bug: Suffix may only have one addgroup rule: ") + suffix->getValue());
											}
											break;
										case changecharge:
										{
											int chargeChange = std::stoi(suffixRule->getAttributeValue(XmlDeclarations::SUFFIXRULES_CHARGE_ATR));
											int protonChange = std::stoi(suffixRule->getAttributeValue(XmlDeclarations::SUFFIXRULES_PROTONS_ATR));
											...
											{ //a suffix prefixed acylium suffix
												if (suffixFrag == nullptr)
												{
													throw StructureBuildingException(L"OPSIN bug: ordering of elements in suffixRules.xml wrong; changeCharge found before addGroup");
												}
												Set<Bond*> *bonds = state->fragManager->getInterFragmentBonds(suffixFrag);
												if (bonds->size() != 1)
												{
													throw StructureBuildingException(L"OPSIN bug: Wrong number of bonds between suffix and group");
												}
												for (auto bond : bonds)
												{
										...
										case setOutAtom:
										{
											std::wstring outValencyAtr = suffixRule->getAttributeValue(XmlDeclarations::SUFFIXRULES_OUTVALENCY_ATR);
											...
											{ //something like oyl on a ring, which means it is now carbonyl and the outAtom is on the suffix and not frag
												if (suffixFrag == nullptr)
												{
													throw StructureBuildingException(L"OPSIN bug: ordering of elements in suffixRules.xml wrong; setOutAtom found before addGroup");
												}
												Set<Bond*> *bonds = state->fragManager->getInterFragmentBonds(suffixFrag);
												if (bonds->size() != 1)
												{
										...
										{
											ChemEl chemEl = StringHelper::fromString<ChemEl>(suffixRule->getAttributeValue(XmlDeclarations::SUFFIXRULES_ELEMENT_ATR));
									...
									if (suffixFrag != nullptr)
									{ //merge suffix frag and parent fragment
							...
							for (auto suffixFrag : fragsToMerge)
							{
								...
								{
									if (std::isdigit(suffixLocant.charAt(0)))
									{ //check that numeric locants do not conflict with the parent fragment e.g. hydrazide 2' with biphenyl 2'
						...
						void SuffixApplier::applyIsotopeToSuffix(Fragment *frag, Element *isotopeSpecification, bool mustBeApplied) throw(StructureBuildingException)
						{
							...
							if (locants.empty())
							{
								...
								if (chemEl == ChemEl::H)
								{
									std::vector<Atom*> parentAtomsToApplyTo = FragmentTools::findnAtomsForSubstitution(atoms, nullptr, multiplier, 1, true);
...
//JAVA TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'toString':
						...
						std::vector<Atom*> SuffixApplier::getProKetonePositions(std::vector<Atom*> &atoms)
						{
							...
							for (auto atom : atoms)
							{
								...
								if (bonds.size() == 2 && bonds[0]->getOrder() == 1 && bonds[1]->getOrder() == 1 && bonds[0]->getOtherAtom(atom).getElement() == ChemEl::C && bonds[1]->getOtherAtom(atom).getElement() == ChemEl::C)
								{
						...
						void SuffixApplier::processCycleFormingSuffix(Fragment *suffixFrag, Fragment *suffixableFragment, Element *suffix) throw(StructureBuildingException, ComponentGenerationException)
						{
...
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
								...
								{
									throw ComponentGenerationException(std::wstring(L"Incorrect number of locants associated with cycle forming suffix, expected 2 found: ") + locants.size());
								}
							}
							else if (locantId != L"")
							{
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
								...
								if (locantIds.size() != 2)
								{
									throw ComponentGenerationException(L"OPSIN bug: Should be exactly 2 locants associated with a cyclic suffix");
								}
							...
							else
							{
								...
								else
								{
									...
									{
										throw ComponentGenerationException(std::wstring(L"cycle forming suffix: ") + suffix->getValue() + std::wstring(L" should be locanted!"));
									}
								}
							}
							if (parentAtom1->equals(parentAtom2))
							{
								throw ComponentGenerationException(std::wstring(L"cycle forming suffix: ") + suffix->getValue() + std::wstring(L" attempted to form a cycle involving the same atom twice!"));
							}
							...
							if (suffixableFragment->getType() == XmlDeclarations::CARBOHYDRATE_TYPE_VAL)
							{
								...
								std::vector<Atom*> chainHydroxy = FragmentTools::findHydroxyLikeTerminalAtoms(parentAtom2->getAtomNeighbours(), ChemEl::O);
								...
								{
									throw ComponentGenerationException(L"The second locant of a carbohydrate lactone should point to a carbon in the chain with a hydroxyl group");
								}
							}
							else
							{
								if (parentAtom2->getElement() == ChemEl::O)
								{ //cyclic suffixes like lactone formally indicate the removal of hydroxy cf. 1979 rule 472.1
						...
						Atom *SuffixApplier::getFragAtomToUse(Fragment *frag, Element *suffix, const std::wstring &suffixTypeToUse) throw(StructureBuildingException)
						{
							std::wstring locant = suffix->getAttributeValue(XmlDeclarations::LOCANT_ATR);
							if (locant != L"")
							{
							...
							else if (suffixTypeToUse == XmlDeclarations::ACIDSTEM_TYPE_VAL || suffixTypeToUse == XmlDeclarations::NONCARBOXYLICACID_TYPE_VAL || suffixTypeToUse == XmlDeclarations::CHALCOGENACIDSTEM_TYPE_VAL)
							{ //means that e.g. sulfonyl, has an explicit outAtom
								return frag->getFirstAtom();
							}
							return nullptr;
						}

						void SuffixApplier::applyUnlocantedChargeModification(std::vector<Atom*> &atomList, int chargeChange, int protonChange)
						{
...
//ORIGINAL LINE: throw new RuntimeException("OPSIN Bug: List of atoms to add charge suffix to was empty");
							...
							for (auto a : atomList)
							{
								...
								std::vector<boost::optional<int>> stableValencies = ValencyChecker::getPossibleValencies(chemEl, a->getCharge() + chargeChange);
								if (stableValencies.empty())
								{ //unstable valency so seems unlikely
									continue;
								}
								int resultantExpectedValency = (a->getLambdaConventionValency() == nullptr ? ValencyChecker::getDefaultValency(chemEl) : a->getLambdaConventionValency()) + a->getProtonsExplicitlyAddedOrRemoved() + protonChange;
								...
								if (protonChange < 0)
								{
									...
									if (a->hasSpareValency() && !std::find(a->getFrag()->getIndicatedHydrogen().begin(), a->getFrag()->getIndicatedHydrogen().end(), a) != a->getFrag()->getIndicatedHydrogen().end())
									{
										substitableHydrogen--;
									}
							...
							if (!nitrogens.empty())
							{
								...
								if (XmlDeclarations::AMINOACID_TYPE_VAL == atomList[0]->getFrag().getType())
								{
									...
									if (std::find(listFromWhichToChoose.begin(), listFromWhichToChoose.end(), atomList[0]) != listFromWhichToChoose.end())
									{
							...
							{
								state->addIsAmbiguous(std::wstring(L"Addition of charge suffix to: ") + chosenAtom->getFrag()->getTokenEl()->getValue());
							}

							chosenAtom->addChargeAndProtons(chargeChange, protonChange);
						}

						void SuffixApplier::swapElementsSuchThatThisElementIsAcidic(Fragment *frag, ChemEl chemEl) throw(StructureBuildingException)
						{
						...
						std::vector<SuffixRule*> SuffixApplier::getSuffixRuleTags(const std::wstring &suffixTypeToUse, const std::wstring &suffixValue, const std::wstring &subgroupType) throw(ComponentGenerationException)
						{
