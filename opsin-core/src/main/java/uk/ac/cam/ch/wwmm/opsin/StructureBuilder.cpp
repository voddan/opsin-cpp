...
//						import static uk.ac.cam.ch.wwmm.opsin.StructureBuildingMethods.*;
						...
						Fragment *StructureBuilder::buildFragment(Element *molecule) throw(StructureBuildingException)
						{
							...
							{
								wordRules = molecule->getChildElements(XmlDeclarations::WORDRULE_EL); //very rarely a word rule adds a top level word rule
							}

							std::vector<Element*> groupElements = OpsinTools::getDescendantElementsWithTagName(molecule, XmlDeclarations::GROUP_EL);
						...
						{
							WordRule wordRule = StringHelper::fromString<WordRule>(wordRuleEl->getAttributeValue(XmlDeclarations::WORDRULE_ATR));
							std::vector<Element*> words = OpsinTools::getChildElementsWithTagNames(wordRuleEl, std::vector<std::wstring>{XmlDeclarations::WORD_EL, XmlDeclarations::WORDRULE_EL});
...
//JAVA TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'toString':
									if (!word->getName().equals(XmlDeclarations::WORD_EL) || !word->getAttributeValue(XmlDeclarations::TYPE_ATR).equals(WordType::full->toString()))
									{
							...
							case substituent:
								...
								{
//JAVA TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'toString':
									if (!word->getName().equals(XmlDeclarations::WORD_EL) || !word->getAttributeValue(XmlDeclarations::TYPE_ATR).equals(WordType::substituent->toString()) || !state->n2sConfig->isAllowRadicals())
									{
...
//JAVA TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'toString':
										...
										{
											throw StructureBuildingException(L"OPSIN bug: Non substituent word found where substituent expected in ester");
										}
									}
									...
									if (traditionalEster)
									{ //e.g. ethylidene dipropanoate
										br = new BuildResults(word);
										outAtomCount = br->getOutAtomCount();
									}
									if (outAtomCount == 1)
									{ //TODO add support for locanted terepthaloyl
								...
								{
									std::wstring locant = word->getAttributeValue(XmlDeclarations::LOCANT_ATR); //specifying a locant for an ateWord is very unusual as this information is typically redundant c.f. dodecamethylene 1,12-bis(chloroformate)
							...
							{
								throw StructureBuildingException(std::wstring(L"There are more radicals in the substituents(") + outAtomCount + std::wstring(L") than there are places to form esters(") + esterIdCount + std::wstring(L")"));
							}
							if (esterIdCount > outAtomCount && outAtomCount % ateGroups.size() != 0)
							{
								...
								throw StructureBuildingException(std::wstring(L"There are less radicals in the substituents(") + outAtomCount + std::wstring(L") than there are places to form esters(") + esterIdCount + std::wstring(L")"));
							}
							for (int i = 0; i < outAtomCount; i++)
							{
								...
								else
								{
									...
									if (!outAtomPosition)
									{
										throw StructureBuildingException(std::wstring(L"Unable to find substituent with locant: ") + locant + std::wstring(L" to form ester!"));
									}
...
//JAVA TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'toString':
							...
							{
								throw StructureBuildingException(std::wstring(L"word: ") + wordIndice + std::wstring(L" was expected to be a substituent"));
							}
							...
							{
								throw StructureBuildingException(std::wstring(L"OutAtom has unexpected valency. Expected 1. Actual: ") + std::to_wstring(substituent1->getOutAtom(0)->getValency()));
							}
							BuildResults *substituent2;
							if (substituent1->getOutAtomCount() == 2)
							{ // e.g. tetramethylene sulfone
								if (substituent1->getOutAtom(1)->getValency() != 1)
								{
									throw StructureBuildingException(std::wstring(L"OutAtom has unexpected valency. Expected 1. Actual: ") + std::to_wstring(substituent1->getOutAtom(1)->getValency()));
								}
							...
							else
							{
								...
								{
									throw StructureBuildingException(std::wstring(L"Expected one outAtom. Found ") + std::to_wstring(substituent1->getOutAtomCount()));
								}
								wordIndice++;
//JAVA TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'toString':
								if (words[wordIndice]->getAttributeValue(XmlDeclarations::TYPE_ATR) == WordType::functionalTerm->toString())
								{ //e.g. methyl sulfoxide rather than dimethyl sulfoxide
								...
								{
									throw StructureBuildingException(std::wstring(L"Expected one outAtom. Found ") + std::to_wstring(substituent2->getOutAtomCount()));
								}
								if (substituent2->getOutAtom(0)->getValency() != 1)
								{
									throw StructureBuildingException(std::wstring(L"OutAtom has unexpected valency. Expected 1. Actual: ") + std::to_wstring(substituent2->getOutAtom(0)->getValency()));
								}
							}
							wordIndice++;
//JAVA TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'toString':
							if (words[wordIndice] == nullptr || words[wordIndice]->getAttributeValue(XmlDeclarations::TYPE_ATR) != WordType::functionalTerm->toString())
							{
								throw StructureBuildingException(words[wordIndice]->getValue() + std::wstring(L" was expected to be a functionalTerm"));
							}
							std::vector<Element*> functionalGroup = OpsinTools::getDescendantElementsWithTagName(words[wordIndice], XmlDeclarations::FUNCTIONALGROUP_EL);
							if (functionalGroup.size() != 1)
							{
								throw StructureBuildingException(L"Unexpected number of functionalGroups found, could be a bug in OPSIN's grammar");
							}
							std::wstring smilesOfGroup = functionalGroup[0]->getAttributeValue(XmlDeclarations::VALUE_ATR);
							Fragment *diValentGroup = state->fragManager->buildSMILES(smilesOfGroup, XmlDeclarations::FUNCTIONALCLASS_TYPE_VAL, XmlDeclarations::NONE_LABELS_VAL);
							...
							{ //c.f. peroxide where it is a linker
						...
						void StructureBuilder::buildMonovalentFunctionalGroup(std::vector<Element*> &words) throw(StructureBuildingException)
						{
							...
							{ //replaces outAtoms with valency greater than 1 with multiple outAtoms; e.g. ylidene -->diyl
							...
							for (int i = 1; i < words.size(); i++)
							{
								...
								{
									throw StructureBuildingException(std::wstring(L"Expected exactly 1 functionalGroup. Found ") + functionalGroups.size());
								}

								Fragment *monoValentFunctionGroup = state->fragManager->buildSMILES(functionalGroups[0]->getAttributeValue(XmlDeclarations::VALUE_ATR), XmlDeclarations::FUNCTIONALCLASS_TYPE_VAL, XmlDeclarations::NONE_LABELS_VAL);
								if (functionalGroups[0]->getAttributeValue(XmlDeclarations::TYPE_ATR) == XmlDeclarations::MONOVALENTSTANDALONEGROUP_TYPE_VAL)
								{
							...
							{ //something like isophthaloyl chloride (more precisely written isophthaloyl dichloride)
							...
							{
								throw StructureBuildingException(L"There are more function groups to attach than there are positions to attach them to!");
							}
							for (int i = 0; i < outAtomCount; i++)
							{
...
//JAVA TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'toString':
							if (wordCountMinus1 < 2 || words[wordCountMinus1]->getAttributeValue(XmlDeclarations::TYPE_ATR) != WordType::functionalTerm->toString())
							{
								throw StructureBuildingException(L"OPSIN Bug: Bug in functionalClassEster rule; 'ester' not found where it was expected");
							}
...
//JAVA TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'toString':
								...
								{
//JAVA TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'toString':
									if (wordType == WordType::functionalTerm->toString() && currentWord->getAttributeValue(XmlDeclarations::VALUE_ATR).equalsIgnoreCase(L"ester"))
									{
										//superfluous ester word
										continue;
									}
									throw StructureBuildingException(std::wstring(L"OPSIN Bug: Bug in functionalClassEster rule; Encountered: ") + currentWord->getAttributeValue(XmlDeclarations::VALUE_ATR));
								}
								...
								for (int j = 0; j < outAtomCount; j++)
								{
									...
									{
										throw StructureBuildingException(L"Substituent was expected to have only have an outgoing valency of 1");
									}
...
//JAVA TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'toString':
							...
							if (words[0]->getName() == XmlDeclarations::WORDRULE_EL)
							{ //e.g. Nicotinic acid N-oxide
//JAVA TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'toString':
								std::vector<Element*> fullWords = OpsinTools::getDescendantElementsWithTagNameAndAttribute(words[0], XmlDeclarations::WORD_EL, XmlDeclarations::TYPE_ATR, WordType::full->toString());
								if (fullWords.empty())
								{
									throw StructureBuildingException(L"OPSIN is entirely unsure where the oxide goes so has decided not to guess");
								}
							...
							std::vector<Element*> multipliers = OpsinTools::getDescendantElementsWithTagName(words[1], XmlDeclarations::MULTIPLIER_EL);
							...
							{
								if (XmlDeclarations::ELEMENTARYATOM_SUBTYPE_VAL == rightMostGroup->getAttributeValue(XmlDeclarations::SUBTYPE_ATR))
								{
							...
							std::vector<Element*> functionalGroup = OpsinTools::getDescendantElementsWithTagName(words[1], XmlDeclarations::FUNCTIONALGROUP_EL);
							if (functionalGroup.size() != 1)
							{
							...
							{
								oxideFragments.push_back(state->fragManager->buildSMILES(smilesReplacement, XmlDeclarations::FUNCTIONALCLASS_TYPE_VAL, labels != L"" ? labels : XmlDeclarations::NONE_LABELS_VAL));
							}
							std::vector<Element*> locantEls = OpsinTools::getDescendantElementsWithTagName(words[1], XmlDeclarations::LOCANT_EL);
							if (locantEls.size() > 1)
							{
...
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
							...
							{ //suffixes (if any) from right to left
							...
							for (int i = 0; i < oxideFragments.size(); i++)
							{
								...
								if (!locantsForOxide.empty())
								{
									...
									if (atomToAddOxideTo->getElement() == ChemEl::C && XmlDeclarations::ELEMENTARYATOM_SUBTYPE_VAL != groupToModify->getSubType())
									{
										throw StructureBuildingException(std::wstring(L"Locant ") + locantsForOxide[i] + std::wstring(L" indicated oxide applied to carbon, but this would lead to hypervalency!"));
									}
						...
						void StructureBuilder::formAppropriateBondToOxideAndAdjustCharges(Atom *atomToAddOxideTo, Atom *oxideAtom) throw(StructureBuildingException)
						{
							boost::optional<int> maxVal = ValencyChecker::getMaximumValency(atomToAddOxideTo->getElement(), atomToAddOxideTo->getCharge());
							if (!maxVal || (atomToAddOxideTo->getIncomingValency() + atomToAddOxideTo->getOutValency() + 2) <= maxVal)
							{
								if (!atomToAddOxideTo->getLambdaConventionValency() || !ValencyChecker::checkValencyAvailableForBond(atomToAddOxideTo, 2))
								{ //probably in well formed names 2 protons should always be added but some names use the lambdaConvention to specify the valency after oxide has been applied
							...
							else
							{
								...
								{
									throw StructureBuildingException(L"Oxide appeared to refer to an atom that has insufficent valency to accept the addition of oxygen");
								}
...
//JAVA TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'toString':
							...
							{
								throw StructureBuildingException(L"OPSIN bug: Wrong word type encountered when applying carbonylDerivative wordRule");
							}
							std::vector<Fragment*> replacementFragments;
							std::vector<std::wstring> locantForFunctionalTerm; //usually not specified
//JAVA TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'toString':
							if (words[1]->getAttributeValue(XmlDeclarations::TYPE_ATR) != WordType::functionalTerm->toString())
							{ //e.g. acetone O-ethyloxime or acetone 1-chloro-1-methylhydrazone
								for (int i = 1; i < words.size(); i++)
								{
									...
									if (children.size() == 1 && children[0]->getName() == XmlDeclarations::BRACKET_EL && children[0]->getAttribute(XmlDeclarations::LOCANT_ATR) != nullptr)
									{
									...
									else if (children.size() == 2 && children[0]->getAttribute(XmlDeclarations::LOCANT_ATR) != nullptr)
									{
										...
										if (children[1]->getName() == XmlDeclarations::ROOT_EL && !frag->hasLocant(locant) && OpsinTools::MATCH_NUMERIC_LOCANT->matcher(locant).matches())
										{ //e.g. 1,3-benzothiazole-2-carbaldehyde 2-phenylhydrazone
											locantForFunctionalTerm.push_back(children[0]->getAttributeValue(XmlDeclarations::LOCANT_ATR));
							...
							{ //e.g. butan-2,3-dione dioxime or hexan2,3-dione 2-oxime
								...
								if (functionalGroup.size() != 1)
								{
									throw StructureBuildingException(std::wstring(L"Expected 1 functionalGroup element found: ") + functionalGroup.size());
								}
								std::wstring smilesReplacement = functionalGroup[0]->getAttributeValue(XmlDeclarations::VALUE_ATR);
								std::wstring labels = functionalGroup[0]->getAttributeValue(XmlDeclarations::LABELS_ATR);
								for (int i = 0; i < numberOfCarbonylReplacements; i++)
								{
									Fragment *replacementFragment = state->fragManager->buildSMILES(smilesReplacement, XmlDeclarations::FUNCTIONALCLASS_TYPE_VAL, labels != L"" ? labels : XmlDeclarations::NONE_LABELS_VAL);
									if (i > 0)
									{
...
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
							...
							if (replacementFragments.size() > 0)
							{
								...
								for (ListIterator<Fragment*> *iterator = fragments.listIterator(fragments.size()); iterator->hasPrevious();)
								{ //iterate in reverse order - right most groups preferred
						...
						void StructureBuilder::replaceCarbonylOxygenWithReplacementFragments(std::vector<Element*> &words, std::vector<Fragment*> &replacementFragments, std::vector<Atom*> &carbonylOxygens, int functionalReplacementsToPerform) throw(StructureBuildingException)
						{
...
//ORIGINAL LINE: throw new RuntimeException("OPSIN Bug: Carbonyl replacement fragment expected to have one outatom");
								...
								if (replacementFrag->getTokenEl()->getParent() == nullptr)
								{ //incorporate only for the case that replacementFrag came from a functional class element
									state->fragManager->incorporateFragment(replacementFrag, carbonylFrag);
								}
							}
						}

						std::vector<Atom*> StructureBuilder::findCarbonylOxygens(Fragment *fragment, std::vector<std::wstring> &locantForCarbonylAtom) throw(StructureBuildingException)
						{
							std::vector<Atom*> matches;
							std::vector<Atom*> rootFragAtomList = fragment->getAtomList();
							for (auto atom : rootFragAtomList)
							{ //find all carbonyl oxygen
									...
									if (neighbours.size() == 1)
									{
										if (neighbours[0]->getElement() == ChemEl::C)
										{
											if (!locantForCarbonylAtom.empty())
											{
												...
												if (numericLocantAtomConnectedToCarbonyl != nullptr)
												{ //could be the carbon of the carbonyl or the ring the carbonyl connects to in say a carbaldehyde
						...
						void StructureBuilder::buildAnhydride(std::vector<Element*> &words) throw(StructureBuildingException)
						{
							...
							{
								throw StructureBuildingException(L"Unexpected number of words in anhydride. Check wordRules.xml, this is probably a bug");
							}
							Element *anhydrideWord = words[words.size() - 1];
							std::vector<Element*> functionalClass = OpsinTools::getDescendantElementsWithTagName(anhydrideWord, XmlDeclarations::FUNCTIONALGROUP_EL);
							...
							std::vector<Element*> multipliers = OpsinTools::getDescendantElementsWithTagName(anhydrideWord, XmlDeclarations::MULTIPLIER_EL);
							if (multipliers.size() > 1)
							{
								throw StructureBuildingException(std::wstring(L"Expected 0 or 1 multiplier found: ") + multipliers.size());
							}
							...
							std::vector<Element*> anhydrideLocants = OpsinTools::getDescendantElementsWithTagNames(anhydrideWord, std::vector<std::wstring>{XmlDeclarations::LOCANT_EL, XmlDeclarations::COLONORSEMICOLONDELIMITEDLOCANT_EL});
							if (anhydrideLocants.size() > 1)
							{
								throw StructureBuildingException(std::wstring(L"Expected 0 or 1 anhydrideLocants found: ") + anhydrideLocants.size());
							}
							if (anhydrideLocants.size() == 1)
							{
							...
							{ //symmetric anhydride
								if (br1->getFunctionalAtomCount() > 1)
								{ //cyclic anhydride
									...
									{ //cyclic anhydride where group has more than 2 acids
										if (anhydrideLocant == L"")
										{
											throw StructureBuildingException(L"Anhydride formation appears to be ambiguous; More than 2 acids, no locants");
										}
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
										std::vector<std::wstring> acidLocants = OpsinTools::MATCH_COLONORSEMICOLON->split(StringTools::removeDashIfPresent(anhydrideLocant));
										if (acidLocants.size() != numberOfAnhydrideLinkages)
										{
											throw StructureBuildingException(L"Mismatch between number of locants and number of anhydride linkages to form");
										}
										if (br1->getFunctionalAtomCount() < (numberOfAnhydrideLinkages *2))
										{
											throw StructureBuildingException(L"Mismatch between number of acid atoms and number of anhydride linkages to form");
										}
...
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
						...
						void StructureBuilder::formAnhydrideLink(const std::wstring &anhydrideSmiles, BuildResults *acidBr1, BuildResults *acidBr2) throw(StructureBuildingException)
						{
						...
						void StructureBuilder::formAnhydrideLink(const std::wstring &anhydrideSmiles, Atom *oxygen1, Atom *oxygen2) throw(StructureBuildingException)
						{
							...
							Fragment *anhydride = state->fragManager->buildSMILES(anhydrideSmiles, XmlDeclarations::FUNCTIONALCLASS_TYPE_VAL, XmlDeclarations::NONE_LABELS_VAL);
							...
							state->fragManager->createBond(atomsInAnhydrideLinkage[atomsInAnhydrideLinkage.size() - 1], atomOnSecondAcidToConnectTo, 1);
...
//JAVA TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'toString':
							...
							{
								throw StructureBuildingException(L"Mismatch between number of halide/pseudo halide fragments and acidic oxygens");
							}
							for (int i = halideCount - 1; i >= 0; i--)
							{
								...
								{
									throw StructureBuildingException(std::wstring(L"Atom type expected to be oxygen but was: ") + acidAtom->getElement());
								}
...
//JAVA TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'toString':
							...
							for (int i = 1; i < words.size(); i++)
							{
								...
								Fragment *monoValentFunctionGroup = state->fragManager->buildSMILES(functionGroup->getAttributeValue(XmlDeclarations::VALUE_ATR), XmlDeclarations::FUNCTIONALCLASS_TYPE_VAL, XmlDeclarations::NONE_LABELS_VAL);
								if (functionGroup->getAttributeValue(XmlDeclarations::TYPE_ATR) == XmlDeclarations::MONOVALENTSTANDALONEGROUP_TYPE_VAL)
								{
									Atom *ideAtom = monoValentFunctionGroup->getDefaultInAtomOrFirstAtom();
									ideAtom->addChargeAndProtons(1, 1); //e.g. make cyanide charge netural
								}
								...
								{ //silicon chloride -->silicon tetrachloride
									...
									else
									{
										...
										else
										{
											...
											{
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
												std::vector<std::wstring> typicalOxidationStates = elementaryAtomEl->getAttributeValue(XmlDeclarations::COMMONOXIDATIONSTATESANDMAX_ATR).split(L":")[0]->split(L",");
							...
							int halideCount = functionalGroupFragments.size();
							boost::optional<int> maximumVal = ValencyChecker::getMaximumValency(elementaryAtom->getElement(), elementaryAtom->getCharge());
							if (maximumVal && halideCount > maximumVal)
							{
								throw StructureBuildingException(std::wstring(L"Too many halides/psuedo halides addded to ") + elementaryAtom->getElement());
							}
							for (int i = halideCount - 1; i >= 0; i--)
							{
								Fragment *ideFrag = functionalGroupFragments[i];
						...
						void StructureBuilder::applyAluminiumHydrideSpecialCase(Element *firstWord, Atom *elementaryAtom, std::vector<Fragment*> &functionalGroupFragments) throw(StructureBuildingException)
						{
							if ((elementaryAtom->getElement() == ChemEl::Al || elementaryAtom->getElement() == ChemEl::B) && elementaryAtom->getCharge() == 0)
							{
								...
								{
									if (functionalGroupFragments[0]->getDefaultInAtomOrFirstAtom().getElement() == ChemEl::H && functionalGroupFragments[1]->getDefaultInAtomOrFirstAtom().getElement() == ChemEl::H && functionalGroupFragments[2]->getDefaultInAtomOrFirstAtom().getElement() == ChemEl::H)
									{
										...
										if (counterCationWordRule != nullptr && counterCationWordRule->getChildCount() == 1)
										{
											...
											if (word != nullptr && word->getChildCount() == 1)
											{
												...
												if (root != nullptr && root->getChildCount() == 1)
												{
													...
													if (group != nullptr && XmlDeclarations::ELEMENTARYATOM_SUBTYPE_VAL == group->getAttributeValue(XmlDeclarations::SUBTYPE_ATR))
													{
														...
														{
															functionalGroupFragments.push_back(state->fragManager->copyFragment(functionalGroupFragments[0]));
								...
								{
									if (functionalGroupFragments[0]->getDefaultInAtomOrFirstAtom().getElement() == ChemEl::H && functionalGroupFragments[1]->getDefaultInAtomOrFirstAtom().getElement() == ChemEl::H && functionalGroupFragments[2]->getDefaultInAtomOrFirstAtom().getElement() == ChemEl::H && functionalGroupFragments[3]->getDefaultInAtomOrFirstAtom().getElement() == ChemEl::H)
									{
										elementaryAtom->setCharge(-1);
									}
								}
						...
						void StructureBuilder::buildGlycol(std::vector<Element*> &words) throw(StructureBuildingException)
						{
							...
							{
								throw StructureBuildingException(std::wstring(L"Glycol class names (e.g. ethylene glycol) expect two outAtoms. Found: ") + std::to_wstring(theDiRadical->getOutAtomCount()));
							}
							wordIndice++;
//JAVA TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'toString':
							if (wordIndice >= words.size() || words[wordIndice]->getAttributeValue(XmlDeclarations::TYPE_ATR) != WordType::functionalTerm->toString())
							{
								throw StructureBuildingException(L"Glycol functionalTerm word expected");
							}
							std::vector<Element*> functionalClassEls = OpsinTools::getDescendantElementsWithTagName(words[wordIndice], XmlDeclarations::FUNCTIONALCLASS_EL);
							...
							Atom *chosenAtom1 = outAtom1->isSetExplicitly() ? outAtom1->getAtom() : StructureBuildingMethods::findAtomForUnlocantedRadical(state, theDiRadical, outAtom1);
							Fragment *functionalFrag = state->fragManager->buildSMILES(functionalClassEls[0]->getAttributeValue(XmlDeclarations::VALUE_ATR), XmlDeclarations::FUNCTIONALCLASS_TYPE_VAL, XmlDeclarations::NONE_LABELS_VAL);
							if (outAtom1->getValency() != 1)
							{
								throw StructureBuildingException(std::wstring(L"OutAtom has unexpected valency. Expected 1. Actual: ") + std::to_wstring(outAtom1->getValency()));
							}
							...
							Atom *chosenAtom2 = outAtom2->isSetExplicitly() ? outAtom2->getAtom() : StructureBuildingMethods::findAtomForUnlocantedRadical(state, theDiRadical, outAtom2);
							Fragment *hydroxy = state->fragManager->buildSMILES(L"O", XmlDeclarations::FUNCTIONALCLASS_TYPE_VAL, XmlDeclarations::NONE_LABELS_VAL);
							if (outAtom2->getValency() != 1)
							{
								throw StructureBuildingException(std::wstring(L"OutAtom has unexpected valency. Expected 1. Actual: ") + std::to_wstring(outAtom2->getValency()));
							}
						...
						void StructureBuilder::buildGlycolEther(std::vector<Element*> &words) throw(StructureBuildingException)
						{
							...
							StructureBuildingMethods::resolveWordOrBracket(state, glycol); //if this actually is something like ethylene glycol this is a no-op as it will already have been resolved
//JAVA TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'toString':
...
//JAVA TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'toString':
								...
								{
									throw StructureBuildingException(std::wstring(L"Unexpected word encountered when applying glycol ether word rule ") + wordOrWordRule->getAttributeValue(XmlDeclarations::VALUE_ATR));
								}
							}
							...
							{
								throw StructureBuildingException(std::wstring(L"No hydroxy groups found in: ") + finalGroup->getValue() + std::wstring(L" to form ether"));
							}
							if (hydroxyAtoms.size() < numOfEthers)
							{
								throw StructureBuildingException(std::wstring(L"Insufficient hydroxy groups found in: ") + finalGroup->getValue() + std::wstring(L" to form required number of ethers"));
							}
							for (int i = 0; i < numOfEthers; i++)
							{
								...
								else if (br->getFunctionalAtomCount() > 0)
								{ //form ester
									Atom *ateAtom = br->getFunctionalAtom(0);
									ateAtom->neutraliseCharge();
									state->fragManager->replaceAtomWithAnotherAtomPreservingConnectivity(hydroxyAtoms[i], br->getFunctionalAtom(0));
									br->removeFunctionalAtom(0);
								}
								else
								{
									throw StructureBuildingException(std::wstring(L"Word had neither an outAtom or a functionalAtom! hence neither and ether or ester could be formed : ") + wordsToAttachToGlycol[i]->getAttributeValue(XmlDeclarations::VALUE_ATR));
								}
							}
						}
						...
						void StructureBuilder::buildAcetal(std::vector<Element*> &words) throw(StructureBuildingException)
						{
							...
							std::vector<Element*> functionalClasses = OpsinTools::getDescendantElementsWithTagName(functionalWord, XmlDeclarations::FUNCTIONALCLASS_EL);
...
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
							...
							{
								throw StructureBuildingException(std::wstring(L"Insufficient carbonyls to form ") + numberOfAcetals + std::wstring(L" ") + functionalClass);
							}
							...
							{
								throw StructureBuildingException(std::wstring(L"incorrect number of susbtituents when forming ") + functionalClass);
							}
							connectSubstituentsToAcetal(acetalFrags, substituentsBr, hemiacetal);
						}

						void StructureBuilder::replaceChalcogensInAcetal(Element *functionalClassEl, std::vector<std::wstring> &elements) throw(StructureBuildingException)
						{
							...
							while (currentEl != functionalClassEl)
							{
								if (currentEl->getName() == XmlDeclarations::GROUP_EL)
								{
									for (int j = 0; j < multiplier; j++)
									{
										...
										if (elements[i] != L"O")
										{
											throw StructureBuildingException(std::wstring(L"Replacement on ") + functionalClassEl->getValue() + std::wstring(L" can only be used to replace oxygen!"));
										}
						...
						Fragment *StructureBuilder::formAcetal(std::vector<Atom*> &carbonylOxygen, std::vector<std::wstring> &elements) throw(StructureBuildingException)
						{
							...
							Fragment *acetalFrag = state->fragManager->buildSMILES(StringTools::arrayToString(elements, L"."),L"",XmlDeclarations::NONE_LABELS_VAL);
						...
						bool StructureBuilder::buildAlcoholEster(std::vector<Element*> &words, int numberOfWordRules) throw(StructureBuildingException)
						{
							for (auto word : words)
							{
//JAVA TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'toString':
							...
							for (int i = 1; i < words.size(); i++)
							{
								...
								if (isAppropriateAteGroupForAlcoholEster(ateWord, wordBr))
								{
									...
									if (locant != L"")
									{
										...
										if (!std::find(hydroxyAtoms.begin(), hydroxyAtoms.end(), atomOnAlcoholFragment) != hydroxyAtoms.end() || std::find(chosenHydroxyAtoms.begin(), chosenHydroxyAtoms.end(), atomOnAlcoholFragment) != chosenHydroxyAtoms.end())
										{
											atomOnAlcoholFragment = potentialAlcoholFragment->getAtomByLocantOrThrow(std::wstring(L"O") + locant);
										}
										if (!std::find(hydroxyAtoms.begin(), hydroxyAtoms.end(), atomOnAlcoholFragment) != hydroxyAtoms.end() || std::find(chosenHydroxyAtoms.begin(), chosenHydroxyAtoms.end(), atomOnAlcoholFragment) != chosenHydroxyAtoms.end())
										{
											throw StructureBuildingException(locant + std::wstring(L" did not point to a hydroxy group to be used for ester formation"));
										}
...
//ORIGINAL LINE: throw new RuntimeException("OPSIN Bug: Either all or none of the esters should be locanted in alcohol ester rule");
								...
								if (hydroxyAtoms.size() == ateWords || hydroxyAtoms.size() > ateWords && (AmbiguityChecker::allAtomsEquivalent(hydroxyAtoms) || potentialAlcoholFragment->getTokenEl()->getValue() == L"glycerol"))
								{
							...
							for (int i = 0; i < ateWords; i++)
							{
								...
								if (ateGroup->getAttribute(XmlDeclarations::NUMBEROFFUNCTIONALATOMSTOREMOVE_ATR) == nullptr && numberOfWordRules == 1)
								{
...
//JAVA TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'toString':
							...
							if (words.size() != 3)
							{
								throw StructureBuildingException(std::wstring(L"Unexpected number of words encountered when processing name of type amineDiConjunctiveSuffix, expected 3 but found: ") + words.size());
							}
...
//ORIGINAL LINE: throw new RuntimeException("OPSIN Bug: locant expected to be N but was: " + locant);
						...
						bool StructureBuilder::isAppropriateAteGroupForAlcoholEster(Element *ateWord, BuildResults *wordBr) throw(StructureBuildingException)
						{
...
//JAVA TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'toString':
							...
							wordRule->getAttribute(XmlDeclarations::VALUE_ATR)->setValue(firstGroup->getAttributeValue(XmlDeclarations::VALUE_ATR));
...
//JAVA TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'toString':
						...
						void StructureBuilder::connectSubstituentsToAcetal(std::vector<Fragment*> &acetalFrags, BuildResults *subBr, bool hemiacetal) throw(StructureBuildingException)
						{
...
//JAVA TO C++ CONVERTER TODO TASK: The Java Collection 'remove(Object)' method is not converted:
						...
						void StructureBuilder::buildCyclicPeptide(std::vector<Element*> &words) throw(StructureBuildingException)
						{
							if (words.size() != 2)
							{
								throw StructureBuildingException(std::wstring(L"OPSIN Bug: Expected 2 words in cyclic peptide name, found: ") + words.size());
							}
							...
							if (peptideBr->getOutAtomCount() == 1)
							{
								...
								std::vector<Element*> aminoAcids = OpsinTools::getDescendantElementsWithTagNameAndAttribute(peptide, XmlDeclarations::GROUP_EL, XmlDeclarations::TYPE_ATR, XmlDeclarations::AMINOACID_TYPE_VAL);
							...
							{
								throw StructureBuildingException(std::wstring(L"Cyclic peptide building failed: Expected 1 outAtoms, found: ") + std::to_wstring(peptideBr->getOutAtomCount()));
							}
						}
						...
						void StructureBuilder::buildPolymer(std::vector<Element*> &words) throw(StructureBuildingException)
						{
							...
							if (polymerBr->getOutAtomCount() == 2)
							{
								 ...
								 */
//JAVA TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'toString':
								Atom *rGroup1 = state->fragManager->buildSMILES(std::wstring(L"[") + outAtom->getElement()::toString() + std::wstring(L"|") + polymerBr->getOutAtom(0)->getValency() + std::wstring(L"]"), L"", L"alpha")->getFirstAtom();
								rGroup1->setProperty(Atom::ATOM_CLASS, 1);
								state->fragManager->createBond(inAtom, rGroup1, polymerBr->getOutAtom(0)->getValency());

//JAVA TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'toString':
								Atom *rGroup2 = state->fragManager->buildSMILES(std::wstring(L"[") + inAtom->getElement()::toString() + std::wstring(L"|") + polymerBr->getOutAtom(1)->getValency() + std::wstring(L"]"), L"", L"omega")->getFirstAtom();
							...
							{
								throw StructureBuildingException(std::wstring(L"Polymer building failed: Two termini were not found; Expected 2 outAtoms, found: ") + std::to_wstring(polymerBr->getOutAtomCount()));
							}
						}

						Atom *StructureBuilder::determineFunctionalAtomToUse(const std::wstring &locant, BuildResults *mainGroupBR) throw(StructureBuildingException)
						{
...
//JAVA TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'toString':
							...
							throw StructureBuildingException(std::wstring(L"Cannot find functional atom with locant: ") + locant + std::wstring(L" to form an ester with"));
						}

						void StructureBuilder::manipulateStoichiometry(Element *molecule, std::vector<Element*> &wordRules) throw(StructureBuildingException)
						{
							...
							for (auto wordRule : wordRules)
							{
								...
								if (fractionalMultiplier->getName() == XmlDeclarations::FRACTIONALMULTIPLIER_EL)
								{
									...
									{
										throw StructureBuildingException(L"Fractional multipliers should not be used in conjunction with explicit stoichiometry");
									}
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
									std::vector<std::wstring> value = fractionalMultiplier->getAttributeValue(XmlDeclarations::VALUE_ATR).split(L"/");
									if (value.size() != 2)
									{
//JAVA TO C++ CONVERTER TODO TASK: The std::exception constructor has no parameters:
...
//ORIGINAL LINE: throw new RuntimeException("OPSIN Bug: malformed fractional multiplier: " + fractionalMultiplier.getAttributeValue(VALUE_ATR));
							...
							if (wordRulesWithFractionalMultipliers.size() > 0)
							{
								...
								if (chargedFractionalGroup)
								{
									for (auto wordRule : wordRules)
									{
										if (std::find(wordRulesWithFractionalMultipliers.begin(), wordRulesWithFractionalMultipliers.end(), wordRule) != wordRulesWithFractionalMultipliers.end())
										{
											continue;
										}
										Element *clone = state->fragManager->cloneElement(state, wordRule);
							...
							{ //a net charge is present! Could just mean the counterion has not been specified though
						...
						bool StructureBuilder::applyExplicitStoichiometryIfProvided(std::vector<Element*> &wordRules) throw(StructureBuildingException)
						{
						...
						void StructureBuilder::deprotonateAcidIfSaltWithMetal(Element *molecule)
						{
							...
							if (negativelyChargedComponents.empty() && (positivelyChargedComponents.size() > 0 || getMetalsThatCanBeImplicitlyCations(molecule).size() > 0))
							{
						...
						void StructureBuilder::balanceChargeIfPossible(Element *molecule, int overallCharge, bool explicitStoichiometryPresent) throw(StructureBuildingException)
						{
							std::vector<Element*> wordRules = molecule->getChildElements(XmlDeclarations::WORDRULE_ATR);
							...
							{ //e.g. nickel tetrachloride [Ni2+]-->[Ni4+]
							...
							if (!explicitStoichiometryPresent && (positivelyChargedComponents.size() == 1 && cationicElements.empty() && negativelyChargedComponents.size() >= 1 || positivelyChargedComponents.size() >= 1 && negativelyChargedComponents.size() == 1))
							{
								bool success = multiplyChargedComponents(negativelyChargedComponents, positivelyChargedComponents, componentToChargeMapping, overallCharge);
								if (success)
								{
									return;
								}
							...
							{ //e.g. magnesium monochloride [Mg2+]-->[Mg+]
						...
						std::vector<Element*> StructureBuilder::getMetalsThatCanBeImplicitlyCations(Element *molecule)
						{
							...
							std::vector<Element*> elementaryAtoms = OpsinTools::getDescendantElementsWithTagNameAndAttribute(molecule, XmlDeclarations::GROUP_EL, XmlDeclarations::SUBTYPE_ATR, XmlDeclarations::ELEMENTARYATOM_SUBTYPE_VAL);
							for (auto elementaryAtom : elementaryAtoms)
							{
								if (elementaryAtom->getAttribute(XmlDeclarations::COMMONOXIDATIONSTATESANDMAX_ATR) != nullptr)
								{
									...
									if (metalAtom->getCharge() == 0 && metalAtom->getProperty(Atom::OXIDATION_NUMBER) == nullptr)
									{ //if not 0 charge cannot be implicitly modified
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
										std::vector<std::wstring> typicalOxidationStates = elementaryAtom->getAttributeValue(XmlDeclarations::COMMONOXIDATIONSTATESANDMAX_ATR).split(L":")[0]->split(L",");
...
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
								std::vector<std::wstring> typicalOxidationStates = cationicElement->getAttributeValue(XmlDeclarations::COMMONOXIDATIONSTATESANDMAX_ATR).split(L":")[0]->split(L",");
						...
						bool StructureBuilder::triHalideSpecialCase(std::vector<Element*> &wordRules)
						{
							for (auto wordRule : wordRules)
							{
								if (wordRule->getChildCount() == 3)
								{
									...
									if ((std::wstring(L"tribromide")) == value || (std::wstring(L"tribromid")) == value || (std::wstring(L"triiodide")) == value || (std::wstring(L"triiodid")) == value)
									{
										std::vector<Element*> groups1 = OpsinTools::getDescendantElementsWithTagName(wordRule->getChild(0), XmlDeclarations::GROUP_EL);
										std::vector<Element*> groups2 = OpsinTools::getDescendantElementsWithTagName(wordRule->getChild(1), XmlDeclarations::GROUP_EL);
										std::vector<Element*> groups3 = OpsinTools::getDescendantElementsWithTagName(wordRule->getChild(2), XmlDeclarations::GROUP_EL);
...
//ORIGINAL LINE: throw new RuntimeException("OPSIN Bug: Unexpected trihalide representation");
						...
						bool StructureBuilder::multiplyChargedComponents(std::vector<Element*> &negativelyChargedComponents, std::vector<Element*> &positivelyChargedComponents, std::unordered_map<Element*, int> &componentToChargeMapping, int overallCharge) throw(StructureBuildingException)
						{
							...
							{ //e.g. iron(3+) sulfate -->2:3 mixture
								...
								{
									OpsinTools::insertAfter(negativelyChargedComponents[0], state->fragManager->cloneElement(state, negativelyChargedComponents[0]));
								}
								for (int i = (targetTotalAbsoluteCharge / positiveCharge); i > 1; i--)
								{
									OpsinTools::insertAfter(positivelyChargedComponents[0], state->fragManager->cloneElement(state, positivelyChargedComponents[0]));
								}
							}
							return true;
						}
						...
						{
//JAVA TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'toString':
							if (componentToMultiply->getAttributeValue(XmlDeclarations::WORDRULE_ATR) == WordRule::simple->toString() && OpsinTools::getChildElementsWithTagNameAndAttribute(componentToMultiply, XmlDeclarations::WORD_EL, XmlDeclarations::TYPE_ATR, WordType::full->toString()).size() > 1)
							{
							...
							if (firstChild->getName() == XmlDeclarations::MULTIPLIER_EL || firstChild->getName() == XmlDeclarations::FRACTIONALMULTIPLIER_EL)
							{ //e.g. monochloride. Allows specification of explicit stoichiometry
								return false;
							}
							return true;
						}
...
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
							int maximumCharge = static_cast<Integer>(cationicElement->getAttributeValue(XmlDeclarations::COMMONOXIDATIONSTATESANDMAX_ATR).split(L":")[1]);
						...
						Element *StructureBuilder::findRightMostGroupInWordOrWordRule(Element *wordOrWordRule) throw(StructureBuildingException)
						{
							if (wordOrWordRule->getName() == XmlDeclarations::WORDRULE_EL)
							{
								std::vector<Element*> words = OpsinTools::getDescendantElementsWithTagName(wordOrWordRule, XmlDeclarations::WORD_EL);
								for (int i = words.size() - 1 ; i >= 0; i--)
								{ //ignore functionalTerm Words
//JAVA TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'toString':
...
//JAVA TO C++ CONVERTER TODO TASK: The Java Collection 'remove(Object)' method is not converted:
							...
							else if (wordOrWordRule->getName() == XmlDeclarations::WORD_EL)
							{ //word element can be treated just like a bracket
						...
						void StructureBuilder::processOxidoAndMethionineSpecialCases(std::vector<Element*> &groups)
						{
							for (auto group : groups)
							{
								...
								if (XmlDeclarations::OXIDOLIKE_SUBTYPE_VAL == subType)
								{
									...
									if (checkForConnectedOxo(connectedAtom))
									{ //e.g. not oxido(trioxo)ruthenium
										continue;
									}
									if (XmlDeclarations::ELEMENTARYATOM_SUBTYPE_VAL == connectedAtom->getFrag()->getSubType() || ((chemEl == ChemEl::S || chemEl == ChemEl::P) && connectedAtom->getCharge() == 0 && ValencyChecker::checkValencyAvailableForBond(connectedAtom, 1)))
									{
									...
									else if (chemEl == ChemEl::N && connectedAtom->getCharge() == 0)
									{
										...
										{
											if (connectedAtom->getLambdaConventionValency() && connectedAtom->getLambdaConventionValency() == 5)
											{
								...
								else if (XmlDeclarations::AMINOACID_TYPE_VAL == group->getAttributeValue(XmlDeclarations::TYPE_ATR))
								{
									...
									{
										if (atom->getElement()::isChalcogen() && atom->getElement() != ChemEl::O && atom->getBondCount() == 3 && atom->getIncomingValency() == 3 && atom->getCharge() == 0)
										{
								...
								if (XmlDeclarations::ELEMENTARYATOM_SUBTYPE_VAL == group->getAttributeValue(XmlDeclarations::SUBTYPE_ATR))
								{
									...
									if (atom->getProperty(Atom::OXIDATION_NUMBER) != nullptr)
									{
										...
										for (auto neighbour : neighbours)
										{
											...
											if (!((neighbourEl->getValue() == L"carbon" && XmlDeclarations::NONCARBOXYLICACID_TYPE_VAL == neighbourEl->getAttributeValue(XmlDeclarations::TYPE_ATR)) || neighbourEl->getValue() == L"nitrosyl"))
											{
						...
						std::vector<Element*> StructureBuilder::findStereochemistryElsInProcessingOrder(Element *parentEl)
						{
							...
							matchingElements.insert(matchingElements.end(), stereochemistryElsAtThisLevel.begin(), stereochemistryElsAtThisLevel.end());
						...
						Atom *StructureBuilder::getOutAtomTakingIntoAccountWhetherSetExplicitly(BuildResults *buildResults, int i) throw(StructureBuildingException)
						{
