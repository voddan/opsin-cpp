			...
			namespace ch
			{
				namespace wwmm
				{
					namespace opsin
					{
						...
						AnnotatorState::AnnotatorState(int state, wchar_t annot, int posInName, bool isCaseSensitive, AnnotatorState *previousAs) : state(state), annot(annot), posInName(posInName), isCaseSensitive(isCaseSensitive), previousAs(previousAs)
						{
						}
