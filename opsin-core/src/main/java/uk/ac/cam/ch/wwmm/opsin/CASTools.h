			...
			namespace ch
			{
				namespace wwmm
				{
					namespace opsin
					{
						...
						private:
							static Pattern *const matchCasCollectiveIndex = Pattern::compile(L"([\\[\\(\\{]([1-9][0-9]?[cC][iI][, ]?)+[\\]\\)\\}])+|[1-9][0-9]?[cC][iI]", Pattern::CASE_INSENSITIVE);
							...
							static Pattern *const matchCompoundWithPhrase = Pattern::compile(L"(compd\\. with|compound with|and) ", Pattern::CASE_INSENSITIVE);
							static Pattern *const matchFunctionalTermAllowingSubstituentPrefix = Pattern::compile(L"(amide|hydrazide|(thi|selen|tellur)?oxime|hydrazone|(iso)?(semicarbazone|thiosemicarbazone|selenosemicarbazone|tellurosemicarbazone)|imide|imine|semioxamazone)[\\]\\)\\}]*", Pattern::CASE_INSENSITIVE);
