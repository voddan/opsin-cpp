...
const std::unordered_map<ChemEl, std::vector<Integer>> SMILESWriter::organicAtomsToStandardValencies = new java::util::EnumMap<ChemEl, std::vector<Integer>>(ChemEl::typeid);
const std::vector<std::wstring> SMILESWriter::closureSymbols = std::vector<std::wstring>();
						...
						SMILESWriter::StaticConstructor::StaticConstructor()
						{
															...
															organicAtomsToStandardValencies[ChemEl::N] = std::vector<boost::optional<int>*>{3,5}; //note that OPSIN doesn't accept valency 5 nitrogen without the lambda convention
...
SMILESWriter::StaticConstructor SMILESWriter::staticConstructor;
						...
						SMILESWriter::SMILESWriter(Fragment *structure, bool outputExtendedSmiles) : structure(structure), outputExtendedSmiles(outputExtendedSmiles)
						{
						}
						...
						std::wstring SMILESWriter::writeSmiles()
						{
							...
							for (auto currentAtom : atomList)
							{
								...
								if (visitedDepth && visitedDepth == 0)
								{ //new component
									if (!isEmpty)
									{
...
//ORIGINAL LINE: throw new RuntimeException("OPSIN Bug: Failed to resolve position variation bond atom");
							...
							if (lastLabel)
							{
								extendedSmiles.push_back(std::wstring(L"$") + StringTools::stringListToString(atomLabels.subList(0, lastLabel + 1), L";") + std::wstring(L"$"));
							}
						...
						void SMILESWriter::assignSmilesOrder()
						{
							...
							for (auto a : atomList)
							{
								if (a->getProperty(Atom::VISITED) == nullptr && !isSmilesImplicitProton(a))
						...
						SMILESWriter::TraversalState::TraversalState(Atom *atom, Bond *bondTaken, int depth) : atom(atom), bondTaken(bondTaken), depth(depth)
						{
						}
...
//JAVA TO C++ CONVERTER TODO TASK: The Java Collection 'remove(Object)' method is not converted:
									...
									{ //can be 0 for imines
										if (bondToNextAtomMap.find(bondsFrom2ndAtom[0]) != bondToNextAtomMap.end())
										{ //ignore bonds to implicit hydrogen
...
//JAVA TO C++ CONVERTER TODO TASK: The Java Collection 'remove(Object)' method is not converted:
									...
									//One of the bonds may have already have a defined slash from a previous bond stereo. If so make sure that we don't change it.
									if (bond1Other != nullptr && bond1Other->getSmilesStereochemistry() != nullptr && !bond1Other->getSmilesStereochemistry()::equals(bond1OtherDirection))
									{
										bond1Direction = bond1Direction::equals(SMILES_BOND_DIRECTION::LSLASH) ? SMILES_BOND_DIRECTION::RSLASH : SMILES_BOND_DIRECTION::LSLASH;
										bond2Direction = bond2Direction::equals(SMILES_BOND_DIRECTION::LSLASH) ? SMILES_BOND_DIRECTION::RSLASH : SMILES_BOND_DIRECTION::LSLASH;
										bond1OtherDirection = bond1OtherDirection::equals(SMILES_BOND_DIRECTION::LSLASH) ? SMILES_BOND_DIRECTION::RSLASH : SMILES_BOND_DIRECTION::LSLASH;
									...
									else if (bond2Other != nullptr && bond2Other->getSmilesStereochemistry() != nullptr && !bond2Other->getSmilesStereochemistry()::equals(bond2OtherDirection))
									{
										bond1Direction = bond1Direction::equals(SMILES_BOND_DIRECTION::LSLASH) ? SMILES_BOND_DIRECTION::RSLASH : SMILES_BOND_DIRECTION::LSLASH;
										bond2Direction = bond2Direction::equals(SMILES_BOND_DIRECTION::LSLASH) ? SMILES_BOND_DIRECTION::RSLASH : SMILES_BOND_DIRECTION::LSLASH;
										bond2OtherDirection = bond2OtherDirection::equals(SMILES_BOND_DIRECTION::LSLASH) ? SMILES_BOND_DIRECTION::RSLASH : SMILES_BOND_DIRECTION::LSLASH;
						...
						std::wstring SMILESWriter::atomToSmiles(Atom *atom, int depth, Bond *bondtaken)
						{
							...
							else
							{
								...
								{ //spare valency corresponds directly to lower case SMILES in OPSIN's SMILES reader
...
//JAVA TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'toString':
						...
						std::wstring SMILESWriter::atomParityToSmiles(Atom *currentAtom, int depth, Bond *bondtaken)
						{
							...
							{ //lone pair as in tetrahedral sulfones
							...
							{ //replace mentions of explicit hydrogen with the central atom the hydrogens are attached to, to be consistent with the SMILES representation
							...
							bool equivalent = StereochemistryHandler::checkEquivalencyOfAtomsRefs4AndParity(atomRefs4, atomParity->getParity(), atomrefs4CurrentArr, 1);
							if (equivalent)
							{
								return L"@@";
							}
