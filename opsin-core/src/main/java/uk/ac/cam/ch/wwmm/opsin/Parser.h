...
namespace uk { namespace ac { namespace cam { namespace ch { namespace wwmm { namespace opsin { class Tokeniser; } } } } } }
namespace uk { namespace ac { namespace cam { namespace ch { namespace wwmm { namespace opsin { class WordRules; } } } } } }
namespace uk { namespace ac { namespace cam { namespace ch { namespace wwmm { namespace opsin { class ResourceManager; } } } } } }
...
//						import static uk.ac.cam.ch.wwmm.opsin.XmlDeclarations.*;
						...
						private:
							...
							static Pattern *const matchStoichiometryIndication = Pattern::compile(L"[ ]?[\\{\\[\\(](\\d+|\\?)([:/](\\d+|\\?))+[\\}\\]\\)]$");
							static Logger *const LOG = Logger::getLogger(Parser::typeid);
						...
						public:
							...
							virtual std::vector<Element*> parse(NameToStructureConfig *n2sConfig, const std::wstring &name) throw(ParsingException);

							static std::vector<boost::optional<int>> processStoichiometryIndication(const std::wstring &ratioString) throw(ParsingException);

						private:
							void generateExactParseFailureReason(TokenizationResult *tokenizationResult, const std::wstring &name) throw(ParsingException);
							...
							/// <exception cref="ParsingException"> </exception>
						public:
							virtual void writeWordXML(Element *wordEl, std::vector<std::wstring> &tokens, std::vector<std::vector<wchar_t>> &annotations) throw(ParsingException);
							...
							/// <exception cref="ParsingException"> </exception>
						private:
							void applyStoichiometryIndicationToWordRules(Element *moleculeEl, std::vector<Integer> &componentRatios) throw(ParsingException);
