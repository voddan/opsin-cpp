...
//						import static uk.ac.cam.ch.wwmm.opsin.OpsinTools.*;
						...
						int ComponentGenerator::VonBaeyerSecondaryBridgeSort::compare(std::unordered_map<std::wstring, int> &bridge1, std::unordered_map<std::wstring, int> &bridge2)
						{
...
const std::wstring ComponentGenerator::elementSymbols = L"(?:He|Li|Be|B|C|N|O|F|Ne|Na|Mg|Al|Si|P|S|Cl|Ar|K|Ca|Sc|Ti|V|Cr|Mn|Fe|Co|Ni|Cu|Zn|Ga|Ge|As|Se|Br|Kr|Rb|Sr|Y|Zr|Nb|Mo|Tc|Ru|Rh|Pd|Ag|Cd|In|Sn|Sb|Te|I|Xe|Cs|Ba|La|Ce|Pr|Nd|Pm|Sm|Eu|Gd|Tb|Dy|Ho|Er|Tm|Yb|Lu|Hf|Ta|W|Re|Os|Ir|Pt|Au|Hg|Tl|Pb|Po|At|Rn|Fr|Ra|Ac|Th|Pa|U|Np|Pu|Am|Cm|Bk|Cf|Es|Fm|Md|No|Lr|Rf|Db|Sg|Bh|Hs|Mt|Ds)";
						...
						{
							std::vector<Element*> substituentsAndRoot = OpsinTools::getDescendantElementsWithTagNames(parse, std::vector<std::wstring>{XmlDeclarations::SUBSTITUENT_EL, XmlDeclarations::ROOT_EL});
						...
						void ComponentGenerator::resolveAmbiguities(Element *subOrRoot) throw(ComponentGenerationException)
						{
							...
							{
								if (XmlDeclarations::BASIC_TYPE_VAL != apparentMultiplier->getAttributeValue(XmlDeclarations::TYPE_ATR) && XmlDeclarations::VONBAEYER_TYPE_VAL != apparentMultiplier->getAttributeValue(XmlDeclarations::TYPE_ATR))
								{
									continue;
								}
								...
								if (multiplierNum >= 3)
								{ //detects ambiguous use of things like tetradeca
									if (nextEl != nullptr)
									{
										if (nextEl->getName() == XmlDeclarations::ALKANESTEMCOMPONENT)
...
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
												if (isThisALocant == nullptr || isThisALocant->getName() != XmlDeclarations::LOCANT_EL || isThisALocant->getValue().split(L",")->length != multiplierNum)
												{
													throw ComponentGenerationException(apparentMultiplier->getValue() + nextEl->getValue() + std::wstring(L" should not have been lexed as two tokens!"));
												}
											}
										}
									}
								...
								if (multiplierNum >= 4 && nextEl != nullptr && nextEl->getName() == XmlDeclarations::HYDROCARBONFUSEDRINGSYSTEM_EL && nextEl->getValue() == L"phen" && (std::wstring(L"e")) != nextEl->getAttributeValue(XmlDeclarations::SUBSEQUENTUNSEMANTICTOKEN_ATR))
								{ //deals with tetra phenyl vs tetraphen yl
									Element *possibleLocantOrMultiplierOrSuffix = OpsinTools::getNextSibling(nextEl);
									if (possibleLocantOrMultiplierOrSuffix != nullptr)
									{ //null if not used as substituent
										if (possibleLocantOrMultiplierOrSuffix->getName() == XmlDeclarations::SUFFIX_EL)
										{ //for phen the aryl substituent, expect an adjacent suffix e.g. phenyl, phenoxy
											Element *isThisALocant = OpsinTools::getPreviousSibling(apparentMultiplier);
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
											if (isThisALocant == nullptr || isThisALocant->getName() != XmlDeclarations::LOCANT_EL || isThisALocant->getValue().split(L",")->length != 1)
											{
												std::wstring multiplierAndGroup = apparentMultiplier->getValue() + nextEl->getValue();
												throw ComponentGenerationException(multiplierAndGroup + std::wstring(L" should not have been lexed as one token!"));
											}
										}
									}
								}
								if (multiplierNum > 4 && !apparentMultiplier->getValue().endsWith(L"a"))
								{ //disambiguate pent oxy and the like. Assume it means pentanoxy rather than 5 oxys
									if (nextEl != nullptr && nextEl->getName() == XmlDeclarations::GROUP_EL && matchInlineSuffixesThatAreAlsoGroups->matcher(nextEl->getValue()).matches())
									{
										throw ComponentGenerationException(apparentMultiplier->getValue() + nextEl->getValue() + std::wstring(L" should have been lexed as [alkane stem, inline suffix], not [multiplier, group]!"));
									}
								}
							}
							...
							for (auto fusion : fusions)
							{
								...
								{
									Element *possibleHWRing = OpsinTools::getNextSiblingIgnoringCertainElements(fusion, std::vector<std::wstring>{XmlDeclarations::MULTIPLIER_EL, XmlDeclarations::HETEROATOM_EL});
									if (possibleHWRing != nullptr && XmlDeclarations::HANTZSCHWIDMAN_SUBTYPE_VAL == possibleHWRing->getAttributeValue(XmlDeclarations::SUBTYPE_ATR))
									{
										int heteroCount = 0;
...
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
										...
										if (locants.size() == heteroCount)
										{
											...
											{
												if (std::stoi(locant) > (possibleHWRing->getAttributeValue(XmlDeclarations::VALUE_ATR).length() - 2))
												{
											...
											{
												throw ComponentGenerationException(L"This fusion bracket is in fact more likely to be a description of the locants of a HW ring");
											}
										}
									}
								}
						...
						void ComponentGenerator::processLocants(Element *subOrRoot) throw(ComponentGenerationException)
						{
							...
							for (auto locant : locants)
							{
								std::vector<std::wstring> individualLocants = splitIntoIndividualLocants(StringTools::removeDashIfPresent(locant->getValue()));
								for (int i = 0, locantCount = individualLocants.size(); i < locantCount; i++)
								{
									std::wstring locantText = individualLocants[i];

									if (locantText.find(L"-") != std::wstring::npos)
									{ //avoids this regex being invoked typically
									...
									if (std::isalpha(locantText[0]))
									{
										...
										if (locantText.length() >= 3)
										{
											...
											while (m->find())
											{
												locantText = locantText.substr(0, m->start()) + m->group()->toLowerCase(Locale::ROOT) + locantText.substr(m->end());
											}
										}
									}
...
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
												...
												for (auto addedHydrogen : addedHydrogens)
												{
													...
													Attribute tempVar(XmlDeclarations::LOCANT_ATR, addedHydrogen.substr(0, addedHydrogen.length() - 1));
												...
												{
													Attribute tempVar2(XmlDeclarations::TYPE_ATR, XmlDeclarations::ADDEDHYDROGENLOCANT_TYPE_VAL);
													locant->addAttribute(&tempVar2); //this locant must not be used as an indirect locant
												}
											}
											else if (StringTools::endsWithCaseInsensitive(brackettedText, L"R") || StringTools::endsWithCaseInsensitive(brackettedText, L"S"))
											{
												locantText = m->replaceFirst(L""); //strip the bracket from the locantText
												std::wstring rs;
...
//JAVA TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'toString':
												...
												Element *newStereoChemEl = new TokenEl(XmlDeclarations::STEREOCHEMISTRY_EL, std::wstring(L"(") + locantText + rs + std::wstring(L")"));
...
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
										...
										if (locantValues.size() == std::stoi(possibleMultiplier->getAttributeValue(XmlDeclarations::VALUE_ATR)))
										{
						...
						void ComponentGenerator::convertOrthoMetaParaToLocants(Element *subOrRoot) throw(ComponentGenerationException)
						{
							...
							for (auto ompLocant : ompLocants)
							{
								...
								if (orthoMetaParaLocantIsTwoLocants(ompLocant))
								{
									...
									{
										throw ComponentGenerationException(locantText + std::wstring(L" was not identified as being either ortho, meta or para but according to the chemical grammar it should of been"));
									}
								}
								else
								{
						...
						bool ComponentGenerator::orthoMetaParaLocantIsTwoLocants(Element *ompLocant)
						{
							...
							if (afterOmpLocant != nullptr)
							{
								...
								if (elName == XmlDeclarations::MULTIPLIER_EL && afterOmpLocant->getAttributeValue(XmlDeclarations::VALUE_ATR) == L"2")
								{
									//e.g. p-dimethyl
									return true;
								}
...
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
								...
								if (elName == XmlDeclarations::GROUP_EL)
								{
									...
									if (multiplier != nullptr && multiplier->getName() == XmlDeclarations::MULTIPLIER_EL && multiplier->getAttributeValue(XmlDeclarations::VALUE_ATR) == L"2")
									{
										Element *suffix = OpsinTools::getNextSiblingIgnoringCertainElements(multiplier, std::vector<std::wstring>{XmlDeclarations::INFIX_EL, XmlDeclarations::SUFFIXPREFIX_EL});
						...
						{
							Deque<Element*> *alkaneStemComponents = new ArrayDeque<Element*>(subOrRoot->getChildElements(XmlDeclarations::ALKANESTEMCOMPONENT));
						...
						void ComponentGenerator::processAlkaneStemModifications(Element *subOrRoot) throw(ComponentGenerationException)
						{
							...
							for (auto alkaneStemModifier : alkaneStemModifiers)
							{
								...
								if (alkane == nullptr || XmlDeclarations::CHAIN_TYPE_VAL != alkane->getAttributeValue(XmlDeclarations::TYPE_ATR) || XmlDeclarations::ALKANESTEM_SUBTYPE_VAL != alkane->getAttributeValue(XmlDeclarations::SUBTYPE_ATR))
								{
								...
								else if (type == L"tert")
								{
									if (chainLength < 4)
									{
										throw ComponentGenerationException(std::wstring(L"ChainLength to small for tert modifier, required minLength 4. Found: ") + std::to_wstring(chainLength));
									}
									if (chainLength > 8)
									{
										throw ComponentGenerationException(std::wstring(L"Interpretation of tert on an alkane chain of length: ") + chainLength + std::wstring(L" is ambiguous"));
									}
									if (chainLength == 8)
									{
								...
								else if (type == L"iso")
								{
									if (chainLength < 3)
									{
										throw ComponentGenerationException(std::wstring(L"ChainLength to small for iso modifier, required minLength 3. Found: ") + std::to_wstring(chainLength));
									}
									bool suffixPresent = subOrRoot->getChildElements(XmlDeclarations::SUFFIX_EL).size() > 0;
									if (chainLength == 3 && !suffixPresent)
									{
										throw ComponentGenerationException(L"iso has no meaning without a suffix on an alkane chain of length 3");
									}
								...
								else if (type == L"sec")
								{
									if (chainLength < 3)
									{
										throw ComponentGenerationException(std::wstring(L"ChainLength to small for sec modifier, required minLength 3. Found: ") + std::to_wstring(chainLength));
									}
									bool suffixPresent = subOrRoot->getChildElements(XmlDeclarations::SUFFIX_EL).size() > 0;
									if (!suffixPresent)
									{
								...
								else if (type == L"neo")
								{
									if (chainLength < 5)
									{
										throw ComponentGenerationException(std::wstring(L"ChainLength to small for neo modifier, required minLength 5. Found: ") + std::to_wstring(chainLength));
									}
						...
						void ComponentGenerator::processHeterogenousHydrides(Element *subOrRoot) throw(ComponentGenerationException)
						{
							...
							for (int i = 0; i < multipliers.size(); i++)
							{
								...
								if (multipliedElem->getName() == XmlDeclarations::GROUP_EL && multipliedElem->getAttribute(XmlDeclarations::SUBTYPE_ATR) != nullptr && multipliedElem->getAttributeValue(XmlDeclarations::SUBTYPE_ATR) == XmlDeclarations::HETEROSTEM_SUBTYPE_VAL)
								{
...
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
									if (possiblyALocant != nullptr && possiblyALocant->getName() == XmlDeclarations::LOCANT_EL && mvalue == possiblyALocant->getValue().split(L",")->length)
									{
										Element *suffix = OpsinTools::getNextSibling(multipliedElem, XmlDeclarations::SUFFIX_EL);
										if (suffix != nullptr && suffix->getAttributeValue(XmlDeclarations::TYPE_ATR) == XmlDeclarations::INLINE_TYPE_VAL)
										{
											...
											{ //NOT something like 3,3'-diselane-1,2-diyl
												continue;
											}
										}
									}
									...
									if (heteroatomSmiles == L"B" && OpsinTools::getPreviousSibling(m) == nullptr)
									{
										...
										if (possibleUnsaturator != nullptr && possibleUnsaturator->getName() == XmlDeclarations::UNSATURATOR_EL && possibleUnsaturator->getAttributeValue(XmlDeclarations::VALUE_ATR) == L"1")
										{
							...
							for (auto m : multipliers)
							{
								...
								if (multipliedElem->getName() == XmlDeclarations::HETEROATOM_EL)
								{
									...
									if (possiblyAnotherHeteroAtom != nullptr && possiblyAnotherHeteroAtom->getName() == XmlDeclarations::HETEROATOM_EL)
									{
										Element *possiblyAnUnsaturator = OpsinTools::getNextSiblingIgnoringCertainElements(possiblyAnotherHeteroAtom, std::vector<std::wstring>{XmlDeclarations::LOCANT_EL, XmlDeclarations::MULTIPLIER_EL}); //typically ane but can be ene or yne e.g. triphosphaza-1,3-diene
										if (possiblyAnUnsaturator != nullptr && possiblyAnUnsaturator->getName() == XmlDeclarations::UNSATURATOR_EL)
										{
											...
											{
												checkForAmbiguityWithHWring(multipliedElem->getAttributeValue(XmlDeclarations::VALUE_ATR), possiblyAnotherHeteroAtom->getAttributeValue(XmlDeclarations::VALUE_ATR));
											}
											int mvalue = std::stoi(m->getAttributeValue(XmlDeclarations::VALUE_ATR));
											...
											if (possiblyARingFormingEl != nullptr && (possiblyARingFormingEl->getName() == XmlDeclarations::CYCLO_EL || possiblyARingFormingEl->getName() == XmlDeclarations::VONBAEYER_EL || possiblyARingFormingEl->getName() == XmlDeclarations::SPIRO_EL))
											{
											...
											smiles = matchHdigit->matcher(smiles).replaceAll(L"H?"); //hydrogen count will be determined by standard valency
										...
										else if (possiblyAnUnsaturator != nullptr && possiblyAnUnsaturator->getValue() == L"an" && XmlDeclarations::HANTZSCHWIDMAN_SUBTYPE_VAL == possiblyAnUnsaturator->getAttributeValue(XmlDeclarations::SUBTYPE_ATR))
										{
...
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
											...
											{
												checkForAmbiguityWithHeterogenousHydride(multipliedElem->getAttributeValue(XmlDeclarations::VALUE_ATR), possiblyAnotherHeteroAtom->getAttributeValue(XmlDeclarations::VALUE_ATR));
											}
										}
									}
								}
						...
						void ComponentGenerator::checkForAmbiguityWithHWring(const std::wstring &firstHeteroAtomSMILES, const std::wstring &secondHeteroAtomSMILES) throw(ComponentGenerationException)
						{
							...
							{
								if (atom2ChemEl == ChemEl::O || atom2ChemEl == ChemEl::S || atom2ChemEl == ChemEl::Se || atom2ChemEl == ChemEl::Te || atom2ChemEl == ChemEl::Bi || atom2ChemEl == ChemEl::Hg)
								{
									if (!hasSiorGeorSnorPb(atom1ChemEl, atom2ChemEl))
									{
										throw ComponentGenerationException(L"Hantzch-widman ring misparsed as a heterogeneous hydride with alternating atoms");
									}
								}
							}
						}
						...
						void ComponentGenerator::checkForAmbiguityWithHeterogenousHydride(const std::wstring &firstHeteroAtomSMILES, const std::wstring &secondHeteroAtomSMILES) throw(ComponentGenerationException)
						{
							Matcher *m = OpsinTools::MATCH_ELEMENT_SYMBOL->matcher(firstHeteroAtomSMILES);
							if (!m->find())
							{
							...
							if (AtomProperties::getHwpriority(StringHelper::fromString<ChemEl>(atom2Element)) > AtomProperties::getHwpriority(StringHelper::fromString<ChemEl>(atom1Element)))
							{
								throw ComponentGenerationException(L"heterogeneous hydride with alternating atoms misparsed as a Hantzch-widman ring");
							}
						}
						...
						void ComponentGenerator::processIndicatedHydrogens(Element *subOrRoot) throw(ComponentGenerationException)
						{
							...
							for (auto indicatedHydrogenGroup : indicatedHydrogens)
							{
								...
								if (!StringTools::endsWithCaseInsensitive(txt, L"h"))
...
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
						...
						void ComponentGenerator::processStereochemistry(Element *subOrRoot) throw(ComponentGenerationException)
						{
							...
							{
								if (stereoChemistryElement->getAttributeValue(XmlDeclarations::TYPE_ATR).equals(XmlDeclarations::STEREOCHEMISTRYBRACKET_TYPE_VAL))
								{
									processStereochemistryBracket(stereoChemistryElement);
								}
								else if (stereoChemistryElement->getAttributeValue(XmlDeclarations::TYPE_ATR).equals(XmlDeclarations::CISORTRANS_TYPE_VAL))
								{
									assignLocantUsingPreviousElementIfPresent(stereoChemistryElement); //assign a locant if one is directly before the cis/trans
								}
								else if (stereoChemistryElement->getAttributeValue(XmlDeclarations::TYPE_ATR).equals(XmlDeclarations::E_OR_Z_TYPE_VAL))
								{
									...
									if (assignLocantUsingPreviousElementIfPresent(stereoChemistryElement))
									{ //assign a locant if one is directly before the E/Z
										locantedUnbrackettedEzTerms.push_back(stereoChemistryElement);
									}
								}
								else if (stereoChemistryElement->getAttributeValue(XmlDeclarations::TYPE_ATR).equals(XmlDeclarations::ENDO_EXO_SYN_ANTI_TYPE_VAL))
								{
								...
								else if (stereoChemistryElement->getAttributeValue(XmlDeclarations::TYPE_ATR).equals(XmlDeclarations::ALPHA_OR_BETA_TYPE_VAL))
								{
									processUnbracketedAlphaBetaStereochemistry(stereoChemistryElement);
								}
								else if (stereoChemistryElement->getAttributeValue(XmlDeclarations::TYPE_ATR).equals(XmlDeclarations::RELATIVECISTRANS_TYPE_VAL))
								{
						...
						void ComponentGenerator::processStereochemistryBracket(Element *stereoChemistryElement) throw(ComponentGenerationException)
						{
							...
							if (txt.length() > 0)
							{ //if txt is just "rel- or rac-" then it will be length 0 at this point
									...
									for (auto stereoChemistryDescriptor : stereoChemistryDescriptors)
									{
										...
										if (m->matches())
										{
											...
											else if (matchAlphaBetaStereochem->matcher(m->group(2)).matches())
											{
												...
												{
													throw ComponentGenerationException(std::wstring(L"Malformed alpha/beta stereochemistry element: ") + stereoChemistryElement->getValue());
												}
											}
											 ...
											 {
												throw ComponentGenerationException(std::wstring(L"Malformed stereochemistry element: ") + stereoChemistryElement->getValue());
											 }
										}
										else
										{
...
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
							if (possibleLocant != nullptr && possibleLocant->getName() == XmlDeclarations::LOCANT_EL && possibleLocant->getValue().split(L",")->length == 1)
							{
...
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
							if (possibleLocant != nullptr && possibleLocant->getName() == XmlDeclarations::LOCANT_EL && possibleLocant->getValue().split(L",")->length == 1)
							{
								...
								if (group != nullptr && (XmlDeclarations::CYCLICUNSATURABLEHYDROCARBON_SUBTYPE_VAL == group->getAttributeValue(XmlDeclarations::SUBTYPE_ATR) || OpsinTools::getPreviousSibling(group)->getName() == XmlDeclarations::VONBAEYER_EL))
								{
						...
						void ComponentGenerator::processUnbracketedAlphaBetaStereochemistry(Element *stereoChemistryElement) throw(ComponentGenerationException)
						{
...
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
							...
							if (!createLocantsEl)
							{
								...
								createLocantsEl = true;
								std::vector<Element*> groups = OpsinTools::getNextSiblingsOfType(stereoChemistryElement, XmlDeclarations::GROUP_EL);
								for (auto group : groups)
								{
...
//ORIGINAL LINE: throw new RuntimeException("Malformed relativeCisTrans element");
						...
						void ComponentGenerator::duplicateLocantFromStereoTermIfAdjacentToEneOrYlidene(std::vector<Element*> &locantedUnbrackettedEzTerms)
						{
							for (int i = 0, l = locantedUnbrackettedEzTerms.size(); i < l; i++)
							{
								...
								if (groupedTerms.size() > 1)
								{
									...
									if (!(multiplier != nullptr && multiplier->getName() == XmlDeclarations::MULTIPLIER_EL && static_cast<String>(groupedTerms.size()).equals(multiplier->getAttributeValue(XmlDeclarations::VALUE_ATR))))
									{
										continue;
									}
								...
								if (eneOrYlidene != nullptr)
								{
									...
									{
											if ((name == XmlDeclarations::UNSATURATOR_EL && eneOrYlidene->getAttributeValue(XmlDeclarations::VALUE_ATR) == L"2") || (name == XmlDeclarations::SUFFIX_EL && eneOrYlidene->getAttributeValue(XmlDeclarations::VALUE_ATR) == L"ylidene"))
											{
											std::vector<std::wstring> locants;
											...
											Element *newLocant = new TokenEl(XmlDeclarations::LOCANT_EL, StringTools::stringListToString(locants, L","));
...
//ORIGINAL LINE: throw new RuntimeException("After E/Z stereo expected yldiene but found: " + eneOrYlidene.getValue());
						...
						void ComponentGenerator::processSuffixPrefixes(Element *subOrRoot) throw(ComponentGenerationException)
						{
							...
							for (auto suffixPrefix : suffixPrefixes)
							{
								...
								{
									throw ComponentGenerationException(std::wstring(L"OPSIN bug: suffix not found after suffixPrefix: ") + suffixPrefix->getValue());
								}
								Attribute tempVar(XmlDeclarations::SUFFIXPREFIX_ATR, suffixPrefix->getAttributeValue(XmlDeclarations::VALUE_ATR));
						...
						void ComponentGenerator::processInfixes(Element *subOrRoot) throw(ComponentGenerationException)
						{
							...
							{
								Element *suffix = OpsinTools::getNextSiblingIgnoringCertainElements(infix, std::vector<std::wstring>{XmlDeclarations::INFIX_EL, XmlDeclarations::SUFFIXPREFIX_EL, XmlDeclarations::MULTIPLIER_EL});
								if (suffix == nullptr || suffix->getName() != XmlDeclarations::SUFFIX_EL)
								{
									throw ComponentGenerationException(std::wstring(L"No suffix found next next to infix: ") + infix->getValue());
								}
								...
								{
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
									currentInfixInformation = StringTools::arrayToList(suffix->getAttributeValue(XmlDeclarations::INFIX_ATR).split(L";"));
								}
								...
								if (possibleMultiplier->getName() == XmlDeclarations::MULTIPLIER_EL)
								{
									...
									Element *possibleSuffixPrefix = OpsinTools::getPreviousSiblingIgnoringCertainElements(infix, std::vector<std::wstring>{XmlDeclarations::MULTIPLIER_EL, XmlDeclarations::INFIX_EL});
									if (possibleSuffixPrefix != nullptr && possibleSuffixPrefix->getName() == XmlDeclarations::SUFFIXPREFIX_EL)
									{
									...
									if (elementBeforeMultiplier->getName() == XmlDeclarations::MULTIPLIER_EL || currentInfixInformation.size() > 1)
									{
										multiplierKnownToIndicateInfixMultiplicationPresent = true;
									}
								...
								{ //multiplier unambiguously means multiplication of the infix
									int multiplierVal = std::stoi(possibleMultiplier->getAttributeValue(XmlDeclarations::VALUE_ATR));
									for (int i = 1; i < multiplierVal; i++)
									{
										currentInfixInformation.push_back(infixValue);
								...
								else if (possibleMultiplier != nullptr && XmlDeclarations::GROUP_TYPE_VAL == possibleMultiplier->getAttributeValue(XmlDeclarations::TYPE_ATR))
								{ //e.g. ethanbisthioic acid == ethanbis(thioic acid)
									infix->detach();
								}
								suffix->getAttribute(XmlDeclarations::INFIX_ATR)->setValue(StringTools::stringListToString(currentInfixInformation, L";"));
							}
						}
...
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
								std::vector<std::wstring> lambdaValues = StringTools::removeDashIfPresent(lambdaConventionEl->getValue()).split(L",");
								...
								while (possibleHeteroatomOrMultiplier != nullptr)
								{
									...
									{
										multiplierValue = std::stoi(possibleHeteroatomOrMultiplier->getAttributeValue(XmlDeclarations::VALUE_ATR));
									}
									else
									{
										break;
								...
								if (lambdaValues.size() == heteroCount)
								{ //heteroatom and number of locants +lambdas must match
									if (fusedRingPresent && possibleHeteroatomOrMultiplier != nullptr && possibleHeteroatomOrMultiplier->getName() == XmlDeclarations::GROUP_EL && possibleHeteroatomOrMultiplier->getAttributeValue(XmlDeclarations::SUBTYPE_ATR) == XmlDeclarations::HANTZSCHWIDMAN_SUBTYPE_VAL)
									{
								...
								else if (possibleHeteroatomOrMultiplier != nullptr && ((heteroCount == 0 && OpsinTools::getNextSibling(lambdaConventionEl)->equals(possibleHeteroatomOrMultiplier) && fusedRingPresent && possibleHeteroatomOrMultiplier->getName() == XmlDeclarations::GROUP_EL && (possibleHeteroatomOrMultiplier->getValue() == L"benzo" || possibleHeteroatomOrMultiplier->getValue() == L"benz") && OpsinTools::getNextSibling(possibleHeteroatomOrMultiplier)->getName() != XmlDeclarations::FUSION_EL && OpsinTools::getNextSibling(possibleHeteroatomOrMultiplier)->getName() != XmlDeclarations::LOCANT_EL) || (possibleHeteroatomOrMultiplier->getName() == XmlDeclarations::POLYCYCLICSPIRO_EL && (possibleHeteroatomOrMultiplier->getAttributeValue(XmlDeclarations::VALUE_ATR) == L"spirobi" || possibleHeteroatomOrMultiplier->getAttributeValue(XmlDeclarations::VALUE_ATR) == L"spiroter"))))
								{
								...
								{ //populate heteroAtoms, multiplied heteroatoms are multiplied out
								...
								{ //assign all the lambdas to heteroatoms or to newly created lambdaConvention elements
									...
									{ //a lambda
										...
										if (frontLocantsExpected)
										{
											if (m->group(1) == nullptr)
											{
												throw ComponentGenerationException(L"Locant not found for lambda convention before a benzo fused ring system");
											}
									...
									{ //just a locant e.g 1,3lambda5
										if (!assignLambdasToHeteroAtoms)
										{
											if (!frontLocantsExpected)
											{
												throw ComponentGenerationException(std::wstring(L"Lambda convention not specified for locant: ") + lambdaValue);
											}
										}
										else
										{
						...
						void ComponentGenerator::findAndStructureBrackets(std::vector<Element*> &substituentsAndRoot) throw(ComponentGenerationException)
						{
							int blevel = 0;
								...
								for (auto child : children)
								{
									...
									else if (name == XmlDeclarations::CLOSEBRACKET_EL)
									{
										...
										if (blevel == 0)
										{
											...
											if (nestedBrackets)
											{
												findAndStructureBrackets(OpsinTools::getDescendantElementsWithTagNames(bracket, std::vector<std::wstring>{XmlDeclarations::SUBSTITUENT_EL, XmlDeclarations::ROOT_EL}));
											}
											openBracket = nullptr;
						...
						Element *ComponentGenerator::structureBrackets(Element *openBracket, Element *closeBracket) throw(ComponentGenerationException)
						{
						...
						void ComponentGenerator::processHydroCarbonRings(Element *subOrRoot) throw(ComponentGenerationException)
						{
							...
							std::vector<Element*> hydrocarbonFRSystems = subOrRoot->getChildElements(XmlDeclarations::HYDROCARBONFUSEDRINGSYSTEM_EL);
							for (auto hydrocarbonFRSystem : hydrocarbonFRSystems)
							{
								...
								if (multiplier != nullptr && multiplier->getName() == XmlDeclarations::MULTIPLIER_EL)
								{
									...
									std::wstring classOfHydrocarbonFRSystem = hydrocarbonFRSystem->getAttributeValue(XmlDeclarations::VALUE_ATR);
									StringBuilder *smilesSB = new StringBuilder();
									...
									Element *newGroup = new TokenEl(XmlDeclarations::GROUP_EL, multiplier->getValue() + hydrocarbonFRSystem->getValue());
						...
						void ComponentGenerator::handleSuffixIrregularities(Element *subOrRoot) throw(ComponentGenerationException)
						{
							...
							for (auto suffix : suffixes)
							{
								...
								else if (suffixValue == L"ylium" && (std::wstring(L"acylium")) == suffix->getAttributeValue(XmlDeclarations::VALUE_ATR) && suffix->getAttribute(XmlDeclarations::SUFFIXPREFIX_ATR) == nullptr && suffix->getAttribute(XmlDeclarations::INFIX_ATR) == nullptr)
								{ //disambiguate between ylium the charge modifying suffix and ylium the acylium suffix
									Element *group = OpsinTools::getPreviousSibling(suffix, XmlDeclarations::GROUP_EL);
									if (group == nullptr || (XmlDeclarations::ACIDSTEM_TYPE_VAL != group->getAttributeValue(XmlDeclarations::TYPE_ATR) && XmlDeclarations::CHALCOGENACIDSTEM_TYPE_VAL != group->getAttributeValue(XmlDeclarations::TYPE_ATR) && XmlDeclarations::NONCARBOXYLICACID_TYPE_VAL != group->getAttributeValue(XmlDeclarations::TYPE_ATR)))
									{
										...
										{
											if (group != nullptr && XmlDeclarations::ARYLSUBSTITUENT_SUBTYPE_VAL == group->getAttributeValue(XmlDeclarations::SUBTYPE_ATR))
											{
											...
											{
												throw ComponentGenerationException(L"ylium is intended to be the removal of H- in this context not the formation of an acylium ion");
											}
										}
									}
								}
...
//ORIGINAL LINE: throw new RuntimeException("OPSIN Bug: nitrolic acid not expected to have sibilings");
										...
										if (precedingSubstituent == nullptr || precedingSubstituent->getName() != XmlDeclarations::SUBSTITUENT_EL)
										{
											throw ComponentGenerationException(L"Expected substituent before nitrolic acid");
										}

										std::vector<Element*> existingSuffixes = precedingSubstituent->getChildElements(XmlDeclarations::SUFFIX_EL);
										...
										{
											throw ComponentGenerationException(L"Only the nitrolic acid case where it is preceded by an yl suffix is supported");
										}
									}
								}
							}
						...
						void ComponentGenerator::detectAlkaneFusedRingBridges(Element *group)
						{
							if (XmlDeclarations::ALKANESTEM_SUBTYPE_VAL == group->getAttributeValue(XmlDeclarations::SUBTYPE_ATR))
							{
								...
								{
									Element *possibleBridgeFormer = OpsinTools::getNextSiblingIgnoringCertainElements(group, std::vector<std::wstring>{XmlDeclarations::UNSATURATOR_EL});
									if (possibleBridgeFormer != nullptr && possibleBridgeFormer->getName() == XmlDeclarations::BRIDGEFORMINGO_EL)
									{
						...
						{
							Element *previous = OpsinTools::getPreviousSiblingIgnoringCertainElements(group, std::vector<std::wstring>{XmlDeclarations::LOCANT_EL});
							if (previous != nullptr)
							{
						...
						void ComponentGenerator::processSpiroSystem(Element *chainGroup, Element *spiroEl) throw(NumberFormatException, ComponentGenerationException)
						{
							std::vector<std::vector<int>> spiroDescriptors = getSpiroDescriptors(StringTools::removeDashIfPresent(spiroEl->getValue()));

							Element *multiplier = OpsinTools::getPreviousSibling(spiroEl);
							int numberOfSpiros = 1;
							if (multiplier != nullptr && multiplier->getName() == XmlDeclarations::MULTIPLIER_EL && XmlDeclarations::BASIC_TYPE_VAL == multiplier->getAttributeValue(XmlDeclarations::TYPE_ATR))
							{
							...
							{
								throw ComponentGenerationException(std::wstring(L"Disagreement between number of atoms in spiro descriptor: ") + numberOfCarbonInDescriptors + std::wstring(L" and number of atoms in chain: ") + std::to_wstring(chainGroup->getAttributeValue(XmlDeclarations::VALUE_ATR).length()));
							}

							int numOfOpenedBrackets = 1;
							int curIndex = 2;
							std::wstring smiles = std::wstring(L"C0") + StringTools::multiplyString(L"C", spiroDescriptors[0][0]) + std::wstring(L"10(");
							...
							for (int i = 1; i < spiroDescriptors.size(); i++)
							{
								if (spiroDescriptors[i][1] >= 0)
								{
									...
									{
										while (smiles[ringOpeningPos] >= L'0' && smiles[ringOpeningPos] <= L'9' && ringOpeningPos < smiles.length())
										{
									...
									if (smiles.find(std::wstring(L"C") + ringOpeningLabel, ringOpeningPos) != std::wstring::npos)
									{
										...
										smiles += std::wstring(L"(") + StringTools::multiplyString(L"C", spiroDescriptors[i][0]) + ringClosure(curIndex) + std::wstring(L")");
										curIndex++;
									}
									else
									{
										smiles += StringTools::multiplyString(L"C", spiroDescriptors[i][0]) + ringOpeningLabel + std::wstring(L")");
									}
								}
...
//ORIGINAL LINE: int[][] spiroDescriptors = new int[spiroDescriptorStrings.length][2];
							std::vector<std::vector<int>> spiroDescriptors = RectangularVectors::ReturnRectangularIntVector(spiroDescriptorStrings.size(), 2); // array of descriptors where number of elements and super string present
...
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
								...
								if (elements.size() > 1)
								{ //a "superscripted" number is present
									spiroDescriptors[i][0] = std::stoi(elements[0]);
									StringBuilder *superScriptedNumber = new StringBuilder();
									for (int j = 1; j < elements.size(); j++)
									{ //may be more than one non digit as there are many ways of indicating superscripts
						...
						boost::optional<int> ComponentGenerator::findIndexOfRingOpenings(const std::wstring &smiles, int locant) throw(ComponentGenerationException)
						{
							int count = 0;
							int pos = -1;
							...
							if (pos == -1)
							{
								throw ComponentGenerationException(L"Unable to find atom corresponding to number indicated by superscript in spiro descriptor");
							}
							pos++;
							return pos;
						}

						void ComponentGenerator::processVonBaeyerSystem(Element *chainEl, Element *vonBaeyerBracketEl) throw(ComponentGenerationException)
						{
							...
							{ //only able to interpret the SMILES that should be in an unmodified unbranched chain
								wchar_t currentChar = smilesArray[i];
								if (currentChar == L'[')
								{
									if (smilesArray[i + 2] == L']')
									{
										elementSymbolArray->add(std::wstring(L"[") + StringHelper::toString(smilesArray[i + 1]) + std::wstring(L"]"));
										i = i + 2;
									}
									else
									{
										elementSymbolArray->add(std::wstring(L"[") + StringHelper::toString(smilesArray[i + 1]) + StringHelper::toString(smilesArray[i + 2]) + std::wstring(L"]"));
										i = i + 3;
									}
								}
								else
							...
							{
								vonBaeyerBracket = vonBaeyerBracket.substr(7, (vonBaeyerBracket.length() - 1) - 7); //cut off cyclo-[ and terminal ]
							}
							else
							{
								vonBaeyerBracket = vonBaeyerBracket.substr(6, (vonBaeyerBracket.length() - 1) - 6); //cut off cyclo[ and terminal ]
							}
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
							std::vector<std::wstring> bridgeDescriptors = matchCommaOrDot->split(vonBaeyerBracket); //the bridgelengths and positions for secondary bridges
							...
							for (int i = 0; i < bridgeDescriptors.size(); i++)
							{
								...
								int bridgeLength = 0;
								if (i > 2)
								{ //this is a secondary bridge (chain start/end locations should have been specified)
									i++;
...
//JAVA TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'toString':
										...
										{
//JAVA TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'toString':
											bridgeLength = static_cast<Integer>(Character::toString(tempCharArray[0]) + Character::toString(tempCharArray[1]));
//JAVA TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'toString':
											coordinatesStr1 = Character::toString(tempCharArray[2]) + Character::toString(tempCharArray[3]);
										}
										else
										{
											throw ComponentGenerationException(std::wstring(L"Unsupported Von Baeyer locant description: ") + bridgeDescriptor);
										}
									}
									else
									{ //bracket or other delimiter detected, no ambiguity!
									...
									{
										throw ComponentGenerationException(std::wstring(L"Indicated bridge position is not on chain: ") + coordinates1 + std::wstring(L",") + std::to_wstring(coordinates2));
									}
									if (coordinates2 > coordinates1)
									{ //makes sure that bridges are built from highest coord to lowest
									...
									bridgeLabelsUsed++;
									if (bridgeLength == 0)
									{ //0 length bridge, hence want atoms with the same labels so they can join together without a bridge
...
//ORIGINAL LINE: java.util.Collections.sort(secondaryBridges, sortBridges);
						...
						void ComponentGenerator::processCyclisedChain(Element *chainGroup, Element *cycloEl) throw(ComponentGenerationException)
						{
							...
							if (chainlen < 3)
							{
								throw ComponentGenerationException(std::wstring(L"Heteroatom chain too small to create a ring: ") + std::to_wstring(chainlen));
							}
							smiles += std::wstring(L"1");
							if (smiles[0] == L'[')
							{
							...
							{
								if (Character::getType(smiles[1]) == Character::LOWERCASE_LETTER)
								{ //element is 2 letters long
							...
							chainGroup->getAttribute(XmlDeclarations::VALUE_ATR)->setValue(smiles);
							if (chainlen == 6)
							{ //6 membered rings have ortho/meta/para positions
						...
						void ComponentGenerator::handleGroupIrregularities(Element *group) throw(ComponentGenerationException)
						{
							...
							if (!n2sConfig->allowInterpretationOfAcidsWithoutTheWordAcid())
							{
								if (group->getAttribute(XmlDeclarations::FUNCTIONALIDS_ATR) != nullptr && (boost::ends_with(groupValue, L"ic") || boost::ends_with(groupValue, L"ous")))
								{
							...
							if (groupValue == L"thiophen" || groupValue == L"selenophen" || groupValue == L"tellurophen")
							{ //thiophenol is generally phenol with an O replaced with S not thiophene with a hydroxy
								Element *possibleSuffix = OpsinTools::getNextSibling(group);
								if ((std::wstring(L"e")) != group->getAttributeValue(XmlDeclarations::SUBSEQUENTUNSEMANTICTOKEN_ATR) && possibleSuffix != nullptr && possibleSuffix->getName() == XmlDeclarations::SUFFIX_EL)
								{
...
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
										if (isThisALocant == nullptr || isThisALocant->getName() != XmlDeclarations::LOCANT_EL || isThisALocant->getValue().split(L",")->length != 1)
										{
											throw ComponentGenerationException(groupValue + std::wstring(L"ol has been incorrectly interpreted as ") + groupValue + std::wstring(L", ol instead of phenol with the oxgen replaced"));
										}
									}
								}
							}
							else if (groupValue == L"chromen")
							{ //chromene in IUPAC nomenclature is fully unsaturated, but sometimes is instead considered to be chromane with a front locanted double bond
								Element *possibleLocant = OpsinTools::getPreviousSibling(group);
								if (possibleLocant != nullptr && possibleLocant->getName() == XmlDeclarations::LOCANT_EL && (possibleLocant->getValue() == L"2" || possibleLocant->getValue() == L"3"))
								{
									Element *possibleSuffix = OpsinTools::getNextSibling(group);
									if (possibleSuffix == nullptr || possibleSuffix->getName() == XmlDeclarations::LOCANT_EL)
									{ //if there is a suffix assume the locant refers to that rather than the double bond
							...
							else if (groupValue == L"methylene" || groupValue == L"methylen")
							{ //e.g. 3,4-methylenedioxyphenyl
								Element *nextSub = OpsinTools::getNextSibling(group->getParent());
								if (nextSub != nullptr && nextSub->getName() == XmlDeclarations::SUBSTITUENT_EL && OpsinTools::getNextSibling(group) == nullptr && (OpsinTools::getPreviousSibling(group) == nullptr || OpsinTools::getPreviousSibling(group)->getName() != XmlDeclarations::MULTIPLIER_EL))
								{ //not trimethylenedioxy
...
//JAVA TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'toString':
									if (possibleRoot == nullptr && OpsinTools::getParentWordRule(group)->getAttributeValue(XmlDeclarations::WORDRULE_ATR) == WordRule::glycol->toString())
									{ //e.g. dodecaethylene glycol
...
//JAVA TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'toString':
										group->getAttribute(XmlDeclarations::OUTIDS_ATR)->setValue(std::wstring(L"1,") + Integer::toString(3*(multiplierValue-1) + 2));
									...
									else if (possibleRoot != nullptr && possibleRoot->getName() == XmlDeclarations::ROOT_EL)
									{
										...
										if (children.size() == 2)
										{
											...
											if (amineMultiplier->getName() == XmlDeclarations::MULTIPLIER_EL && (amine->getValue() == L"amine" || amine->getValue() == L"amin"))
											{ //e.g. Triethylenetetramine
												if (std::stoi(amineMultiplier->getAttributeValue(XmlDeclarations::VALUE_ATR)) != multiplierValue +1)
												{
								...
								{
									Element *nextSub = OpsinTools::getNextSibling(group->getParent());
									if (nextSub != nullptr && nextSub->getName() == XmlDeclarations::SUBSTITUENT_EL && OpsinTools::getNextSibling(group) == nullptr)
									{
										std::vector<Element*> children = nextSub->getChildElements();
										if (children.size() >= 2 && children[0]->getValue() == L"di" && children[1]->getValue() == L"oxy")
										{
...
//JAVA TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'toString':
										group->getAttribute(XmlDeclarations::OUTIDS_ATR)->setValue(std::wstring(L"2,") + Integer::toString(4*(multiplierValue-1) + 3));
										group->getAttribute(XmlDeclarations::VALUE_ATR)->setValue(smiles->toString());
										if (group->getAttribute(XmlDeclarations::LABELS_ATR) != nullptr)
										{
											group->getAttribute(XmlDeclarations::LABELS_ATR)->setValue(XmlDeclarations::NONE_LABELS_VAL);
							...
							else if (groupValue == L"anthr" || groupValue == L"phenanthr" || groupValue == L"acrid" || groupValue == L"xanth" || groupValue == L"thioxanth" || groupValue == L"selenoxanth" || groupValue == L"telluroxanth" || groupValue == L"xanthen")
							{
								Element *possibleLocant = OpsinTools::getPreviousSibling(group);
								if (possibleLocant == nullptr || possibleLocant->getName() != XmlDeclarations::LOCANT_EL)
								{ //only need to give one a locant of 9 if no locant currently present
									Element *possibleSuffix = OpsinTools::getNextSibling(group);
									if (possibleSuffix != nullptr && (std::wstring(L"one")) == possibleSuffix->getAttributeValue(XmlDeclarations::VALUE_ATR))
									{
										//Rule C-315.2
									...
									else if (possibleSuffix != nullptr && possibleSuffix->getName() == XmlDeclarations::SUFFIX_EL && groupValue == L"xanth" || groupValue == L"thioxanth" || groupValue == L"selenoxanth" || groupValue == L"telluroxanth")
									{
										...
										{
											throw ComponentGenerationException(groupValue + possibleSuffix->getValue() + std::wstring(L" is not a derivative of xanthene"));
										}
									}
								}
							}
							else if (groupValue == L"phospho")
							{ //is this the organic meaning (P(=O)=O) or biochemical meaning (P(=O)(O)O)
								...
								for (auto otherGroup : OpsinTools::getDescendantElementsWithTagName(wordRule, XmlDeclarations::GROUP_EL))
								{
									...
									if (type == XmlDeclarations::AMINOACID_TYPE_VAL || type == XmlDeclarations::CARBOHYDRATE_TYPE_VAL || XmlDeclarations::BIOCHEMICAL_SUBTYPE_VAL == subType || (XmlDeclarations::YLFORACYL_SUBTYPE_VAL == subType && ((std::wstring(L"glycol")) == otherGroup->getValue() || (std::wstring(L"diglycol")) == otherGroup->getValue())))
									{
							...
							else if (groupValue == L"hydrogen")
							{
								...
								if (nextSubOrRoot != nullptr)
								{
									...
									if (possibleSuitableAteGroup->getName() != XmlDeclarations::GROUP_EL || XmlDeclarations::NONCARBOXYLICACID_TYPE_VAL != possibleSuitableAteGroup->getAttributeValue(XmlDeclarations::TYPE_ATR))
									{
							...
							else if (groupValue == L"azo" || groupValue == L"azoxy" || groupValue == L"nno-azoxy" || groupValue == L"non-azoxy" || groupValue == L"onn-azoxy" || groupValue == L"diazoamino" || groupValue == L"hydrazo")
							{
								Element *enclosingSub = group->getParent();
								Element *next = OpsinTools::getNextSiblingIgnoringCertainElements(enclosingSub, std::vector<std::wstring>{XmlDeclarations::HYPHEN_EL});
								if (next == nullptr && OpsinTools::getPreviousSibling(enclosingSub) == nullptr)
								{ //e.g. [(E)-NNO-azoxy]benzene
									next = OpsinTools::getNextSiblingIgnoringCertainElements(enclosingSub->getParent(), std::vector<std::wstring>{XmlDeclarations::HYPHEN_EL});
								}
								if (next != nullptr && next->getName() == XmlDeclarations::ROOT_EL)
								{
									if (!(next->getChild(0)->getName() == XmlDeclarations::MULTIPLIER_EL))
									{
										...
										if (suffixes.empty())
										{ //only case without locants is handled so far. suffixes only apply to one of the fragments rather than both!!!
											...
											if (interSubstituentHyphen != nullptr && interSubstituentHyphen->getName() != XmlDeclarations::HYPHEN_EL)
											{ //prevent implicit bracketting
							...
							else if (groupValue == L"coenzyme a" || groupValue == L"coa")
							{
								...
								if (previous != nullptr)
								{
									std::vector<Element*> groups = OpsinTools::getDescendantElementsWithTagName(previous, XmlDeclarations::GROUP_EL);
									if (groups.size() > 0)
									{
										...
										{
											if (possibleAcid->getAttribute(XmlDeclarations::SUFFIXAPPLIESTO_ATR) != nullptr)
											{ //multi acid. yl should be one oyl and the rest carboxylic acids
											...
											if (subType == XmlDeclarations::YLFORYL_SUBTYPE_VAL || subType == XmlDeclarations::YLFORNOTHING_SUBTYPE_VAL)
											{
												possibleAcid->getAttribute(XmlDeclarations::SUBTYPE_ATR)->setValue(XmlDeclarations::YLFORACYL_SUBTYPE_VAL); //yl always means an acyl when next to coenzyme A
											}
										}
									}
								}
							...
							else if (groupValue == L"sphinganine" || groupValue == L"icosasphinganine" || groupValue == L"eicosasphinganine" || groupValue == L"phytosphingosine" || groupValue == L"sphingosine" || groupValue == L"sphinganin" || groupValue == L"icosasphinganin" || groupValue == L"eicosasphinganin" || groupValue == L"phytosphingosin" || groupValue == L"sphingosin")
							{
								Element *enclosingSubOrRoot = group->getParent();
								Element *previous = OpsinTools::getPreviousSibling(enclosingSubOrRoot);
								if (previous != nullptr)
								{
									...
									{
										Element *possibleAcid = groups[groups.size() - 1];
										if (XmlDeclarations::ALKANESTEM_SUBTYPE_VAL == possibleAcid->getAttributeValue(XmlDeclarations::SUBTYPE_ATR))
										{
											std::vector<Element*> inlineSuffixes = OpsinTools::getChildElementsWithTagNameAndAttribute(possibleAcid->getParent(), XmlDeclarations::SUFFIX_EL, XmlDeclarations::TYPE_ATR, XmlDeclarations::INLINE_TYPE_VAL);
											if (inlineSuffixes.size() == 1 && inlineSuffixes[0]->getAttributeValue(XmlDeclarations::VALUE_ATR) == L"yl")
											{
												inlineSuffixes[0]->getAttribute(XmlDeclarations::VALUE_ATR).setValue(L"oyl"); //yl on a systematic acid next to a fatty acid means acyl
							...
							else if (groupValue == L"sel")
							{
								...
								if (XmlDeclarations::HETEROSTEM_SUBTYPE_VAL == group->getAttributeValue(XmlDeclarations::SUBTYPE_ATR) && group->getAttribute(XmlDeclarations::SUBSEQUENTUNSEMANTICTOKEN_ATR) == nullptr)
								{
									Element *unsaturator = OpsinTools::getNextSibling(group);
									if (unsaturator != nullptr && unsaturator->getName() == XmlDeclarations::UNSATURATOR_EL && unsaturator->getValue() == L"en" && group->getAttribute(XmlDeclarations::SUBSEQUENTUNSEMANTICTOKEN_ATR) == nullptr)
									{
										Element *ium = OpsinTools::getNextSibling(unsaturator);
										if (ium != nullptr && ium->getName() == XmlDeclarations::SUFFIX_EL && ium->getValue() == L"ium")
										{
											throw ComponentGenerationException(L"<multiplier>selenium does not indicate a chain of selenium atoms with a double bond and a positive charge");
										}
									}
								}
							}
							else if ((groupValue == L"keto" || groupValue == L"aldehydo") && XmlDeclarations::SIMPLESUBSTITUENT_SUBTYPE_VAL == group->getAttributeValue(XmlDeclarations::SUBTYPE_ATR))
							{
								...
								if (previousEl == nullptr || previousEl->getName() != XmlDeclarations::LOCANT_EL || groupValue == L"aldehydo")
								{
									...
									while (parentOfCarbohydate != nullptr)
									{
										...
										if (possibleCarbohydrate != nullptr && possibleCarbohydrate->getAttributeValue(XmlDeclarations::TYPE_ATR) == XmlDeclarations::CARBOHYDRATE_TYPE_VAL)
										{
									...
									if (carbohydrate != nullptr)
									{
										...
										{
											throw ComponentGenerationException(std::wstring(L"Carbohydrate has a specified ring size but ") + groupValue + std::wstring(L" indicates the open chain form!"));
										}
										for (auto suffix : parentOfCarbohydate->getChildElements(XmlDeclarations::SUFFIX_EL))
										{
											...
											{
												throw ComponentGenerationException(std::wstring(L"Carbohydrate appears to be a glycosyl, but ") + groupValue + std::wstring(L" indicates the open chain form!"));
											}
										}
										Element *alphaOrBetaLocantEl = OpsinTools::getPreviousSiblingIgnoringCertainElements(carbohydrate, std::vector<std::wstring>{XmlDeclarations::STEREOCHEMISTRY_EL});
										if (alphaOrBetaLocantEl != nullptr && alphaOrBetaLocantEl->getName() == XmlDeclarations::LOCANT_EL)
										{
											...
											{
												throw ComponentGenerationException(std::wstring(L"Carbohydrate has alpha/beta anomeric form but ") + groupValue + std::wstring(L" indicates the open chain form!"));
											}
										}
										group->detach();
										...
										if (XmlDeclarations::RING_SUBTYPE_VAL == carbohydrate->getAttributeValue(XmlDeclarations::SUBTYPE_ATR))
										{
											std::wstring carbohydrateAdditionValue = carbohydrate->getAttributeValue(XmlDeclarations::ADDITIONALVALUE_ATR);
											...
											{
												throw ComponentGenerationException(carbohydrate->getValue() + std::wstring(L" can only describe the cyclic form but ") + groupValue + std::wstring(L" indicates the open chain form!"));
											}
							...
							else if (groupValue == L"bor" || groupValue == L"antimon" || groupValue == L"arsen" || groupValue == L"phosphor" || groupValue == L"phosphate" || groupValue == L"phosphat" || groupValue == L"silicicacid" || groupValue == L"silicic acid" || groupValue == L"silicate" || groupValue == L"silicat")
							{ //fluoroboric acid/fluoroborate are trivial rather than systematic; tetra(fooyl)borate is inorganic
								...
								else
								{
									...
									if (suffix != nullptr && suffix->getName() == XmlDeclarations::SUFFIX_EL && suffix->getAttribute(XmlDeclarations::INFIX_ATR) == nullptr && OpsinTools::getNext(suffix) == nullptr)
									{
								...
								if (isAcid)
								{ //check for inorganic interpretation
									Element *substituent = OpsinTools::getPreviousSibling(group->getParent());
									if (substituent != nullptr && (substituent->getName() == XmlDeclarations::SUBSTITUENT_EL || substituent->getName() == XmlDeclarations::BRACKET_EL))
									{
										std::vector<Element*> children = substituent->getChildElements();
										Element *firstChild = children[0];
										bool matched = false;
										if (children.size() == 1 && firstChild->getName() == XmlDeclarations::GROUP_EL && (firstChild->getValue() == L"fluoro" || firstChild->getValue() == L"fluor"))
										{
											if (groupValue == L"bor")
											{
												group->getAttribute(XmlDeclarations::VALUE_ATR)->setValue(isAcid ? L"F[B-](F)(F)F.[H+]" : L"F[B-](F)(F)F");
												matched = true;
											}
											else if (groupValue == L"antimon")
											{
												group->getAttribute(XmlDeclarations::VALUE_ATR)->setValue(isAcid ? L"F[Sb-](F)(F)(F)(F)F.[H+]" : L"F[Sb-](F)(F)(F)(F)F");
												matched = true;
											}
											else if (boost::starts_with(groupValue, L"silicic"))
											{
												group->getAttribute(XmlDeclarations::VALUE_ATR)->setValue(isAcid ? L"F[Si|6-2](F)(F)(F)(F)F.[H+].[H+]" : L"F[Si|6-2](F)(F)(F)(F)F");
												matched = true;
											}
											if (matched)
											{
										...
										else if (firstChild->getName() == XmlDeclarations::MULTIPLIER_EL)
										{
											...
											if (groupValue == L"bor")
											{
												if (multiplierVal == L"4" || (multiplierVal == L"3" && OpsinTools::getPreviousSibling(substituent) != nullptr))
												{
											...
											{
												group->getAttribute(XmlDeclarations::VALUE_ATR)->setValue(isAcid ? L"[Si|6-2].[H+].[H+]" :L"[Si|6-2]");
												matched = true;
											}
										}
										if (matched)
										{
											...
											group->getAttribute(XmlDeclarations::SUBTYPE_ATR)->setValue(XmlDeclarations::SIMPLEGROUP_SUBTYPE_VAL);
							...
							else if (XmlDeclarations::ENDINIC_SUBTYPE_VAL == group->getAttributeValue(XmlDeclarations::SUBTYPE_ATR) && XmlDeclarations::AMINOACID_TYPE_VAL == group->getAttributeValue(XmlDeclarations::TYPE_ATR))
							{
								//aspartyl and glutamyl typically mean alpha-aspartyl/alpha-glutamyl
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
								std::vector<std::wstring> suffixAppliesTo = group->getAttributeValue(XmlDeclarations::SUFFIXAPPLIESTO_ATR).split(L",");
							...
							else if (XmlDeclarations::SALTCOMPONENT_SUBTYPE_VAL == group->getAttributeValue(XmlDeclarations::SUBTYPE_ATR))
							{
								...
								{
									throw ComponentGenerationException(std::wstring(L"Group expected to be part of a salt but only one component found. Could be a class of compound: ") + groupValue);
								}
								if (groupValue.length() > 0)
								{
									...
									if (firstChar >= L'1' && firstChar <= L'9')
									{
										...
										if (shouldntBeAmultiplier != nullptr && shouldntBeAmultiplier->getName() == XmlDeclarations::MULTIPLIER_EL)
										{
											throw ComponentGenerationException(std::wstring(L"Unepxected multiplier found before: ") + groupValue);
										}
