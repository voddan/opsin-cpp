...
PropertyKey<java::util::Set<Atom*>*> *const Atom::AMBIGUOUS_ELEMENT_ASSIGNMENT = new PropertyKey<java::util::Set<Atom*>*>(L"ambiguousElementAssignment");
...
PropertyKey<std::vector<Atom*>> *const Atom::POSITION_VARIATION_BOND = new PropertyKey<std::vector<Atom*>>(L"positionVariationBond");
...
PropertyKey<int> *const Atom::VISITED = new PropertyKey<int>(L"visited");
						...
						int Atom::determineValency(bool considerOutValency)
						{
							...
							boost::optional<int> calculatedMinValency = !minimumValency ? nullptr : minimumValency + protonsExplicitlyAddedOrRemoved;
...
//JAVA TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'toString':
								if (chemEl::toString()->equals(m->group(1)))
								{ //element symbol
									if (!m->group(2).equals(L"") && (!hasLocant(m->group(1) + m->group(2))))
									{ //has primes
...
//ORIGINAL LINE: throw new RuntimeException("Isotopic mass cannot be less than the element's number of protons: " + chemEl.toString() + " " + isotope + " < " + chemEl.ATOMIC_NUM);
						...
						void Atom::addBond(Bond *b)
						{
							...
							{
								throw std::invalid_argument("Atom already has given bond (This is not allowed as this would give two bonds between the same atoms!)");
							}
							bonds.push_back(b);
						}
