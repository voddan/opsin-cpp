...
bool AmbiguityChecker::isSubstitutionAmbiguous(std::vector<Atom *> &substitutableAtoms, int numberToBeSubstituted) {
    ...
    return !(allAtomsEquivalent(substitutableAtoms) &&
...
StereoAnalyser *AmbiguityChecker::analyseRelevantAtomsAndBonds(Collection < Atom * > *startingAtoms) {
    ...
    for (auto atom : atoms) {
        if (atom->getFrag()->getSubType() ==
            ELEMENTARYATOM_SUBTYPE_VAL) { //these do not have implicit hydrogen e.g. phosphorus is literally just a phosphorus atom
            continue;
        }
...
AmbiguityChecker::useAtomEnvironmentsToGivePlausibleSubstitution(std::vector<Atom *> &substitutableAtoms,
    ...
    std::vector<Atom *> preferredAtoms = findPlausibleSubstitutionPatternUsingSymmmetry(substitutableAtoms,
                                                                                        numberToBeSubstituted);
    ...
    if (preferredAtoms.empty()) {
        ...
        for (auto atoms : atomsInEachEnvironment) {
            if (atoms.second->size() == (numberToBeSubstituted * 2)) {
                Set < Atom * > *uniquified = new LinkedHashSet<Atom *>(
                        atoms.second); //retain deterministic atom ordering
...
//JAVA TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'toString':
        std::wstring s = a->getElement()
        ::toString() + std::wstring(L"\t") + valency + std::wstring(L"\t") + numOfBonds + std::wstring(L"\t") +
        StringHelper::toString(a->hasSpareValency());
