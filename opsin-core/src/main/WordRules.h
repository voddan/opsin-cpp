...
public:
    ...
    virtual void groupWordsIntoWordRules(Element *moleculeEl, NameToStructureConfig *n2sConfig, bool allowSpaceRemoval,
...
private:
    ...
    public:
        ...
        WordRulesInstance(WordRules *outerInstance, Element *moleculeEl, NameToStructureConfig *n2sConfig,
                          bool allowSpaceRemoval, std::vector<Integer> &componentRatios);

    private:
        bool matchWordRule(std::vector<Element *> &wordEls, int indexOfFirstWord) throw(ParsingException);

        bool
        endsWithGroupPredicateSatisfied(Element *wordEl, EndsWithGroup endsWithGroupPredicate) throw(ParsingException);

        bool oxidationNumberOrMultiplierIs(Element *elementaryAtomEl, Element *functionalGroupWord,
        ...
        /// <exception cref="ParsingException"> </exception>
        void joinWords(std::vector<Element *> &wordEls, Element *firstWord,
