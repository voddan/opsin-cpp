...
void WordRulesOmittedSpaceCorrector::correctOmittedSpaces() throw(StructureBuildingException) {
    std::vector<Element *> wordRules = OpsinTools::getDescendantElementsWithTagName(parse,
                                                                                    XmlDeclarations::WORDRULE_EL);
    for (auto wordRule : wordRules) {
        WordRule wordRuleVal = StringHelper::fromString<WordRule>(
                wordRule->getAttributeValue(XmlDeclarations::WORDRULE_ATR));
...
void WordRulesOmittedSpaceCorrector::checkAndCorrectOmittedSpacesInDivalentFunctionalGroupRule(
        Element *divalentFunctionalGroupWordRule) {
    std::vector<Element *> substituentWords = OpsinTools::getChildElementsWithTagNameAndAttribute(
            divalentFunctionalGroupWordRule, XmlDeclarations::WORD_EL, XmlDeclarations::TYPE_ATR,
            XmlDeclarations::SUBSTITUENT_TYPE_VAL);
    if (substituentWords.size() ==
        1) { //potentially has been "wrongly" interpreted e.g. ethylmethyl ketone is more likely to mean ethyl methyl ketone
        std::vector<Element *> children = OpsinTools::getChildElementsWithTagNames(substituentWords[0],
        ...
        if (children.size() == 2) {
            ...
            if (firstSubOrbracket->getAttribute(XmlDeclarations::LOCANT_ATR) == nullptr &&
                firstSubOrbracket->getAttribute(XmlDeclarations::MULTIPLIER_ATR) == nullptr) {
...
void
WordRulesOmittedSpaceCorrector::checkAndCorrectOmittedSpaceEster(Element *wordRule) throw(StructureBuildingException) {
    ...
    if (matchAteOrIteEnding->matcher(wordRuleContents).find()) {
        std::vector<Element *> children = OpsinTools::getChildElementsWithTagNames(word, std::vector<std::wstring>{
                XmlDeclarations::SUBSTITUENT_EL, XmlDeclarations::BRACKET_EL, XmlDeclarations::ROOT_EL});
        if (children.size() >= 2) {
            ...
            if (functionalAtomsCount > 0) {
                ...
                if (specialCaseWhereEsterPreferred(findRightMostGroupInSubBracketOrRoot(firstChild), multiplierValue,
                                                   rootGroup, substituentCount)) {
                    transformToEster(wordRule, firstChild);
                } else if (substituentCount > 1 && (allBarFirstSubstituentHaveLocants(substituents) ||
                                                    insufficientSubstitutableHydrogenForSubstitution(substituents,
                                                                                                     rootFrag,
                                                                                                     rootMultiplier))) {
                    transformToEster(wordRule, firstChild);
                } else if ((substituentCount == 1 || rootMultiplier > 1) &&
                           substitutionWouldBeAmbiguous(rootFrag, multiplierValue)) {
...
bool
WordRulesOmittedSpaceCorrector::allBarFirstSubstituentHaveLocants(std::vector<Element *> &substituentsAndBrackets) {
...
bool WordRulesOmittedSpaceCorrector::insufficientSubstitutableHydrogenForSubstitution(
        std::vector<Element *> &substituentsAndBrackets, Fragment *frag, int rootMultiplier) {
    ...
    int firstFragSubstitutableHydrogenRequired = getTotalOutAtomValency(
            findRightMostGroupInSubBracketOrRoot(potentialEsterSub).getFrag());
    std::wstring multiplierValue = potentialEsterSub->getAttributeValue(XmlDeclarations::MULTIPLIER_ATR);
    int multiplier = 1;
    if (multiplierValue != L"") {
        multiplier = std::stoi(multiplierValue);
    }
    if (substitutableHydrogens >= 0 &&
        (substitutableHydrogens - (firstFragSubstitutableHydrogenRequired * multiplier)) < 0) {
        return true;
    }
    return false;
}
...
bool WordRulesOmittedSpaceCorrector::specialCaseWhereEsterPreferred(Element *substituentGroupEl,
                                                                    const std::wstring &multiplierValue,
                                                                    Element *rootGroup, int numOfSubstituents) {
    if (multiplierValue != L"" && std::stoi(multiplierValue) == 1) {
        return true;
    }
    std::wstring rootGroupName = rootGroup->getParent()->getValue();
    if (substituentGroupEl->getAttributeValue(XmlDeclarations::TYPE_ATR) == XmlDeclarations::CHAIN_TYPE_VAL &&
        XmlDeclarations::ALKANESTEM_SUBTYPE_VAL ==
        substituentGroupEl->getAttributeValue(XmlDeclarations::SUBTYPE_ATR)) {
        if (std::regex_match(substituentGroupEl->getParent()->getValue(),
                             std::regex(substituentGroupEl->getValue() + std::wstring(L"yl-?"))) &&
            std::regex_match(rootGroupName, std::regex(L".*(form|methan|acet|ethan)[o]?ate?"))) {
            return true;
        }
    }
    if ((boost::ends_with(rootGroupName, L"carbamate") || boost::ends_with(rootGroupName, L"carbamat")) &&
        numOfSubstituents >= 2) {
...
bool WordRulesOmittedSpaceCorrector::substitutionWouldBeAmbiguous(Fragment *frag, const std::wstring &multiplierValue) {
    ...
    if (uniqueEnvironments->size() == 1 &&
        (multiplier == 1 || multiplier == atomForEachSubstitutableHydrogen.size() - 1)) {
        return false;
    }
    return true;
}

bool WordRulesOmittedSpaceCorrector::checkSuitabilityOfSubstituentForEsterFormation(Element *subOrBracket,
                                                                                    int rootFunctionalAtomsCount) {
...
void WordRulesOmittedSpaceCorrector::transformToEster(Element *parentSimpleWordRule,
