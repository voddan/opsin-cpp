...
ParseRules::ParseRules(ResourceManager *resourceManager) : chemAutomaton(resourceManager->getChemicalAutomaton()),
                                                           stateSymbols(chemAutomaton->getCharIntervals()),
                                                           symbolTokenNamesDict(
                                                                   resourceManager->getSymbolTokenNamesDict()),
                                                           symbolRegexAutomataDict(
                                                                   resourceManager->getSymbolRegexAutomataDict()),
                                                           symbolRegexesDict(resourceManager->getSymbolRegexesDict()),
                                                           initialState(
                                                                   new AnnotatorState(chemAutomaton->getInitialState(),
                                                                                      L'\0', 0, true, nullptr)) {
}
        ...
        for (int i = 0; i < stateSymbolsSize; i++) {
            ...
            if (potentialNextState != -1) { //-1 means this state is not accessible from the previous state
                ...
                if (possibleTokenisationsTrie != nullptr) {
                    std::vector<int> possibleTokenisations = possibleTokenisationsTrie->findMatches(
...
ParseTokens *ParseRules::convertAnnotationStateToParseTokens(AnnotatorState *as, const std::wstring &chemicalWord,
    ...
    while ((previousAs = as->getPreviousAs()) != nullptr) {
        if (as->isCaseSensitive()) {
            tokens.push_back(
                    chemicalWord.substr(previousAs->getPosInName(), as->getPosInName() - previousAs->getPosInName()));
        } else {
            tokens.push_back(chemicalWordLowerCase.substr(previousAs->getPosInName(),
                                                          as->getPosInName() - previousAs->getPosInName()));
        }
