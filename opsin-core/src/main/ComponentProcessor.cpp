...
ComponentProcessor::ComponentProcessor(BuildState *state, SuffixApplier *suffixApplier) : functionalReplacement(
        new FunctionalReplacement(state)), suffixApplier(suffixApplier), state(state) {
}
...
//JAVA TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'toString':
        ...
        if (roots.size() > 1) {
            throw ComponentGenerationException(
                    std::wstring(L"Multiple roots, but only 0 or 1 were expected. Found: ") + roots.size());
        }
        std::vector<Element *> substituents = OpsinTools::getDescendantElementsWithTagName(word,
                                                                                           XmlDeclarations::SUBSTITUENT_EL);
        std::vector<Element *> substituentsAndRoot = OpsinTools::combineElementLists(substituents, roots);
        std::vector<Element *> brackets = OpsinTools::getDescendantElementsWithTagName(word,
                                                                                       XmlDeclarations::BRACKET_EL);
        std::vector<Element *> substituentsAndRootAndBrackets = OpsinTools::combineElementLists(substituentsAndRoot,
                                                                                                brackets);
        ...
        for (auto subOrRoot : substituentsAndRoot) {
            ...
            processCarbohydrates(
                    subOrRoot); //e.g. glucopyranose (needs to be done before determineLocantMeaning to cope with alpha,beta for undefined anomer stereochemistry)
        }

        Element *finalSubOrRootInWord = word->getChild(word->getChildCount() - 1);
        while (finalSubOrRootInWord->getName() != XmlDeclarations::ROOT_EL &&
               finalSubOrRootInWord->getName() != XmlDeclarations::SUBSTITUENT_EL) {
            std::vector<Element *> children = OpsinTools::getChildElementsWithTagNames(finalSubOrRootInWord,
        ...
        for (auto subOrRoot : substituentsAndRoot) {
            ...
            if (groupsOfSubOrRoot.size() > 0) {
                ...
                preliminaryProcessSuffixes(lastGroupInSubOrRoot,
                                           subOrRoot->getChildElements(XmlDeclarations::SUFFIX_EL));
            }
        }
        for (int j = substituents.size() - 1; j >= 0; j--) {
            ...
            if (substituent->getChildElements(XmlDeclarations::GROUP_EL).empty()) {
                bool removed = removeAndMoveToAppropriateGroupIfHydroSubstituent(
                        substituent); //this REMOVES a substituent just containing hydro/perhydro elements and moves these elements in front of an appropriate ring
                if (!removed) {
...
//JAVA TO C++ CONVERTER TODO TASK: The Java Collection 'remove(Object)' method is not converted:
        ...
        if (functionalReplacement->processPrefixFunctionalReplacementNomenclature(groups,
                                                                                  substituents)) { //true if functional replacement performed, 1 or more substituents will have been removed
        ...
        for (auto subOrRoot : substituentsAndRoot) {
            ...
            suffixApplier->resolveSuffixes(subOrRoot->getFirstChildElement(XmlDeclarations::GROUP_EL),
                                           subOrRoot->getChildElements(XmlDeclarations::SUFFIX_EL));
        }

        moveErroneouslyPositionedLocantsAndMultipliers(brackets); //e.g. (tetramethyl)azanium == tetra(methyl)azanium
        std::vector<Element *> children = OpsinTools::getChildElementsWithTagNames(word, std::vector<std::wstring>{
                XmlDeclarations::ROOT_EL, XmlDeclarations::SUBSTITUENT_EL, XmlDeclarations::BRACKET_EL});
        addImplicitBracketsWhenFirstSubstituentHasTwoMultipliers(children[0],
                                                                 brackets); //e.g. ditrifluoroacetic acid --> di(trifluoroacetic acid)
        while (children.size() == 1) {
            children = OpsinTools::getChildElementsWithTagNames(children[0],
                                                                std::vector<std::wstring>{XmlDeclarations::ROOT_EL,
                                                                                          XmlDeclarations::SUBSTITUENT_EL,
                                                                                          XmlDeclarations::BRACKET_EL});
        }
        if (children.size() > 0) {
            assignLocantsToMultipliedRootIfPresent(children[children.size() -
                                                            1]); //multiplicative nomenclature e.g. methylenedibenzene or 3,4'-oxydipyridine
        }
        substituentsAndRootAndBrackets = OpsinTools::combineElementLists(substituentsAndRoot,
                                                                         brackets); //implicit brackets may have been created
...
Fragment *ComponentProcessor::resolveGroup(BuildState *state,
                                           Element *group) throw(StructureBuildingException, ComponentGenerationException) {
    ...
    Fragment *thisFrag = state->fragManager->buildSMILES(groupValue, group, labelsValue != L"" ? labelsValue
...
ComponentProcessor::AtomReference::AtomReference(AtomReferenceType referenceType, const std::wstring &reference)
...
ComponentProcessor::AddHeteroatom::AddHeteroatom(const std::wstring &heteroAtomSmiles, AtomReference *atomReference)
        : heteroAtomSmiles(heteroAtomSmiles) {
    this->atomReference = atomReference;
}
...
void ComponentProcessor::processXyleneLikeNomenclature(BuildState *state, Element *group,
...
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
            ...
            Fragment *fragToAdd;
            if (description.size() == 4) { //labels may optionally be specified for the group to be added
        ...
        if (previousEl != nullptr && previousEl->getName() ==
                                     XmlDeclarations::LOCANT_EL) { //has the name got specified locants to override the default ones
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
            ...
            if ((locantValues.size() == groupsToBeAdded.size() || locantValues.size() + 1 == groupsToBeAdded.size()) &&
                locantAreAcceptableForXyleneLikeNomenclatures(locantValues,
                                                              group)) { //one locant can be implicit in some cases
                ...
                if (locantValues.size() != groupsToBeAdded.size()) {
                    ...
                    switch (groupInformation->atomReference.referenceType) {
                        case DEFAULTLOCANT:
                        case LOCANT:
                            locant = parentFrag->getAtomByLocantOrThrow(
                                    groupInformation->atomReference.reference)->getFirstLocant();
                            break;
                        case DEFAULTID:
                        case ID:
                            locant = parentFrag->getAtomByIDOrThrow(parentFrag->getIdOfFirstAtom() +
                                                                    static_cast<Integer>(groupInformation->atomReference.reference) -
                                                                    1)->getFirstLocant();
                            break;
                        default:
                            throw ComponentGenerationException(L"malformed addGroup tag");
                ...
                if (assignlocants) {
                    for (int i = groupsToBeAdded.size() - 1; i >= 0; i--) {
                        ...
                        if (locantValues.size() > 0) {
                            groupInformation->atomReference = new AtomReference(AtomReferenceType::LOCANT,
                                                                                locantValues[locantValues.size() - 1]);
        ...
        for (int i = 0; i < groupsToBeAdded.size(); i++) {
            ...
            switch (groupInformation->atomReference.referenceType) {
                ...
                case LOCANT:
                    if (groupInformation->atomReference.reference.equals(L"required")) {
                        throw ComponentGenerationException(
                                group->getValue() + std::wstring(L" requires an allowed locant"));
                    }
                ...
                case DEFAULTID:
                    ambiguous = true;
                case ID:
                    atomOnParentFrag = parentFrag->getAtomByIDOrThrow(parentFrag->getIdOfFirstAtom() +
                                                                      static_cast<Integer>(groupInformation->atomReference.reference) -
                                                                      1);
                    break;
                default:
            ...
            if (newFrag->getOutAtomCount() == 1) {
                ...
                state->fragManager->incorporateFragment(newFrag, newFragOutAtom->getAtom(), parentFrag,
                                                        atomOnParentFrag, newFragOutAtom->getValency());
            } else {
...
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
            std::vector<std::wstring> locantValues = StringTools::arrayToList(previousEl->getValue().split(L","));
            if (locantValues.size() == heteroAtomsToBeAdded.size() &&
                locantAreAcceptableForXyleneLikeNomenclatures(locantValues, group)) {
                for (int i = heteroAtomsToBeAdded.size() - 1;
                     i >= 0; i--) { //all heteroatoms must have a locant or default locants will be used
        ...
        for (int i = 0; i < heteroAtomsToBeAdded.size(); i++) {
            ...
            switch (heteroAtomInformation->atomReference.referenceType) {
                ...
                case LOCANT:
                    ...
                    atomOnParentFrag = parentFrag->getAtomByLocantOrThrow(
                            heteroAtomInformation->atomReference.reference);
                    break;
                case DEFAULTID:
                    ambiguous = true;
                case ID:
                    atomOnParentFrag = parentFrag->getAtomByIDOrThrow(parentFrag->getIdOfFirstAtom() +
                                                                      static_cast<Integer>(heteroAtomInformation->atomReference.reference) -
                                                                      1);
                    break;
                default:
                    throw ComponentGenerationException(L"malformed addHeteroAtom tag");
    ...
    if (group->getAttributeValue(XmlDeclarations::ADDBOND_ATR) != L"" && XmlDeclarations::HANTZSCHWIDMAN_SUBTYPE_VAL !=
                                                                         group->getAttributeValue(
                                                                                 XmlDeclarations::SUBTYPE_ATR)) { //HW add bond is handled later
...
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
            std::vector<std::wstring> locantValues = StringTools::arrayToList(previousEl->getValue().split(L","));
            if (locantValues.size() == bondsToBeAdded.size() &&
                locantAreAcceptableForXyleneLikeNomenclatures(locantValues, group)) {
                for (int i = bondsToBeAdded.size() - 1;
                     i >= 0; i--) { //all bond order changes must have a locant or default locants will be used
                    AddBond *bondInformation = bondsToBeAdded[i];
                    bondInformation->atomReference = new AtomReference(AtomReferenceType::LOCANT,
                                                                       locantValues[locantValues.size() - 1]);
                    locantValues.pop_back();
                }
                group->removeAttribute(group->getAttribute(XmlDeclarations::FRONTLOCANTSEXPECTED_ATR));
        ...
        for (int i = 0; i < bondsToBeAdded.size(); i++) {
            ...
            switch (bondInformation->atomReference.referenceType) {
                ...
                case ID:
                    atomOnParentFrag = parentFrag->getAtomByIDOrThrow(parentFrag->getIdOfFirstAtom() +
                                                                      static_cast<Integer>(bondInformation->atomReference.reference) -
                                                                      1);
                    break;
                default:
                    throw ComponentGenerationException(L"malformed addBond tag");
            ...
            if (!locanted && b->getOrder() == 2 && parentFrag->getAtomCount() == 5 &&
                b->getFromAtom()->getAtomIsInACycle() && b->getToAtom()->getAtomIsInACycle()) {
...
bool ComponentProcessor::locantAreAcceptableForXyleneLikeNomenclatures(std::vector<std::wstring> &locantValues,
                                                                       Element *group) {
...
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
    std::vector<std::wstring> allowedLocants = Arrays::asList(
            group->getAttributeValue(XmlDeclarations::FRONTLOCANTSEXPECTED_ATR).split(L","));
...
void ComponentProcessor::setFragmentDefaultInAtomIfSpecified(Fragment *thisFrag,
    ...
    if (defaultInLocant != L"") { //sets the atom at which substitution will occur to by default
        thisFrag->setDefaultInAtom(thisFrag->getAtomByLocantOrThrow(defaultInLocant));
    } else if (defaultInId != L"") {
        thisFrag->setDefaultInAtom(
                thisFrag->getAtomByIDOrThrow(thisFrag->getIdOfFirstAtom() + std::stoi(defaultInId) - 1));
    }
}

void ComponentProcessor::setFragmentFunctionalAtomsIfSpecified(Element *group,
                                                               Fragment *thisFrag) throw(StructureBuildingException) {
    if (group->getAttribute(XmlDeclarations::FUNCTIONALIDS_ATR) != nullptr) {
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
        std::vector<std::wstring> functionalIDs = group->getAttributeValue(XmlDeclarations::FUNCTIONALIDS_ATR).split(
                L",");
        for (auto functionalID : functionalIDs) {
            thisFrag->addFunctionalAtom(
                    thisFrag->getAtomByIDOrThrow(thisFrag->getIdOfFirstAtom() + std::stoi(functionalID) - 1));
        }
    }
}
...
//JAVA TO C++ CONVERTER TODO TASK: The Java Collection 'remove(Object)' method is not converted:
    ...
    } else if (groupType == XmlDeclarations::CHAIN_TYPE_VAL &&
               XmlDeclarations::ALKANESTEM_SUBTYPE_VAL == group->getAttributeValue(XmlDeclarations::SUBTYPE_ATR)) {
        std::vector<Atom *> atomList = thisFrag->getAtomList();
        if (atomList.size() == 1) {
            return;
        }
        ...
        if (possibleSuffix != nullptr &&
            XmlDeclarations::TERMINAL_SUBTYPE_VAL == possibleSuffix->getAttributeValue(XmlDeclarations::SUBTYPE_ATR) &&
            possibleSuffix->getAttribute(XmlDeclarations::SUFFIXPREFIX_ATR) == nullptr) {
            terminalSuffixWithNoSuffixPrefixPresent = true;
        }
        for (auto atom : atomList) {
            std::wstring firstLocant = atom->getFirstLocant();
            if (!atom->getAtomIsInACycle() && firstLocant != L"" && firstLocant.length() == 1 &&
                std::isdigit(firstLocant[0])) {
...
void ComponentProcessor::processChargeAndOxidationNumberSpecification(Element *group, Fragment *frag) {
    ...
    if (nextEl != nullptr) {
        ...
        if (nextEl->getName() == XmlDeclarations::OXIDATIONNUMBERSPECIFIER_EL) {
            frag->getFirstAtom()->setProperty(Atom::OXIDATION_NUMBER,
                                              std::stoi(nextEl->getAttributeValue(XmlDeclarations::VALUE_ATR)));
            nextEl->detach();
        }
    }
}

bool ComponentProcessor::removeAndMoveToAppropriateGroupIfHydroSubstituent(
        Element *substituent) throw(ComponentGenerationException) {
...
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
            if (possibleLocantInFrontOfHydro != nullptr &&
                possibleLocantInFrontOfHydro->getName() == XmlDeclarations::LOCANT_EL &&
                possibleLocantInFrontOfHydro->getValue().split(L",")->length == 1) {
                //e.g.4-decahydro-1-naphthalenyl
                targetRing = potentialRing;
            } else {
                Element *possibleLocantInFrontOfRing = OpsinTools::getPreviousSibling(potentialRing,
                                                                                      XmlDeclarations::LOCANT_EL);
                if (possibleLocantInFrontOfRing != nullptr) {
                    if (potentialRing->getAttribute(XmlDeclarations::FRONTLOCANTSEXPECTED_ATR) !=
                        nullptr) { //check whether the group was expecting a locant e.g. 2-furyl
                        std::wstring locantValue = possibleLocantInFrontOfRing->getValue();
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
                        std::vector<std::wstring> expectedLocants = potentialRing->getAttributeValue(
                                XmlDeclarations::FRONTLOCANTSEXPECTED_ATR).split(L",");
                    ...
                    if (XmlDeclarations::FUSIONRING_SUBTYPE_VAL ==
                        potentialRing->getAttributeValue(XmlDeclarations::SUBTYPE_ATR) &&
                        (potentialRing->getValue() == L"benzo" || potentialRing->getValue() == L"benz") &&
                        OpsinTools::getNextSibling(potentialRing)->getName() != XmlDeclarations::FUSION_EL) {
                        targetRing = potentialRing;
                    }
                } else {
                    targetRing = potentialRing;
        ...
        if (targetRing == nullptr) {
            Element *nextSubOrRootOrBracketFromLast = substituent->getParent()->getChild(
                    substituent->getParent()->getChildCount() - 1); //the last sibling
        ...
        if (targetSubstituent->equals(adjacentSubOrRootOrBracket)) {
            ...
            for (int i = children.size() - 1; i >= 0; i--) {
                ...
                if (elName == XmlDeclarations::HYPHEN_EL) {
                    ...
                    throw ComponentGenerationException(
                            std::wstring(L"Unexpected term found before detachable hydro prefix: ") +
                            child->getValue());
                }
            }
        }
...
bool ComponentProcessor::removeAndMoveToAppropriateGroupIfSubtractivePrefix(
        Element *substituent) throw(ComponentGenerationException) {
    ...
    if (subtractivePrefixes.size() > 0) {
        ...
        if (nextSubOrRootOrBracket == nullptr) {
            throw ComponentGenerationException(
                    std::wstring(L"Unable to find group for: ") + subtractivePrefixes[0]->getValue() +
                    std::wstring(L" to apply to!"));
        }
        ...
        while (nextSubOrRootOrBracket != nullptr) {
            ...
            if (groupToConsider != nullptr) {
                ...
                if (XmlDeclarations::BIOCHEMICAL_SUBTYPE_VAL == subType ||
                    XmlDeclarations::CARBOHYDRATE_TYPE_VAL == type || XmlDeclarations::AMINOACID_TYPE_VAL == type) {
        ...
        if (targetSubstituent->equals(adjacentSubOrRootOrBracket)) {
            ...
            for (int i = children.size() - 1; i >= 0; i--) {
                ...
                if (elName == XmlDeclarations::HYPHEN_EL) {
                    ...
                    throw ComponentGenerationException(
                            std::wstring(L"Unexpected term found before detachable substractive prefix: ") +
                            child->getValue());
                }
            }
        }
...
bool ComponentProcessor::removeAndMoveToAppropriateGroupIfRingBridge(
        Element *substituent) throw(ComponentGenerationException) {
    ...
    if (ringBridges.size() > 0) {
        ...
        if (nextSubOrRootOrBracket == nullptr) {
            throw ComponentGenerationException(
                    std::wstring(L"Unable to find group for: ") + ringBridges[0]->getValue() +
                    std::wstring(L" to apply to!"));
        }
        ...
        while (nextSubOrRootOrBracket != nullptr) {
            ...
            if (groupToConsider != nullptr) {
                if (containsCyclicAtoms(groupToConsider) &&
                    OpsinTools::getPreviousSiblingsOfType(groupToConsider, XmlDeclarations::LOCANT_EL).empty()) {
        ...
        if (targetSubstituent->equals(adjacentSubOrRootOrBracket)) {
            ...
            for (int i = children.size() - 1; i >= 0; i--) {
                ...
                if (elName == XmlDeclarations::HYPHEN_EL) {
                    continue;
                } else if (inDetachablePrefix && (elName == XmlDeclarations::FUSEDRINGBRIDGE_EL ||
                                                  elName == XmlDeclarations::COLONORSEMICOLONDELIMITEDLOCANT_EL ||
                                                  elName == XmlDeclarations::LOCANT_EL)) {
                    child->detach();
                    targetSubstituent->insertChild(child, 0);
                } else if (elName == XmlDeclarations::STEREOCHEMISTRY_EL) {
                    inDetachablePrefix = false;
                    ...
                    throw ComponentGenerationException(
                            std::wstring(L"Unexpected term found before detachable ring bridge: ") + child->getValue());
                }
            }
        }
        substituent->detach();
...
void ComponentProcessor::determineLocantMeaning(Element *subOrBracketOrRoot,
    ...
    Element *group = subOrBracketOrRoot->getFirstChildElement(
            XmlDeclarations::GROUP_EL); //will be null if element is a bracket
        ...
        if (locantValues.size() > 1) {
            ...
            while (afterLocant != nullptr) {
                ...
                if (elName == XmlDeclarations::LOCANT_EL) {
                    ...
                    if (locantValues.size() == std::stoi(afterLocant->getAttributeValue(XmlDeclarations::VALUE_ATR))) {
                        if (afterLocant->equals(OpsinTools::getNextSiblingIgnoringCertainElements(locant,
                                                                                                  std::vector<std::wstring>{
                                                                                                          XmlDeclarations::INDICATEDHYDROGEN_EL}))) {
                            ...
                            if (afterMultiplier != nullptr &&
                                (afterMultiplier->getName() == XmlDeclarations::SUFFIX_EL ||
                                 afterMultiplier->getName() == XmlDeclarations::INFIX_EL ||
                                 afterMultiplier->getName() == XmlDeclarations::UNSATURATOR_EL ||
                                 afterMultiplier->getName() == XmlDeclarations::GROUP_EL)) {
                                multiplierEl = afterLocant; //indirect locant
                                break;
                            }
                        }
                    }
                    if (afterLocant->equals(OpsinTools::getNextSibling(
                            locant))) { //if nothing better can be found report this as a locant/multiplier mismatch
                        multiplierEl = afterLocant;
                    }
                } else if (elName == XmlDeclarations::RINGASSEMBLYMULTIPLIER_EL &&
                           afterLocant->equals(OpsinTools::getNextSibling(locant))) { //e.g. 1,1'-biphenyl
                    multiplierEl = afterLocant;
                    if (!FragmentTools::allAtomsInRingAreIdentical(
                            group->getFrag())) { //if all atoms are identical then the locant may refer to suffixes
                        break;
                    }
                } else if (elName == XmlDeclarations::FUSEDRINGBRIDGE_EL && locantValues.size() == 2 &&
                           afterLocant->equals(OpsinTools::getNextSibling(locant))) { //e.g. 1,8-methano
                    break;
                }
                afterLocant = OpsinTools::getNextSibling(afterLocant);
            }
            if (multiplierEl != nullptr) {
                if (std::stoi(multiplierEl->getAttributeValue(XmlDeclarations::VALUE_ATR)) == locantValues.size()) {
                    ...
                    if (boost::ends_with(locantValues[locantValues.size() - 1], L"'") && group != nullptr &&
                        subOrBracketOrRoot->find(group) >
                        subOrBracketOrRoot->find(locant)) { //quite possible that this is referring to a multiplied root
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
                        if (group->getAttribute(XmlDeclarations::OUTIDS_ATR) != nullptr &&
                            group->getAttributeValue(XmlDeclarations::OUTIDS_ATR).split(L",")->length > 1) {
                            ...
                            while (afterGroup != nullptr) {
                                ...
                                } else if (afterGroup->getName() == XmlDeclarations::SUFFIX_EL &&
                                           afterGroup->getAttributeValue(XmlDeclarations::TYPE_ATR) ==
                    ...
                    if (!locantModified && !OpsinTools::getNextSibling(locant)->equals(
                            multiplierEl)) { //the locants apply indirectly the multiplier e.g. 2,3-butandiol
...
//JAVA TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'toString':
                        throw ComponentGenerationException(
                                std::wstring(L"Mismatch between locant and multiplier counts (") +
                                Integer::toString(locantValues.size()) + std::wstring(L" and ") +
                                multiplierEl->getAttributeValue(XmlDeclarations::VALUE_ATR) + std::wstring(L"):") +
                                locant->getValue());
                    }
                }
            } else {
                ...
                if (!checkSpecialLocantUses(locant, locantValues, finalSubOrRootInWord)) {
                    throw ComponentGenerationException(
                            std::wstring(L"Multiple locants without a multiplier: ") + locant->toXML());
                }
            }
        }
    }
...
bool ComponentProcessor::checkSpecialLocantUses(Element *locant, std::vector<std::wstring> &locantValues,
    ...
    if (currentElem != nullptr && currentElem->getName() == XmlDeclarations::GROUP_EL) {
        if (currentElem->getAttributeValue(XmlDeclarations::SUBTYPE_ATR) ==
        ...
        if (heteroCount == 0 &&
            currentElem->getAttribute(XmlDeclarations::OUTIDS_ATR) != nullptr) { //e.g. 1,4-phenylene
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
            std::vector<std::wstring> outIDs = currentElem->getAttributeValue(XmlDeclarations::OUTIDS_ATR).split(L",",
                                                                                                                 -1);
            Fragment *groupFragment = currentElem->getFrag();
            if (count == outIDs.size() &&
                groupFragment->getAtomCount() > 1) { //things like oxy do not need to have their outIDs specified
...
//JAVA TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'toString':
                ...
                if (!foundLocantNotPresentOnFragment) {
                    currentElem->getAttribute(XmlDeclarations::OUTIDS_ATR)->setValue(
                            StringTools::arrayToString(outIDs, L","));
                    locant->detach();
                    return true;
                }
            }
        } else if (currentElem->getValue() == L"benz" || currentElem->getValue() == L"benzo") {
            Element *potentialGroupAfterBenzo = OpsinTools::getNextSibling(currentElem,
                                                                           XmlDeclarations::GROUP_EL); //need to make sure this isn't benzyl
    ...
    if (currentElem != nullptr) {
        ...
        if (name == XmlDeclarations::POLYCYCLICSPIRO_EL) {
            ...
            return true;
        } else if (name == XmlDeclarations::SUFFIX_EL && XmlDeclarations::CYCLEFORMER_SUBTYPE_VAL ==
                                                         currentElem->getAttributeValue(XmlDeclarations::SUBTYPE_ATR) &&
                   count == 2) {
            Attribute tempVar(XmlDeclarations::LOCANT_ATR, locant->getValue());
            currentElem->addAttribute(&tempVar);
            locant->detach();
            return true;
        } else if (name == XmlDeclarations::SUBTRACTIVEPREFIX_EL &&
                   XmlDeclarations::ANHYDRO_TYPE_VAL == currentElem->getAttributeValue(XmlDeclarations::TYPE_ATR)) {
            if (count != 2) {
                throw ComponentGenerationException(
                        std::wstring(L"Two locants are required before an anhydro prefix, but found: ") +
                        locant->getValue());
            }
    ...
    bool detectedMultiplicativeNomenclature = detectMultiplicativeNomenclature(locant, locantValues,
                                                                               finalSubOrRootInWord);
    ...
    if (count == 2 && parentElem->getName() ==
                      XmlDeclarations::BRACKET_EL) { //e.g. 3,4-(dichloromethylenedioxy) this is changed to (dichloro3,4-methylenedioxy)
...
bool ComponentProcessor::detectMultiplicativeNomenclature(Element *locant, std::vector<std::wstring> &locantValues,
                                                          Element *finalSubOrRootInWord) {
    int count = locantValues.size();
    Element *multiplier = finalSubOrRootInWord->getChild(0);
    if (finalSubOrRootInWord->getParent()->getName() ==
        XmlDeclarations::BRACKET_EL) { //e.g. 1,1'-ethynediylbis(1-cyclopentanol)
        if (multiplier->getName() != XmlDeclarations::MULTIPLIER_EL) {
            ...
            if (elName == XmlDeclarations::HETEROATOM_EL || elName == XmlDeclarations::SUBTRACTIVEPREFIX_EL ||
                (elName == XmlDeclarations::HYDRO_EL && !boost::starts_with(elAfterMultiplier->getValue(), L"per")) ||
                elName == XmlDeclarations::FUSEDRINGBRIDGE_EL) {
                multiplier = finalSubOrRootInWord->getParent()->getChild(0);
            }
        }
    }
    ...
    while (parentOfMultiplier != nullptr) {
        if (commonParent->equals(parentOfMultiplier)) {
            if (boost::ends_with(locantValues[count - 1], L"'") &&
                multiplier->getName() == XmlDeclarations::MULTIPLIER_EL &&
                OpsinTools::getNextSibling(multiplier)->getName() != XmlDeclarations::MULTIPLICATIVELOCANT_EL &&
                std::stoi(multiplier->getAttributeValue(XmlDeclarations::VALUE_ATR)) ==
                count) { //multiplicative nomenclature
...
void ComponentProcessor::applyDLPrefixes(Element *subOrRoot) throw(ComponentGenerationException) {
    std::vector<Element *> dlStereochemistryEls = OpsinTools::getChildElementsWithTagNameAndAttribute(subOrRoot,
                                                                                                      XmlDeclarations::STEREOCHEMISTRY_EL,
                                                                                                      XmlDeclarations::TYPE_ATR,
                                                                                                      XmlDeclarations::DLSTEREOCHEMISTRY_TYPE_VAL);
...
bool ComponentProcessor::applyDlStereochemistryToAminoAcid(Element *aminoAcidEl,
...
void ComponentProcessor::applyDlStereochemistryToCarbohydrate(Element *carbohydrateEl,
                                                              const std::wstring &dlStereochemistryValue) throw(ComponentGenerationException) {
    ...
    if (atomsWithParities.empty()) {
        throw ComponentGenerationException(std::wstring(L"D/L stereochemistry :") + dlStereochemistryValue +
                                           std::wstring(
                                                   L" found before achiral carbohydrate")); //sounds like a vocab bug...
    }
    ...
    if (dlStereochemistryValue == L"dl") {
        ...
        if (dlStereochemistryValue == L"d" || dlStereochemistryValue == L"dg") {
            ...
            throw ComponentGenerationException(
                    std::wstring(L"Unexpected value for D/L stereochemistry found before carbohydrate: ") +
                    dlStereochemistryValue);
        }
...
void ComponentProcessor::applyDlStereochemistryToCarbohydrateConfigurationalPrefix(Element *elementToApplyTo,
                                                                                   const std::wstring &dlStereochemistryValue) throw(ComponentGenerationException) {
    if (dlStereochemistryValue == L"d" || dlStereochemistryValue == L"dg") {
...
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
        ...
        throw ComponentGenerationException(
                std::wstring(L"Unexpected value for D/L stereochemistry found before carbohydrate prefix: ") +
                dlStereochemistryValue);
    }
}

void ComponentProcessor::processCarbohydrates(Element *subOrRoot) throw(StructureBuildingException) {
    std::vector<Element *> carbohydrates = OpsinTools::getChildElementsWithTagNameAndAttribute(subOrRoot,
    ...
    for (auto carbohydrate : carbohydrates) {
        ...
        if (XmlDeclarations::CARBOHYDRATESTEMKETOSE_SUBTYPE_VAL == subtype) {
            isAldose = false;
        } else if (XmlDeclarations::CARBOHYDRATESTEMALDOSE_SUBTYPE_VAL == subtype ||
                   XmlDeclarations::SYSTEMATICCARBOHYDRATESTEMALDOSE_SUBTYPE_VAL == subtype) {
            isAldose = true;
        } else {
            Attribute *anomericId = carbohydrate->getAttribute(XmlDeclarations::SUFFIXAPPLIESTO_ATR);
            if (anomericId != nullptr) {
                Atom *anomericCarbon = carbohydrateFrag->getAtomByID(
                        carbohydrateFrag->getIdOfFirstAtom() + std::stoi(anomericId->getValue()) - 1);
        ...
        if (anomericId == nullptr) {
            throw StructureBuildingException(
                    std::wstring(L"OPSIN bug: Missing suffixAppliesTo on: ") + carbohydrate->getValue());
        }
        Atom *potentialCarbonyl = carbohydrateFrag->getAtomByID(
                carbohydrateFrag->getIdOfFirstAtom() + std::stoi(anomericId->getValue()) - 1);
        if (potentialCarbonyl == nullptr) {
            throw StructureBuildingException(std::wstring(L"OPSIN bug: ") + anomericId->getValue() +
                                             std::wstring(L" did not point to an atom on: ") +
            ...
            if (elName == XmlDeclarations::SUFFIX_EL) {
                ...
                if (value == L"dialdose" || value == L"aric acid" || value == L"arate") {
                    ...
                    if (cyclisationPerformed) {
                        throw StructureBuildingException(std::wstring(L"OPSIN bug: ") + value +
                                                         std::wstring(L" not expected after carbohydrate cycliser"));
                    }
                    ...
                    if (value == L"ulose") {
                        ...
                        if (XmlDeclarations::SYSTEMATICCARBOHYDRATESTEMALDOSE_SUBTYPE_VAL == subtype) {
                            carbohydrate->getAttribute(XmlDeclarations::SUBTYPE_ATR)->setValue(
...
//JAVA TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'toString':
                    if (value == L"glycoside" &&
                        OpsinTools::getParentWordRule(subOrRoot)->getAttributeValue(XmlDeclarations::WORDRULE_ATR) ==
                        WordRule::simple->toString()) {
                        throw StructureBuildingException(
                                L"A glycoside requires a space-separated substituent e.g. methyl alpha-D-glucopyranoside");
                    }
                }
            ...
            } else if (elName != XmlDeclarations::LOCANT_EL && elName != XmlDeclarations::MULTIPLIER_ATR &&
                       elName != XmlDeclarations::UNSATURATOR_EL &&
                       elName != XmlDeclarations::COLONORSEMICOLONDELIMITEDLOCANT_EL) {
                break;
            }
...
void ComponentProcessor::applyUnspecifiedRingSizeCyclisationIfPresent(Element *group,
    ...
    if (possibleYl != nullptr && possibleYl->getName() == XmlDeclarations::SUFFIX_EL) {
        if (possibleYl->getAttributeValue(XmlDeclarations::VALUE_ATR) == L"yl") {
            ...
            if (possibleYl != nullptr && possibleYl->getName() == XmlDeclarations::SUFFIX_EL &&
                possibleYl->getAttributeValue(XmlDeclarations::VALUE_ATR) == L"yl") {
                cyclise = true;
            }
        }
    }
    if (!cyclise) {
        Element *alphaOrBetaLocantEl = OpsinTools::getPreviousSiblingIgnoringCertainElements(group,
                                                                                             std::vector<std::wstring>{
                                                                                                     XmlDeclarations::STEREOCHEMISTRY_EL});
...
Atom *ComponentProcessor::processUloseSuffix(Element *group, Element *suffix,
                                             Atom *potentialCarbonyl) throw(StructureBuildingException) {
...
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
            ...
            if (locantStrs.size() != multVal) {
                throw StructureBuildingException(
                        std::wstring(L"Mismatch between locant and multiplier counts (") + locantStrs.size() +
                        std::wstring(L" and ") + multVal + std::wstring(L"):") + locant->getValue());
            }
...
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
            if (locantStr.split(L",")->length == 1) {
                ...
                throw StructureBuildingException(
                        std::wstring(L"Incorrect number of locants for ul suffix: ") + locantStr);
            }
            locant->detach();
        } else {
    ...
    if (suffix->getAttributeValue(XmlDeclarations::VALUE_ATR) == L"ulose") { //convert aldose to ketose
        ...
        for (auto bond : aldehydeAtom->getBonds()) {
            if (bond->getOrder() == 2) {
                ...
                if (otherAtom->getElement() == ChemEl::O && otherAtom->getCharge() == 0 &&
                    otherAtom->getBondCount() == 1) {
                    bond->setOrder(1);
                    foundBond = true;
                    break;
                }
...
void ComponentProcessor::cycliseCarbohydrateAndApplyAlphaBetaStereo(Element *carbohydrateGroup, Element *ringSize,
                                                                    Atom *potentialCarbonyl) throw(StructureBuildingException) {
...
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
        ...
        if (locants.size() != 2) {
            throw StructureBuildingException(
                    std::wstring(L"Expected 2 locants in front of sugar ring size specifier but found: ") +
                    potentialLocant->getValue());
        }
        try {
            ...
            if (std::abs(secondLocant - firstLocant) != (std::stoi(ringSizeVal) - 2)) {
                throw StructureBuildingException(std::wstring(L"Mismatch between ring size: ") + ringSizeVal +
                                                 std::wstring(L" and ring size specified by locants: ") +
                                                 (std::abs(secondLocant - firstLocant) + 2));
            }
        }
        catch (const NumberFormatException &e) {
            throw StructureBuildingException(
                    std::wstring(L"Locants for ring should be numeric but were: ") + potentialLocant->getValue());
        }
...
//ORIGINAL LINE: throw new RuntimeException("OPSIN bug: Could not determine locant of carbonyl carbon in carbohydrate", e);
    ...
    if (atomToJoinWith == nullptr) {
        ...
        if (atomToJoinWith == nullptr) {
            throw StructureBuildingException(
                    std::wstring(L"Carbohydrate was not an inappropriate length to form a ring of size: ") +
                    ringSizeVal);
        }
    }
...
void ComponentProcessor::applyAlphaBetaStereoToCyclisedCarbohydrate(Element *carbohydrateGroup, Atom *carbonylCarbon) {
    Fragment *frag = carbohydrateGroup->getFrag();
    Element *alphaOrBetaLocantEl = OpsinTools::getPreviousSiblingIgnoringCertainElements(carbohydrateGroup,
                                                                                         std::vector<std::wstring>{
                                                                                                 XmlDeclarations::STEREOCHEMISTRY_EL});
    if (alphaOrBetaLocantEl != nullptr && alphaOrBetaLocantEl->getName() == XmlDeclarations::LOCANT_EL) {
...
//ORIGINAL LINE: throw new RuntimeException("OPSIN bug: Unable to determine anomeric reference atom in: " +carbohydrateGroup.getValue());
        ...
        if (carbonylCarbon->getAtomParity() != nullptr &&
            (XmlDeclarations::SYSTEMATICCARBOHYDRATESTEMALDOSE_SUBTYPE_VAL ==
             carbohydrateGroup->getAttributeValue(XmlDeclarations::SUBTYPE_ATR) ||
             XmlDeclarations::SYSTEMATICCARBOHYDRATESTEMKETOSE_SUBTYPE_VAL ==
             carbohydrateGroup->getAttributeValue(XmlDeclarations::SUBTYPE_ATR))) {
            ...
            if (val.substr(val.length() - 1, val.length() - (val.length() - 1))->equals(L"l")) { //"r" if D, "l" if L
                //flip if L
...
void ComponentProcessor::processAldoseDiSuffix(const std::wstring &suffixValue, Element *group,
...
void ComponentProcessor::applyAnomerStereochemistryIfPresent(Element *alphaOrBetaLocantEl, Atom *anomericAtom,
                                                             Atom *anomericReferenceAtom) {
    std::wstring value = alphaOrBetaLocantEl->getValue();
    if (value == L"alpha" || value == L"beta") {
        ...
        bool flip = StereochemistryHandler::checkEquivalencyOfAtomsRefs4AndParity(referenceAtomRefs4, 1,
                                                                                  anomericReferenceAtom->getAtomParity()->getAtomRefs4(),
                                                                                  anomericReferenceAtom->getAtomParity()->getParity());
...
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
        ...
        if (nextName == XmlDeclarations::UNSATURATOR_EL || nextName == XmlDeclarations::SUFFIX_EL ||
            nextName == XmlDeclarations::SUBTRACTIVEPREFIX_EL || (nextName == XmlDeclarations::HETEROATOM_EL &&
                                                                  XmlDeclarations::GROUP_TYPE_VAL !=
                                                                  multiplier->getAttributeValue(
                                                                          XmlDeclarations::TYPE_ATR)) ||
            nextName == XmlDeclarations::HYDRO_EL) {
...
void ComponentProcessor::detectConjunctiveSuffixGroups(Element *subOrRoot,
    ...
    if (groups.size() > 1) {
        ...
        for (int i = groups.size() - 1; i >= 0; i--) {
            ...
            if (group->getAttributeValue(XmlDeclarations::TYPE_ATR) !=
                XmlDeclarations::RING_TYPE_VAL) { //e.g. the methanol in benzenemethanol.
        ...
        if (ringGroup == nullptr) {
            throw ComponentGenerationException(
                    L"OPSIN bug: unable to find ring associated with conjunctive suffix group");
        }
        if (conjunctiveGroups.size() != 1) {
            throw ComponentGenerationException(
                    L"OPSIN Bug: Two groups exactly should be present at this point when processing conjunctive nomenclature");
        }
...
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
                ...
                if (locants.size() != multiplier) {
                    throw ComponentGenerationException(
                            L"mismatch between number of locants and multiplier in conjunctive nomenclature routine");
                }
...
void ComponentProcessor::matchLocantsToDirectFeatures(Element *subOrRoot) throw(ComponentGenerationException) {
    ...
    std::vector<Element *> groups = subOrRoot->getChildElements(XmlDeclarations::GROUP_EL);
    for (auto group : groups) {
        if (group->getAttributeValue(XmlDeclarations::SUBTYPE_ATR).equals(
                XmlDeclarations::HANTZSCHWIDMAN_SUBTYPE_VAL)) { //handle Hantzch-widman systems
            if (group->getAttribute(XmlDeclarations::ADDBOND_ATR) != nullptr) { //special case for partunsatring
                ...
                if (deltas.empty()) {
                    ...
                    Element *appropriateLocant = OpsinTools::getPreviousSiblingIgnoringCertainElements(group,
                                                                                                       std::vector<std::wstring>{
                                                                                                               XmlDeclarations::HETEROATOM_EL,
                                                                                                               XmlDeclarations::MULTIPLIER_EL});
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
                    if (appropriateLocant != nullptr && appropriateLocant->getName() == XmlDeclarations::LOCANT_EL &&
                        appropriateLocant->getValue().split(L",")->length == 1) {
...
//JAVA TO C++ CONVERTER TODO TASK: The Java Collection 'remove(Object)' method is not converted:
                        ...
                        delta->setValue(L"");
                        subOrRoot->insertChild(delta,
                                               0); //no obvious attempt to set double bond position, potentially ambiguous, valency will be used to choose later
                    }
                }
            }
            if (locants.size() > 0) {
                ...
                for (int j = indexOfGroup - 1; j >= 0; j--) {
                    ...
                    if (elName == XmlDeclarations::LOCANT_EL) {
                        ...
                        if (heteroAtom->getAttribute(XmlDeclarations::LOCANT_ATR) !=
                            nullptr) { //locants already assigned, assumedly by process multipliers
                            break;
                        }
                    } else {
                        break;
...
//JAVA TO C++ CONVERTER TODO TASK: The Java Collection 'remove(Object)' method is not converted:
                            ...
                            throw ComponentGenerationException(
                                    L"Mismatch between number of locants and Hantzsch-Widman heteroatoms");
                        }
                    }
                }
            }
...
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
        ...
        if (referent != nullptr && locantValues.size() == 1) {
            ...
            if (referent->getAttribute(XmlDeclarations::LOCANT_ATR) == nullptr &&
                referent->getAttribute(XmlDeclarations::MULTIPLIED_ATR) == nullptr &&
                (refName == XmlDeclarations::UNSATURATOR_EL || refName == XmlDeclarations::SUFFIX_EL ||
                 refName == XmlDeclarations::HETEROATOM_EL || refName == XmlDeclarations::CONJUNCTIVESUFFIXGROUP_EL ||
                 refName == XmlDeclarations::SUBTRACTIVEPREFIX_EL || (refName == XmlDeclarations::HYDRO_EL &&
                                                                      !boost::starts_with(referent->getValue(),
                                                                                          L"per")))) { //not perhydro
...
void ComponentProcessor::preliminaryProcessSuffixes(Element *group,
    ...
    if (group->getAttribute(XmlDeclarations::SUFFIXAPPLIESTO_ATR) !=
        nullptr) { //typically a trivial polyAcid or aminoAcid
        ...
        for (auto suffix : suffixes) {
            if (suffix->getAttribute(XmlDeclarations::ADDITIONALVALUE_ATR) != nullptr) {
                throw ComponentGenerationException(std::wstring(L"suffix: ") + suffix->getValue() +
                                                   std::wstring(L" used on an inappropriate group"));
            }
        }
    }
    ...
    if (group->getAttributeValue(XmlDeclarations::TYPE_ATR) ==
        XmlDeclarations::CHALCOGENACIDSTEM_TYPE_VAL) { //merge the suffix into the chalcogen acid stem e.g sulfonoate needs to be one fragment for infix replacement
    ...
    if (group->getValue() ==
        L"oxal") { //oxalic acid is treated as a non carboxylic acid for the purposes of functional replacment. See P-65.2.3
        suffixApplier->resolveSuffixes(group, suffixes);
...
//JAVA TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'toString':
                    Attribute tempVar(XmlDeclarations::DEFAULTLOCANTID_ATR,
                                      Integer::toString(firstIdInFragment + std::stoi(suffixInstruction) - 1));
...
void ComponentProcessor::processSuffixAppliesTo(Element *group, std::vector<Element *> &suffixes,
    ...
    if (suffix == nullptr) {
        ...
        if (suffixes.size() > 1 &&
            group->getAttributeValue(XmlDeclarations::TYPE_ATR) == XmlDeclarations::ACIDSTEM_TYPE_VAL) {
            throw ComponentGenerationException(
                    L"More than one suffix detected on trivial polyAcid. Not believed to be allowed");
        }
...
//JAVA TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'toString':
        ...
        if (suffix->getAttribute(XmlDeclarations::ADDITIONALVALUE_ATR) !=
            nullptr) { //handles amic, aldehydic, anilic and amoyl suffixes properly
            if (suffixInstructions.size() < 2) {
                throw ComponentGenerationException(std::wstring(L"suffix: ") + suffix->getValue() +
                                                   std::wstring(L" used on an inappropriate group"));
            }
...
//JAVA TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'toString':
            Attribute tempVar2(XmlDeclarations::LOCANTID_ATR,
                               Integer::toString(firstIdInFragment + std::stoi(suffixInstructions[0]) - 1));
            suffix->addAttribute(&tempVar2);
        }
        for (int i = 1; i < suffixInstructions.size(); i++) {
            ...
            if (symmetricSuffixes) {
                ...
                if (suffix->getAttribute(XmlDeclarations::SUBTYPE_ATR) != nullptr) {
                    Attribute tempVar5(XmlDeclarations::SUBTYPE_ATR,
                ...
                if (suffix->getAttribute(XmlDeclarations::INFIX_ATR) != nullptr &&
                    boost::starts_with(suffix->getAttributeValue(XmlDeclarations::INFIX_ATR),
                                       L"=")) { //clone infixes that effect double bonds but not single bonds e.g. maleamidate still should have one functional atom
                    Attribute tempVar6(XmlDeclarations::INFIX_ATR,
                                       suffix->getAttributeValue(XmlDeclarations::INFIX_ATR));
                    newSuffix->addAttribute(&tempVar6);
                }
            } else {
                Attribute tempVar7(XmlDeclarations::VALUE_ATR,
                                   suffix->getAttributeValue(XmlDeclarations::ADDITIONALVALUE_ATR));
...
//JAVA TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'toString':
            Attribute tempVar9(XmlDeclarations::LOCANTID_ATR,
                               Integer::toString(firstIdInFragment + std::stoi(suffixInstructions[i]) - 1));
...
std::vector<Fragment *> ComponentProcessor::resolveGroupAddingSuffixes(std::vector<Element *> &suffixes,
    ...
    for (auto suffix : suffixes) {
        ...
        std::vector<SuffixRule *> suffixRules = suffixApplier->getSuffixRuleTags(suffixTypeToUse, suffixValue,
                                                                                 subgroupType);
        ...
        for (auto suffixRule : suffixRules) {
            switch (suffixRule->getType()) {
                case addgroup: {
                    ...
                    suffixFrag = state->fragManager->buildSMILES(
                            suffixRule->getAttributeValue(XmlDeclarations::SUFFIXRULES_SMILES_ATR),
                            XmlDeclarations::SUFFIX_TYPE_VAL, labels);
                    std::vector<Atom *> atomList = suffixFrag->getAtomList();
                    std::wstring functionalIdsAtr = suffixRule->getAttributeValue(
                            XmlDeclarations::SUFFIXRULES_FUNCTIONALIDS_ATR);
...
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
                        ...
                        for (auto relativeId : relativeIdsOfFunctionalAtoms) {
                            ...
                            if (atomIndice >= atomList.size()) {
                                throw StructureBuildingException(
                                        L"Check suffixRules.xml: Atom requested to have a functionalAtom was not within the suffix fragment");
                            }
...
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
                        ...
                        for (auto relativeId : relativeIdsOfOutAtoms) {
                            ...
                            if (atomIndice >= atomList.size()) {
                                throw StructureBuildingException(
                                        L"Check suffixRules.xml: Atom requested to have a outAtom was not within the suffix fragment");
                            }
                ...
                case addFunctionalAtomsToHydroxyGroups:
                    if (suffixFrag != nullptr) {
                        throw ComponentGenerationException(
                                L"addFunctionalAtomsToHydroxyGroups is not currently compatable with the addGroup suffix rule");
                    }
                ...
                case chargeHydroxyGroups:
                    if (suffixFrag != nullptr) {
                        throw ComponentGenerationException(
                                L"chargeHydroxyGroups is not currently compatable with the addGroup suffix rule");
                    }
                ...
                case removeTerminalOxygen: {
                    if (suffixFrag != nullptr) {
                        throw ComponentGenerationException(
                                L"removeTerminalOxygen is not currently compatible with the addGroup suffix rule");
                    }
...
void ComponentProcessor::processRemovalOfHydroxyGroupsRules(std::vector<Element *> &suffixes,
...
void ComponentProcessor::addFunctionalAtomsToHydroxyGroups(Atom *atom) throw(StructureBuildingException) {
    ...
    for (auto neighbour : neighbours) {
        if (neighbour->getElement() == ChemEl::O && neighbour->getCharge() == 0 && neighbour->getBondCount() == 1 &&
            atom->getBondToAtomOrThrow(neighbour)->getOrder() == 1) {
...
void ComponentProcessor::processSuffixPrefixes(std::vector<Element *> &suffixes) throw(StructureBuildingException) {
    for (auto suffix : suffixes) {
        if (suffix->getAttribute(XmlDeclarations::SUFFIXPREFIX_ATR) != nullptr) {
            Fragment *suffixPrefixFrag = state->fragManager->buildSMILES(
                    suffix->getAttributeValue(XmlDeclarations::SUFFIXPREFIX_ATR), XmlDeclarations::SUFFIX_TYPE_VAL,
                    XmlDeclarations::NONE_LABELS_VAL);
...
bool ComponentProcessor::checkLocantPresentOnPotentialRoot(BuildState *state, Element *startingElement,
    ...
    while (s->size() > 0) {
        ...
        std::vector<Element *> siblings = OpsinTools::getChildElementsWithTagNames(parent, std::vector<std::wstring>{
                XmlDeclarations::BRACKET_EL, XmlDeclarations::SUBSTITUENT_EL, XmlDeclarations::ROOT_EL});
        ...
        for (auto bracketOrSub : siblings) {
            ...
            if (bracketOrSub->getName().equals(
                    XmlDeclarations::BRACKET_EL)) { //only want to consider implicit brackets, not proper brackets
                ...
                std::vector<Element *> conjunctiveGroups = OpsinTools::getNextSiblingsOfType(group,
    ...
    if (!foundSibling) { //Special case: anything the group could potentially substitute onto is in a bracket. The bracket is checked recursively
        ...
        while (s->size() > 0) {
            Element *currentElement = s->removeLast();
            Element *parent = currentElement->getParent();
            std::vector<Element *> siblings = OpsinTools::getChildElementsWithTagNames(parent,
                                                                                       std::vector<std::wstring>{
                                                                                               XmlDeclarations::BRACKET_EL,
                                                                                               XmlDeclarations::SUBSTITUENT_EL,
                                                                                               XmlDeclarations::ROOT_EL});
...
void ComponentProcessor::handleGroupIrregularities(
    ...
    for (auto group : groups) {
        ...
        if (groupValue == L"porphyrin" || groupValue == L"porphin") {
            std::vector<Element *> hydrogenAddingEls = group->getParent()->getChildElements(
                    XmlDeclarations::INDICATEDHYDROGEN_EL);
        ...
        } else if (groupValue == L"xanthate" || groupValue == L"xanthat" || groupValue == L"xanthic acid" ||
                   groupValue == L"xanthicacid") {
...
//JAVA TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'toString':
            if (wordRule->getAttributeValue(XmlDeclarations::WORDRULE_ATR) == WordRule::simple->toString()) {
                if (OpsinTools::getDescendantElementsWithTagName(wordRule, XmlDeclarations::SUBSTITUENT_EL).empty()) {
                    throw ComponentGenerationException(groupValue + std::wstring(
                            L" describes a class of compounds rather than a particular compound"));
                }
            }
        } else if (groupValue == L"adenosin" || groupValue == L"cytidin" || groupValue == L"guanosin" ||
                   groupValue == L"inosin" || groupValue == L"uridin" || groupValue == L"xanthosin") {
            //These groups are 2'-deoxy by convention
            Element *previous = OpsinTools::getPreviousSibling(group);
            if (previous != nullptr && previous->getName() == XmlDeclarations::SUBTRACTIVEPREFIX_EL &&
                previous->getAttributeValue(XmlDeclarations::TYPE_ATR) == XmlDeclarations::DEOXY_TYPE_VAL &&
                previous->getAttributeValue(XmlDeclarations::VALUE_ATR) == L"O" &&
                previous->getAttribute(XmlDeclarations::LOCANT_ATR) == nullptr) {
        ...
        if ((std::wstring(L"yes")) == group->getAttributeValue(XmlDeclarations::USABLEASJOINER_ATR) &&
            group->getAttribute(XmlDeclarations::DEFAULTINID_ATR) == nullptr &&
            group->getAttribute(XmlDeclarations::DEFAULTINLOCANT_ATR) == nullptr) {
            //makes linkers by default attach end to end
            Fragment *frag = group->getFrag();
            int chainLength = frag->getChainLength();
            if (chainLength > 1) {
                bool connectEndToEndWithPreviousSub = true;
                if (group->getAttributeValue(XmlDeclarations::TYPE_ATR).equals(XmlDeclarations::CHAIN_TYPE_VAL) &&
                    XmlDeclarations::ALKANESTEM_SUBTYPE_VAL == group->getAttributeValue(
                            XmlDeclarations::SUBTYPE_ATR)) { //don't do this if the group is preceded by another alkaneStem e.g. methylethyl makes more sense as prop-2-yl rather than propyl
                    Element *previousSubstituent = OpsinTools::getPreviousSibling(group->getParent());
                    if (previousSubstituent != nullptr) {
                        std::vector<Element *> previousSubstGroups = previousSubstituent->getChildElements(
                                XmlDeclarations::GROUP_EL);
                        if (previousSubstGroups.size() == 1) {
                            Element *previousGroup = previousSubstGroups[0];
                            if (previousGroup->getAttributeValue(XmlDeclarations::TYPE_ATR) ==
                                XmlDeclarations::CHAIN_TYPE_VAL && XmlDeclarations::ALKANESTEM_SUBTYPE_VAL ==
                                                                   previousGroup->getAttributeValue(
                                                                           XmlDeclarations::SUBTYPE_ATR)) {
                                Element *suffixAfterGroup = OpsinTools::getNextSibling(previousGroup,
                                                                                       XmlDeclarations::SUFFIX_EL);
                                if (suffixAfterGroup == nullptr || suffixAfterGroup->getFrag() == nullptr ||
                                    suffixAfterGroup->getFrag()->getOutAtomCount() == 0) {
...
void ComponentProcessor::processHW(Element *subOrRoot) throw(StructureBuildingException, ComponentGenerationException) {
    std::vector<Element *> hwGroups = OpsinTools::getChildElementsWithTagNameAndAttribute(subOrRoot,
                                                                                          XmlDeclarations::GROUP_EL,
                                                                                          XmlDeclarations::SUBTYPE_ATR,
                                                                                          XmlDeclarations::HANTZSCHWIDMAN_SUBTYPE_VAL);
    for (auto group : hwGroups) {
        ...
        if (atomList.size() == 6 && group->getValue().equals(L"an")) {
            ...
            for (auto heteroatom : heteroatomsToProcess) {
                ...
                if (heteroAtomElement == L"Si" || heteroAtomElement == L"Ge" || heteroAtomElement == L"Sn" ||
                    heteroAtomElement == L"Pb") {
            ...
            if (saturatedRing && !hasNitrogen && hasSiorGeorSnorPb) {
                throw ComponentGenerationException(
                        L"Blocked Hantzsch-Widman system (6 member saturated ring with no nitrogen but has Si/Ge/Sn/Pb)");
            }
        }
                ...
                if (specialInstruction != L"") {
                    if (specialInstruction == L"blocked") {
                        ...
                        if (group->getAttribute(XmlDeclarations::SUBSEQUENTUNSEMANTICTOKEN_ATR) == nullptr) {
                            Element *nextEl = OpsinTools::getNextSibling(group);
                            if (nextEl != nullptr && nextEl->getName() == XmlDeclarations::SUFFIX_EL &&
                                nextEl->getAttribute(XmlDeclarations::LOCANT_ATR) == nullptr &&
                                nextEl->getAttributeValue(XmlDeclarations::VALUE_ATR) == L"ic") {
                                throw ComponentGenerationException(name + nextEl->getValue() + std::wstring(
                                        L" appears to be a generic class name, not a Hantzsch-Widman ring"));
                            }
                        }
                        ...
                        if (group->getAttribute(XmlDeclarations::SUBSEQUENTUNSEMANTICTOKEN_ATR) == nullptr) {
                            Element *nextEl = OpsinTools::getNextSibling(group);
                            if (nextEl == nullptr ||
                                (nextEl != nullptr && nextEl->getName() == XmlDeclarations::SUFFIX_EL &&
                                 nextEl->getAttribute(XmlDeclarations::LOCANT_ATR) == nullptr &&
                                 nextEl->getAttributeValue(XmlDeclarations::VALUE_ATR) == L"ate")) {
                                throw ComponentGenerationException(name + std::wstring(
                                        L" has the syntax for a Hantzsch-Widman ring but probably does not mean that in this context"));
                            }
                        }
                    } else {
                        throw ComponentGenerationException(
                                L"OPSIN Bug: Unrecognised special Hantzsch-Widman ring instruction");
                    }
                }
...
//JAVA TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'toString':
        ...
        for (std::vector<Element *>::const_iterator it = heteroatomsToProcess.begin();
             it != heteroatomsToProcess.end(); ++it) {
        ...
        if (hetAtomsToProcess > 0) {
            ...
            if (hetAtomsToProcess > 1 && hetAtomsToProcess < (carbonAtomsInRing.size() - 1)) {
                ...
                if (!(possibleBenzo != nullptr &&
                      (possibleBenzo->getValue() == L"benz" || possibleBenzo->getValue() == L"benzo") ||
                      (std::wstring(L"o")) ==
                      group->getAttributeValue((XmlDeclarations::SUBSEQUENTUNSEMANTICTOKEN_ATR)))) {
            ...
            if (hetAtomsToProcess > carbonAtomsInRing.size()) {
                throw StructureBuildingException(std::to_wstring(hetAtomsToProcess) +
                                                 L" heteroatoms were specified for a Hantzsch-Widman ring with only " +
                                                 carbonAtomsInRing.size() + std::wstring(L" atoms"));
            }
            for (int i = 0; i < hetAtomsToProcess; i++) {
                ...
                a->setElement(StringHelper::fromString<ChemEl>(elementReplacement));
                if (heteroatom->getAttribute(XmlDeclarations::LAMBDA_ATR) != nullptr) {
                    a->setLambdaConventionValency(
                            std::stoi(heteroatom->getAttributeValue(XmlDeclarations::LAMBDA_ATR)));
                }
                heteroatom->detach();
            }
        }
...
void ComponentProcessor::processRingAssemblies(
        Element *subOrRoot) throw(ComponentGenerationException, StructureBuildingException) {
    std::vector<Element *> ringAssemblyMultipliers = subOrRoot->getChildElements(
            XmlDeclarations::RINGASSEMBLYMULTIPLIER_EL);
    for (auto multiplier : ringAssemblyMultipliers) {
        ...
        if (potentialLocant != nullptr &&
            (potentialLocant->getName() == XmlDeclarations::COLONORSEMICOLONDELIMITEDLOCANT_EL ||
             potentialLocant->getName() ==
             XmlDeclarations::LOCANT_EL)) { //a locant appears to have been provided to indicate how to connect the rings of the ringAssembly
            if (XmlDeclarations::ORTHOMETAPARA_TYPE_VAL == potentialLocant->getAttributeValue(
                    XmlDeclarations::TYPE_ATR)) { //an OMP locant has been provided to indicate how to connect the rings of the ringAssembly
...
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
                ...
                if (perRingLocantArray.size() != (mvalue - 1)) {
                    throw ComponentGenerationException(
                            std::wstring(L"Disagreement between number of locants(") + locantText +
                            std::wstring(L") and ring assembly multiplier: ") + std::to_wstring(mvalue));
                }
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
                if (perRingLocantArray.size() != 1 || perRingLocantArray[0].split(L",")->length !=
                                                      1) { //if there is just a single locant it doesn't relate to how the rings are connected
...
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
                        ...
                        if (locantArray.size() != 2) {
                            throw ComponentGenerationException(
                                    std::wstring(L"missing locant, expected 2 locants: ") + ringLocantArray);
                        }
        ...
        if (nextEl->getName() ==
            XmlDeclarations::STRUCTURALOPENBRACKET_EL) { //brackets have been provided to aid disambiguation. These brackets are detached e.g. bi(cyclohexyl)
            ...
            elementToResolve = determineElementsToResolveIntoRingAssembly(multiplier, ringJoiningLocants.size(),
                                                                          fragmentToResolveAndDuplicate->getOutAtomCount());
        }
        ...
        if (fragmentToResolveAndDuplicate->getOutAtomCount() > 1) {
            throw StructureBuildingException(
                    L"Ring assembly fragment should have one or no OutAtoms; not more than one!");
        }
        ...
        bool twoRingsJoinedUsingSuffixPosition =
                ringJoiningLocants.empty() && mvalue == 2 && fragmentToResolveAndDuplicate->getOutAtomCount() == 1;
        ...
        if (twoRingsJoinedUsingSuffixPosition) {
            ...
            state->fragManager->incorporateFragment(clone, atomOnClone, fragmentToResolveAndDuplicate, atomOnParent,
                                                    bondOrder);
        } else {
            ...
            for (int i = 0; i < mvalue - 1; i++) {
                ...
                if (ringJoiningLocants.size() > 0) { //locants defined
                    ...
                    if (lastRingUnlocantedBondedTo == nullptr) {
                        potentialAtomsOnParent = FragmentTools::findSubstituableAtoms(fragmentToResolveAndDuplicate,
                                                                                      bondOrder);
                    } else {
                        potentialAtomsOnParent = FragmentTools::findSubstituableAtoms(lastRingUnlocantedBondedTo,
                                                                                      bondOrder);
                    }
                    ...
                    if (potentialAtomsOnParent.empty() || potentialAtomsOnClone.empty()) {
                        throw StructureBuildingException(
                                L"Unable to find suitable atom for unlocanted ring assembly construction");
                    }
                    if (AmbiguityChecker::isSubstitutionAmbiguous(potentialAtomsOnParent, 1)) {
                        state->addIsAmbiguous(
                                std::wstring(L"Choice of atoms to form ring assembly: ") + group->getValue());
                    }
                ...
                state->fragManager->incorporateFragment(clone, atomOnLatestClone, fragmentToResolveAndDuplicate,
                                                        atomOnParent, bondOrder);
            }
        }
        ...
        if (possibleOpenStructuralBracket != nullptr && possibleOpenStructuralBracket->getName() ==
                                                        XmlDeclarations::STRUCTURALOPENBRACKET_EL) { //e.g. [2,2'-bipyridin].
            //To emphasise there can actually be two sets of structural brackets e.g. [1,1'-bi(cyclohexyl)]
            OpsinTools::getNextSibling(possibleOpenStructuralBracket,
                                       XmlDeclarations::STRUCTURALCLOSEBRACKET_EL)->detach();
...
Element *ComponentProcessor::determineElementsToResolveIntoRingAssembly(Element *multiplier, int ringJoiningLocants,
    ...
    while (currentEl != nullptr) {
        ...
        if (!groupFound) {
            ...
            if (currentEl->getName() == XmlDeclarations::SUFFIX_EL) {
                ...
                if (suffixType == XmlDeclarations::CHARGE_TYPE_VAL &&
                    currentEl->getAttribute(XmlDeclarations::LOCANT_ATR) == nullptr) {
                    currentEl->detach();
                    elementToResolve->addChild(currentEl);
                } else if (!inlineSuffixSeen && suffixType == XmlDeclarations::INLINE_TYPE_VAL &&
                           currentEl->getAttributeValue(XmlDeclarations::MULTIPLIED_ATR) == L"" &&
                           (currentEl->getAttribute(XmlDeclarations::LOCANT_ATR) == nullptr ||
                            ((std::wstring(L"2")) == multiplier->getAttributeValue(XmlDeclarations::VALUE_ATR) &&
                             ringJoiningLocants == 0)) && currentEl->getFrag() == nullptr) {
            ...
            } else if (currentEl->getName() == XmlDeclarations::UNSATURATOR_EL &&
                       currentEl->getAttribute(XmlDeclarations::LOCANT_ATR) == nullptr) {
                currentEl->detach();
                elementToResolve->addChild(currentEl);
            } else {
    ...
    if (parent->getName() != XmlDeclarations::SUBSTITUENT_EL &&
        OpsinTools::getChildElementsWithTagNameAndAttribute(parent, XmlDeclarations::SUFFIX_EL,
                                                            XmlDeclarations::TYPE_ATR,
                                                            XmlDeclarations::INLINE_TYPE_VAL).size() > 0) {
        throw ComponentGenerationException(L"Unexpected radical adding suffix on ring assembly");
    }
    return elementToResolve;
}

void ComponentProcessor::processPolyCyclicSpiroNomenclature(
        Element *subOrRoot) throw(ComponentGenerationException, StructureBuildingException) {
...
void ComponentProcessor::processNonIdenticalPolyCyclicSpiro(
    ...
    std::vector<Element *> spiroBracketElements = OpsinTools::getSiblingsUpToElementWithTagName(openBracket,
                                                                                                XmlDeclarations::STRUCTURALCLOSEBRACKET_EL);
...
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
            ...
            if (locants.size() != 2) {
                throw ComponentGenerationException(
                        L"Incorrect number of locants found before component of polycyclic spiro system");
            }
            bool changed = false;
            ...
            if (m1->find()) {
                ...
                if (primes > 0 && primes == (groups.size() -
                                             1)) { //rings are primeless before spiro fusion (hydrogen is currently added before spiro fusion)
            ...
            if (m2->find()) {
                ...
                int primes = StringTools::countTerminalPrimes(addedHydrogenLocant);
                if (primes > 0 && primes ==
                                  groups.size()) { //rings are primeless before spiro fusion (hydrogen is currently added before spiro fusion)
                    addedHydrogenLocant = addedHydrogenLocant.substr(0, addedHydrogenLocant.length() - primes);
                }
...
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
        ...
        int indexOfNextSpiroLocantOrEndOfSpiro = subOrRoot->find(
                i + 1 < groupCount ? OpsinTools::getNextSibling(nextGroup, XmlDeclarations::SPIROLOCANT_EL)
                                   : OpsinTools::getNextSibling(nextGroup, XmlDeclarations::STRUCTURALCLOSEBRACKET_EL));
        ...
        if (atomToBeReplaced == nullptr) {
            throw ComponentGenerationException(std::wstring(L"Could not find the atom with locant ") + locants[0] +
                                               std::wstring(L" for use in polycyclic spiro system"));
        }
        ...
        if (atomToBeReplaced->getElement() != atomOnNextFragment->getElement()) {
            ...
            if (atomToBeReplaced->getElement() != ChemEl::C && atomOnNextFragment->getElement() == ChemEl::C) {
                ...
                throw ComponentGenerationException(
                        std::wstring(L"Disagreement between which element the spiro atom should be: ") +
                        atomToBeReplaced->getElement() + std::wstring(L" and ") + atomOnNextFragment->getElement());
            }
        }
    ...
    if (spiroAtoms->size() > 1) {
        ...
        if (expectedMultiplier != nullptr && expectedMultiplier->getName() == XmlDeclarations::MULTIPLIER_EL &&
            std::stoi(expectedMultiplier->getAttributeValue(XmlDeclarations::VALUE_ATR)) == spiroAtoms->size()) {
...
void ComponentProcessor::processOldMethodPolyCyclicSpiro(
    ...
    std::vector<Element *> elementsToResolve = OpsinTools::getSiblingsUpToElementWithTagName(firstEl,
    ...
    for (int i = 0; i < spiroElements.size(); i++) {
        ...
        elementsToResolve = OpsinTools::getSiblingsUpToElementWithTagName(currentSpiro,
                                                                          XmlDeclarations::POLYCYCLICSPIRO_EL);
        resolveFeaturesOntoGroup(elementsToResolve);
...
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
        ...
        if (locant1 != L"") {
            ...
            if (AmbiguityChecker::isSubstitutionAmbiguous(potentialAtoms, 1)) {
                state->addIsAmbiguous(
                        std::wstring(L"Choice of atom for spiro fusion on: ") + previousGroup->getValue());
            }
...
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
        ...
        if (atomToBeReplaced->getCharge() != 0 && atomOnParentFrag->getCharge() == 0) {
            ...
            atomOnParentFrag->setProtonsExplicitlyAddedOrRemoved(
...
void ComponentProcessor::processSpiroBiOrTer(Element *polyCyclicSpiroDescriptor,
    ...
    if (locant == nullptr || locant->getName() != XmlDeclarations::LOCANT_EL) {
        if (components == 2) {
            ...
            throw ComponentGenerationException(
                    L"Unable to find locant indicating atoms to form polycyclic spiro system!");
        }
    } else {
...
void ComponentProcessor::processDispiroter(
...
void ComponentProcessor::determineFeaturesToResolveInSingleComponentSpiro(
        Element *polyCyclicSpiroDescriptor) throw(StructureBuildingException, ComponentGenerationException) {
    ...
    if (possibleOpenBracket->getName() == XmlDeclarations::STRUCTURALOPENBRACKET_EL) {
        ...
        elementsToResolve = OpsinTools::getSiblingsUpToElementWithTagName(polyCyclicSpiroDescriptor,
                                                                          XmlDeclarations::STRUCTURALCLOSEBRACKET_EL);
...
void ComponentProcessor::resolveFeaturesOntoGroup(
    ...
    Element *substituentToResolve = new GroupingEl(
            XmlDeclarations::SUBSTITUENT_EL); //temporary element containing elements that should be resolved before the ring is cloned
    ...
    if (locant != nullptr) { //locant is probably an indirect locant, try and assign it
...
ComponentProcessor::SortBridgesByHighestLocantedBridgehead::SortBridgesByHighestLocantedBridgehead(
        std::unordered_map<Fragment *, std::vector<Atom *>> &bridgeToRingAtoms) : bridgeToRingAtoms(bridgeToRingAtoms) {
}
    ...
    for (auto bridge : bridges) {
        ...
        if (possibleMultiplier != nullptr) {
            ...
            if (possibleMultiplier->getName() == XmlDeclarations::MULTIPLIER_EL) {
                ...
                possibleMultiplier->detach();
                if (possibleLocant != nullptr &&
                    possibleLocant->getName() == XmlDeclarations::COLONORSEMICOLONDELIMITEDLOCANT_EL) {
                    locants = std::vector<std::vector<std::wstring>>();
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
                    std::vector<std::wstring> locantsForEachMultiple = StringTools::removeDashIfPresent(
                            possibleLocant->getValue()).split(L":");
...
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
        ...
        for (int i = 0; i < multiplier; i++) {
            Fragment *bridgeFrag = state->fragManager->buildSMILES(
                    bridge->getAttributeValue(XmlDeclarations::VALUE_ATR), groupEl, XmlDeclarations::NONE_LABELS_VAL);
            ...
            if (locants.size() > 0) {
                ...
                ringAtoms = StructureBuildingMethods::formEpoxide(state, bridgeFrag,
                                                                  ringFrag->getDefaultInAtomOrFirstAtom());
            } else {
...
void ComponentProcessor::applyLambdaConvention(Element *subOrRoot) throw(StructureBuildingException) {
    ...
    for (auto lambdaConventionEl : lambdaConventionEls) {
        ...
        if (lambdaConventionEl->getAttribute(XmlDeclarations::LOCANT_ATR) != nullptr) {
            frag->getAtomByLocantOrThrow(
                    lambdaConventionEl->getAttributeValue(XmlDeclarations::LOCANT_ATR))->setLambdaConventionValency(
                    std::stoi(lambdaConventionEl->getAttributeValue(XmlDeclarations::LAMBDA_ATR)));
        } else {
            if (frag->getAtomCount() != 1) {
                throw StructureBuildingException(
                        L"Ambiguous use of lambda convention. Fragment has more than 1 atom but no locant was specified for the lambda");
            }
            frag->getFirstAtom()->setLambdaConventionValency(
                    std::stoi(lambdaConventionEl->getAttributeValue(XmlDeclarations::LAMBDA_ATR)));
        }
...
void ComponentProcessor::handleMultiRadicals(
        Element *subOrRoot) throw(ComponentGenerationException, StructureBuildingException) {
    Element *group = subOrRoot->getFirstChildElement(XmlDeclarations::GROUP_EL);
    std::wstring groupValue = group->getValue();
    Fragment *thisFrag = group->getFrag();
    if (groupValue == L"methylene" || groupValue == L"methylen" || groupValue == L"oxy" ||
        matchChalcogenReplacement->matcher(
                groupValue).matches()) { //resolves for example trimethylene to propan-1,3-diyl or dithio to disulfan-1,2-diyl. Locants may not be specified before the multiplier
        Element *beforeGroup = OpsinTools::getPreviousSibling(group);
        if (beforeGroup != nullptr && beforeGroup->getName() == XmlDeclarations::MULTIPLIER_ATR &&
            beforeGroup->getAttributeValue(XmlDeclarations::TYPE_ATR) == XmlDeclarations::BASIC_TYPE_VAL &&
            OpsinTools::getPreviousSibling(beforeGroup) == nullptr) {
            ...
            if (!unsuitableForFormingChainMultiradical(group, beforeGroup)) {
                if (groupValue == L"methylene" || groupValue == L"methylen") {
                    group->getAttribute(XmlDeclarations::VALUE_ATR)->setValue(
                            StringTools::multiplyString(L"C", multiplierVal));
                } else if (groupValue == L"oxy") {
                    group->getAttribute(XmlDeclarations::VALUE_ATR)->setValue(
                            StringTools::multiplyString(L"O", multiplierVal));
                } else if (groupValue == L"thio") {
                    group->getAttribute(XmlDeclarations::VALUE_ATR)->setValue(
                            StringTools::multiplyString(L"S", multiplierVal));
                } else if (groupValue == L"seleno") {
                    group->getAttribute(XmlDeclarations::VALUE_ATR)->setValue(
                            StringTools::multiplyString(L"[SeH?]", multiplierVal));
                } else if (groupValue == L"telluro") {
                    group->getAttribute(XmlDeclarations::VALUE_ATR)->setValue(
                            StringTools::multiplyString(L"[TeH?]", multiplierVal));
                } else {
                    throw ComponentGenerationException(L"unexpected group value");
                }
                group->getAttribute(XmlDeclarations::OUTIDS_ATR)->setValue(std::wstring(L"1,") + std::to_wstring(
                        std::stoi(beforeGroup->getAttributeValue(XmlDeclarations::VALUE_ATR))));
    ...
    if (group->getAttribute(XmlDeclarations::OUTIDS_ATR) != nullptr) { //adds outIDs at the specified atoms
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
    ...
    if (outAtomCount >= 2) {
        if (groupValue == L"amine" || groupValue ==
                                      L"amin") { //amine is a special case as it shouldn't technically be allowed but is allowed due to it's common usage in EDTA
            Element *previousGroup = OpsinTools::getPreviousGroup(group);
            Element *nextGroup = OpsinTools::getNextGroup(group);
            if (previousGroup == nullptr || previousGroup->getFrag()->getOutAtomCount() < 2 ||
                nextGroup == nullptr) { //must be preceded by a multi radical
    ...
    if (outAtomCount == 2 &&
        XmlDeclarations::EPOXYLIKE_SUBTYPE_VAL == group->getAttributeValue(XmlDeclarations::SUBTYPE_ATR)) {
...
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
    ...
    int totalOutAtoms = outAtomCount + calculateOutAtomsToBeAddedFromInlineSuffixes(group, subOrRoot->getChildElements(
            XmlDeclarations::SUFFIX_EL));
    if (totalOutAtoms >= 2) {
//JAVA TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'toString':
...
bool ComponentProcessor::unsuitableForFormingChainMultiradical(Element *group, Element *multiplierBeforeGroup) {
    ...
    if (previousGroup != nullptr) {
        if (previousGroup->getAttribute(XmlDeclarations::ISAMULTIRADICAL_ATR) != nullptr) {
            if (previousGroup->getAttributeValue(XmlDeclarations::ACCEPTSADDITIVEBONDS_ATR) != L"" &&
                OpsinTools::getPreviousSibling(previousGroup->getParent()) != nullptr) {
                return false;
            }
            ...
            if (previousGroup->getAttributeValue(XmlDeclarations::ISAMULTIRADICAL_ATR) ==
                multiplierBeforeGroup->getAttributeValue(XmlDeclarations::VALUE_ATR)) {
...
int ComponentProcessor::calculateOutAtomsToBeAddedFromInlineSuffixes(Element *group,
...
void
ComponentProcessor::addImplicitBracketsToAminoAcids(std::vector<Element *> &groups, std::vector<Element *> &brackets) {
    for (int i = groups.size() - 1; i >= 0; i--) {
        Element *group = groups[i];
        if (group->getAttributeValue(XmlDeclarations::TYPE_ATR) == XmlDeclarations::AMINOACID_TYPE_VAL &&
            OpsinTools::getNextGroup(group) != nullptr) {
            Element *possibleLocant = OpsinTools::getPreviousSiblingIgnoringCertainElements(group,
                                                                                            std::vector<std::wstring>{
                                                                                                    XmlDeclarations::MULTIPLIER_EL});
            if (possibleLocant != nullptr && possibleLocant->getName() == XmlDeclarations::LOCANT_EL) {
                continue;
            ...
            while (previous != nullptr) {
                if (previous->getName() != XmlDeclarations::SUBSTITUENT_EL &&
                    previous->getName() != XmlDeclarations::BRACKET_EL) {
                    break;
                }
...
void ComponentProcessor::implicitlyBracketToPreviousSubstituentIfAppropriate(Element *substituent,
    ...
    if (elementBeforeSubstituent == nullptr || elementBeforeSubstituent->getName() != XmlDeclarations::SUBSTITUENT_EL &&
                                               elementBeforeSubstituent->getName() != XmlDeclarations::BRACKET_EL) {
        return;
    }
    ...
    if (elementAftersubstituent != nullptr) {
        ...
        if (elementBeforeSubstituent->getName() == XmlDeclarations::BRACKET_EL && XmlDeclarations::IMPLICIT_TYPE_VAL !=
                                                                                  elementBeforeSubstituent->getAttributeValue(
                                                                                          XmlDeclarations::TYPE_ATR) &&
            elementAftersubstituent->getName() == XmlDeclarations::BRACKET_EL) {
            Element *firstChildElementOfElementAfterSubstituent = elementAftersubstituent->getChild(0);
            if ((firstChildElementOfElementAfterSubstituent->getName() == XmlDeclarations::SUBSTITUENT_EL ||
                 firstChildElementOfElementAfterSubstituent->getName() == XmlDeclarations::BRACKET_EL) &&
                OpsinTools::getPrevious(firstChildElementOfElementAfterSubstituent)->getName() !=
    ...
    if (!isSubBracketOrRoot(elementAftersubstituent)) {
        if (!(elementAftersubstituent == nullptr &&
              locantedEsterImplicitBracketSpecialCase(substituent, elementBeforeSubstituent))) {
            return;
        }
    }
    ...
    Element *elementDirectlyBeforeSubstituent = OpsinTools::getPrevious(
            substituent->getChild(0)); //can't return null as we know elementBeforeSubstituent is not null
    if (elementDirectlyBeforeSubstituent->getName() == XmlDeclarations::HYPHEN_EL &&
    ...
    std::vector<Element *> groupElements = OpsinTools::getDescendantElementsWithTagName(elementBeforeSubstituent,
                                                                                        XmlDeclarations::GROUP_EL); //one for a substituent, possibly more for a bracket
    ...
    if (theSubstituentType == XmlDeclarations::CHAIN_TYPE_VAL &&
        theSubstituentSubType == XmlDeclarations::ALKANESTEM_SUBTYPE_VAL &&
        lastGroupOfElementBeforeSub->getAttributeValue(XmlDeclarations::TYPE_ATR) == XmlDeclarations::CHAIN_TYPE_VAL &&
        lastGroupOfElementBeforeSub->getAttributeValue(XmlDeclarations::SUBTYPE_ATR) ==
        XmlDeclarations::ALKANESTEM_SUBTYPE_VAL) {
        ...
        if (!placeInImplicitBracket) {
            ...
            if (foundLocantNotReferringToChain &&
                !foundLocantNotReferringToChain) { //a locant was found and it appeared to refer to the other chain
                placeInImplicitBracket = true;
            }
        }
        if (!placeInImplicitBracket) {
    ...
    if (lastGroupOfElementBeforeSub->getAttribute(XmlDeclarations::ISAMULTIRADICAL_ATR) != nullptr &&
        lastGroupOfElementBeforeSub->getAttribute(XmlDeclarations::ACCEPTSADDITIVEBONDS_ATR) == nullptr &&
        lastGroupOfElementBeforeSub->getAttribute(XmlDeclarations::IMINOLIKE_ATR) == nullptr) {
        return;
    }
    if (substituentGroup->getAttribute(XmlDeclarations::ISAMULTIRADICAL_ATR) != nullptr) {
        if (substituentGroup->getAttribute(XmlDeclarations::ACCEPTSADDITIVEBONDS_ATR) == nullptr &&
            substituentGroup->getAttribute(XmlDeclarations::IMINOLIKE_ATR) == nullptr) {
        ...
        if (!isSubstitutable && elementAftersubstituent != nullptr &&
            elementAftersubstituent->getChild(0)->getName() == XmlDeclarations::MULTIPLIER_EL) {
            ...
            if (frag->getOutAtomCount() ==
                std::stoi(elementAftersubstituent->getChild(0)->getAttributeValue(XmlDeclarations::VALUE_ATR))) {
                ...
                if (elType == XmlDeclarations::ROOT_EL) {
                    ...
                    std::vector<Element *> groups = OpsinTools::getDescendantElementsWithTagName(
                            elementAftersubstituent, XmlDeclarations::GROUP_EL);
                    for (auto group : groups) {
                        if (group->getAttribute(XmlDeclarations::ISAMULTIRADICAL_ATR) != nullptr) {
                            return; //a multi radical
                        }
                    }
                } else if (elType == XmlDeclarations::BRACKET_EL &&
                           OpsinTools::getDescendantElementsWithTagName(elementAftersubstituent,
                                                                        XmlDeclarations::ROOT_EL).size() > 0) {
                    return;
                }
            }
        }
    }
    if (lastGroupOfElementBeforeSub->getAttribute(XmlDeclarations::IMINOLIKE_ATR) != nullptr &&
        substituentGroup->getAttribute(XmlDeclarations::IMINOLIKE_ATR) != nullptr) {
    ...
    if (substituentGroup->getValue() == L"sulf" && frag->getAtomCount() == 1) {
        Element *suffix = OpsinTools::getNextSiblingIgnoringCertainElements(substituentGroup, std::vector<std::wstring>{
    ...
    if (XmlDeclarations::PERHALOGENO_SUBTYPE_VAL ==
        lastGroupOfElementBeforeSub->getAttributeValue(XmlDeclarations::SUBTYPE_ATR)) {
        return;
    }
...
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
    ...
    if (locantValues.size() > 0) {
        ...
        for (auto locantText : locantValues) {
            ...
            if (frag->getAtomCount() == 1 || !frag->hasLocant(locantText) ||
                matchElementSymbolOrAminoAcidLocant->matcher(locantText).find() ||
                (locantValues.size() == 1 && elAfterLocant->getName() == XmlDeclarations::MULTIPLIER_EL)) {
        ...
        if (moveLocants && locantValues.size() > 1) {
            if (elAfterLocant != nullptr && elAfterLocant->getName() == XmlDeclarations::MULTIPLIER_EL) {
                Element *shouldBeAGroupOrSubOrBracket = OpsinTools::getNextSiblingIgnoringCertainElements(elAfterLocant,
                                                                                                          std::vector<std::wstring>{
                                                                                                                  XmlDeclarations::MULTIPLIER_EL});
                if (shouldBeAGroupOrSubOrBracket != nullptr) {
                    if ((shouldBeAGroupOrSubOrBracket->getName() == XmlDeclarations::GROUP_EL &&
                         elAfterLocant->getAttributeValue(XmlDeclarations::TYPE_ATR) ==
                         XmlDeclarations::GROUP_TYPE_VAL) || (matchGroupsThatAreAlsoInlineSuffixes->matcher(
                            substituentGroup->getValue()).matches())) { //e.g. 4,4'-dimethoxycarbonyl-2,2'-bioxazole --> 4,4'-di(methoxycarbonyl)-2,2'-bioxazole - e.g. 2,5-bisaminothiobenzene --> 2,5-bis(aminothio)benzene
                        locantRelatedElements.push_back(
                                elAfterLocant); //e.g. 1,5-bis-(4-methylphenyl)sulfonyl --> 1,5-bis-((4-methylphenyl)sulfonyl)
                    } else if (XmlDeclarations::ORTHOMETAPARA_TYPE_VAL == locantRelatedElements[0]->getAttributeValue(
                            XmlDeclarations::TYPE_ATR)) { //e.g. p-dimethylamino[ring]
                        locantRelatedElements[0]->setValue(locantValues[1]);
                    } else if (frag->getAtomCount() ==
                               1) { //e.g. 1,3,4-trimethylthiobenzene --> 1,3,4-tri(methylthio)benzene
    ...
    if (locantRelatedElements.empty()) {
        ...
        if (possibleMultiplier->getName() == XmlDeclarations::MULTIPLIER_EL &&
            (matchGroupsThatAreAlsoInlineSuffixes->matcher(substituentGroup->getValue()).matches() ||
             possibleMultiplier->getAttributeValue(XmlDeclarations::TYPE_ATR) == XmlDeclarations::GROUP_TYPE_VAL)) {
            Element *desiredGroup = OpsinTools::getNextSiblingIgnoringCertainElements(possibleMultiplier,
                                                                                      std::vector<std::wstring>{
                                                                                              XmlDeclarations::MULTIPLIER_EL});
...
bool ComponentProcessor::implicitBracketWouldPreventAdditiveBonding(Element *elementBeforeSubstituent,
                                                                    Element *elementAftersubstituent) {
    if (elementAftersubstituent != nullptr && elementAftersubstituent->getName() == XmlDeclarations::SUBSTITUENT_EL) {
        ...
        if (groupAfterSubstituent->getAttribute(XmlDeclarations::ACCEPTSADDITIVEBONDS_ATR) != nullptr &&
            ...
            if (elementBeforeSubstituent->getChild(0)->getName() == XmlDeclarations::LOCANT_EL) {
                ...
                while (viableSubstituent != nullptr) {
                    if (viableSubstituent->getName() == XmlDeclarations::SUBSTITUENT_EL ||
                        viableSubstituent->getName() == XmlDeclarations::BRACKET_EL) {
                        Element *possibleLocant = viableSubstituent->getChild(0);
                        if (possibleLocant->getName() == XmlDeclarations::LOCANT_EL) {
                            if (additiveAcceptingFrag->getFirstAtom()->equals(
                                    additiveAcceptingFrag->getAtomByLocant(possibleLocant->getValue()))) {
                                return false;
                            }
                        }
                    }
...
bool
ComponentProcessor::locantedEsterImplicitBracketSpecialCase(Element *substituent, Element *elementBeforeSubstituent) {
    if (substituent->getParent()->getName() == XmlDeclarations::WORD_EL &&
        OpsinTools::getPreviousSibling(elementBeforeSubstituent) == nullptr &&
        (state->currentWordRule == WordRule::ester || state->currentWordRule == WordRule::functionalClassEster ||
         state->currentWordRule == WordRule::multiEster || state->currentWordRule == WordRule::acetal)) {
        return true;
    }
    return false;
}
...
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
        ...
        if (locantValues.size() == 1 && group->getAttribute(XmlDeclarations::FRONTLOCANTSEXPECTED_ATR) !=
                                        nullptr) { //some trivial retained names like 2-furyl expect locants to be in front of them. For these the indirect intepretation will always be used rather than checking whether 2-(furyl) even makes sense
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
            std::vector<std::wstring> allowedLocants = group->getAttributeValue(
                    XmlDeclarations::FRONTLOCANTSEXPECTED_ATR).split(L",");
            for (auto allowedLocant : allowedLocants) {
                if (locantValues[0] == allowedLocant) {
                    ...
                    if (expectedSuffix != nullptr && expectedSuffix->getName() == XmlDeclarations::SUFFIX_EL &&
                        expectedSuffix->getAttribute(XmlDeclarations::LOCANT_ATR) == nullptr) {
                        Attribute tempVar(XmlDeclarations::LOCANT_ATR, locantValues[0]);
        ...
        if (state->currentWordRule == WordRule::multiEster && XmlDeclarations::ADDEDHYDROGENLOCANT_TYPE_VAL !=
                                                              lastLocant->getAttributeValue(
                                                                      XmlDeclarations::TYPE_ATR)) { //special case e.g. 1-benzyl 4-butyl terephthalate (locants do not apply to yls)
            Element *parentEl = subOrRoot->getParent();
            if (parentEl->getName() == XmlDeclarations::WORD_EL &&
                parentEl->getAttributeValue(XmlDeclarations::TYPE_ATR) == XmlDeclarations::SUBSTITUENT_EL &&
                parentEl->getChildCount() == 1 && locantValues.size() == 1 &&
                XmlDeclarations::ORTHOMETAPARA_TYPE_VAL != lastLocant->getAttributeValue(XmlDeclarations::TYPE_ATR)) {
        ...
        if (allowIndirectLocants) {
            /* The first locant is most likely a locant indicating where this subsituent should be attached.
									 ...
									 */
            if (XmlDeclarations::ADDEDHYDROGENLOCANT_TYPE_VAL !=
                lastLocant->getAttributeValue(XmlDeclarations::TYPE_ATR) && locantEls.size() == 1 &&
                group->getAttribute(XmlDeclarations::ISAMULTIRADICAL_ATR) == nullptr && locantValues.size() == 1 &&
                checkLocantPresentOnPotentialRoot(state, subOrRoot, locantValues[0]) &&
                OpsinTools::getPreviousSibling(lastLocant, XmlDeclarations::LOCANT_EL) == nullptr) {
                return;
            }
            ...
            if (locantAble.size() < locantValues.size()) {
                ...
                for (auto locantValue : locantValues) {
                    if (!fragmentAfterLocant->hasLocant(
                            locantValue)) { //locant is not available on the group adjacent to the locant!
                        assignableToIndirectFeatures = false;
                    }
                }
            }
                    ...
                    for (auto suffix : suffixes) {
                        if (suffix->hasLocant(locantValues[0])) { //e.g. 2'-Butyronaphthone
                            ...
                            if (b != nullptr) {
                                ...
                                state->fragManager->createBond(dummyRAtom,
                                                               suffix->getAtomByLocantOrThrow(locantValues[0]),
                                                               b->getOrder());
                                lastLocant->detach();
                            }
                        }
...
std::vector<Element *> ComponentProcessor::findLocantsThatCouldBeIndirectLocants(Element *subOrRoot) {
    ...
    for (auto el : children) {
        if (el->getName().equals(XmlDeclarations::LOCANT_EL)) {
            ...
            if (afterLocant != nullptr && afterLocant->getName() ==
                                          XmlDeclarations::MULTIPLIER_EL) { //locant should not be followed by a multiplier. c.f. 1,2,3-tributyl 2-acetyloxypropane-1,2,3-tricarboxylate
                continue;
            }
...
std::vector<Element *> ComponentProcessor::findElementsMissingIndirectLocants(Element *subOrRoot, Element *locantEl) {
    ...
    for (auto el : childrenOfSubOrBracketOrRoot) {
        ...
        if (name == XmlDeclarations::SUFFIX_EL || name == XmlDeclarations::UNSATURATOR_EL ||
            name == XmlDeclarations::CONJUNCTIVESUFFIXGROUP_EL) {
            if (el->getAttribute(XmlDeclarations::LOCANT_ATR) == nullptr &&
                el->getAttribute(XmlDeclarations::LOCANTID_ATR) == nullptr &&
                el->getAttribute(XmlDeclarations::MULTIPLIED_ATR) ==
                nullptr) { // shouldn't already have a locant or be multiplied (should of already had locants assignd to it if that were the case)
                if (subOrRoot->find(el) > subOrRoot->find(locantEl)) {
                    if (name == XmlDeclarations::SUFFIX_EL) { //check a few special cases that must not be locanted
                        ...
                        if ((type == XmlDeclarations::ACIDSTEM_TYPE_VAL && XmlDeclarations::CYCLEFORMER_SUBTYPE_VAL !=
                                                                           el->getAttributeValue(
                                                                                   XmlDeclarations::SUBTYPE_ATR)) ||
                            type == XmlDeclarations::NONCARBOXYLICACID_TYPE_VAL ||
                            type == XmlDeclarations::CHALCOGENACIDSTEM_TYPE_VAL) {
                            continue;
                        }
                    }
...
void
ComponentProcessor::assignImplicitLocantsToDiTerminalSuffixes(Element *subOrRoot) throw(StructureBuildingException) {
    ...
    if (terminalSuffix1 != nullptr) {
        if (isATerminalSuffix(terminalSuffix1) && OpsinTools::getNextSibling(terminalSuffix1) != nullptr) {
            ...
            if (isATerminalSuffix(terminalSuffix2)) {
                ...
                if (hopefullyAChain != nullptr &&
                    hopefullyAChain->getAttributeValue(XmlDeclarations::TYPE_ATR) == XmlDeclarations::CHAIN_TYPE_VAL) {
...
bool ComponentProcessor::isATerminalSuffix(Element *suffix) {
    return suffix->getName() == XmlDeclarations::SUFFIX_EL &&
           suffix->getAttribute(XmlDeclarations::LOCANT_ATR) == nullptr &&
           (suffix->getAttributeValue(XmlDeclarations::TYPE_ATR) == XmlDeclarations::INLINE_TYPE_VAL ||
            XmlDeclarations::TERMINAL_SUBTYPE_VAL == suffix->getAttributeValue(XmlDeclarations::SUBTYPE_ATR));
}

void ComponentProcessor::processConjunctiveNomenclature(
        Element *subOrRoot) throw(ComponentGenerationException, StructureBuildingException) {
    std::vector<Element *> conjunctiveGroups = subOrRoot->getChildElements(XmlDeclarations::CONJUNCTIVESUFFIXGROUP_EL);
    ...
    if (conjunctiveGroupCount > 0) {
        ...
        for (int i = 0; i < conjunctiveGroupCount; i++) {
            ...
            if (atomToConnectToOnConjunctiveFrag == nullptr) {
                throw ComponentGenerationException(
                        L"OPSIN Bug: Unable to find non suffix carbon with sufficient valency");
            }
            if (locant != L"") {
                state->fragManager->createBond(atomToConnectToOnConjunctiveFrag,
                                               ringFrag->getAtomByLocantOrThrow(locant), 1);
            } else {
                std::vector<Atom *> possibleAtoms = FragmentTools::findSubstituableAtoms(ringFrag, 1);
                if (possibleAtoms.empty()) {
                ...
                if (AmbiguityChecker::isSubstitutionAmbiguous(possibleAtoms, 1)) {
                    state->addIsAmbiguous(
                            std::wstring(L"Connection of conjunctive group to: ") + ringGroup->getValue());
                }
...
void ComponentProcessor::processBiochemicalLinkageDescriptors(std::vector<Element *> &substituents,
...
//ORIGINAL LINE: throw new RuntimeException("OPSIN Bug: More than 1 biochemical linkage locant associated with subsituent");
            ...
            int secondLocantStartPos =
                    std::max((int) bioLinkLocantStr.rfind(L'>'), (int) bioLinkLocantStr.rfind(L'-')) + 1;
                ...
                if (previousElements.size() > 0) { //an explicit bracket is needed
                    ...
                    if (substituent->getAttribute(XmlDeclarations::LOCANT_ATR) != nullptr) {
                        throw StructureBuildingException(std::wstring(
                                L"Substituent with biochemical linkage descriptor should not also have a locant: ") +
                                                         substituent->getAttributeValue(XmlDeclarations::LOCANT_ATR));
                    }
                }
            }
            ...
            if (!bracketAdded) {
                ...
                if (elToAddAtrTo->getAttribute(XmlDeclarations::LOCANT_ATR) != nullptr) {
                    throw StructureBuildingException(std::wstring(
                            L"Substituent with biochemical linkage descriptor should not also have a locant: ") +
                                                     elToAddAtrTo->getAttributeValue(XmlDeclarations::LOCANT_ATR));
                }
...
//ORIGINAL LINE: throw new RuntimeException("OPSIN Bug: Substituent expected before biochemical linkage locant");
            ...
            if (bracket->getAttribute(XmlDeclarations::LOCANT_ATR) != nullptr) {
                throw StructureBuildingException(std::wstring(
                        L"Substituent with biochemical linkage descriptor should not also have a locant: ") +
                                                 bracket->getAttributeValue(XmlDeclarations::LOCANT_ATR));
            }
...
bool ComponentProcessor::isSubBracketOrRoot(Element *element) {
    return element != nullptr && (element->getName() == XmlDeclarations::SUBSTITUENT_EL ||
                                  element->getName() == XmlDeclarations::BRACKET_EL ||
                                  element->getName() == XmlDeclarations::ROOT_EL);
}

void ComponentProcessor::checkAndApplyFirstLocantOfBiochemicalLinkage(Element *substituent,
    ...
    if (group->getAttributeValue(XmlDeclarations::TYPE_ATR) == XmlDeclarations::CARBOHYDRATE_TYPE_VAL) {
        ...
        if (!anomericIsOutAtom) {
            throw StructureBuildingException(
                    std::wstring(L"Invalid glycoside linkage descriptor. Locant: ") + firstLocant +
                    std::wstring(L" should point to the anomeric carbon"));
        }
    } else {
        Atom *positionOfPhospho = frag->getAtomByLocantOrThrow(std::wstring(L"O") + firstLocant);
        if (positionOfPhospho->getBondCount() != 1) {
            throw StructureBuildingException(
                    firstLocant + std::wstring(L" should be the carbon to which a hydroxy group is attached!"));
        }
...
//ORIGINAL LINE: throw new RuntimeException("OPSIN Bug: Biochemical linkage only expected on groups with 1 OutAtom");
    ...
    if (OpsinTools::getNextGroup(group) == nullptr) {
        throw StructureBuildingException(
                std::wstring(L"Biochemical linkage descriptor should be followed by another biochemical: ") +
                biochemicalLinkage);
    }
}
    ...
    for (int i = brackets.size() - 1; i >= 0; i--) {
        ...
        if (childCount == 1 || hyphenPresent && childCount == 2) {
            ...
            if (substituentContent.size() >= 2) {
                ...
                if (substituentContent[0]->getName() == XmlDeclarations::LOCANT_EL) { //probably erroneous locant
                    locant = substituentContent[0];
                    possibleMultiplier = substituentContent[1];
                }
                if (possibleMultiplier->getName() ==
                    XmlDeclarations::MULTIPLIER_EL) { //erroneously placed multiplier present
                    multiplier = possibleMultiplier;
                }
                if (locant != nullptr) {
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
                    if (multiplier == nullptr || locant->getValue().split(L",")->length ==
                                                 std::stoi(multiplier->getAttributeValue(XmlDeclarations::VALUE_ATR))) {
                        locant->detach();
...
void ComponentProcessor::addImplicitBracketsWhenFirstSubstituentHasTwoMultipliers(Element *substituent,
...
void ComponentProcessor::assignLocantsToMultipliedRootIfPresent(
...
void ComponentProcessor::addImplicitBracketsWhenSubstituentHasTwoLocants(Element *substituent,
                                                                         std::vector<Element *> &brackets) {
    ...
    if (siblingSubstituent != nullptr && siblingSubstituent->getName() == XmlDeclarations::SUBSTITUENT_EL) {
        ...
        if (locants.size() == 2 && locantsAreSingular(locants) &&
            getLocantsAtStartOfSubstituent(siblingSubstituent).empty()) {
            Element *bracket = new GroupingEl(XmlDeclarations::BRACKET_EL);
            Attribute tempVar(XmlDeclarations::TYPE_ATR, XmlDeclarations::IMPLICIT_TYPE_VAL);
            bracket->addAttribute(&tempVar);
            Element *parent = substituent->getParent();
...
void ComponentProcessor::assignLocantsAndMultipliers(
        Element *subOrBracket) throw(ComponentGenerationException, StructureBuildingException) {
    ...
    if (multipliers.size() > 0) {
        if (multipliers.size() > 1) {
            throw ComponentGenerationException(
                    subOrBracket->getName() + std::wstring(L" has multiple multipliers, unable to determine meaning!"));
        }
        if (oneBelowWordLevel && OpsinTools::getNextSibling(subOrBracket) == nullptr &&
            OpsinTools::getPreviousSibling(subOrBracket) == nullptr) {
            return; //word level multiplier
        }
        multiplier = std::stoi(multipliers[0]->getAttributeValue(XmlDeclarations::VALUE_ATR));
        Attribute tempVar(XmlDeclarations::MULTIPLIER_ATR,
                          multipliers[0]->getAttributeValue(XmlDeclarations::VALUE_ATR));
        ...
        if (groupIfPresent != nullptr && XmlDeclarations::PERHALOGENO_SUBTYPE_VAL ==
                                         groupIfPresent->getAttributeValue(XmlDeclarations::SUBTYPE_ATR)) {
            throw StructureBuildingException(groupIfPresent->getValue() + std::wstring(L" cannot be multiplied"));
        }
    }
    if (locants.size() > 0) {
        if (multiplier == 1 && oneBelowWordLevel &&
            OpsinTools::getPreviousSibling(subOrBracket) == nullptr) { //locant might be word Level locant
            if (wordLevelLocantsAllowed(subOrBracket,
                                        locants.size())) { //something like S-ethyl or S-(2-ethylphenyl) or S-4-tert-butylphenyl
                Element *locant = locants.erase(locants.begin());
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
                if (locant->getValue().split(L",")->length != 1) {
                    throw ComponentGenerationException(
                            L"Multiplier and locant count failed to agree; All locants could not be assigned!");
                }
                Attribute tempVar2(XmlDeclarations::LOCANT_ATR, locant->getValue());
...
//JAVA TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'toString':
        if (parent->getName() != XmlDeclarations::WORD_EL ||
            parent->getAttributeValue(XmlDeclarations::TYPE_ATR) != WordType::full->toString() ||
            !state->currentWordRule::equals(WordRule::carbonylDerivative)) {
...
bool ComponentProcessor::wordLevelLocantsAllowed(Element *subBracketOrRoot, int numberOflocants) {
    ...
    if (StringHelper::fromString<WordType>(parentElem->getAttributeValue(XmlDeclarations::TYPE_ATR)) ==
        WordType::substituent && (OpsinTools::getNextSibling(subBracketOrRoot) == nullptr || numberOflocants >= 2)) {
        if (state->currentWordRule == WordRule::ester || state->currentWordRule == WordRule::functionalClassEster ||
            state->currentWordRule == WordRule::multiEster || state->currentWordRule == WordRule::acetal) {
            return true;
        }
    }
    if ((state->currentWordRule == WordRule::potentialAlcoholEster ||
         state->currentWordRule == WordRule::amineDiConjunctiveSuffix || (state->currentWordRule == WordRule::ester &&
                                                                          (OpsinTools::getNextSibling(
                                                                                  subBracketOrRoot) == nullptr ||
                                                                           numberOflocants >= 2))) &&
        parentElem->getName() == XmlDeclarations::WORD_EL) {
    ...
    if (state->currentWordRule == WordRule::acidReplacingFunctionalGroup &&
        parentElem->getName() == XmlDeclarations::WORD_EL &&
        (OpsinTools::getNextSibling(subBracketOrRoot) == nullptr || numberOflocants >= 2)) {
...
void ComponentProcessor::processWordLevelMultiplierIfApplicable(Element *word, std::vector<Element *> &roots,
    ...
    if (word->getChildCount() == 1) {
        ...
        if (multiplier != nullptr) {
            ...
            bool wordLevelLocants = wordLevelLocantsAllowed(firstSubBrackOrRoot,
                                                            locants.size()); //something like O,S-dimethyl phosphorothioate
            if (locants.size() > 1) {
...
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
            ...
            if (wordCount == 1) {
                if (!isMonoFollowedByElement(multiplier, multiVal)) {
                    throw StructureBuildingException(
                            L"Unexpected multiplier found at start of word. Perhaps the name is trivial e.g. triphosgene");
                }
            }
            if (multiVal == 1) { //mono
                return;
            ...
            for (auto el : elementsNotToBeMultiplied) { //re-add anything before multiplier to original word
                firstSubBrackOrRoot->insertChild(el, 0);
            }
        }
    } else if (roots.size() == 1) {
        if (OpsinTools::getDescendantElementsWithTagName(roots[0], XmlDeclarations::FRACTIONALMULTIPLIER_EL).size() >
            0) {
...
void ComponentProcessor::checkForNonConfusedWithNona(Element *multiplier) throw(StructureBuildingException) {
    if (multiplier->getValue() == L"non") {
        std::wstring subsequentUnsemanticToken = multiplier->getAttributeValue(
                XmlDeclarations::SUBSEQUENTUNSEMANTICTOKEN_ATR);
        if (subsequentUnsemanticToken != L"" && subsequentUnsemanticToken.toLowerCase(Locale::ROOT)->startsWith(L"a")) {
            return;
        }
        throw StructureBuildingException(
                L"\"non\" probably means \"not\". If a multiplier of value 9 was intended \"nona\" should be used");
    }
}
...
bool ComponentProcessor::isMonoFollowedByElement(Element *multiplier, int multiVal) {
    if (multiVal == 1) {
        ...
        if (possibleElement != nullptr && possibleElement->getName() == XmlDeclarations::GROUP_EL &&
            (XmlDeclarations::ELEMENTARYATOM_SUBTYPE_VAL ==
             possibleElement->getAttributeValue(XmlDeclarations::SUBTYPE_ATR) ||
             possibleElement->getValue() == L"hydrogen")) {
            return true;
        }
    }
    return false;
