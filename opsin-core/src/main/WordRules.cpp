...
WordRules::WordDescription::WordDescription(XMLStreamReader *reader) : type(static_cast<WordType>(atrValue)),
                                                                       endsWithGroup(
                                                                               static_cast<EndsWithGroup>(atrValue)),
                                                                       endsWithPattern(Pattern::compile(
                                                                               atrValue + std::wstring(L"$"),
                                                                               Pattern::CASE_INSENSITIVE)),
                                                                       value(atrValue), functionalGroupType(atrValue),
...
WordRules::WordRuleDescription::WordRuleDescription(XMLStreamReader *reader) throw(XMLStreamException)
        : wordDescriptions(Collections::unmodifiableList(wordDescriptions)),
          ruleName(static_cast<WordRule>(reader->getAttributeValue(nullptr, L"name"))),
          ruleType(static_cast<WordType>(reader->getAttributeValue(nullptr, L"type"))) {
...
WordRules::WordRules(ResourceGetter *resourceGetter) throw(IOException) : wordRuleList(
        Collections::unmodifiableList(wordRuleList)) {
...
void WordRules::groupWordsIntoWordRules(Element *moleculeEl, NameToStructureConfig *n2sConfig, bool allowSpaceRemoval,
                                        std::vector<Integer> &componentRatios) throw(ParsingException) {
    WordRulesInstance *instance = new WordRulesInstance(this, moleculeEl, n2sConfig, allowSpaceRemoval,
                                                        componentRatios);
    ...
    for (auto wordRuleEl : wordRuleEls) {
        if (!wordRuleEl->getName().equals(XmlDeclarations::WORDRULE_EL)) {
            throw ParsingException(std::wstring(L"Unable to assign wordRule to: ") +
                                   wordRuleEl->getAttributeValue(XmlDeclarations::VALUE_ATR));
        }
    }
}

WordRules::WordRulesInstance::WordRulesInstance(WordRules *outerInstance, Element *moleculeEl,
                                                NameToStructureConfig *n2sConfig, bool allowSpaceRemoval,
                                                std::vector<Integer> &componentRatios) : moleculeEl(moleculeEl),
                                                                                         allowRadicals(
                                                                                                 n2sConfig->isAllowRadicals()),
                                                                                         allowSpaceRemoval(
                                                                                                 allowSpaceRemoval),
                                                                                         expectedNumOfComponents(
                                                                                                 componentRatios.size() >
                                                                                                 0
                                                                                                 ? componentRatios.size()
                                                                                                 : nullptr),
...
bool WordRules::WordRulesInstance::matchWordRule(std::vector<Element *> &wordEls,
                                                 int indexOfFirstWord) throw(ParsingException) {
...
//JAVA TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'toString':
                    if (!WordType::functionalTerm->toString()->equals(
                            wordEl->getAttributeValue(XmlDeclarations::TYPE_ATR))) {
                    ...
                    while (lastEl->getName() == XmlDeclarations::CLOSEBRACKET_EL ||
                           lastEl->getName() == XmlDeclarations::STRUCTURALCLOSEBRACKET_EL) {
                        lastEl = OpsinTools::getPreviousSibling(lastEl);
                        if (lastEl == nullptr) {
                            throw ParsingException(
                                    L"OPSIN Bug: Cannot find the functional element in a functionalTerm");
                        }
                    }
                ...
                if (endsWithGroupPredicate != nullptr &&
                    !endsWithGroupPredicateSatisfied(wordEl, endsWithGroupPredicate)) {
                    goto wordRuleLoopContinue;
                }

                std::wstring valuePredicate = wd->getValue();
                if (valuePredicate != L"" &&
                    !wordEl->getAttributeValue(XmlDeclarations::VALUE_ATR).toLowerCase(Locale::ROOT).equals(
                            valuePredicate)) { //word string contents mismatch
                    goto wordRuleLoopContinue;
                }

                Pattern *endsWithPatternPredicate = wd->getEndsWithPattern();
                if (endsWithPatternPredicate != nullptr) {
                    if (!endsWithPatternPredicate->matcher(
                            wordEl->getAttributeValue(XmlDeclarations::VALUE_ATR)).find()) {
                        goto wordRuleLoopContinue;
                    }
                }
                ...
                if (endsWithGroupTypePredicate != L"") {
                    ...
                    if (lastGroupInWordRule == nullptr || endsWithGroupTypePredicate !=
                                                          lastGroupInWordRule->getAttributeValue(
                ...
                if (endsWithSubGroupTypePredicate != L"") {
                    Element *lastGroupInWordRule = getLastGroupInWordRule(wordEl);
                    if (lastGroupInWordRule == nullptr || endsWithSubGroupTypePredicate !=
                                                          lastGroupInWordRule->getAttributeValue(
...
//JAVA TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'toString':
            Attribute tempVar(XmlDeclarations::TYPE_ATR, wordRuleDesc->getRuleType()
            ::toString());
...
//JAVA TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'toString':
            ...
            switch (wordRule) {
                case functionGroupAsGroup: {
                    ...
                    if (functionalWord->getAttributeValue(XmlDeclarations::TYPE_ATR) !=
                        XmlDeclarations::FUNCTIONALTERM_EL || wordsInWordRule > 2) {
...
//JAVA TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'toString':
                ...
                case acidReplacingFunctionalGroup:
                    ...
                    for (int j = 1; j < (wordsInWordRule - 1); j++) {
                        Element *wordEl = wordEls[i + j];
//JAVA TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'toString':
                        if (WordType::substituent->toString()->equals(
                                wordEl->getAttributeValue(XmlDeclarations::TYPE_ATR))) {
                            joinWords(wordEls, wordEls[i + j], wordEls[i + j + 1]);
                            wordsInWordRule--;
                            std::vector<Element *> functionalTerm = OpsinTools::getDescendantElementsWithTagName(
                                    wordEls[i + j],
                                    XmlDeclarations::FUNCTIONALTERM_EL); //rename functionalTerm element to root
                            if (functionalTerm.size() != 1) {
                                throw ParsingException(std::wstring(L"OPSIN bug: Problem with ") + wordRule +
                                                       std::wstring(L" wordRule"));
                            }
                            functionalTerm[0]->setName(XmlDeclarations::ROOT_EL);
                            std::vector<Element *> functionalGroups = OpsinTools::getDescendantElementsWithTagName(
                                    functionalTerm[0],
                                    XmlDeclarations::FUNCTIONALGROUP_EL); //rename functionalGroup element to group
...
//JAVA TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'toString':
                            wordEls[i + j]->getAttribute(XmlDeclarations::TYPE_ATR).setValue(
                ...
                case oxide: {
                    ...
                    std::vector<Element *> elementaryAtoms = OpsinTools::getDescendantElementsWithTagNameAndAttribute(
                            possibleElementaryAtomContainingWord, XmlDeclarations::GROUP_EL,
                            XmlDeclarations::SUBTYPE_ATR, XmlDeclarations::ELEMENTARYATOM_SUBTYPE_VAL);
                    if (elementaryAtoms.size() == 1) {
                        ...
                        if (wordRule == WordRule::oxide) {
                            ...
                            for (int j = 1; j < wordsInWordRule; j++) {
                                ...
                                if (!FragmentTools::isCovalent(chemEl1,
                                                               chemEl2)) { //use separate word rules for ionic components
                                    ...
                                    if (chemEl2::isHalogen() && wordsInWordRule == 2) {
                                        switch (chemEl1) {
                                            case Mg:
                                                ...
                                                break;
                                            case Al:
                                                if (chemEl2 == ChemEl::Cl || chemEl2 == ChemEl::Br ||
                                                    chemEl2 == ChemEl::I) {
                                                    specialCaseCovalency = true;
                                                }
                                                break;
                                            case Ti:
                                                if (oxidationNumberOrMultiplierIs(elementaryAtom, functionalGroup, 4) &&
                                                    (chemEl2 == ChemEl::Cl || chemEl2 == ChemEl::Br ||
                                                     chemEl2 == ChemEl::I)) {
                                                    specialCaseCovalency = true;
                                                }
                                                break;
...
//JAVA TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'toString':
    if (firstWord->getName() == XmlDeclarations::WORD_EL && WordType::full->toString()->equals(
            firstWord->getAttributeValue(XmlDeclarations::TYPE_ATR))) { //No wordRule -->wordRule="simple"
        applySimpleWordRule(wordEls, indexOfFirstWord, firstWord);
        return false;
    }
//JAVA TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'toString':
    else if (allowSpaceRemoval &&
             WordType::substituent->toString()->equals(firstWord->getAttributeValue(XmlDeclarations::TYPE_ATR))) {
        /*
...
//JAVA TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'toString':
            if (WordType::full->toString()->equals(
                    wordToPotentiallyCombineWith->getAttributeValue(XmlDeclarations::TYPE_ATR)) ||
                WordType::substituent->toString()->equals(
                        wordToPotentiallyCombineWith->getAttributeValue(XmlDeclarations::TYPE_ATR))) {
...
//JAVA TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'toString':
    else if (WordType::functionalTerm->toString()->equals(firstWord->getAttributeValue(XmlDeclarations::TYPE_ATR)) &&
             firstWord->getAttributeValue(XmlDeclarations::VALUE_ATR).toLowerCase(Locale::ROOT).equals(L"salt")) {
...
//JAVA TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'toString':
    if (allowRadicals && wordEls.size() == 1 && indexOfFirstWord == 0 &&
        firstWord->getName() == XmlDeclarations::WORD_EL &&
        WordType::substituent->toString()->equals(firstWord->getAttributeValue(XmlDeclarations::TYPE_ATR))) {
...
bool WordRules::WordRulesInstance::endsWithGroupPredicateSatisfied(Element *wordEl,
                                                                   EndsWithGroup endsWithGroupPredicate) throw(ParsingException) {
    Element *lastEl = getLastElementInWord(wordEl);
    if (lastEl == nullptr) {
        return false;
    }
    std::wstring elName = lastEl->getName();
    while (elName == XmlDeclarations::CLOSEBRACKET_EL || elName == XmlDeclarations::STRUCTURALCLOSEBRACKET_EL ||
           elName == XmlDeclarations::ISOTOPESPECIFICATION_EL) {
        lastEl = OpsinTools::getPreviousSibling(lastEl);
        if (lastEl == nullptr) {
            return false;
        }
    ...
    if (endsWithGroupPredicate == EndsWithGroup::acid) {
        if (elName == XmlDeclarations::SUFFIX_EL) {
            ...
            if (lastEl->getAttribute(XmlDeclarations::FUNCTIONALIDS_ATR) != nullptr &&
                icOrOusAcid->matcher(lastEl->getValue()).find()) {
                return true;
            }
        }
        ...
        if (elName == XmlDeclarations::GROUP_EL) {
            if (lastEl->getAttribute(XmlDeclarations::FUNCTIONALIDS_ATR) != nullptr &&
                ateOrIteOrAmide->matcher(lastEl->getValue()).find()) {
                return true;
            }
        } else {
...
bool
WordRules::WordRulesInstance::oxidationNumberOrMultiplierIs(Element *elementaryAtomEl, Element *functionalGroupWord,
                                                            int expectedVal) throw(ParsingException) {
    ...
    if (possibleMultiplier != nullptr && possibleMultiplier->getName() == XmlDeclarations::MULTIPLIER_EL) {
        ...
        if (possibleOxidationNumber != nullptr &&
            possibleOxidationNumber->getName() == XmlDeclarations::OXIDATIONNUMBERSPECIFIER_EL) {
...
void WordRules::WordRulesInstance::applySimpleWordRule(std::vector<Element *> &wordEls, int indexOfFirstWord,
                                                       Element *firstWord) {
...
void WordRules::WordRulesInstance::applySubstituentWordRule(std::vector<Element *> &wordEls, int indexOfFirstWord,
                                                            Element *firstWord) {
    Element *parentEl = firstWord->getParent();
    int indexToInsertAt = parentEl->find(firstWord);
    Element *wordRuleEl = new GroupingEl(XmlDeclarations::WORDRULE_EL);
...
void WordRules::WordRulesInstance::joinWords(std::vector<Element *> &wordEls, Element *firstWord,
                                             Element *wordToPotentiallyCombineWith) throw(ParsingException) {
//JAVA TO C++ CONVERTER TODO TASK: The Java Collection 'remove(Object)' method is not converted:
    ...
    if (finalSubstituentChildren[finalSubstituentChildren.size() - 1]->getName() !=
        XmlDeclarations::HYPHEN_EL) { //add an implicit hyphen if one is not already present
...
//JAVA TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'toString':
    ...
    firstWord->getAttribute(XmlDeclarations::VALUE_ATR)->setValue(
            firstWord->getAttributeValue(XmlDeclarations::VALUE_ATR) +
            wordToPotentiallyCombineWith->getAttributeValue(XmlDeclarations::VALUE_ATR));
}
...
//JAVA TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'toString':
    ...
    std::vector<Element *> functionalTerms = OpsinTools::getDescendantElementsWithTagName(word,
                                                                                          XmlDeclarations::FUNCTIONALTERM_EL);
...
void WordRules::WordRulesInstance::setOxideStructureAppropriately(Element *oxideGroup, Element *elementaryAtom) {
    ...
    if (possibleMultiplier != nullptr && possibleMultiplier->getName() == XmlDeclarations::MULTIPLIER_EL) {
        ...
        if (multiplierVal > 1) {
            std::wstring commonOxidationStatesAndMax = elementaryAtom->getAttributeValue(
                    XmlDeclarations::COMMONOXIDATIONSTATESANDMAX_ATR);
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
            if (commonOxidationStatesAndMax == L"" ||
                static_cast<Integer>(commonOxidationStatesAndMax.split(L":")[1]) <= 2) {
...
ChemEl
WordRules::WordRulesInstance::getChemElFromWordWithFunctionalGroup(Element *functionalWord) throw(ParsingException) {
    std::vector<Element *> functionalGroups = OpsinTools::getDescendantElementsWithTagName(functionalWord,
                                                                                           XmlDeclarations::FUNCTIONALGROUP_EL);
    if (functionalGroups.size() != 1) {
        throw ParsingException(L"OPSIN bug: Unable to find functional group in oxide or addition compound rule");
    }
