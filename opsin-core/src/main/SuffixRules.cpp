...
SuffixRules::ApplicableSuffix::ApplicableSuffix(const std::wstring &requiredSubType,
                                                std::vector<SuffixRule *> &suffixRules) : requiredSubType(
        requiredSubType), suffixRules(suffixRules) {
}

SuffixRules::SuffixRules(ResourceGetter *resourceGetter) throw(IOException) : suffixApplicability(
        generateSuffixApplicabilityMap(resourceGetter, suffixRulesMap)) {
...
SuffixRules::generateSuffixRulesMap(ResourceGetter *resourceGetter) throw(IOException) {
    ...
    try {
        while (reader->hasNext()) {
            if (reader->next() == XMLStreamConstants::START_ELEMENT &&
                reader->getLocalName().equals(XmlDeclarations::SUFFIXRULES_RULE_EL)) {
...
SuffixRules::generateSuffixApplicabilityMap(ResourceGetter *resourceGetter,
    ...
    try {
        while (reader->hasNext()) {
            if (reader->next() == XMLStreamConstants::START_ELEMENT &&
                reader->getLocalName().equals(XmlDeclarations::SUFFIXAPPLICABILITY_GROUPTYPE_EL)) {
                std::unordered_map<std::wstring, std::vector<ApplicableSuffix *>> suffixToRuleMap;
                suffixApplicability[reader->getAttributeValue(nullptr,
                                                              XmlDeclarations::SUFFIXAPPLICABILITY_TYPE_ATR)] = suffixToRuleMap;
                while (reader->hasNext()) {
                    int event_Renamed = reader->next();
                    if (event_Renamed == XMLStreamConstants::START_ELEMENT &&
                        reader->getLocalName().equals(XmlDeclarations::SUFFIXAPPLICABILITY_SUFFIX_EL)) {
                        std::wstring suffixValue = reader->getAttributeValue(nullptr,
                                                                             XmlDeclarations::SUFFIXAPPLICABILITY_VALUE_ATR);
                        ...
                        std::wstring requiredSubType = reader->getAttributeValue(nullptr,
                                                                                 XmlDeclarations::SUFFIXAPPLICABILITY_SUBTYPE_ATR);
...
SuffixRules::getSuffixRuleTags(const std::wstring &suffixTypeToUse, const std::wstring &suffixValue,
    ...
    if (groupToSuffixMap.empty()) {
        throw ComponentGenerationException(std::wstring(L"Suffix Type: ") + suffixTypeToUse + std::wstring(
                L" does not have a corresponding groupType entry in suffixApplicability.xml"));
    }
    ...
    if (potentiallyApplicableSuffixes.empty() || potentiallyApplicableSuffixes.empty()) {
        throw ComponentGenerationException(std::wstring(L"Suffix: ") + suffixValue + std::wstring(
                L" does not apply to the group it was associated with (type: ") + suffixTypeToUse +
                                           std::wstring(L") according to suffixApplicability.xml"));
    }
    ...
    for (auto suffix : potentiallyApplicableSuffixes) {
        ...
        if (suffixRules.size() > 0) {
            throw ComponentGenerationException(std::wstring(L"Suffix: ") + suffixValue +
                                               std::wstring(L" appears multiple times in suffixApplicability.xml"));
        }
    ...
    if (suffixRules.empty()) {
        throw ComponentGenerationException(std::wstring(L"Suffix: ") + suffixValue + std::wstring(
                L" does not apply to the group it was associated with (type: ") + suffixTypeToUse +
                                           std::wstring(L") due to the group's subType: ") + subgroupType +
                                           std::wstring(L" according to suffixApplicability.xml"));
    }
    return suffixRules;
}
