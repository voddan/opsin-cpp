...
ReverseParseRules::ReverseParseRules(ResourceManager *resourceManager) throw(IOException) : chemAutomaton(
        resourceManager->getReverseChemicalAutomaton()), stateSymbols(chemAutomaton->getCharIntervals()),
                                                                                            symbolTokenNamesDictReversed(
                                                                                                    resourceManager->getSymbolTokenNamesDictReversed()),
                                                                                            symbolRegexAutomataDictReversed(
                                                                                                    resourceManager->getSymbolRegexAutomataDictReversed()),
                                                                                            symbolRegexesDictReversed(
                                                                                                    resourceManager->getSymbolRegexesDictReversed()) {
...
ParseRulesResults *ReverseParseRules::getParses(const std::wstring &chemicalWord) throw(ParsingException) {
    AnnotatorState *initialState = new AnnotatorState(chemAutomaton->getInitialState(), L'\0', chemicalWord.length(),
                                                      true, nullptr);
    ...
    while (!asStack->isEmpty()) {
        ...
        if (chemAutomaton->isAccept(as->getState())) {
            if (posInName <= posInNameOfLastSuccessfulAnnotations) { //this annotation is worthy of consideration
                if (posInName <
                    posInNameOfLastSuccessfulAnnotations) { //this annotation is longer than any previously found annotation
                    successfulAnnotations.clear();
                    posInNameOfLastSuccessfulAnnotations = posInName;
                } else if (successfulAnnotations.size() > 128) {
                    throw ParsingException(
                            L"Ambiguity in OPSIN's chemical grammar has produced more than 128 annotations. Parsing has been aborted. Please report this as a bug");
                }
        ...
        for (int i = 0; i < stateSymbolsSize; i++) {
            ...
            if (potentialNextState != -1) { //-1 means this state is not accessible from the previous state
                ...
                if (possibleTokenisationsTrie != nullptr) {
                    std::vector<int> possibleTokenisations = possibleTokenisationsTrie->findMatchesReadingStringRightToLeft(
                            chemicalWordLowerCase, posInName);
                    if (possibleTokenisations.size() > 0) { //next could be a token
                        for (int j = 0, l = possibleTokenisations.size();
                             j < l; j++) { //typically list size will be 1 so this is faster than an iterator
                            int tokenizationIndex = possibleTokenisations[j];
                            AnnotatorState *newAs = new AnnotatorState(potentialNextState, annotationCharacter,
                                                                       tokenizationIndex, false, as);
                ...
                if (possibleAutomata != nullptr) { //next could be an automaton
                ...
                if (possibleRegex != nullptr) { //next could be a regex
    ...
    if (successfulAnnotations.size() >
        0) { //at least some of the name could be interpreted into a substituent/full/functionalTerm
...
ParseTokens *
ReverseParseRules::convertAnnotationStateToParseTokens(AnnotatorState *as, const std::wstring &chemicalWord,
    ...
    while ((previousAs = as->getPreviousAs()) != nullptr) {
        if (as->isCaseSensitive()) {
            ...
            tokens.push_back(
                    chemicalWordLowerCase.substr(as->getPosInName(), previousAs->getPosInName() - as->getPosInName()));
        }
