...
std::wstring
StringTools::convertNonAsciiAndNormaliseRepresentation(const std::wstring &s) throw(PreProcessingException) {
    ...
    for (int i = 0, l = s.length(); i < l; i++) {
        ...
        switch (c) {
            ...
            default:
                if (c >= 128) {
                    sb->append(getReplacementForNonASCIIChar(
                            c)); //replace non ascii characters with hard coded ascii strings
