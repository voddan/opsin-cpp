...
FusedRingBuilder::FusedRingBuilder(BuildState *state, std::vector<Element *> &groupsInFusedRing) : state(state),
                                                                                                   groupsInFusedRing(
                                                                                                           groupsInFusedRing),
                                                                                                   lastGroup(
                                                                                                           groupsInFusedRing[
                                                                                                                   groupsInFusedRing.size() -
                                                                                                                   1]),
                                                                                                   parentRing(
                                                                                                           lastGroup->getFrag()) {
    ...
    for (int i = groups.size() - 1; i >= 0; i--) { //group groups into fused rings
        ...
        if (i != 0) {
            ...
            if ((group->getValue() == L"benz" || group->getValue() == L"benzo") &&
                XmlDeclarations::FUSIONRING_SUBTYPE_VAL == group->getAttributeValue(XmlDeclarations::SUBTYPE_ATR)) {
            ...
            Element *possibleGroup = OpsinTools::getPreviousSiblingIgnoringCertainElements(startingEl,
                                                                                           std::vector<std::wstring>{
                                                                                                   XmlDeclarations::MULTIPLIER_EL,
                                                                                                   XmlDeclarations::FUSION_EL});
            if (!groups[i - 1]->equals(possibleGroup)) { //end of fused ring system
...
//JAVA TO C++ CONVERTER TODO TASK: The Java Collection 'remove(Object)' method is not converted:
    ...
    if (nameComponents.size() > 0 && possibleMultiplier != nullptr &&
        possibleMultiplier->getName() == XmlDeclarations::MULTIPLIER_EL) {
    ...
    int ncIndice = processMultiParentSystem(parentFragments, nameComponents,
                                            componentFragments); //handle multiparent systems
    /*
    ...
    for (; ncIndice >= 0; ncIndice--) {
        ...
        int multiplier = 1;
        Element *possibleMultiplierEl = OpsinTools::getPreviousSibling(
                nameComponents[ncIndice]); //e.g. the di of difuro
        ...
        if (fusion != nullptr) {
            std::wstring fusionDescriptorString = fusion->getValue().toLowerCase(Locale::ROOT)->substr(1,
                                                                                                       (fusion->getValue().length() -
                                                                                                        1) - 1);
            if (multiplier == 1) {
...
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
                    ...
                    if (ncIndice != 0) {
                        throw StructureBuildingException(
                                std::wstring(L"Unexpected multiplier: ") + possibleMultiplierEl->getValue() +
                                std::wstring(L" or incorrect fusion descriptor: ") + fusionDescriptorString);
                    }
                    multiplier = 1;
                    fusionDescriptors = std::vector<std::wstring>{fusionDescriptorString};
                }
        ...
        for (int j = 0; j < multiplier; j++) {
            ...
            if (fusion != nullptr) {
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
                if (fusionDescriptors[j].split(L":")->length ==
                    1) { //A fusion bracket without a colon is used when applying to the parent component (except in a special case where locants are ommitted)
                    //check for case of omitted locant from a higher order fusion bracket e.g. cyclopenta[4,5]pyrrolo[2,3-c]pyridine
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
                    if (fusionDescriptors[j].split(L"-")->length == 1 && fusionDescriptors[j].split(L",")->length > 1 &&
                        FragmentTools::allAtomsInRingAreIdentical(component) &&
                        ((StringTools::countTerminalPrimes(fusionDescriptors[j].split(L",")[0])) !=
                         fusionLevel)) { //Could be like cyclopenta[3,4]cyclobuta[1,2]benzene where the first fusion to occur has parent locants omitted not child locants
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
                        ...
                        if (numberOfPrimes + 1 != fusionLevel) {
                            if (numberOfPrimes + 2 ==
                                fusionLevel) { //ring could be in previous fusion level e.g. the benzo in benzo[10,11]phenanthro[2',3',4',5',6':4,5,6,7]chryseno[1,2,3-bc]coronene
                                fusionLevel--;
                            } else {
                                throw StructureBuildingException(
                                        std::wstring(L"Incorrect number of primes in fusion bracket: ") +
                                        fusionDescriptors[j]);
                            }
                        }
                        relabelAccordingToFusionLevel(component, fusionLevel);
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
                        std::vector<std::wstring> numericalLocantsOfParent = Arrays::asList(
                                fusionDescriptors[j].split(L","));
                        std::vector<std::wstring> numericalLocantsOfChild = findPossibleNumericalLocants(component,
                                                                                                         determineAtomsToFuse(
                                                                                                                 fragmentInScopeForEachFusionLevel[fusionLevel],
                                                                                                                 numericalLocantsOfParent,
                                                                                                                 boost::none).size() -
                                                                                                         1);
                        processHigherOrderFusionDescriptors(component, fragmentInScopeForEachFusionLevel[fusionLevel],
                        ...
                        if (fusionArray[1] != L"") {
                            ...
                            if (fusionArray[0] == L"") {
                                ...
                                fusionDescriptor =
                                        fusionArray[0] + std::wstring(L"-") + fusionArray[1].replaceAll(L"'", L"");
                            }
                            if (numberOfPrimes >= parentFragments.size()) {
                                throw StructureBuildingException(L"Unexpected prime in fusion descriptor");
                            }
                        }
                        performSimpleFusion(fusionDescriptor, component,
                                            parentFragments[numberOfPrimes]); //e.g. pyrano[3,2-b]imidazo[4,5-e]pyridine where both are level 0 fusions
                    }
                } else {
...
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
                    ...
                    relabelAccordingToFusionLevel(component, fusionLevel);
                    performHigherOrderFusion(fusionDescriptors[j], component,
                                             fragmentInScopeForEachFusionLevel[fusionLevel]);
                }
            } else {
                relabelAccordingToFusionLevel(component, fusionLevel);
            ...
            for (auto atom : atomList) {
                if (!atom->getAtomIsInACycle()) {
                    throw StructureBuildingException(std::wstring(
                            L"Inappropriate group used in fusion nomenclature. Only groups composed entirely of atoms in cycles may be used. i.e. not: ") +
                                                     group->getValue());
                }
            }
            if (group->getAttribute(XmlDeclarations::FUSEDRINGNUMBERING_ATR) != nullptr) {
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
                std::vector<std::wstring> standardNumbering = group->getAttributeValue(
                        XmlDeclarations::FUSEDRINGNUMBERING_ATR).split(L"/", -1);
...
void FusedRingBuilder::processPartiallyUnsaturatedHWSystems(Element *group, Fragment *ring) {
    if (XmlDeclarations::HANTZSCHWIDMAN_SUBTYPE_VAL == group->getAttributeValue(XmlDeclarations::SUBTYPE_ATR) &&
        group->getAttribute(XmlDeclarations::ADDBOND_ATR) != nullptr) {
        std::vector<Element *> unsaturators = OpsinTools::getNextAdjacentSiblingsOfType(group,
                                                                                        XmlDeclarations::UNSATURATOR_EL);
        if (unsaturators.size() > 0) {
            Element *unsaturator = unsaturators[0];
            if (unsaturator->getAttribute(XmlDeclarations::LOCANT_ATR) == nullptr &&
                unsaturator->getAttributeValue(XmlDeclarations::VALUE_ATR) == L"2") {
                unsaturator->detach();
...
int FusedRingBuilder::processMultiParentSystem(std::vector<Fragment *> &parentFragments,
                                               std::vector<Element *> &nameComponents,
    ...
    if (i >= 0 && parentFragments.size() > 1) {
        ...
        for (; i >= 0; i--) {
            ...
            if (multiplier > 1 && multiplier != previousFusionLevelFragments.size()) {
                throw StructureBuildingException(
                        L"Mismatch between number of components and number of parents in fused ring system");
            }
            std::wstring fusionDescriptorString = fusion->getValue().toLowerCase(Locale::ROOT)->substr(1,
                                                                                                       (fusion->getValue().length() -
                                                                                                        1) - 1);
...
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
            ...
            if (fusionDescriptors.size() != previousFusionLevelFragments.size()) {
                throw StructureBuildingException(std::wstring(L"Invalid fusion descriptor: ") + fusionDescriptorString +
                                                 std::wstring(L"(Number of locants disagrees with number of parents)"));
            }
...
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
                ...
                if (simpleFusion) {
                    ...
                    if (fusionArray[1].length() != 0) {
                        ...
                        if (numberOfPrimes !=
                            j) { //check the number of primes on the letter part agree with the parent to use e.g.[4,5-bcd:1,2-c']difuran
                            throw StructureBuildingException(
                                    std::wstring(L"Incorrect number of primes in fusion descriptor: ") +
                                    fusionDescriptor);
                        }
                    }
...
std::vector<std::wstring>
FusedRingBuilder::determineNumericalAndLetterComponents(const std::wstring &fusionDescriptor) {
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
    ...
    if (fusionArray.size() == 2) {
        ...
        if (fusionArray[0].find(L",") != std::wstring::npos) { //the digit section
        ...
        if (group->getValue() == L"benz" || group->getValue() == L"benzo") {
            ...
            if (possibleFusionbracket->getName() != XmlDeclarations::FUSION_EL) {
                ...
                if (possibleMultiplier == nullptr || possibleMultiplier->getName() != XmlDeclarations::MULTIPLIER_EL ||
                    possibleMultiplier->getAttributeValue(XmlDeclarations::TYPE_ATR) ==
...
void FusedRingBuilder::performSimpleFusion(const std::wstring &fusionDescriptor, Fragment *childRing,
...
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
            ...
            if (fusionArray[0].find(L",") != std::wstring::npos) { //only has digits
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
    ...
    if (numericalLocantsOfChild.empty() || letterLocantsOfParent.empty()) {
        throw StructureBuildingException(
                L"Unable to find bond to form fused ring system. Some information for forming fused ring system was only supplyed implicitly");
    }

    processFirstOrderFusionDescriptors(childRing, parentRing, numericalLocantsOfChild,
                                       letterLocantsOfParent); //fuse the rings
}
...
std::vector<std::wstring> FusedRingBuilder::findPossibleLetterLocants(Fragment *ring, int edgeLength) {
    ...
    for (int i = 0; i <= numberOfAtoms; i++) {
        ...
        if (atom->getElement() == ChemEl::C && atom->getBondCount() == 2 && (carbonAtomIndexes.empty() || std::find(
                atom->getAtomNeighbours().begin(), atom->getAtomNeighbours().end(), cyclicAtomList->peekNext()) !=
                                                                                                          atom->getAtomNeighbours().end())))
        {
            carbonAtomIndexes.push_back(cyclicAtomList->getIndex());
            if (carbonAtomIndexes.size() ==
                edgeLength + 1) { //as many carbons in a row as to give that edgelength ->use these side/s
                std::reverse(carbonAtomIndexes.begin(), carbonAtomIndexes.end());
                std::vector<std::wstring> letterLocantsOfParent;
                for (int j = 0; j < edgeLength; j++) {
                    letterLocantsOfParent.push_back(StringHelper::toString(
                            static_cast<wchar_t>(97 + carbonAtomIndexes[j]))); //97 is ascii for a
                }
...
std::vector<std::wstring> FusedRingBuilder::findPossibleNumericalLocants(Fragment *ring, int edgeLength) {
    ...
    for (int i = 0; i <= numberOfAtoms; i++) {
        ...
        //want non-bridgehead carbon atoms. Double-check that these carbon atoms are actually bonded (e.g. von baeyer systems have non-consecutive atom numbering!)
        if (atom->getElement() == ChemEl::C && atom->getBondCount() == 2 && (carbonLocants.empty() || std::find(
                atom->getAtomNeighbours().begin(), atom->getAtomNeighbours().end(), cyclicAtomList->peekPrevious()) !=
                                                                                                      atom->getAtomNeighbours().end())))
        {
            carbonLocants.push_back(atom->getFirstLocant());
            if (carbonLocants.size() ==
                edgeLength + 1) { //as many carbons in a row as to give that edgelength ->use these side/s
                std::vector<std::wstring> numericalLocantsOfChild;
...
void FusedRingBuilder::processFirstOrderFusionDescriptors(Fragment *childRing, Fragment *parentRing,
                                                          std::vector<std::wstring> &numericalLocantsOfChild,
                                                          std::vector<std::wstring> &letterLocantsOfParent) throw(StructureBuildingException) {
    std::vector<Atom *> childAtoms = determineAtomsToFuse(childRing, numericalLocantsOfChild,
                                                          letterLocantsOfParent.size() + 1);
    if (childAtoms.empty()) {
    ...
    CyclicAtomList *cyclicListAtomsOnSurfaceOfParent = new CyclicAtomList(parentPeripheralAtomList,
                                                                          static_cast<int>(letterLocantsOfParent[0][0]) -
                                                                          97); //convert from lower case character through ascii to 0-23
...
void FusedRingBuilder::performHigherOrderFusion(const std::wstring &fusionDescriptor, Fragment *nextComponent,
...
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
    ...
    processHigherOrderFusionDescriptors(nextComponent, fusedRing, numericalLocantsOfChild,
                                        numericalLocantsOfParent); //fuse the rings
}

void FusedRingBuilder::processHigherOrderFusionDescriptors(Fragment *childRing, Fragment *parentRing,
                                                           std::vector<std::wstring> &numericalLocantsOfChild,
...
std::vector<Atom *>
FusedRingBuilder::determineAtomsToFuse(Fragment *ring, std::vector<std::wstring> &numericalLocantsOnRing,
    ...
    int indexfinal = parentPeripheralAtomList.find(
            ring->getAtomByLocantOrThrow(numericalLocantsOnRing[numericalLocantsOnRing.size() - 1]));
    ...
    if (!expectedNumberOfAtomsToBeUsedForFusion ||
        expectedNumberOfAtomsToBeUsedForFusion == potentialFusionAtomsAscending.size()) {
        bool notInPotentialParentAtoms = false;
        for (int i = 1; i < numericalLocantsOnRing.size() - 1; i++) {
            if (!std::find(potentialFusionAtomsAscending.begin(), potentialFusionAtomsAscending.end(),
                           ring->getAtomByLocantOrThrow(numericalLocantsOnRing[i])) !=
                potentialFusionAtomsAscending.end()))
            {
    ...
    if (fusionAtoms.empty() ||
        !expectedNumberOfAtomsToBeUsedForFusion) { //that didn't work, so try assuming the numbers are descending
        ...
        if (!expectedNumberOfAtomsToBeUsedForFusion ||
            expectedNumberOfAtomsToBeUsedForFusion == potentialFusionAtomsDescending.size()) {
            bool notInPotentialParentAtoms = false;
            for (int i = 1; i < numericalLocantsOnRing.size() - 1; i++) {
                if (!std::find(potentialFusionAtomsDescending.begin(), potentialFusionAtomsDescending.end(),
                               ring->getAtomByLocantOrThrow(numericalLocantsOnRing[i])) !=
                    potentialFusionAtomsDescending.end()))
                {
                    notInPotentialParentAtoms = true;
                }
            }
...
void FusedRingBuilder::fuseRings(std::vector<Atom *> &childAtoms,
                                 std::vector<Atom *> &parentAtoms) throw(StructureBuildingException) {
    if (parentAtoms.size() != childAtoms.size()) {
        throw StructureBuildingException(
                std::wstring(L"Problem with fusion descriptors: Parent atoms specified: ") + parentAtoms.size() +
                std::wstring(L" Child atoms specified: ") + childAtoms.size() +
                std::wstring(L" These should have been identical!"));
    }
    ...
    for (int i = 0; i < childAtoms.size(); i++) {
        ...
        if (parentAtom->getElement() != childAtom->getElement()) {
            throw StructureBuildingException(
                    L"Invalid fusion descriptor: Heteroatom placement is ambiguous as it is not present in both components of the fusion");
        }
        atomsToRemoveToReplacementAtom[childAtom] = parentAtom;
    }

    Set < Bond * >
    *fusionEdgeBonds = std::unordered_set<Bond *>(); //these bonds already exist in both the child and parent atoms
...
void FusedRingBuilder::setBenzoHeteroatomPositioning(Element *benzoEl,
                                                     Fragment *fusedRing) throw(StructureBuildingException) {
...
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
        ...
        if (locantsCouldApplyToHeteroatomPositions(locants, benzoEl)) {
            ...
            for (auto atom : atomList) { //this iterates in the same order as the numbering system
                if (atom->getElement() != ChemEl::C) {
                    heteroatoms.push_back(atom);
                    elementOfHeteroAtom.push_back(atom->getElement());
                }
            }
            if (locants.size() == heteroatoms.size()) { //as many locants as there are heteroatoms to assign
                //check for special case of a single locant indicating where the group substitutes e.g. 4-benzofuran-2-yl
                if (!(locants.size() == 1 && OpsinTools::getPreviousSibling(locantEl) == nullptr &&
                ...
                throw StructureBuildingException(
                        L"Unable to assign all locants to benzo-fused ring or multiplier was mising");
            }
        }
    }
}
...
bool FusedRingBuilder::locantsCouldApplyToHeteroatomPositions(std::vector<std::wstring> &locants, Element *benzoEl) {
    ...
    if (locants.size() ==
        suffixesWithoutLocants) { //In preference locants will be assigned to suffixes rather than to this nomenclature
        return false;
    }
    return true;
}
