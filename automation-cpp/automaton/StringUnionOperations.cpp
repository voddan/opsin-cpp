        ...
        namespace automaton {
            ...
            State * StringUnionOperations::State::newState(wchar_t label) {
                assert(Arrays::binarySearch(labels, label) < 0,
                       std::wstring(L"State already has transition labeled: ") + StringHelper::toString(label));
            ...
            void StringUnionOperations::add(std::string * current) {
                ...
                assert(previous == nullptr || LEXICOGRAPHIC_ORDER::compare(previous, current) <= 0,
                       std::wstring(L"Input must be sorted: ") + previous + std::wstring(L" >= ") + current);
            ...
            dk::brics::automaton::State * StringUnionOperations::convert(State * s,
