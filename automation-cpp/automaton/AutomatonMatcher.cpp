...
namespace dk {
    namespace brics {
        namespace automaton {
            ...
            AutomatonMatcher::AutomatonMatcher(std::string * const chars, RunAutomaton * const automaton) : automaton(
                    automaton), chars(chars) {
            }
            ...
            void AutomatonMatcher::setMatch(int const matchStart, int const matchEnd) throw(std::invalid_argument) {
                if (matchStart > matchEnd) {
                    throw std::invalid_argument(
                            std::wstring(L"Start must be less than or equal to end: ") + matchStart +
                            std::wstring(L", ") + std::to_wstring(matchEnd));
                }
