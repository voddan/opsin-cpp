                ...
                if (suspend_shuffle && min <= suspend_shuffle && suspend_shuffle <= max && min != max) {
                    if (min < suspend_shuffle) {
                        add(suspend_shuffle, resume_shuffle, pending, visited, c, i1, t1, t2, min,
                            static_cast<wchar_t>(suspend_shuffle - 1));
                    }
                    ...
                    add(suspend_shuffle, resume_shuffle, pending, visited, c, i1, t1, t2,
                        static_cast<wchar_t>(HIGH_SURROGATE_END + 1), max);
                } else {
...
//ORIGINAL LINE: @SuppressWarnings("unused") private ShuffleConfiguration()
            ...
            ShuffleOperations::ShuffleConfiguration::ShuffleConfiguration(std::vector<Automaton *> * ca,
                                                                          Automaton * a) {
            ...
            ShuffleOperations::ShuffleConfiguration::ShuffleConfiguration(ShuffleConfiguration * c, int i1, State * s1,
                                                                          wchar_t min) {
                prev = c;
            ...
            bool ShuffleOperations::ShuffleConfiguration::equals(void * obj) {
                if (dynamic_cast<ShuffleConfiguration *>(obj) != nullptr) {
                    ...
                    return shuffle_suspended == c->shuffle_suspended && surrogate == c->surrogate &&
                           suspended1 == c->suspended1 && Arrays::equals(ca_states, c->ca_states) &&
                           a_state == c->a_state;
                }
                return false;
            }
