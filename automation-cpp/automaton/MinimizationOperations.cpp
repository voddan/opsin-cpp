...
namespace dk
{
	namespace brics
	{
		namespace automaton
		{
			...
			bool MinimizationOperations::statesAgree(std::vector<std::vector<Transition*>> &transitions, std::vector<std::vector<bool>> &mark, int n1, int n2)
			{
			...
			void MinimizationOperations::addTriggers(std::vector<std::vector<Transition*>> &transitions, std::vector<std::vector<std::unordered_set<IntPair*>>> &triggers, int n1, int n2)
			{
				std::vector<Transition*> t1 = transitions[n1];
				std::vector<Transition*> t2 = transitions[n2];
				for (int k1 = 0, k2 = 0; k1 < t1.size() && k2 < t2.size();)
			...
			void MinimizationOperations::markPair(std::vector<std::vector<bool>> &mark, std::vector<std::vector<std::unordered_set<IntPair*>>> &triggers, int n1, int n2)
			{
				mark[n1][n2] = true;
...
//ORIGINAL LINE: boolean[][] mark = new boolean[states.length][states.length];
			...
			void MinimizationOperations::minimizeHopcroft(Automaton *a)
			{
				...
				if (tr->size() == 1)
				{
					...
					if (t->to == a->initial && t->min == std::numeric_limits<wchar_t>::min() && t->max == std::numeric_limits<wchar_t>::max())
					{
						return;
					}
				}
...
//JAVA TO C++ CONVERTER TODO TASK: There is no direct native C++ equivalent to the Java String 'split' method:
				...
				for (int t = 0; t < transitionCount; ++t)
				{
					if (blocks->locations[tails[t]] == blocks->first[blocks->setNo[tails[t]]])
					{
						...
						Transition tempVar2(static_cast<wchar_t>(labels[t]->n1), static_cast<wchar_t>(labels[t]->n2), head);
			...
			void MinimizationOperations::makeAdjacent(std::vector<int> &A, std::vector<int> &F, std::vector<int> &K, int nn, int mm)
			{
				for (int q = 0; q <= nn; ++q)
				{
					F[q] = 0;
				...
				for (auto s : states)
				{
					...
					for (auto t : transitions)
					{
						...
						for (* : :optional<wchar_t> c : intersection)
						{
							...
							if (c - start > 1)
							{
								Transition tempVar3(static_cast<wchar_t>(start + 1), static_cast<wchar_t>(c - 1), t->to);
