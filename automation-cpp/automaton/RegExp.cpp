...
namespace dk {
    namespace brics {
        namespace automaton {
                ...
                if (s.length() == 0) {
                    ...
                    if (pos < b.length()) {
                        throw std::invalid_argument(
                                std::wstring(L"end-of-string expected at position ") + std::to_wstring(pos));
                    }
                }
                kind = e->kind;
                exp1 = e->exp1;
            ...
            Automaton *
            RegExp::toAutomaton(AutomatonProvider * automaton_provider, bool minimize) throw(std::invalid_argument) {
                return toAutomatonAllowMutate(nullptr, automaton_provider, minimize);
            }

            Automaton *
            RegExp::toAutomaton(std::unordered_map<std::wstring, Automaton *> & automata) throw(std::invalid_argument) {
                return toAutomatonAllowMutate(automata, nullptr, true);
            }

            Automaton * RegExp::toAutomaton(std::unordered_map<std::wstring, Automaton *> & automata,
                                            bool minimize) throw(std::invalid_argument) {
            ...
            Automaton * RegExp::toAutomatonAllowMutate(std::unordered_map<std::wstring, Automaton *> & automata,
            ...
            void RegExp::findLeaves(RegExp * exp, Kind kind, std::vector<Automaton *> & list,
                                    std::unordered_map<std::wstring, Automaton *> & automata,
                                    AutomatonProvider * automaton_provider, bool minimize) {
                if (exp->kind == kind) {
            ...
            RegExp * RegExp::parseRepeatExp() throw(std::invalid_argument) {
                ...
                while (peek(L"?*+{")) {
                    if (match(L'?')) {
                        ...
                        if (start == pos) {
                            throw std::invalid_argument(
                                    std::wstring(L"integer expected at position ") + std::to_wstring(pos));
                        }
                        ...
                        if (!match(L'}')) {
                            throw std::invalid_argument(
                                    std::wstring(L"expected '}' at position ") + std::to_wstring(pos));
                        }
                        if (m == -1) {
                            e = makeRepeat(e, n);
                        } else {
            ...
            RegExp * RegExp::parseSimpleExp() throw(std::invalid_argument) {
                if (match(L'.')) {
                    ...
                    if (i == -1) {
                        if (!check(AUTOMATON)) {
                            throw std::invalid_argument(
                                    std::wstring(L"interval syntax error at position ") + std::to_wstring(pos - 1));
                        }
                        return makeAutomaton(s);
                    } else {
                        if (!check(INTERVAL)) {
                            throw std::invalid_argument(
                                    std::wstring(L"illegal identifier at position ") + std::to_wstring(pos - 1));
                        }
                        try {
