...
namespace dk {
    namespace brics {
        namespace automaton {
            ...
            Automaton *
            SpecialOperations::subst(Automaton * a, std::unordered_map<wchar_t, std::set<wchar_t> *> & map) {
                if (map.empty()) {
            ...
            Automaton *
            SpecialOperations::homomorph(Automaton * a, std::vector<wchar_t> & source, std::vector<wchar_t> & dest) {
                a = a->cloneExpandedIfRequired();
                for (auto s : a->getStates()) {
                    std::set<Transition *> * st = s->transitions;
                    s->resetTransitions();
                    for (auto t : st) {
                        ...
                        while (min <= t->max) {
                            ...
                            int end = (n + 1 == source.size()) ? std::numeric_limits<wchar_t>::max() : source[n + 1] -
                                                                                                       1;
                            int length;
                            if (end < t->max) {
                                length = end + 1 - min;
            ...
            Automaton * SpecialOperations::projectChars(Automaton * a, std::set<wchar_t> * chars) {
                std::vector<boost::optional<wchar_t>> c = chars->toArray(
                        std::vector<boost::optional<wchar_t> *>(chars->size()));
            ...
            bool SpecialOperations::isFinite(State * s, std::unordered_set<State *> & path,
                                             std::unordered_set<State *> & visited) {
                path.insert(s);
                for (auto t : s->transitions) {
                    if (std::find(path.begin(), path.end(), t->to) != path.end() ||
                        (!std::find(visited.begin(), visited.end(), t->to) != visited.end() &&
            ...
            void SpecialOperations::getStrings(State * s, std::set<std::wstring> * strings, StringBuilder * path,
                                               int length) {
                if (length == 0) {
                    if (s->accept) {
            ...
            std::set<std::wstring> * SpecialOperations::getFiniteStrings(Automaton * a) {
                ...
                if (a->isSingleton()) {
                    strings.insert(a->singleton);
                } else if (!getFiniteStrings(a->initial, std::unordered_set<State *>(), strings, new StringBuilder(),
                                             -1)) {
                    return nullptr;
                }
                return strings;
            ...
            bool SpecialOperations::getFiniteStrings(State * s, std::unordered_set<State *> & pathstates,
                                                     std::unordered_set<std::wstring> & strings, StringBuilder * path,
                                                     int limit) {
                pathstates.insert(s);
                for (auto t : s->transitions) {
