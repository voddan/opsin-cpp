		...
		namespace automaton
		{
			...
			bool BasicOperations::isTotal(Automaton *a)
			{
				...
				if (a->initial->accept && a->initial->transitions->size() == 1)
				{
					...
					return t->to == a->initial && t->min == std::numeric_limits<wchar_t>::min() && t->max == std::numeric_limits<wchar_t>::max();
				}
				return false;
			}
			...
			std::wstring BasicOperations::getShortestExample(State *s, bool accepted)
			{
				...
				while (!queue.empty())
				{
					...
					{
						if (best == L"" || p.length() < best.length() || (p.length() == best.length() && p.compare(best) < 0))
						{
							best = p;
						}
					}
