    ...
    namespace brics {
        namespace automaton {
            ...
            void RunAutomaton::setAlphabet() {
                classmap = std::vector<int>(
                        std::numeric_limits<wchar_t>::max() - std::numeric_limits<wchar_t>::min() + 1);
                int i = 0;
...
//			}
            ...
            RunAutomaton *
            RunAutomaton::load(std::istream stream) throw(IOException, ClassCastException, ClassNotFoundException) {
