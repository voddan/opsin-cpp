...
//JAVA TO C++ CONVERTER TODO TASK: There is no native C++ equivalent to 'toString':
			...
			State *BasicAutomata::between(const std::wstring &x, const std::wstring &y, int n, Collection<State*> *initials, bool zeros)
			{
				State *s = new State();
				if (x.length() == n)
				{
				...
				else
				{
					...
					{ // cx<cy
						...
						if (cx + 1 < cy)
						{
							Transition tempVar4(static_cast<wchar_t>(cx + 1), static_cast<wchar_t>(cy - 1), anyOfRightLength(x, n + 1));
...
//ORIGINAL LINE: throw new IllegalArgumentException();
			...
			void BasicAutomata::maxInteger(const std::wstring &n, int i, StringBuilder *b)
			{
				...
				if (i < n.length())
				{
					...
					if (c != L'0')
					{
						b->append(L"[0-")->append(static_cast<wchar_t>(c - 1))->append(L"][0-9]{")->append(n.length() - i - 1)->append(L"}|");
					}
					b->append(c);
					maxInteger(n, i + 1, b);
				}
			...
			void BasicAutomata::minInteger(const std::wstring &n, int i, StringBuilder *b)
			{
				...
				if (i < n.length())
				{
					...
					if (c != L'9')
					{
						b->append(L"[")->append(static_cast<wchar_t>(c + 1))->append(L"-9][0-9]{")->append(n.length() - i - 1)->append(L"}|");
					}
					b->append(c);
					minInteger(n, i + 1, b);
				}
			...
			{
				RegExp tempVar(std::wstring(L"[ \t\n\r]*[-+]?0*([0-9]{0,") + i + std::wstring(L"}|((([0-9]\\.*){0,") + i + std::wstring(L"})&@\\.@)0*)[ \t\n\r]*"));
				return Automaton::minimize((&tempVar)->toAutomaton());
			}
			...
			Automaton *BasicAutomata::makeIntegerValue(const std::wstring &value)
			{
				...
				return Automaton::minimize(ws->concatenate(s->concatenate(Automaton::makeChar(L'0')->repeat())->concatenate(Automaton::makeString(b->toString())))->concatenate(ws));
			}
			...
			Automaton *BasicAutomata::makeDecimalValue(const std::wstring &value)
			{
				...
				{
					d = Automaton::makeChar(L'.')->concatenate(Automaton::makeString(b2->toString()))->concatenate(Automaton::makeChar(L'0')->repeat());
				}
				Automaton *ws = Datatypes::getWhitespaceAutomaton();
				return Automaton::minimize(ws->concatenate(s->concatenate(Automaton::makeChar(L'0')->repeat())->concatenate(Automaton::makeString(b1->toString()))->concatenate(d))->concatenate(ws));
			}
